Current handoff r131: Frozen v38 integrated MAP01 first allocation completed six Luna-low decisions/no retry in a continuously advancing enemy-visible fixture. One model-authored schema-v6 primary program admitted with exact running-action-v3 semantic steps/program SHA/intent-token binding; one active answer rejected at immediate fresh action predicate, four pending turns interrupted at authored health hard floors. Seven accepted cover/plan programs all empty-release;119 typed health/ammo captures reconciled with exact frames. No natural active revocation, so early physical release remains unexposed. Independent score0 kills/0 deaths/no map exit,31.731s control/27.489s model-wall. Raw first outcome retained265 files/41,480,362 bytes; Windows/WSL audit passes. Liveness1/6 admissions, zero renewal. Next version a bounded policy-liveness condition and separately expose running-action revocation; preserve lease expiration and cross-domain coverage. See research/doom/MAP01_V38_INTEGRATED_LIVE_V1.md.
