...
Current handoff r133: Posthoc SHA-bound v38/v39 raw-event analysis (no new model/GUI) classifies model-wait program envelopes, not actual held-key duration or useful outcomes. V38:27.489s wait,11.418s motor-capable cover,16.022s coast,0.049s no program,2/6 completed answers,1/6 plans. V39:43.318s wait,21.821s motor-capable cover,21.484s input-free coast,0.013s no program,5/6 completed,3/6 plans. Two post-rejection coast waits lost12 and5 health despite answer liveness; decision2 action still failed fresh validity. One v39 typed revocation reached verified physical empty release26.090ms after typed event and52.961ms before terminal. First exact plan frames51-78ms after acceptance, but independent task-useful feedback remains unavailable. Windows/WSL audit agrees analysis SHA7b754117... . Next measure true held-input occupancy, independently scored first useful result, bounded recovery cover and cross-domain coverage under matched conditions; no causal speed or human-tempo claim. See research/doom/MAP01_V38_V39_CONTROL_TEMPO_POSTHOC_V1.md.

> **Document role:** The newest handoff stays visible. Earlier retained handoffs remain verbatim below for provenance and coordination history.

<details>
<summary><strong>Earlier retained handoff history (r132 and earlier) — expand</strong></summary>

Current handoff r132: V38 attribution corrected: decision1 rejected fresh action predicate, then reusable_cover dropped authored cover; decisions2-5 used unauthored empty coast with runtime zero-loss health guard, causing redundant planner interrupts. V39 removes only that empty-coast policy monitor, retaining exact observation and fresh immediate action validity. Frozen six-turn Luna-low first outcome completed three coast-mode decisions (at least one through health loss), admitted3/6 plans and naturally revoked one running retreat_fire from typed health73→65 beyond authored six-point loss. Matched early input release verified empty before cancelled empty terminal; nine accepted programs all empty release. 218 typed/exact observations but217 PNG due one correct unchanged AIT reuse; original audit failed PNG-count assumption. Retained v2 pixel/receipt audit plus Windows/WSL file-hash manifest pass464 files/63,880,177 bytes. Independent score1 kill/0 death/no exit,50.371s control/43.318s model-wall. Continue with separate useful-control gap, renewal, survival and cross-domain tests; no causal speed or clear claim. See research/doom/MAP01_V39_COAST_LIVENESS_LIVE_V1.md.

Current handoff r131: Frozen v38 integrated MAP01 first allocation completed six Luna-low decisions/no retry in a continuously advancing enemy-visible fixture. One model-authored schema-v6 primary program admitted with exact running-action-v3 semantic steps/program SHA/intent-token binding; one active answer rejected at immediate fresh action predicate, four pending turns interrupted at unauthored runtime zero-loss empty-cover hard floors. Seven accepted cover/plan programs all empty-release;119 typed health/ammo captures reconciled with exact frames. No natural active revocation, so early physical release remains unexposed. Independent score0 kills/0 deaths/no map exit,31.731s control/27.489s model-wall. Raw first outcome retained265 files/41,480,362 bytes; Windows/WSL audit passes. Liveness1/6 admissions, zero renewal. Next version the unauthored empty-cover guard condition and separately expose running-action revocation; preserve lease expiration and cross-domain coverage. See research/doom/MAP01_V38_INTEGRATED_LIVE_V1.md.

Current handoff r130: frozen schema-v6 preflight made one Luna-low no-image call and returned ENDPOINT_COMPATIBLE (8,125 input/43 output); original and raw audit pass. Frozen v32 final-admission six-turn MAP01 run used actual advancing threat state:5 rejected policy-invalidated answers,1 plan admission,0 soft events,229 exact observations,10 accepted cover/plan programs all empty-release. Completed eligible decision4 lost to source_expired at12,214.235ms before admission despite interrupt already_terminal; no plan-4 input. Independent result1 kill/0 deaths/no map exit,59.752s control/57.150s model-wall. Interrupted usage incomplete. First outcomes retained13 files/9,060 bytes and484 files/65,814,543 bytes with manifests; Windows/WSL audit passes. Next preregister and run v38 schema-v6 integrated planner/running guard/early typed/two-phase-release exposure; preserve authored expiry. See research/doom/MAP01_V32_FINAL_ADMISSION_LIVE_V1.md.

Current handoff r129: frozen archived OpenTTD transfer has one no-retry schema preflight and six Luna-low effect decisions. Exact current frame/prompt within A-to-B/B-to-C compares no memory, full pre/post frames, action crop; independent engine transitions90/131 establish observed ground truth. No-memory/full2/2; crop1/2 and falsely contradicted A-to-B. Input18,736/23,748/19,424; images2/6/4. DO_NOT_TRANSFER_CROP; v1 plus raw retention audits pass65 files/56,471 bytes. No new game input. Revisit only on independently current-insufficient context with changed occlusion-cleared or landmark-preserving evidence. Main next priority is Issue #59 real-time threat control in continuously advancing DOOM, retaining failures and testing the integrated guard under live exposure. See research/live_control/OPENTTD_EFFECT_MEMORY_ABLATION_V1.md.

Current handoff r128: frozen action-grounded memory ablation v1 completed first allocation/no retry:1 schema preflight and9 Luna-low Chromium arms. Shifted, duplicate and misleading-old-appearance scenarios had byte-identical current frames across no-memory/full-frame/crop. Every proposed point was actually admitted after a fresh observation; independent server score is9/9 exact,0 decoy/no-effect, all releases empty. No-memory/full/crop each scored3/3; actual input28,035/31,791/28,188 and images3/6/6. Crop is -3,603 tokens (-11.33%) versus full at equal correctness, but +153 (+0.55%) versus perfect no-memory. Trap crop self-reported stale/misleading rejection and chose correctly. It is eligible only to replace full history in a different-domain test. Original audit's `requested_model` KeyError is retained; prereg-hashed runner and all10 raw plans prove Luna-low, and retained audit passes Windows/WSL without rerun. Next OpenTTD/Mindustry history-needed transfer; no-memory stays default when current evidence suffices. See `research/live_control/ACTION_GROUNDED_MEMORY_ABLATION_LIVE_V1.md`.

Current handoff r127: `action_grounded_memory_ablation_live_v1.py` is implemented and preregistered with output absent. One no-input schema preflight precedes nine fresh Chromium/Luna-low arms: no prior visual memory, prior full frame and action-grounded crop, Latin-balanced across shifted, duplicate-label and old-appearance-decoy scenarios. Current frame, URL, prompt, schema and independent HTTP scorer are fixed within each scenario; a fresh post-model observation precedes Executor admission. Correct Submit, decoy Submit and no effect remain distinct, with empty release mandatory. Crop advances only at3/3, zero wrong target, no worse than controls and lower actual input tokens than full-frame. Five tests pass Windows/WSL, Chromium executable was checked, hashes/schedule are frozen and no formal output/model call exists. Commit before running once; retain capacity failure or any comparison outcome without retry. See `research/live_control/ACTION_GROUNDED_MEMORY_ABLATION_LIVE_V1.md`.

Current handoff r126: `action_grounded_visual_memory_v1.py` joins the previously separate target patch, accepted-action lineage, exact effect and recovery memory into one fail-closed receipt. Each retained crop binds exact source bytes/context, target handle/point/hash, admitted Submit program, completed empty release, first successful reconciled semantic effect, independent output and the full local/model repair path. A matching receipt remains a visual reference only: current exact observation, target revalidation and fresh input admission are mandatory. Two42x18 crops were derived from the frozen seed217 local/model cases; distinct hashes preserve the actual hover change. Ten tests and retained audit pass on Windows/WSL. This is retrospective provenance with no new calls/actions or memory-benefit claim. Next preregister matched crop/full/no-prior-memory arms and measure correctness, wrong targets, real tokens/images, fallback and recovery before OpenTTD/Mindustry transfer. See `research/live_control/ACTION_GROUNDED_VISUAL_MEMORY_V1.md`.

Current handoff r125: frozen adaptive semantic repair live v2 passed both seed217 cases once. Resize: shared caller trace association_changed→local repaired,0 repair calls,1 total call/9,348 input, mutation→return651.634ms, Submit→useful248.037ms. Actual Save hover changed the patch: association_changed→local missing→model target→current patch match,1 repair/2 total calls,18,696 input, repair wait7,197.203ms, caller7,871.585ms, Submit→useful249.006ms. Both exact `t000217`, semantic reconciliation and all releases pass.100 files/1,792,829 bytes audit Windows/WSL and retain first outcome/no retry. Mutations differ, so no savings comparison. Next action-grounded crop provenance and held-out crop/full/no-memory comparison. See `research/live_control/ADAPTIVE_SEMANTIC_REPAIR_LIVE_V2.md`.

Current handoff r124: formal adaptive live v1 stopped after its local case. Shared caller trace is reuse association_changed→local repaired with0 repair calls, but final adapter incorrectly treated `expected_crop_missing` as failure before the action that creates the expected completion crop. Current binding was exact and observed crop materialized; no Submit/input/effect occurred, output `{}`, releases empty. Model case did not start.40 files/742,028 bytes and diagnosis pass12-check Windows/WSL audit; never rerun v1. V2 changes only the pre-action semantic gate, requires false/evaluable/current `expected_crop_missing`, keeps handle patch and authority checks, and is frozen seed217 local/model/no-retry. Next execute v2 once. See `research/live_control/results/adaptive-semantic-repair-live-01/diagnosis.json` and `adaptive_semantic_repair_live_v2_prereg.json`.

Current handoff r123: adaptive semantic repair live v1 is preregistered before any formal output. Integration found two concrete shared-caller gaps and repaired them: final revalidation may promote a refreshed target/cache before execution, and typed capacity refusal survives as `TASK_DEFERRED` with attempt image/wait accounting. Frozen seed216 runs two no-retry Linux/X11 cases in local/model order. A -120px width change must take the zero-model local path; a real Save hover patch change must naturally produce missing/ambiguous/changed, take exactly one Luna-low fallback and bind it to a later exact observation before input. Both require independent exact POST, semantic reconciliation and empty release. These unequal mutations provide integration evidence only, never a speed/token comparison. Twelve caller tests, audited nine-scenario offline report and three prereg tests pass Windows/WSL. Next run once and retain any result. See `research/live_control/adaptive_semantic_repair_live_v1_prereg.json`.

Current handoff r122: a primary-source convergence note now maps MementoGUI, AGMem, adaptive VLM routing, Mirage-1 and OSWorld to bounded Agent Interface tests. The common direction is selected ROI/action-grounded evidence, conditional escalation, reusable methods with explicit invalidation and independent execution scoring. Do not import their reported gains. Next attach action/effect/recovery provenance to patches, keep the same model for causal evaluation, first run adaptive caller v3 on natural local and fallback branches, then freeze an action-crop versus full-frame versus no-memory transition comparison. Learned memory control and multi-model routing remain deferred until these simpler discriminators fail. See `research/live_control/CONDITIONAL_REUSE_RESEARCH_V1.md`.

Current handoff r121: `adaptive_acquisition_caller_v3` moves the matched local-first repair policy into one shared route. Reuse first revalidates, may try one exact no-authority zero-model local repair, and only configured `missing`/`ambiguous`/`association_changed` outcomes reach accounted model reacquisition. A completed fallback must bind its call ID to one later passive exact observation, capture clock and pointer binding before final revalidation/input. Changed/unconfigured/malformed evidence stops with zero input. Attempt accounting now includes image submissions and model wait with explicit coverage for failed and completed calls. Ten direct tests and a separately audited seven-scenario report pass Windows/WSL; fresh model/GUI calls0. Next use this exact caller in a finite natural local-repair plus changed-evidence live block. See `research/live_control/ADAPTIVE_ACQUISITION_CALLER_V3.md`.

Current handoff r120: v3 repairs v2 by preserving the model-visible patch, taking exactly one passive exact frame after inference and binding source/current sequence, clocks, model call ID, point and patch hash in a no-authority receipt. Frozen seed215 passed4/4. Local recovery median109.966ms/input9,351 versus model8,115.268ms/input18,702; bounded differences8,005.302ms and9,351 tokens. Post-model refresh was75.592/66.802ms and patch check0.071/0.066ms. Every arm saved `t000215`, reconciled useful frames and released;6 unique calls, retry0.189 files/3,622,385 bytes audit both OSes. Promote only a local-first route candidate for this calibrated unchanged target; next shared-caller integration must exercise missing/ambiguous/changed fallback with full accounting. See `research/live_control/MATCHED_SEMANTIC_REPAIR_V1.md`.

Current handoff r119: matched v2 crossed the prior capacity boundary. Local arm1 passed with one9,351-input-token Luna call, correct `t000214`, recovery122.670ms, useful feedback263.130ms and empty release. Model arm2's two Luna calls completed; post-resize output correctly chose `[270,243]` but arrived8,262.886ms after a source with3,000ms freshness. The derived handle safely refused `STALE`, so no model-arm Submit or balanced comparison occurred and arms3-4 never started. A deterministic retained-frame replay proves identical patch valid fresh and stale after wait. Total3 calls/28,053 input;90 files/1,415,626 bytes pass11 checks on Windows/WSL. Next freeze v3 with exactly one post-model passive snapshot and current-patch revalidation; never rerun v2. See `research/live_control/MATCHED_SEMANTIC_REPAIR_V1.md`.

Current handoff r118: `semantic_repair_model_v2` and `semantic_grounding_admission_v1` convert controlled model execution into COMPLETED/DEFERRED_UPSTREAM/FAILED_UPSTREAM/FAILED_OUTPUT, then TASK_MUTATION_ELIGIBLE/TASK_DEFERRED/TASK_BLOCKED. Every branch grants no semantic or input authority; eligible still requires Executor admission. The retained valid Luna record and v1 capacity refusal plus malformed/authority-escalation controls pass7 tests on Windows/WSL, with no fresh model/GUI call. Next integrate initial grounding before form token entry and typed later reacquisition deferral into a separately frozen matched v2 runner. See `research/live_control/SEMANTIC_GROUNDING_ADMISSION_V1.md`.

Current handoff r117: matched semantic-repair v1 was frozen at `c03fd4fd` and executed once. Arm1 completed the navigate/fill prefix and captured the exact filled Chromium frame, then the required initial Luna-low grounding call returned model-at-capacity. One thread started; model messages, completed turns and usage records were0. No handle, resize, semantic probe or Submit occurred. Both prefix terminals and owner close verified empty input. The failed comparison is retained as34 files/469,683 bytes and passes all11 independent Windows/WSL diagnosis checks. It supplies no token or recovery comparison. Next expose capacity as a typed invocation outcome and gate/defer before task-input acquisition under a separately frozen allocation; never rerun v1. See `research/live_control/MATCHED_SEMANTIC_REPAIR_V1.md`.

Current handoff r116: target-handle semantic repair passed once. In seed212 Chromium, an exact Save handle anchored the future completion crop. After width1050→930, old contract refused `surface_size_changed`; exact handle resolve0.091ms and no-authority contract repair0.145ms, capture→ready110.866ms, frontier resumptions0. Repaired Submit saved `t000212`; useful feedback271.178ms,4.101ms probe,24.195ms before PNG/139.613ms before terminal.31 files/751,360 bytes audit Windows/WSL; retry/subagent/model0. Relation is calibrated, not discovered. Next compare local cached repair against same-model reacquisition on matched geometry changes with full token/image/cost accounting.

Current handoff r115: surface-relative semantic probe v1 is retained safe-failed after an actual `[21,28]` move: stale pre-move binding caused needs_decision before0th pointer step, with no probe/submission and empty release. V2 adds exactly one passive post-move snapshot; fresh seed211 passes. The relative crop follows `[21,28]` while the fixed crop rejects the same frame; useful feedback275.781ms,2.743ms compute,23.828ms before PNG/134.919ms before terminal. Independent `t000211`, reconciliation and releases pass. A same-surface -120px resize refuses before crop hashing.33 files/857,946 bytes audit Windows/WSL; model/subagent/retry0. Next connect this predicate to an existing verified target handle and measure bounded local resize repair.
