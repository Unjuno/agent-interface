Latest OpenTTD effect replication: preregistered v8 keeps the v7 candidate unchanged. It builds A-to-B once, marks the effect uncertain under sign/cost/highlight occlusion, uses two observation-only inspections, then safely stops on turn8 without repeating A-to-B or mutating B-to-C. Independent score is false; forbidden/surrounding state remains valid. It uses133,041 input tokens,30 durable calls,32 frames,116.213s model wait and13.185s proposal-to-feedback. Across v7/v8 the candidate has1/2 same-task hard success and2/2 zero repeated completed-segment drag. HOLD: retain action-effect evidence across inspection turns and improve bounded occlusion recovery before changed geometry. V8 also exposes that finish-outcome-v1 mislabels any abort file as `bounded_turn_limit`; explicit finish-kind v2 separates typed safe stop, limit, cleanup and visual verify in12 valid/4 invalid controls, preserving raw v8. Windows/WSL audits pass. See research/live_control/OPENTTD_EFFECT_REPLICATION_V2.md. No causal speed, token, geometry, human-tempo or cross-domain claim. Full goal stays open.
