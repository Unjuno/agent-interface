Latest OpenTTD live effect evidence: a preregistered v7 allocation keeps the v6 seed991003 task, fixed Astra-medium route, initial state, checkpoint semantics,12-turn limit and independent scorer, while appending bounded path-local before/after/difference panels after each drag. The first run recognizes A-to-B on turn6, builds B-to-C instead of repeating A-to-B, and independently passes all target/connection/forbidden/surrounding gates on turn7. It uses116,879 input tokens,26 durable calls,32 exact frames,94.221s model wait,12.439s proposal-to-feedback and109.034s initial-observation-to-semantic-completion. Descriptively versus retained v6:12->7 turns,199,613->116,879 tokens,2->0 repeated completed-segment drags, hard false->true. Windows/WSL audits pass. Retain for preregistered replication and new geometry; one sequential same-task result is not a causal41% improvement, latency distribution, geometry transfer or human tempo. See research/live_control/OPENTTD_EFFECT_LIVE_V1.md. Full goal stays open.
