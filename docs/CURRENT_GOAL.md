Latest OpenTTD allocation: a tracked pre-action tree-transparency transition costs428.854ms, adds zero model boundaries and reduces the fixed Astra episode from12 turns/198,746 input tokens to7/115,045. Independent correctness still fails: A-to-B is one row high and the model falsely verifies. Adding only the official general sign-to-map-square relation regresses to12 turns/199,613 tokens without verify or safe stop. The limit branch exits before the supervisor abort is consumed, so that run has no independent outcome; artifacts are retained without retry. Next fix that handshake, then test mechanical annotation/surface evidence across held-out geometries. The full live-tempo/domain goal stays open.
