Latest OpenTTD infrastructure: driver v5 fixes the bounded-limit finish race. A model-free live probe applies12 observe-only proposals, then records independent `bounded_turn_limit` through50 durable calls/39 observations with zero pointer/model calls, unchanged task/save state and exit0; audits pass cross-OS. Ctrl+1 changes custom sign presentation on seed991003 L and held-out seed991002 straight geometry without task mutation, but normal simulation progress contaminates full-frame deltas. Next create a paused matched pair, then measure model targeting and independent task correctness. Full live-tempo/domain goal stays open.
