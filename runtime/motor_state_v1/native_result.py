...
def motor_state_from_native_result(raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    surface, frame = target.get("surface_id") or target.get("window_id"), target.get("frame")
    ...
    pointer_ok = isinstance(observed, Mapping) and {"x", "y"} <= set(observed)
    ack_id = raw.get("ack_id")
    ack_ok = isinstance(ack_id, str) and bool(ack_id)
    transport_ok, focus_ok = execution.get("transport_passed") is True, raw.get("focus_confirmed") is True
    uncertainty = "NONE" if isinstance(obs, Mapping) and pointer_ok and transport_ok and focus_ok and ack_ok else ("FOCUS_UNKNOWN" if isinstance(obs, Mapping) else "OS_UNCONFIRMED")
    ...
    row = {"schema": SCHEMA, "state_id": result_id, "owner_id": str(raw.get("owner_id", "owner-unknown")), "owner_revision": raw.get("binding_revision", 0), "observation_id": str(obs.get("observation_id", "obs-unknown")) if isinstance(obs, Mapping) else "obs-unknown", "surface_id": surface, "coordinate_frame": frame, "commanded_pointer": raw.get("commanded_pointer", {}), "observed_pointer": observed, "held_keys": list(raw.get("held_keys", [])), "held_buttons": list(raw.get("held_buttons", [])), "input_ack": {"id": ack_id if ack_ok else "ack-unknown", "status": "ACKED" if transport_ok and ack_ok else "UNKNOWN"}, "release": {"status": release_status, "retained": True}, "uncertainty": uncertainty, "events": [{"type": "RELEASE_TRANSITION", "status": release_status}]}
