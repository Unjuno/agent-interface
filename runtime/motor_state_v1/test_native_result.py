...
class NativeResultTests(unittest.TestCase):
    ...
    def test_missing_observation_is_uncertain_and_valid(self):
        value=raw(); value["observation"]=None; self.assertEqual(motor_state_from_native_result(value)["uncertainty"],"OS_UNCONFIRMED")
    def test_transport_failure_cannot_emit_none(self):
        value=raw(); value["execution"]={"transport_passed":False}; self.assertNotEqual(motor_state_from_native_result(value)["uncertainty"],"NONE")
    def test_missing_pointer_cannot_emit_none(self):
        value=raw(); value["observation"]={"observation_id":"obs-1"}; self.assertNotEqual(motor_state_from_native_result(value)["uncertainty"],"NONE")
    def test_empty_pointer_cannot_emit_none(self):
        value=raw(); value["observation"]["observed_pointer"]={}; self.assertNotEqual(motor_state_from_native_result(value)["uncertainty"],"NONE")
    def test_missing_ack_cannot_emit_acked_or_none(self):
        value=raw(); del value["ack_id"]; row=motor_state_from_native_result(value); self.assertEqual(row["input_ack"]["status"],"UNKNOWN"); self.assertNotEqual(row["uncertainty"],"NONE")
    def test_release_error_precedes_verified_flag(self):
        value=raw(); value["release"]={"error":"release_failed","final_release_verified":True}; self.assertEqual(motor_state_from_native_result(value)["release"]["status"],"FAILED")
    def test_bad_revision_is_rejected(self):
        value=raw(); value["binding_revision"]="1"
        with self.assertRaises(NativeResultError): motor_state_from_native_result(value)
    def test_missing_surface_is_rejected(self):
        value=raw(); value["target"]={"frame":"window-client"}
        with self.assertRaises(NativeResultError): motor_state_from_native_result(value)
    def test_authority_promotion_is_rejected(self):
        value=raw(); value["extends_lease"]=True
        with self.assertRaises(NativeResultError): motor_state_from_native_result(value)
    def test_raw_input_is_not_mutated(self):
        value=raw(); before=repr(value); motor_state_from_native_result(value); self.assertEqual(repr(value),before)
