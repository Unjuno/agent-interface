...
class NativeResultTests(unittest.TestCase):
    ...
    def test_bool_pointer_cannot_emit_none(self):
        value = raw()
        value["observation"]["observed_pointer"] = {"x": True, "y": False}
        row = motor_state_from_native_result(value)
        self.assertNotEqual(row["uncertainty"], "NONE")

    def test_null_pointer_cannot_emit_none(self):
        value = raw()
        value["observation"]["observed_pointer"] = {"x": None, "y": 2}
        row = motor_state_from_native_result(value)
        self.assertNotEqual(row["uncertainty"], "NONE")

    def test_string_pointer_cannot_emit_none(self):
        value = raw()
        value["observation"]["observed_pointer"] = {"x": "1", "y": 2}
        row = motor_state_from_native_result(value)
        self.assertNotEqual(row["uncertainty"], "NONE")

    def test_missing_observation_is_uncertain_and_valid(self):
        value = raw(); value["observation"] = None
        self.assertEqual(motor_state_from_native_result(value)["uncertainty"], "OS_UNCONFIRMED")

    def test_transport_failure_cannot_emit_none(self):
        value = raw(); value["execution"] = {"transport_passed": False}
        self.assertNotEqual(motor_state_from_native_result(value)["uncertainty"], "NONE")

    def test_missing_pointer_cannot_emit_none(self):
        value = raw(); value["observation"] = {"observation_id": "obs-1"}
        self.assertNotEqual(motor_state_from_native_result(value)["uncertainty"], "NONE")

    def test_empty_pointer_cannot_emit_none(self):
        value = raw(); value["observation"]["observed_pointer"] = {}
        self.assertNotEqual(motor_state_from_native_result(value)["uncertainty"], "NONE")

    def test_missing_ack_cannot_emit_acked_or_none(self):
        value = raw(); del value["ack_id"]
        row = motor_state_from_native_result(value)
        self.assertEqual(row["input_ack"]["status"], "UNKNOWN")
        self.assertNotEqual(row["uncertainty"], "NONE")

    def test_release_error_precedes_verified_flag(self):
        value = raw(); value["release"] = {"error": "release_failed", "final_release_verified": True}
