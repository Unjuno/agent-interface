...
async def main():
    out=Path('results-local/native-mcp-direct-image-01').resolve()
    ...
    async def call(client,name,args,label):
        ...
        print(json.dumps({'label':label,'mcp_result':result.model_dump(mode='json')}),flush=True)
        return metadata
    params=StdioServerParameters(command=sys.executable,args=[
        str(Path('research/live_control/native_mcp_v1.py').resolve()),
        '--allocation-directory',str(out/'allocation'),'--app','inkscape',
        '--seed','991119','--max-stages','2','--harness-python','/usr/bin/python3'],env=dict(os.environ))
