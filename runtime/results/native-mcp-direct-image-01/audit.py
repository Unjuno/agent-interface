...
for label in ('start-0', 'submit'):
    ...
    for index,block in images:
        metadata_image=json.loads(response['content'][0]['text'])['image_reference']
        artifact=run/metadata_image['relative_path']
        assert base64.b64decode(block['data'],validate=True) == artifact.read_bytes()
        assert hashlib.sha256(artifact.read_bytes()).hexdigest()==metadata_image['sha256']
...
terminal=json.loads(read('session/status-1.json')['content'][0]['text'])['allocation']
assert terminal['status']=='terminal' and terminal['returncode']==0
assert terminal['pid']==read('session/allocation/run/owner.json')['pid']==17392
events=read('session/timings.json')
assert [e['tool'] for e in events]==['native_start','native_submit','native_status','native_status']
