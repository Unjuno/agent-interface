...
out=Path('results-local/inline-portable-live-01').resolve()
...
try:
    ...
    observed=subprocess.run([sys.executable,str(Path('results-local/inline-portable-01/runtime.pyz').resolve()),'dispatch','--program',str(program_path),'--targets',str(targets),'--current-observation-seq','0','--current-binding-revision','0','--capture-directory',str(out/'images'),'--display',name,'--review'],capture_output=True)
    ...
    raw=row['receipt']['source']['raw_report']
    (out/'review.json').write_bytes(observed.stdout)
    captures=raw['result']['execution']['observations']
    assert len(captures)==2
    assert [c['operation_index'] for c in captures]==[1,2]
    assert row['image_reference']['recorded_capture']['operation_index']==2
    assert row['image_reference']['execution_observation_index']==1
    assert row['image_reference']['sha256']==captures[-1]['artifact']['sha256']
    (out/'comparison.json').write_text(json.dumps({'captures':len(captures),'selected_index':1,'capture_operation_indices':[c['operation_index'] for c in captures],'selected_operation_index':row['image_reference']['recorded_capture']['operation_index'],'selected_hash_matches':True,'selected_dimensions':[captures[-1]['width'],captures[-1]['height']]}))
