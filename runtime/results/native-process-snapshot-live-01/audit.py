...
assert (owner['status'], owner['pid'], owner['returncode']) == ('terminal', 19706, 0)
assert metadata[2]['allocation']['status'] == 'needs_review'
assert metadata[2]['allocation']['owner']['state'] == 'terminal'
assert 'returncode' not in metadata[2]['allocation']
assert metadata[1]['allocation']['source_stage'] == 1
assert metadata[0]['session_context']['goal']['value']['task']['kind'] == 'move_right_preserve_geometry'
for stage in (1, 2):
    assert (run / f'request-{stage}.json').read_bytes() == (root / f'prior-request-{stage}.json').read_bytes()
print(json.dumps({'status': 'TASK_PASS_NO_ROUNDTRIP_REDUCTION', 'manifest_files': len(manifest),
