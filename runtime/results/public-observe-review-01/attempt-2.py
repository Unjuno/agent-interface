...
out=Path('results-local/public-observe-review-live-02').resolve()
out.mkdir(exist_ok=False)
r,w=os.pipe()
server=subprocess.Popen(['Xvfb','-displayfd',str(w),'-screen','0','640x480x24','-nolisten','tcp'],pass_fds=(w,),stdout=subprocess.DEVNULL,stderr=(out/'xvfb.stderr.log').open('wb'))
os.close(w)
d=None
try:
