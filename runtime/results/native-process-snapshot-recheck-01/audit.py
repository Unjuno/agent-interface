...
assert [r['tool'] for r in rows] == ['native_start', 'native_submit', 'native_submit']
...
assert (owner['status'], owner['pid'], owner['returncode']) == ('terminal', 19984, 0)
assert metadata[1]['allocation']['initial_source_stage'] == 1
assert 'source_stage' not in metadata[1]['allocation']
...
print(json.dumps({'status': 'TASK_PASS_NO_SEPARATE_STATUS_CALL', 'manifest_files': len(manifest),
