...
The archive does not retain separate exit codes for the SDK client or owner, so their process exits are unknown from this record. `cleanup.json` records only the two tracked GUI-child return codes (-15 and 0); it does not verify descendants, and must not be read as an owner/client exit-status record. The source revision and file hashes were inspected after the run (no implementation edits occurred during the allocation); this is not full launch attestation. Raw paths in receipts refer to the original results-local/public-mcp-action-01 directory. The archive preserves the original directory structure and every file hash is listed in RESULT.json.
