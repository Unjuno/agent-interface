...
out=Path('results-local/public-inkscape-use-04').resolve();out.mkdir(exist_ok=False)
name=':147';number='147'
...
try:
 ...
 launch(['openbox'],env)
 app=launch(['inkscape','--app-id-tag=AgentInterfacePublic04',str(svg)],env)
 d=display.Display(name);assert (d.screen().width_in_pixels,d.screen().height_in_pixels)==(1024,768)
 window=None
 def scan(win):
  for child in win.query_tree().children:
   try:
    if 'inkscape' in str(child.get_wm_class()).lower() and child.get_attributes().map_state == 2:return child
    found=scan(child)
 ...
 for _ in range(600):
  ...
  time.sleep(.05)
 if window is None:
  (out/'root-children.json').write_text(json.dumps([{'id':c.id,'title':str(c.get_wm_name()),'class':str(c.get_wm_class())} for c in d.screen().root.query_tree().children]))
  raise RuntimeError('window timeout')
