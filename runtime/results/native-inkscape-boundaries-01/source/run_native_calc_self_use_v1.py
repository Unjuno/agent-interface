...
from native_exchange_v1 import publish, encoded
...
def main():
    ...
    parser.add_argument('--app', choices=('calc', 'inkscape'), default='calc')
    ...
    try:
        ...
        goal, output, _ = suite.prepare(session, args.app, args.seed, '')
        save('goal.json', goal)
        ...
        save('evaluation.json', suite.evaluate(args.app, output, goal))
        evaluation = json.loads((out/'evaluation.json').read_text())
        publish(out/f'reply-{stage}.json', encoded({'status': 'finished', 'stage': stage,
        ...
        if decision_hash is not None and not (out/f'reply-{stage}.json').exists():
            publish(out/f'reply-{stage}.json', encoded({'status': 'needs_review', 'stage': stage,
