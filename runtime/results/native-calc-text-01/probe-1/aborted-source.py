    ...
    for block in range(2):
        ...
        for edit, gap in conditions:
            for payload in (('116', '476') if block == 0 else ('476', '116')):
                cases.append(dict(cell=f'A{len(cases)+1}', block=block,
...
def main():
    ...
    try:
        ...
        for i, case in enumerate(cases):
            ...
            if case['gap_ms'] == 0:
                tail.append({'op':'text','text':case['text']})
            else:
                for n,ch in enumerate(case['text']):
                    if n:
                        tail.append({'op':'wait_update','timeout_ms':case['gap_ms']})
                    tail.append({'op':'text','text':ch})
