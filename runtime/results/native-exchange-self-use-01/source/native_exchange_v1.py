...
def run(run_directory, stage, decision, *, timeout=5, resume=False):
    ...
    if type(resume) is not bool or not isinstance(decision, dict):
    ...
    while True:
        if reply.exists():
            displayed = review_native(reply, root)
