...
out=Path('results-local/public-portable-review-live-01').resolve()
...
try:
    ...
    observed=subprocess.run([sys.executable,str(Path('results-local/public-portable-review-01/agent-interface-runtime.pyz').resolve()),'observe','--targets',str(targets),'--target','fixture','--frame','window_client','--region','0','0','400','180','--capture-directory',str(out/'images'),'--display',name],capture_output=True)
    ...
    reviewed=subprocess.run([sys.executable,str(Path('results-local/public-portable-review-01/agent-interface-runtime.pyz').resolve()),'review','--report',str(report),'--run-directory',str(out)],capture_output=True)
