"""Independent raw-only byte-binding audit for Issue #3793."""
...
EXPECTED = {
    ...
    "audit_plan": "79741322f5c4b43d7d6a5a7b847bf3505bf94d2bc2f486ff27e405925dd795",
...
def load_inputs(root: Path, manifest_path: Path) -> tuple[dict, dict[str, bytes], list[str]]:
    ...
    if manifest.get("schema") != "agent-interface/issue-3793-audit-input-manifest-v1":
        errors.append("MANIFEST_SCHEMA_MISMATCH")
    if manifest.get("issue") != 3793 or manifest.get("allocation") != "issue3711-downstream-truncation-audit-v3-01":
...
def bind_frozen_history(files: dict[str, bytes]) -> list[str]:
    errors: list[str] = []
    ...
    checks = [
        ...
        (freeze.get("image_digest") == IMAGE.split("@", 1)[1], "FROZEN_IMAGE_DIGEST_MISMATCH"),
        (freeze.get("platform") == "linux/arm64", "FROZEN_PLATFORM_MISMATCH"),
        (freeze.get("sha256", {}).get("PLAN.md") == EXPECTED["audit_plan"], "FROZEN_PLAN_HASH_MISMATCH"),
        (freeze.get("sha256", {}).get("audit.py") == EXPECTED["audit_source"], "FROZEN_AUDITOR_HASH_MISMATCH"),
        ...
        (audit_v2_01.get("disposition") == "FAIL_AUDIT_V2", "AUDIT_V2_01_FAILURE_NOT_PRESERVED"),
    ]
...
def main() -> int:
    ...
    raw = json.loads(files["formal_raw"]) if "formal_raw" in files else {}
    accepted = files.get("accepted", b"")
    delivered = files.get("delivered_prefix", b"")
    baseline_errors = delivery_errors(accepted, delivered, raw) if not errors else ["BASELINE_NOT_RUN_SOURCE_STOP"]

    needle = b"/out/attempt"
    occurrences = accepted.count(needle)
    mutated_accepted = accepted.replace(needle, b"/bad/attempt", 1)
    accepted_mutation_errors = delivery_errors(mutated_accepted, delivered, raw)
    accepted_control_sound = (
        occurrences == 1 and len(mutated_accepted) == len(accepted)
        and b"/bad/attempt" in mutated_accepted
        and not {"ACCEPTED_JSON_INVALID", "ACCEPTED_CONTENT_MISMATCH"} & set(accepted_mutation_errors)
        and "ACCEPTED_SHA256_MISMATCH" in accepted_mutation_errors
    )
    truncated = delivered[:-1]
    prefix_mutation_errors = delivery_errors(accepted, truncated, raw)
    prefix_control_sound = (
    ...
    elif not accepted_control_sound or not prefix_control_sound:
        disposition = "FAIL_MUTATION_BINDING"
    else:
        disposition = "PASS_AUDIT_BINDING_SCOPED"

    result = {
        "schema": "agent-interface/issue-3793-audit-result-v1",
        "issue": 3793,
        ...
        "audit_invocations": 1,
        "baseline": {
            "raw_sha256": sha(files["formal_raw"]) if "formal_raw" in files else None,
        ...
        "mutations": {
            ...
            "delivered_prefix_23_to_22_bytes": {
                ...
                "control_sound": prefix_control_sound,
            },
        },
        "errors": errors,
    }
