...
def modal_names(rows,calc): return sorted(set(r['name'] for r in rows if r['id']!=calc and r['name'] and r['class'] and any('libreoffice-calc' in str(x).lower() for x in r['class'])))
def proc_uids(fragment):
 ...
 for p in Path('/proc').iterdir():
  ...
  try:
   ...
   out.append({'pid':int(p.name),'euid':int(uid_line.split()[2]),'cmd':cmd[:500]})
...
def publish(staging,target,target_dir):
 ...
 os.replace(tmp,target); fd=os.open(target_dir,os.O_RDONLY); os.fsync(fd); os.close(fd)
def writer_attempt(gui,gui_uid,external,target,result_path):
 code=r'''import os,json,sys,time
src,dst,out=sys.argv[1:]
r={'started_ns':time.monotonic_ns(),'euid':os.geteuid(),'egid':os.getegid(),'src':src,'dst':dst,'operation':'inplace_r+b'}
try:
 payload=open(src,'rb').read()
 with open(dst,'r+b') as f:
  f.seek(0); f.write(payload); f.truncate(); f.flush(); os.fsync(f.fileno())
 r.update(ok=True,errno=None,error=None)
except OSError as e:
...
def run_arm(root,name,dn,adversarial,gui):
 ...
 make_xlsx(target,'seed','old'); make_xlsx(staging,'seed','old'); make_xlsx(external,'external','replacement','external-marker','writer'); os.chmod(target,0o644)
 ...
 auth=out/'controller.Xauthority'; auth.write_bytes(b''); env=os.environ.copy(); env.update(DISPLAY=dn,XAUTHORITY=str(auth)); os.environ['DISPLAY']=dn; os.environ['XAUTHORITY']=str(auth)
 xv=subprocess.Popen(['Xvfb',dn,'-screen','0','1024x768x24','-nolisten','tcp','-ac'],env=env,stdout=(out/'xvfb.stdout').open('w'),stderr=(out/'xvfb.stderr').open('w')); ob=lo=None; b=None
 try:
  wait_display(env,dn); genv=['env',f'DISPLAY={dn}',f'HOME={home}',f'XDG_RUNTIME_DIR={xdg}',f'USER={gui}',f'LOGNAME={gui}']
  ...
  profile_url=f'file://{profile}'; lo=subprocess.Popen(['runuser','-u',gui,'--',*genv,'libreoffice',f'-env:UserInstallation={profile_url}','--nologo','--nodefault','--nofirststartwizard','--norestore',str(staging)],stdout=(out/'libreoffice.stdout').open('w'),stderr=(out/'libreoffice.stderr').open('w'))
  calc=find_calc(dn); result['calc_window_id']=calc; time.sleep(1.5); result['soffice_processes']=proc_uids(str(profile))
  ...
  b=OfficeX11Backend(dn,{'calc':calc}); b.focus('calc'); b.pointer_move('calc','window_client',80,180); b.pointer_button('left',True); b.pointer_button('left',False); b.key_chord(['CTRL','Home']); b.text('office'); b.key_chord(['ENTER']); b.text('preview'); b.key_chord(['ENTER']); time.sleep(.15); b.key_chord(['CTRL','S']); time.sleep(.7); m1=modal_names(windows(dn),calc)
  ...
  m2=modal_names(windows(dn),calc); result['modal_names_1']=m1; result['modal_names_2']=m2; result['release']=b.release_all().__dict__; result['backend_emissions']=b.emissions
  ...
  result['writer']=None
  if adversarial:
   result['writer']=writer_attempt(gui,uid,external,target,gui_dir/'writer_result.json'); result['target_after_writer_attempt']=statrow(target); result['external_exists_after_writer_attempt']=external.exists()
  current=statrow(target); result['publish_recheck_target']=current; result['publish_recheck_valid']=all(current[k]==plan[k] for k in ['sha256','ino','size']); result['publish_started_ns']=time.monotonic_ns()
  if result['publish_recheck_valid']: publish(staging,target,target_dir); result['published']=True
  ...
  okcells=result['final_score']['cells']['A1']=='office' and result['final_score']['cells']['A2']=='preview'; denied=(not adversarial) or (result['writer'] and result['writer']['ok'] is False and result['writer']['errno'] in [errno.EACCES,errno.EPERM]); unchanged=(not adversarial) or (result['target_after_writer_attempt']['sha256']==plan['sha256'] and result['target_after_writer_attempt']['ino']==plan['ino'])
  result['classification']='PASS_CAPABILITY_COMMIT_OWNER' if result['published'] and okcells and denied and unchanged and result['release']['verified'] else 'FAIL_OR_UNCERTAIN'; dump(out/'result.json',result); return result
 except Exception as e: dump(out/'error.json',{'type':type(e).__name__,'detail':str(e),'monotonic_ns':time.monotonic_ns()}); raise
 finally:
  if b:
...
def main():
 ...
 for name,dn,adv in [('01-stable',':345',False),('02-direct-write-attempt',':346',True)]: rows.append(run_arm(args.out,name,dn,adv,args.gui_user))
 summary={'schema':'agent-interface/libreoffice-privsep-commit-owner-v1','allocation':'c318-privsep-06','order':['stable','direct_write_attempt'],'rows':rows,'source_blobs':EXPECTED_BLOBS,'source_sha256':{p.name:sha(p) for p in [Path(__file__),HERE/'audit_v6_direct.py',HERE/'score_xlsx.py',SOURCE/'backend_x11.py',SOURCE/'office_backend.py'] if p.exists()}}; dump(args.out/'SUMMARY.json',summary); print(json.dumps({'allocation':summary['allocation'],'classifications':[r['classification'] for r in rows],'writer':rows[1]['writer'],'final_cells':[r['final_score']['cells'] for r in rows]},indent=2))
