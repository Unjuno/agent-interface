...
def update_one(model,opt,x,y,seen_rows,batch_rows):
    if not set(batch_rows).issubset(set(seen_rows)):
        raise RuntimeError("sampled unrevealed support row")
    # The sampler emits original support-row IDs, not offsets into seen_rows.
    bix=torch.tensor(batch_rows,dtype=torch.long,device=x.device)
    loss=nn.functional.cross_entropy(model(x[bix]),y[bix])
    opt.zero_grad(set_to_none=True);loss.backward();opt.step()
...
def main():
    ...
    out={"allocation":"needle-lora-3441-rank4-minibatch-seed-corrected-index-v1",
