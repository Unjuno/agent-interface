...
def producer_offer(c, cap, seq, eid, sha, policy):
    ...
    if policy == 'arrival_pos':
        pos=(rows[-1][0]+1) if rows else 1
    elif policy == 'sequence_pos':
        pos=seq
    else:
...
def ack_through(c, through, supplied_sha):
    ...
    frontier=max(c.execute('SELECT COALESCE(MAX(seq),0) FROM consumer').fetchone()[0],last)
    if through > frontier: return 'ACK_BEYOND_ACCEPTED'
