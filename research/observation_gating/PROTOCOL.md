...
Status: A1 revision 3 protocol, 2026-09-13 JST. Source and parameters are snapshotted
in `results/frozen-a1r3/freeze.json` before either new fresh replicate. Revisions 1
and 2 stopped on Inkscape baseline failures; they remain INVALID and are never
pooled with this evaluation. Their original protocols and source remain in
`frozen-a1/` and `frozen-a1r2/`.
...
- Revision 2 repairs a missed Inkscape precondition: after Ctrl+A, observe visible
  selector handles before dragging. Both strategies use this same guard. Validate
  on the failed seed as development, another four-pair/app screen and a separate
  12-pair Inkscape development screen. Keep revision 1's failed run.
- Also move Inkscape's visible-target startup wait into setup, before task timing,
  for consistency with Calc. The first measured document image is painted; no
  repeated startup-grey frames contribute to the primary reduction measure.
- Revision 3: visible selection alone did not fix a fresh O0 drag failure. Test
  a conservative 30 ms press-to-motion delivery dwell in development, with both
  pair orders already present during development. This is a local backend
  delivery policy; movement is still verified from public images. Neither the
  dwell nor the image gate can declare a drag successful merely because time
  elapsed. Validate 12 balanced Inkscape pairs and four balanced pairs/app before
  freezing. Old failing seeds can be regression tests, never fresh evidence.
  The revision 3 development seeds are 3201–3212 for Inkscape and 1401–1404 for
  the four-app screen; seed 290102 is a development regression in O1/O0 order.
- Fresh evaluation: two independently launched replicates, 12 pairs/app each,
  seeds 490101–490112 and 590101–590112, **24 pairs/app, 96 pairs total**. These seeds
...
- Local input parameters: 2 ms between text characters, 30 ms press-to-motion
  dwell (revised for this WSL environment), 4 ms between drag motion steps.
