...
def prepare(session, app, seed, chromium):
    ...
    setup_frames = 0
    if app == "calc":
        # Window mapped != worksheet painted. A blank grey client accepts input
        ...
        while True:
            ...
            setup_frames += 1
            arr = np.asarray(im)
            if np.mean(np.all(arr > 245, axis=2)) > .2 and im.getcolors(128) is None:
                break
            if time.perf_counter() >= deadline:
                raise TimeoutError("Painted worksheet readiness")
            time.sleep(.016)
