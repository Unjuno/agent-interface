"""Construction-only coverage for fresh-seed multi-skill CUDA allocation; no training."""
...
class Construction(unittest.TestCase):
 ...
 def test_fresh_seed_and_fixed_shapes(self):
  self.assertIn("SEED = 3914",self.text);self.assertNotIn("SEED = 3444",self.text)
  self.assertEqual(self.mod["BASE_STEPS"],400)
  self.assertEqual(self.mod["STEPS"],120);self.assertEqual(self.mod["RANK"],2)
 ...
 def test_dataset_reproducibility_and_cardinality(self):
  x1,y1,h1=self.mod["dataset"](32,3915)
  x2,y2,h2=self.mod["dataset"](32,3915)
 ...
 def test_full_module_state_roundtrip_and_rollback(self):
  core=self.mod["Core"](); model=self.mod["Adapter"](core,3925)
