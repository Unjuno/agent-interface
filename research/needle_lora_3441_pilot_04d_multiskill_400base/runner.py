"""Fresh-seed complete-evidence successor to Issue #3714; synthetic data only."""
...
BASE_STEPS, STEPS, BATCH, LR = 400, 120, 32, 0.04
EPOCH = 7
...
def fit(model, x, y, seed, steps=STEPS):
...
def main():
    ...
    base = Core().to(DEVICE)
    pretrain_ms = fit(base, xa, ya, SEED + 10, steps=BASE_STEPS)
    ...
    result = {
        "allocation": "needle-lora-3441-pilot-04d-multiskill-400base",
        ...
        "parameters": {"seed": SEED, "features": FEATURES, "hidden": HIDDEN,
                       ...
                       "base_updates": BASE_STEPS, "batch": BATCH, "lr": LR, "support_each": 16, "heldout_each": 4096},
