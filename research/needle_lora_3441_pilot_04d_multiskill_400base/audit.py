...
if r.get("allocation")!="needle-lora-3441-pilot-04d-multiskill-400base":errors.append("allocation")
if r.get("parameters",{}).get("base_updates")!=400:errors.append("base_updates")
...
decision="PASS_MULTI_SKILL_ROUTING_SCOPED" if not errors and quality else ("FAIL_MULTI_SKILL_INTERFERENCE" if not errors else "FAIL_ROUTE_OR_AUDIT_INTEGRITY")
print(json.dumps({"disposition":decision,"integrity":not errors,"errors":sorted(errors),"rows_recomputed":sum(len(stored.get(a,{}).get(k,{}).get("predicted",[])) for a in ("routed","shared") for k in specs),"routed_accuracy":m.get("routed_accuracy"),"shared_sequential_accuracy":m.get("shared_sequential_accuracy"),"quality_gate":quality},sort_keys=True,separators=(",",":")))
