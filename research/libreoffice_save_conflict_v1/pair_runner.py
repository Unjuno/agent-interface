...
ROOT=Path('/mnt/data/libreoffice_conflict_c254/scored-pair-02')
...
def run_arm(name,display_name,replace):
    ...
    ob=lo=None; b=None
    result={'arm':name,'replace':replace,'display':display_name,'initial_file':initial}
    try:
        ...
        if replace:
            ...
            result['replacement_source']=statrow(repl); os.replace(repl,xlsx)
            result['after_external_replace']={'file':statrow(xlsx),'monotonic_ns':time.monotonic_ns()}
        ...
        if not replace:
            ...
            if cells and cells.get('A1')=='external' and cells.get('A2')=='replacement' and cells.get('A3')=='external-marker':
                result['classification']='external_preserved_with_prompt' if (first_modals!=['Confirm File Format'] or second_modals) else 'external_preserved_no_conflict_prompt'
...
rows=[]
for spec in [('stable',':221',False),('replacement',':222',True)]:
    rows.append(run_arm(*spec))
summary={'schema':'agent-interface/libreoffice-save-conflict-pair-v1','allocation':'c254-pair-02','order':['stable','replacement'],'rows':rows,
