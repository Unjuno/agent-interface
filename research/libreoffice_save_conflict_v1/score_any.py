...
a=ap.parse_args(); p=a.xlsx
row={'exists':p.exists()}
if p.exists():
    b=p.read_bytes(); row.update({'sha256':hashlib.sha256(b).hexdigest(),'bytes':len(b)})
    try:
        wb=load_workbook(p,data_only=False,read_only=True); ws=wb.active
        row['cells']={k:ws[k].value for k in ('A1','A2','A3','B1')}; row['xlsx_read_ok']=True
    except Exception as e:
        row['xlsx_read_ok']=False; row['error']=repr(e)
