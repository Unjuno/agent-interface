...
def main():
    ...
    if pre.get("seed")!=284926 or pre.get("status")!="FROZEN_BEFORE_FORMAL_TASK": raise RuntimeError("STOP_BAD_FREEZE")
    ...
    mirror=RUN/"host-model-mirror"; pythonpath=":".join(pre["PYTHONPATH"])
    env={"ISSUE_2849_REPO":str(ROOT),"ISSUE_2849_RUN_ROOT":str(RUN),"ISSUE_2849_RUN_DIR":str(OUT),"ISSUE_2849_SEED":"284926",
      ...
      "AGENT_INTERFACE_DOCKER_IPC":str(RUN/"ipc"),"DOCKER_HOST":"unix:///var/run/docker.sock","PYTHONPATH":pythonpath,"PYTHONDONTWRITEBYTECODE":"1"}
    ...
    outer_cmd += [OUTER,str(HERE/"run_task1_v26.py")]
    ...
    try:
        proc=subprocess.run(outer_cmd,cwd=ROOT,capture_output=True,text=True,timeout=600); status="OUTER_RETURNED"; rc=proc.returncode; stdout=proc.stdout; stderr=proc.stderr
    ...
    result={"seed":284926,"status":status,"started_ns":started,"outer_returncode":rc,"broker_returncode":broker.returncode,
      "counts":{"requests":len(req),"responses":len(rsp),"broker_receipts":len(receipts)},"max_host_model_calls":1,"authority_granted":False}
