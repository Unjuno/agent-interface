 ...
 try:
  for i,(name,mode,delivery,effect,cleanup) in enumerate(CASES):
   ...
   if (out/'events.jsonl').exists():
    ...
    (out/'events.jsonl').write_text(json.dumps({'events': [], 'delivery': delivery, 'status': result.get('status') if isinstance(result,dict) else None, 'diagnostic': result.get('diagnostic') if isinstance(result,dict) else None, 'replay_allowed': False, 'replay_count': 0}, sort_keys=True)+'\\n', encoding='utf-8')
