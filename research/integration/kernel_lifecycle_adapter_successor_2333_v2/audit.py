...
def main():
    ...
    assert payload["schema"] == "agent-interface/kernel-lifecycle-adapter-v2"
    ...
    assert negative["malformed_receipt"]["rejected"]
    assert negative["missing_receipt"]["rejected"]
    assert negative["cleanup_failure"]["rejected"]
    print("KERNEL_LIFECYCLE_AUDIT_PASS positive=verified negative_rejections=7 source_blobs=4")
    return payload
