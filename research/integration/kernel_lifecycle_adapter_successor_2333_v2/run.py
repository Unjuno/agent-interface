...
def negative_checks():
    ...
    malformed = RequestLifecycle(); malformed.record_observation(obs); malformed.bind(binding); malformed.authorize(lease, now_ns=10); malformed.begin_execution(request, now_ns=11)
    try: malformed.record_execution(object())
    except (ContractError, AttributeError, TypeError) as error: results["malformed_receipt"] = {"rejected": True, "error": type(error).__name__}
    missing = RequestLifecycle()
    try: missing.outcome()
    except ContractError as error: results["missing_receipt"] = {"rejected": True, "error": str(error)}
    cleanup = RequestLifecycle(); cleanup.record_observation(obs); cleanup.bind(binding); cleanup.authorize(lease, now_ns=10)
    try: cleanup.stop("explicit_cleanup_failure", release=ReleaseReceipt(20, False, ("KEY",)))
    except ContractError as error: results["cleanup_failure"] = {"rejected": True, "error": str(error)}
    return results

def main():
    payload = {"schema":"agent-interface/kernel-lifecycle-adapter-v2","positive":positive(),"negative":negative_checks()}
