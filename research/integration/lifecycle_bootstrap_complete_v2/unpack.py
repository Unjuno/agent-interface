...
def restore(destination):
    ...
    check(meta['file_count'] == 100 and len(meta['files']) == 100, 'file count')
    check(meta['compressed_bytes'] == 24664 and meta['uncompressed_bytes'] == 474330, 'bounds')
    ...
    for i, item in enumerate(meta['parts'], 1):
        ...
        check(len(raw) <= 8501 and len(raw) == item['bytes'], 'part bytes')
    ...
    check(len(compressed) == 24664, 'compressed bytes')
    check(digest(compressed) == meta['compressed_sha256'], 'compressed digest')
    dec = lzma.LZMADecompressor(memlimit=128 * 1024 * 1024)
    expanded = dec.decompress(compressed, max_length=474331)
    check(dec.eof and not dec.unused_data and len(expanded) == 474330, 'expanded bounds')
    ...
    return {'files': len(prepared), 'bytes': sum(map(len, prepared.values())),
            'scientific_verdict': 'not_evaluated_by_unpacker', 'executed_experiment': False}
