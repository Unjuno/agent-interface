#!/usr/bin/env python3
"""Independent raw audit v2; relaxes only an unsupported Xvfb exit-code assumption."""
import hashlib
import json
...
def audit(evidence_dir):
    ...
    xvfb = summary.get("xvfb_cleanup", {})
    if xvfb.get("reaped") is not True or not isinstance(xvfb.get("exit_code"), int):
        raise ValueError("Xvfb process not reaped with recorded exit status")

    pixels, png_hashes, raw_hashes = {}, {}, {}
    for route in EXPECTED_ROUTES:
        ...
        cleanup = attempt.get("fixture_cleanup", {})
        if cleanup.get("reaped") is not True or not isinstance(cleanup.get("exit_code"), int):
        ...
        if row.get("input_dispatched") is not False or row.get("elapsed_ns", 0) <= 0:
            raise ValueError("input or clock boundary mismatch: "+route)
        req, fixture, capture = row.get("request", {}), row.get("fixture_window", {}), row.get("capture", {})
        ...
        capture_raw = observation.get("observation", {})
        artifact = capture_raw.get("artifact", {})
        raw_sha = capture_raw.get("sha256")
        ...
        png_hashes[route], raw_hashes[route] = row["png_sha256"], raw_sha
        if route == "mcp":
            mcp = row["raw_result"]
            ...
            if not mcp.get("server_pids_during_call"):
    ...
    if len(set(pixels.values())) != 1 or len(set(raw_hashes.values())) != 1:
        raise ValueError("decoded RGB pixels or raw X11 hashes differ")
    return {"audit": "PASS", "decision": "PASS_UNIT_ROUTE_EQUIVALENCE",
            ...
            "xvfb_exit_code_observed": xvfb["exit_code"],
            "audit_v2_note": "Requires reaped and recorded process, not a particular signal exit code."}
