...
from formal_session import DISPLAY, geom, launch, setup_xauthority
...
class ReadinessTests(unittest.TestCase):
    ...
    def test_xauthority_cookie_is_added_to_frozen_display(self):
        result=MagicMock(returncode=0,stderr="")
        with patch("formal_session.subprocess.run",return_value=result) as run:
            setup_xauthority({},"/tmp/session.Xauthority")
        self.assertEqual(run.call_args.args[0], ["xauth","-f","/tmp/session.Xauthority","add",DISPLAY,".","0123456789abcdef0123456789abcdef"])

    def test_xauthority_failure_stops_before_server_launch(self):
        result=MagicMock(returncode=1,stderr="cookie write denied")
        with patch("formal_session.subprocess.run",return_value=result):
            with self.assertRaisesRegex(RuntimeError,"STOP_XAUTH_COOKIE_SETUP"):
                setup_xauthority({},"/tmp/session.Xauthority")
