...
M=['missing_part','alter_part','reorder_parts','wrong_xz','wrong_member_hash','existing_destination']
...
def main():
 ...
 for m in M:
  with tempfile.TemporaryDirectory() as td:
   ...
   for name in ['restore_evidence.py','PARTS.json','COMPACT_EVIDENCE.json','EVIDENCE.json']:
   ...
   elif m=='wrong_member_hash':
    p=r/'EVIDENCE.json'; x=json.loads(p.read_text()); x['members'][0]['sha256']='0'*64; p.write_text(json.dumps(x,indent=2,sort_keys=True)+'\n')
