...
class Fixture:
    def __init__(self, state, duplicate=None):
        self.win = ROOT.create_window(20,20,520,160,0,D.screen().root_depth,
    ...
    def replace(self,state):
        old=self.xid; self.win.destroy(); D.sync(); self.__init__(state)
        return old,self.xid
...
def capture(fx, tag, region=(0,0,520,160)):
    ...
    return {"tag":tag,"before_ns":before,"after_ns":after,"api":row}

def main():
    cases=[]; oracle=[]; prior=None
    ...
    row=decide(artifact=obs.get("artifact",{}),phase="EFFECT_PENDING",binding=binding,intent_epoch="stale",
      captured_ns=obs.get("capture_ended_ns",raw["before_ns"]),now_ns=time.monotonic_ns(),
    ...
    row=decide(artifact=obs.get("artifact",{}),phase="EFFECT_PENDING",binding=newbinding,intent_epoch="replace",
      ...
      prior={"phase":"EFFECT_PENDING","state":(1,1,1,0),"mask":["E","S"],"binding":{"xid":old},"intent_epoch":"replace"})
    ...
    second=Fixture((1,1,1,0)); matches=[fx.xid,second.xid]
    row=decide(artifact={},phase="EFFECT_PENDING",binding=newbinding,intent_epoch="ambiguous",
      captured_ns=time.monotonic_ns(),now_ns=time.monotonic_ns(),ambiguous=(len(matches)!=1),prior=None)
    row.update({"case":"ambiguous_target","matching_xids":matches,"api_calls":0}); cases.append(row)
    oracle.append({"case":"ambiguous_target","matching_xids":matches,"api_calls":0})
    ...
    row=decide(artifact=obs.get("artifact",{}),phase="EFFECT_PENDING",binding=newbinding,intent_epoch="new-intent",
      captured_ns=obs.get("capture_ended_ns",raw["before_ns"]),now_ns=raw["after_ns"],observed_xid=obs.get("native_window_id"),
      prior={"phase":"EFFECT_PENDING","state":(1,1,1,0),"mask":["E","S"],"binding":newbinding,"intent_epoch":"old-intent"})
