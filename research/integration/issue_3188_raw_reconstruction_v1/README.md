...
- **H:** The immutable formal-02 raw truth table has exactly the declared 32 unique Boolean vectors, exactly one all-true `AUTHORIZE`, 31 `HOLD` vectors, and five named negative controls that all remain `HOLD`. The row count is 37. The raw artifact contains only vectors and controls, so this raw-only audit makes no current-snapshot classification.
...
- **U:** This finite audit validates one frozen truth table and the listed challenges only. The preregistration states an expected current class, but the archived raw file does not retain the separate current-gate field values needed to independently recompute that class; current snapshot remains outside this audit. It is not workflow/CI equivalence, MAP01 recovery efficacy, generalized verifier soundness, runtime safety, or production authority.
...
The preceding 7/7 native Python test result applies to the earlier revision, before the review correction, and is not claimed as a rerun for this commit. For this correction, the updated `audit_raw.py` was executed in a no-file-write Python 3.12 process against the exact raw bytes fetched from current main: `PASS_RAW_RECONSTRUCTION`, 32 vectors, 37 total rows, no `current_snapshot` field, and all five corruption challenges rejected.

The updated full unittest suite and `git diff --check` have not been rerun: the attached local C: volume has 0 bytes free and Docker Desktop is not responding. Treat this PR as not merge-ready until the complete native suite is rerun from a valid checkout. No Docker result is claimed.