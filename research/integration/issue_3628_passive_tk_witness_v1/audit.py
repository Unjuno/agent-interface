...
for allocation_path in allocations:
    ...
    passive = [event for event in event_rows
               if event.get('bindtag') == 'AgentInterfacePassiveAudit']
    passive_chord = [event.get('keysym') for event in passive
                     if event.get('keysym') in {'Control_L', 's'}]
    need(passive_chord == ['Control_L', 's'], f'{root.name}:passive-save-chord-exactly-once-ordered')
    passive_typed = ''.join(event.get('char', '') for event in passive)
    need(isinstance(marker, str) and marker in passive_typed,
         f'{root.name}:passive-fixture-received-exact-marker')
    need('Control_L' in keysyms and 's' in keysyms,
         f'{root.name}:legacy-fixture-received-save-chord-diagnostic')
