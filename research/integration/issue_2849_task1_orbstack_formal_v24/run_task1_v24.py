...
RUN=Path(os.environ["ISSUE_2849_RUN_ROOT"]).resolve()
OUT=Path(os.environ["ISSUE_2849_RUN_DIR"]).resolve()
MIRROR=Path(os.environ["AGENT_INTERFACE_DOCKER_SCHEMA"]).resolve().parent
SEED=int(os.environ["ISSUE_2849_SEED"])
for root in (V2,REPO,REPO/"research/live_control",REPO/"research/observation_tiles",REPO/"research/observation_gating",REPO/"research/real_apps_v1",REPO/"runtime"):
    sys.path.insert(0,str(root))
if SEED!=284924: raise RuntimeError("STOP_TASK_SEED_MISMATCH")
if not (OUT/"pre-call-source-manifest.json").is_file(): raise RuntimeError("STOP_SOURCE_FREEZE_MISSING")
if not MIRROR.is_dir() or not (MIRROR/"workspace").is_dir(): raise RuntimeError("STOP_BROKER_MIRROR_MISSING")
if os.environ.get("DOCKER_HOST")!="unix:///var/run/docker.sock": raise RuntimeError("STOP_NESTED_DOCKER_SOCKET")
...
if docker_version!="29.4.0 linux/arm64": raise RuntimeError("STOP_NESTED_DOCKER_DAEMON:"+docker_version)
...
def mirror_call(root,prompt,image,contract,workspace):
    global CALLS
    CALLS+=1
    if CALLS!=1: raise RuntimeError("STOP_MODEL_CALL_LIMIT_EXCEEDED_NO_RETRY")
    source=Path(image).resolve()
    if not source.is_file(): raise FileNotFoundError("STOP_CURRENT_SCREENSHOT_MISSING")
    digest=hashlib.sha256(source.read_bytes()).hexdigest(); destination=MIRROR/"image.png"
...
namespace=runpy.run_path(str(V2/"run_task1_smoke.py"),run_name="issue_2849_task1_v24")
main_globals=namespace["main"].__globals__
if main_globals.get("docker_model_call") is not selected_call: raise RuntimeError("STOP_BASE_CALLBACK_BINDING")
