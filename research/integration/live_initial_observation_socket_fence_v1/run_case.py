...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--case-id',required=True);ap.add_argument('--policy',choices=['endpoint_only','initial_observation_gate'],required=True)
    ...
    candidate_src=a.experiment_dir/'event_socket_initial_observation_gate_v1.py';candidate_exec=live/'event_socket_initial_observation_gate_v1.py'
    if a.policy=='initial_observation_gate':
    ...
    if a.policy=='initial_observation_gate':env['AI_INITIAL_OBSERVATION_TRACE']=str(trace)
    ...
    try:
        ...
        if proc.poll() is None:
            ...
            proc.wait(timeout=3)
        if candidate_exec.exists() and a.policy=='initial_observation_gate':candidate_exec.unlink()
    ...
    result={
        ...
        'initial_trace':json.loads(trace.read_text()) if trace.exists() else None,'stderr':stderr,'stdout_tail':stdout_tail,
