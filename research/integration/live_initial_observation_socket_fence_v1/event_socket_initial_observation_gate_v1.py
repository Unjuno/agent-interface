...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='agent-interface-events-') as private:
        ...
        with BoundedServer(str(socket_path),Handler) as server:
            ...
            initial_ready=threading.Event();initial_trace={}
            def consume():
                try:
                    for line in process.stdout:
                        record=json.loads(line);cursor.append(record)
                        if (record.get('event')=='observation' and record.get('id')=='initial' and record.get('exact') is True):
                            initial_trace['initial_append_ns']=time.perf_counter_ns();initial_ready.set()
                except Exception as exc:failures.append(repr(exc))
                finally:cursor.close()
            reader=threading.Thread(target=consume,daemon=True);reader.start()
            if not initial_ready.wait(8):
                process.terminate();process.wait(timeout=2);reader.join(timeout=2)
                raise RuntimeError('existing initial observation event not observed')
            initial_trace['endpoint_publish_ns']=time.perf_counter_ns()
            trace_path=os.environ.get('AI_INITIAL_OBSERVATION_TRACE')
            if trace_path:Path(trace_path).write_text(json.dumps(initial_trace,sort_keys=True)+'\n')
