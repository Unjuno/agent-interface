...
OUT = HERE / "evidence/formal-seed-284936/run-01"
...
def main():
    ...
    base_commit=subprocess.run(["git","-C",str(ROOT),"rev-parse","HEAD"],capture_output=True,
        text=True,check=True,timeout=10).stdout.strip()
    if base_commit!=lock["base_commit"]: raise SystemExit("STOP_BASE_COMMIT:"+base_commit)
    ...
    smoke=json.loads((HERE/"evidence/runner-smoke-v4/independent-audit.json").read_text())
    ...
    startup=json.loads((HERE/"evidence/runtime-startup-smoke-v2/independent-audit.json").read_text())
    ...
    write(OUT/"environment-gate.json",{"host_codex_sha256":sha(CODEX),"host_codex_version":version,
        ...
        "pre_calls_pass":True,"started_ns":time.time_ns()})
    ...
    write(formal_out/"preregistration.json", {"schema":"integrated_efficiency_preregistration_v1",
        "status":"frozen_before_live_calls","study":"issue-3873-orbstack-live-01",
        "seed":284936,"arm_order":["plain","ephemeral","persistent"],
    ...
    write(OUT / "outer-command.json", {"command": command, "seed": 284936,
