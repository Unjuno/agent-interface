...
class ReceiptCompositionTests(unittest.TestCase):
    ...
    def test_missing_effect_refuses(self):
        row=compose(admission=ADMISSION,source=SOURCE,dispatch=None,effect=None,current=CURRENT)
        self.assertEqual(row["decision"],"REFUSE")
        self.assertIn("EFFECT_RECEIPT_MISSING",row["reasons"])
