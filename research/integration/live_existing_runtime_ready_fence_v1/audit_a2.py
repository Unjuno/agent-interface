...
def main():
    ...
    errors = []
    details = []
    allocation = json.loads((args.formal / "A2_ALLOCATION.json").read_text())
    if allocation != {"formal_allocation": 1, "outer_case_invocations_expected": 6, "formal_reruns": 0}:
        errors.append("A2 allocation marker")
    invocation_rows = load_jsonl(args.formal / "A2_INVOCATIONS.jsonl")
    if len(invocation_rows) != 6:
        errors.append("A2 outer invocation count")
    if len({row.get("case_id") for row in invocation_rows}) != len(invocation_rows):
        errors.append("A2 repeated case id")
    if any(row.get("status") != "complete" for row in invocation_rows):
        errors.append("A2 incomplete outer invocation")

    schedule = json.loads((args.experiment_dir / "schedule_a2.json").read_text())
    ...
    freeze = json.loads((args.experiment_dir / "A2_FREEZE.json").read_text())
    ...
    output = {
        "task": "LIVE-EXISTING-RUNTIME-READY-FENCE-20260917-002",
        "decision": decision,
        "errors": errors,
        "details": details,
        "formal_allocation": allocation.get("formal_allocation"),
        "outer_case_invocations": len(invocation_rows),
        "formal_reruns": 0,
        "core_blobs": core_blobs,
    }
    (args.formal / "AUDIT_A2.json").write_text(json.dumps(output, indent=2, sort_keys=True) + "\n")
