"""Scoped experiment: gate observation-socket publication on the first parsed producer record.

This is an additive #802 experiment. It preserves EventCursor/read semantics and changes only
when the observation endpoint is advertised. It grants no authority.
"""
import argparse,json,socket,socketserver,subprocess,sys,tempfile,threading,time
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='agent-interface-events-') as private:
        ...
        with BoundedServer(str(socket_path),Handler) as server:
            ...
            first_record=threading.Event();first_append_ns=[]
            def consume():
                try:
                    for line in process.stdout:
                        record=json.loads(line)
                        cursor.append(record)
                        if not first_record.is_set():
                            first_append_ns.append(time.perf_counter_ns());first_record.set()
                except Exception as exc:failures.append(repr(exc))
                finally:cursor.close()
            reader=threading.Thread(target=consume,daemon=True);reader.start()
            ready=first_record.wait(READINESS_TIMEOUT_S)
            published_ns=time.perf_counter_ns()
            if ready:
                print(json.dumps(dict(event='observation_socket',socket=str(socket_path),cancel_socket=str(cancel_path),input='optional once-only command forwarding; ordinary runtime admission',authority='no lease renewal; session-local deduplication',readiness='first_record_appended',first_record_appended_ns=first_append_ns[0],endpoint_published_ns=published_ns)),flush=True)
            else:
                print(json.dumps(dict(event='observation_socket_unready',socket=None,cancel_socket=None,authority='none',readiness='bounded_timeout',endpoint_published_ns=published_ns)),flush=True)
