...
def run_case(outdir:Path,case_id:str,policy:str,scope:str,producer_mode='effect'):
    ...
    server='event_socket_v11.py' if policy=='endpoint_only' else 'event_socket_first_record_gate_v1.py'
    args=[sys.executable,'-u',str(UP/server),'serve','--',producer_mode,action_id,request_id,str(telemetry),str(PRODUCER_DELAY_S)]
    ...
    if socket_record.get('event')=='observation_socket':
        ...
        with socket.socket(socket.AF_UNIX,socket.SOCK_STREAM) as s:
            s.settimeout(2);s.connect(socket_record['socket']);read_started_ns=time.perf_counter_ns();s.sendall((json.dumps(req)+'\n').encode());response=recv_line(s);read_returned_ns=time.perf_counter_ns()
    else:
        read_started_ns=read_returned_ns=None
    ...
    row={
        'case_id':case_id,'policy':policy,'scope':scope,'producer_mode':producer_mode,'case_started_ns':started_ns,'endpoint_wait_ns':endpoint_received_ns-started_ns,
        'read_timeout_s':READ_TIMEOUT_S,'producer_delay_s':PRODUCER_DELAY_S,
        'socket_event':socket_record.get('event'),'socket_authority':socket_record.get('authority'),
        'socket_readiness':socket_record.get('readiness'),'first_record_appended_ns':socket_record.get('first_record_appended_ns'),
        ...
        'record_count':len(records),'record':records[-1] if records else None,
