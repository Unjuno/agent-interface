...
def sha(p):
 ...
 return h.hexdigest()
def audit(root: Path):
 e=[];required=['RESULT.json','asset_manifest.json','acquisition.txt','ready.txt','oracle.json','windows.txt','mindustry.stdout.txt','mindustry.stderr.txt','xvfb.stdout.txt','xvfb.stderr.txt','openbox.stdout.txt','openbox.stderr.txt']
 for n in required:
  if not (root/n).is_file():e.append('missing:'+n)
 if e:return {'passed':False,'errors':e}
 r=json.loads((root/'RESULT.json').read_text());a=json.loads((root/'asset_manifest.json').read_text());o=json.loads((root/'oracle.json').read_text())
 checks={'formal_once':r.get('formal_invocations')==1,'outcome':r.get('outcome')==EXPECTED_OUTCOME,
  'jar':a.get('jar',{}).get('bytes')==87022576 and a.get('jar',{}).get('sha256')==JAR_SHA,
  ...
  'oracle_scalar':o.get('width')==300 and o.get('height')==250 and o.get('paused') is True and o.get('core_present') is True and o.get('copper')==200,
  ...
  'zero_authority':all(r.get(k)==0 for k in ('model_calls','provider_calls','controller_calls','task_input_calls')),
  ...
  'ordering':isinstance(r.get('started_ns'),int) and isinstance(r.get('ready_ns'),int) and isinstance(r.get('finished_ns'),int) and r['started_ns']<r['ready_ns']<r['finished_ns']}
 e += [k for k,v in checks.items() if not v];hashes={n:sha(root/n) for n in required}
 return {'passed':not e,'errors':e,'checks':checks,'file_sha256':hashes,'outcome':r.get('outcome')}
def main():
 ap=argparse.ArgumentParser();ap.add_argument('root',type=Path);ap.add_argument('--out',type=Path);a=ap.parse_args();res=audit(a.root)
