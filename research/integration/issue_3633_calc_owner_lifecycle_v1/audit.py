...
def classify(raw):
    if raw.get("allocation_id") != "issue3644-calc-owner-lifecycle-formal-01":
...
def main(raw_path, out_path):
    ...
    for rel, expected in manifest.get("files", {}).items():
        try:
            rel_path = Path(rel)
            source_path = (repo_root / rel_path.relative_to("research/integration/issue_3633_calc_owner_lifecycle_v1")
