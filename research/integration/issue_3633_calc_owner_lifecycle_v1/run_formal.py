...
DISPLAY = ":143"
ALLOC = "issue3644-calc-owner-lifecycle-formal-01"
...
def main(outdir: str) -> int:
    ...
    freeze = json.loads((source_dir / "FREEZE-v2.json").read_text())
    ...
    for rel, expected in source_manifest.get("files", {}).items():
        rel_path = Path(rel)
        source_path = (manifest_root / rel_path.relative_to("research/integration/issue_3633_calc_owner_lifecycle_v1")
    ...
    root = Path(tempfile.mkdtemp(prefix="issue3644-calc-life-"))
