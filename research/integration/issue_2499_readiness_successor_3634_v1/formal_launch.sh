...
docker run "${common[@]}" -v "${formal_output}:/evidence:rw" -e EVIDENCE_PATH=/evidence/result.json \
  --entrypoint /usr/bin/python3 "${image_id}" -B /src/formal_session.py || formal_status=$?
[[ -s "${formal_output}/result.json" ]]
...
[[ "${formal_status}" == 0 || "$(jq -r '.decision' "${formal_output}/result.json")" == STOP_MIXED_APP_READINESS_SUCCESSOR ]]
