...
- Docker Desktop was not used. Initial harness attempts exposed WSL's `/tmp/.X11-unix` mount as read-only with mode 0777: its server created an abstract X socket, so filesystem-socket polling was the wrong readiness condition. An early post-action harness assertion also read the cleanup receipt at the wrong nesting level; the saved fixture effect, returned image and execution had succeeded, but the harness marked the run failed until that evidence path was corrected. The final harness probes the selected display with `xdpyinfo` and reads the retained receipt correctly. These were harness/setup failures, not runtime failure evidence.
