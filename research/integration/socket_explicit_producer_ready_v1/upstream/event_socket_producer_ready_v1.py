...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='agent-interface-events-') as private:
        ...
        with BoundedServer(str(socket_path),Handler) as server:
            ...
            producer_ready=threading.Event();ready_append_ns=[]
            def consume():
                try:
                    for line in process.stdout:
                        record=json.loads(line)
                        cursor.append(record)
                        if (record.get('event')=='producer_ready' and record.get('authority')=='none' and not producer_ready.is_set()):
                            ready_append_ns.append(time.perf_counter_ns());producer_ready.set()
                except Exception as exc:failures.append(repr(exc))
                finally:cursor.close()
            reader=threading.Thread(target=consume,daemon=True);reader.start()
            ready=producer_ready.wait(READINESS_TIMEOUT_S)
            published_ns=time.perf_counter_ns()
            if ready:
                print(json.dumps(dict(event='observation_socket',socket=str(socket_path),cancel_socket=str(cancel_path),input='optional once-only command forwarding; ordinary runtime admission',authority='no lease renewal; session-local deduplication',readiness='producer_ready_appended',producer_ready_appended_ns=ready_append_ns[0],endpoint_published_ns=published_ns)),flush=True)
            else:
