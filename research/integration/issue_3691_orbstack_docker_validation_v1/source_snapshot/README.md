...
No Docker PASS is claimed. After the local Docker engine is available, run the tests in a network-disabled container with read-only source, then run the CLI in a second fresh container. Preserve this construction PASS and Docker STOP as separate outcomes.