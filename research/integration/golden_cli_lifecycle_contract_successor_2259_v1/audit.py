...
def main():
    with (ROOT / "RESULT.json").open(encoding="utf-8") as f:
        result = json.load(f)
    with (ROOT / "SOURCE_MANIFEST.json").open(encoding="utf-8") as f:
    ...
    assert all(
        row["task_success_distinct"]
        and row["partial_effects_representable"]
        and row["unknown_fails_closed"]
