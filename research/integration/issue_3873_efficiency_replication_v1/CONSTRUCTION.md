...
6. The first frozen launcher invocation stopped before creating the formal output directory because its import-closure container command used a host absolute path instead of the container mount path. It made zero model calls; the exact STOP is `evidence/formal-launch-precall-stop-01.json`. This execution-path correction is documented and hash-frozen before any formal allocation.
