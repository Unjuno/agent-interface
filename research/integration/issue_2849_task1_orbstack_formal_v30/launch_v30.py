...
ROOT=Path(__file__).resolve().parents[3]; HERE=Path(__file__).resolve().parent; RUN=HERE/"evidence/task1-seed-284930"; OUT=RUN/"task-run"
OUTER="sha256:436172d89b145c6a9f9a57e655422c9558b3b0235347dd77607e9d61bcfa6393"; INNER="sha256:5cc4d237e6af4548147ddfffc35413faf2487fd585f6a0216221f153f61cf073"; SOCKET=Path("/Users/taka/.orbstack/run/docker.sock")
def sha(p): return hashlib.sha256(p.read_bytes()).hexdigest()
def main():
    pre=json.loads((OUT/"pre-call-source-manifest.json").read_text())
    if pre.get("seed")!=284930 or pre.get("status")!="FROZEN_BEFORE_FORMAL_TASK": raise RuntimeError("STOP_BAD_FREEZE")
    ...
    for image,expected in ((OUTER,pre["outer_image"]),(INNER,pre["nested_image"])):
        got=subprocess.check_output(["docker","--context","orbstack","image","inspect",image,"--format","{{.Id}} {{.Architecture}}"],text=True).strip()
        if got!=expected: raise RuntimeError("STOP_IMAGE_IDENTITY")
    mirror=RUN/"host-model-mirror"; py=":".join(pre["PYTHONPATH"])
    env={"ISSUE_2849_REPO":str(ROOT),"ISSUE_2849_RUN_ROOT":str(RUN),"ISSUE_2849_RUN_DIR":str(OUT),"ISSUE_2849_SEED":"284930",
      "AGENT_INTERFACE_MODEL_BACKEND":"module","AGENT_INTERFACE_MODEL_CALL_MODULE":"docker_model_call_backend_v1",
      "AGENT_INTERFACE_DOCKER_RUNNER":str(mirror/"runner.py"),"AGENT_INTERFACE_DOCKER_SCHEMA":str(mirror/"schema.json"),"AGENT_INTERFACE_DOCKER_INSTRUCTIONS":str(mirror/"instructions.txt"),
      "AGENT_INTERFACE_DOCKER_IMAGE":INNER,"AGENT_INTERFACE_DOCKER_IPC":str(RUN/"ipc"),"DOCKER_HOST":"unix:///var/run/docker.sock","PYTHONPATH":py,"PYTHONDONTWRITEBYTECODE":"1"}
    ...
    outer_cmd += [OUTER,str(HERE/"run_task1_v30.py")]
    (RUN/"launch-command.json").write_text(json.dumps({"outer":outer_cmd,"broker":broker_cmd,"max_model_calls":1,"no_retry":True,"authority_granted":False},indent=2)+"\n")
    ...
    result={"seed":284930,"status":status,"started_ns":started,"outer_returncode":rc,"broker_returncode":broker.returncode,"counts":{"requests":len(req),"responses":len(rsp),"broker_receipts":len(br)},"max_host_model_calls":1,"authority_granted":False}
