...
- A later wrapper-only construction check stopped because it did not start the required X server; no formal allocation began. The wrapper was corrected to start isolated Xvfb and then returned `PASS_READINESS_CONSTRUCTION` with the same three distinct role windows and zero input/model/network calls.
