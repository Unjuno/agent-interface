...
def audit(root, mode):
    ...
    try:
        ...
        require(exact(terminal, {'status': 'COMPLETE', 'rows': len(plan),
                                 'batch_count': len(plan)//2, 'mode': mode}), 'TERMINAL')
        rows = load(root/'ROWS.json')
        require(len(rows) == len(plan), 'DENOMINATOR')
        require(not (root/'STOP.json').exists(), 'STOP_PRESENT')
        for batch_index in range(len(plan)//2):
            label = f'BATCH-{batch_index:02d}'
            consumed = load(root/(label+'.CONSUMED.json'))
            done = load(root/(label+'.DONE.json'))
            external = load(root/(label+'.EXECUTION.json'))
            launch = load(root/f'LAUNCH-{batch_index:02d}.json')
            def prefix_hash(prefix):
                return digest((json.dumps(prefix, sort_keys=True, indent=2)+'\n').encode())
            require(exact(done, {'batch': batch_index, 'range': [2*batch_index, 2*batch_index+2],
                                  'pid': consumed['pid'], 'mode': mode, 'status': 'COMPLETE',
                                  'prefix_count': 2*batch_index+2,
                                  'prefix_sha256': prefix_hash(rows[:2*batch_index+2])}), label+'DONE')
            require(exact(consumed['batch'], batch_index) and consumed['mode'] == mode,
                    label+'CONSUMED_ID')
            require(exact(consumed['range'], [2*batch_index, 2*batch_index+2]), label+'RANGE')
            require(type(consumed['pid']) is int and consumed['pid'] > 0, label+'PID')
            require(consumed['prior_prefix_sha256'] == prefix_hash(rows[:2*batch_index]), label+'PRIOR')
            require(consumed['freeze_sha256'] == launch['freeze_sha256'], label+'FREEZE')
            if batch_index == 0:
                require(start['pid'] == consumed['pid'], label+'START_PID')
            command = external['command']
            require(exact(command, launch['command']), label+'COMMAND')
            require(Path(command[2]).name == 'batch.py' and command[1] == '-B', label+'ENTRY')
            require(command[3] == '--out' and command[5:9] ==
                    ['--index', str(batch_index), '--mode', mode] and command[9] == '--freeze', label+'ARGS')
            require(exact(command[2:], consumed['argv']), label+'ACTUAL_ARGV')
            require(exact(launch['timeout_s'], 25) and exact(launch['batch'], batch_index), label+'LAUNCH')
            require(exact(external['batch'], batch_index) and exact(external['exit'], 0)
                    and external['timed_out'] is False, label+'EXTERNAL_EXIT')
            require(type(external['started_ns']) is int and type(external['finished_ns']) is int
                    and external['finished_ns'] >= external['started_ns'], label+'CLOCK')
            stdout = (root/(label+'.stdout')).read_bytes()
            stderr = (root/(label+'.stderr')).read_bytes()
            require(digest(stdout) == external['stdout_sha256'] and
                    digest(stderr) == external['stderr_sha256'], label+'CHANNEL_HASH')
            require(stderr == b'' and stdout.endswith(b'\n') and exact(json.loads(stdout), done), label+'CHANNEL')
