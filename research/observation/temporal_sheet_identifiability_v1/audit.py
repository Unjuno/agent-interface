...
def evaluate(data):
    ...
    if errors: dec='FAIL_INTEGRITY'
    elif admissible: dec='PASS_RETAINED_TEMPORAL_SHEET_BENEFIT_IDENTIFIABLE_SCOPED'
    else: dec='PASS_RETAINED_TEMPORAL_SHEET_BENEFIT_NOT_IDENTIFIABLE_SCOPED'
    return {'decision':dec,'pass':not errors,'errors':errors,'metrics':{'families':len(fam),'candidate_pairs':len(pairs),'admissible_pairs':len(admissible),'admissible_ids':admissible}}
