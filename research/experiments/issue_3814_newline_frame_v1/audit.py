#!/usr/bin/env python3
"""Independent raw-only auditor for Issue #3814; imports no project modules."""
import base64
import hashlib
import json
import os
...
def main():
    ...
    errors = []
    if doc.get("schema") != "issue-3814/newline-frame-raw-v1":
    ...
    if not isinstance(recorded_sources, list) or not recorded_sources:
        ...
        for row in recorded_sources:
            try:
                rel = row["path"]
                if rel in seen or not rel.startswith(("runtime/cli_v1/", "runtime/motor_state_v1/", "runtime/selector_v1/")):
    ...
    experiment = source / "research/experiments/issue_3814_newline_frame_v1"
    ...
    if (occupied.get("producer_exit") != 2 or occupied.get("dispatch_calls") != 0
            ...
            or not any(row.get("path") == "sentinel" for row in occupied.get("files", []))):
    ...
    result = {"schema": "issue-3814/independent-audit-v1", "raw_sha256": sha(raw_bytes),
