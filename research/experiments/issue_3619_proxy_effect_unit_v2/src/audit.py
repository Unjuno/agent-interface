...
def validate(raw, check_files=True):
    ...
    for r in rows:
        ...
        if case=="ambiguous" and r.get("current_state",{}).get("status")!="ambiguous": errors.append(f"{label}: ambiguity not established")
...
def main():
    ...
    errors=validate(raw)
    if raw.get("allocation_id")!="issue3610-proxy-effect-unit-formal-02" or raw.get("issue")!=3619: errors.append("allocation identity mismatch")
