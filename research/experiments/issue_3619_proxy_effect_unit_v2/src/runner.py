...
def state(conn, rowdir, label, pid_hint=None):
    ...
    if len(wins) > 1: return {"status": "ambiguous", "count": len(wins)}
    win, title = wins[0]
    parts = title.split(":")
...
def run_row(arm, case):
    ...
    try:
        ...
        if case == "stale_version":
            ...
            wait_state(conn, rowdir, "duplicate", pid_hint=app.pid)
        ...
        if conn is not None:
            try:
                query = conn.query_pointer(conn.screen().root)
                row["release_verified"] = not bool(query.mask & X.Button1Mask)
...
def main():
    ...
    result={"allocation_id":"issue3610-proxy-effect-unit-formal-02","issue":3619,
