...
with tempfile.TemporaryDirectory(prefix="issue3631-preflight-") as td:
    ...
    try:
        ...
        result={"preflight":"PASS" if title=="proxy-fixture:1:2" and any(e.get("kind")=="effect" for e in events) and not (release.mask & X.Button1Mask) else "STOP",
                "title":title,"events":events,"xres_pid":pids[0],"derived_coordinate":[x,y],
