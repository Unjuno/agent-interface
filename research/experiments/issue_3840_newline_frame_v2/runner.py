#!/usr/bin/env python3
"""One-shot newline-framing boundary experiment for Issue #3840."""
import base64
import hashlib
import io
import json
...
BASE_COMMIT = "5842cea6b16dde275f79caef3f09fdba828112e0"
...
def main():
    ...
    if os.environ.get("PYTHONPATH") != "/source":
        raise SystemExit("STOP_PYTHONPATH_MISMATCH")
    runner_sha = sha((root / "research/experiments/issue_3840_newline_frame_v2/runner.py").read_bytes())
    auditor_sha = sha((root / "research/experiments/issue_3840_newline_frame_v2/audit.py").read_bytes())
    ...
    rows = []
    with tempfile.TemporaryDirectory(prefix="issue3840-") as temporary:
    ...
    raw_doc = {
        "schema": "issue-3840/newline-frame-raw-v2", "allocation": "issue3814-jsonl-frame-02",
