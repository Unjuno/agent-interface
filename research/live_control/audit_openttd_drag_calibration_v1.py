"""Audit exact programs, independent effects and releases for drag calibration."""
import hashlib
import json
...
def main():
    ...
    assert overall['model_calls'] == 0 and overall['source_sha256'] == sha(HERE / 'probe_openttd_drag_calibration_v1.py')
    rows = []
    for offset in prereg['offsets_y']:
        ...
        assert evaluation == read(trial / 'runtime/evaluation.json')
        ...
        drags = [e for e in events if e.get('event') == 'step_started' and e.get('operation') == 'pointer_drag']
        assert len(drags) == 1
        ...
        rows.append({'offset_y': offset, 'points': result['points'],
                     'target_road_ids': [t['id'] for t in evaluation['observation']['tiles'] if t['id'] in prereg['target_tiles'] and t['road']],
