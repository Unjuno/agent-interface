...
H=Path(__file__).resolve().parent;R=H/'results/recovery-view-controls-01';R.mkdir(exist_ok=False)
...
for case in ['timeout','closed','transport_error','conflicting_echo','matched','no_pending']:
 with tempfile.TemporaryDirectory() as td:
  ...
  if error is None:
   ...
   assert view['new_input']==('requires_fresh_admission' if case=='matched' else 'blocked')
   assert view['task_completion']=='not_scored_here'
  rows.append({'case':case,'read_transport_calls':len(seen),'error':error,'view':result['recovery'] if error is None else None,'pending_retained':after['pending'] is not None})
...
source=read(S/'recovered.json');before=copy.deepcopy(source);view=present(source)
assert source==before and view['status']=='resolved'
view['observation']['sequence']=-1
assert source==before
wrong=copy.deepcopy(source);wrong['request']['action_id']='different-action'
v=present(wrong);assert v['status']=='no_pending' and v['resolution_for_read_action'] is None and v['last_resolution_historical'] is not None
both=copy.deepcopy(source);both['state']['continuation']['channel_closed']=True
v=present(both);assert v['status']=='resolved' and v['channel']=='closed' and v['new_input']=='blocked'
(R/'presentation-controls.json').write_text(json.dumps({'input_unchanged':True,'output_not_aliased':True,'old_resolution_not_current':True,'resolved_and_closed_distinct':True},indent=2)+'\n',encoding='utf-8')
