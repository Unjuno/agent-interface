...
root=HERE/'results/model-context-01';root.mkdir(exist_ok=False)
...
plan={'order':['builtin','responder','responder','builtin'],'model':'gpt-5.6-luna','effort':'low',
      'image_sha256':hashlib.sha256(image.read_bytes()).hexdigest(),'prompt_sha256':hashlib.sha256(prompt.read_bytes()).hexdigest(),
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['probe_model_context_v1.py','model_context_runner_v1.py','screenshot_responder_v1.txt']},
      ...
      'scope':'archived actual Calc dialog screenshot, identical prompt; inference only, no new GUI input; builtin versus constrained responder instructions; default mode both arms; full transmitted context unavailable'}
(root/'plan.json').write_text(json.dumps(plan,indent=2)+'\n',encoding='utf-8')
runs=[]
for n,tier in enumerate(plan['order'],1):
    out=root/f'{n}-{tier}';begin=time.perf_counter_ns()
    result=subprocess.run([sys.executable,str(HERE/'model_context_runner_v1.py'),r'C:\Program Files\nodejs\node.exe',
    ...
    runs.append({'index':n,'instruction_mode':tier,'exit_code':result.returncode,'begin_ns':begin,'end_ns':time.perf_counter_ns()})
