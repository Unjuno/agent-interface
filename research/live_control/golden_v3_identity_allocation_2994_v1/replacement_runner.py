...
def main():
    ...
    try:
        ...
        summary={'schema':'golden_v3_identity_fail_closed_result_v1','allocation':'golden-v3-identity-20260920-a1','display':display,'rows':{'useful_control':{'window_id':xid,'result':control},'target_replacement':{'old_window_id':oldid,'replacement_window_id':newid,'result':replacement},'source_mismatch':{'window_id':sid,'requested_binding_revision':999,'current_binding_revision':1,'result':mismatch}},'decision':'PASS_GOLDEN_V3_IDENTITY_FAIL_CLOSED_SCOPED' if control.get('status') in ('success','partial') and replacement.get('status')=='refused' and mismatch.get('status')=='refused' else 'HOLD_IDENTITY_FAIL_CLOSED_NOT_PROVEN'}
