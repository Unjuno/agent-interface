...
from visual_anchor_v2 import VisualAnchor
from visual_anchor_v4 import VisualAnchor as TrackingAnchor
...
class PatchServo:
    def __init__(self,image,box,observation_id,frame_id,target_delta,max_updates=3,max_step=24):
        ...
        source=self.anchor.locate(image,observation_id,frame_id,allow_source=True)
        if source['status']!='matched' or source['rival_error']<=self.anchor.max_error:raise ValueError('source patch has plausible competing match')
        self.anchor=TrackingAnchor(image,box,observation_id,frame_id)
