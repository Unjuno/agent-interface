...
before=len(seen)
try:request_once('different-socket',state,spec,exchange)
except ValueError:pass
else:raise AssertionError('cross-session call allowed')
assert len(seen)==before
out=HERE/'results/received-exchange-02';out.mkdir(exist_ok=False)
(out/'report.json').write_text(json.dumps({'scope':'injected recorded replies through actual caller function, no network','cross_session_transport_calls':0,'requests':seen,'first_matched_clock':one['matched_clock'],'final_matched_clock':two['matched_clock'],'image_sequence':two['continuation']['observation']['sequence']},indent=2)+'\n');print('Recorded stale-clock caller replay: one command, one read-only followup, current clock matched; image retained.')
