"""Pre-action transparent OpenTTD supervisor with bounded post-drag effect evidence."""
...
base=HERE/'results/timing-envelope-openttd-l-07';root=base/arm;control=base/(arm+'-control');control.mkdir(parents=True,exist_ok=False)
def dump(path,value):
...
plan={'scope':'fresh seed-991003 OpenTTD five-tile L objective after pre-action transparency, with bounded post-drag effect evidence','arm':arm,
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['timing_envelope_openttd_l_supervisor_v7.py','timing_envelope_openttd_l_driver_v5.py','timing_envelope_v1.py','timing_envelope_v2.py','semantic_checkpoint_v4.py','openttd_finish_outcome_v1.py','model_pair_runner_v2.py','session_v22.py','openttd_effect_sheet_v1.py','openttd_contact_sheet_v1.py','openttd_drag_effect_v1.py']},
      'max_turns':12,'seed_semantics':'byte-pinned seed-991003 save; target tiles 977,978,979,1043,1107; closed toolbar',
      'model_route':'all gpt-6-astra medium','prompt_policy':'v6 prompt plus labeled bounded before/after/difference crops after each drag','initial_ui_state':'closed toolbar; interface-tracked transparent trees; signs A, B corner, C and X keep clear'}
...
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_l_driver_v5.py'),arm,'timing-envelope-openttd-l-07'],stdout=(control/'driver-stdout.txt').open('wb'),stderr=(control/'driver-stderr.txt').open('wb'))
...
try:
    ...
    task=ready['task'];observation=ready['observation'];previous=None;planner_image=None;exploration_history=[];checkpoint_required_turn=None
    for index in range(1,13):
        ...
        prompt=f'''You control an isolated OpenTTD road fixture using its current screenshot. Task: {task} OpenTTD signs are floating text labels attached to map squares. Treat the visible A/B/C/X label boxes as annotations; for construction, target the underlying isometric tile surface below the label box rather than the text-box center. Choose the next bounded visual action. Use the road-construction toolbar and straight-road tool when visible. This objective requires two connected directional segments sharing the B corner; do not treat one straight segment as completion. Treat tooltip purpose and target noun as an action guard: Display/list is a status window, and Build tramways is not Build roads. Do not click either for this task. Continue until visible controls specifically indicate road construction or Build roads. If toolbar meanings are uncertain, inspect up to three distinct icons in one act using repeated pointer_move + dwell_observe pairs around 800ms. Spread probes across unexplored toolbar regions. The next planner image preserves intermediate tooltips as labeled strips or, after a drag, appends labeled before/after/absolute-difference crops for the drag path. Do not revisit an inspected coordinate within about 12 pixels unless visible state changed. Close an irrelevant window through its visible close control. A prior needs_decision/focus_changed or zero steps_completed does not mean the application effect failed: inspect the screenshot and never blindly replay.
The interface has already applied one tracked tree-transparency transition before this first planner observation. Trees are transparent and the transition cannot be requested again. Every act must declare intent as progress, inspect or repair, and a concrete expected_effect. Checkpoint is null when no prior drag is pending. After a drag, the next proposal must bind prior_turn to that turn and classify its expected effect as observed, contradicted or uncertain with visual evidence. If uncertain, use intent inspect and only pointer_move/dwell_observe/observe. If contradicted, use intent repair or stop. A new progress mutation is allowed only after observed. Do not call an effect observed from a cost popup alone.
Return ONLY one JSON object of exactly one form:
{{"kind":"act","steps":[...],"rationale":"short explanation","intent":"progress|inspect|repair","expected_effect":"concrete visible effect","checkpoint":null_or_{{"prior_turn":INTEGER,"status":"observed|contradicted|uncertain","evidence":"visual evidence"}}}}
{{"kind":"verify","road_visible":BOOLEAN,"rationale":"short explanation","checkpoint":null_or_object_above}}
{{"kind":"stop","rationale":"why no justified next action is available","checkpoint":null_or_object_above}}
Use verify only when both A-to-B and B-to-C road segments are visibly complete and the X area remains clear; it requests the independent engine guard score. Rationale 1..600 characters. Expected effect 1..300 and checkpoint evidence 1..400 characters. Act has 1..6 steps. Supported exact step shapes:
...
Pending checkpoint prior turn: {json.dumps(checkpoint_required_turn)}
'''
        ...
        proposal=parse(items[0]['text'],checkpoint_required_turn,'transparent');dump(root/f'typed-{index}.json',proposal);row={'turn':index,'model_runner_started_ns':begin,'model_runner_returned_ns':ended,'kind':proposal['kind'],'requested_model':'gpt-6-astra','requested_effort':'medium','checkpoint_required_turn':checkpoint_required_turn};turns.append(row);dump(control/'turns.json',turns)
        if proposal['kind']=='stop':
            dump(root/'abort.json',{'finish':True,'reason':'typed model safe stop: '+proposal['rationale']});failure=wait_file(root/'failure-evaluation.json',30);driver.wait(timeout=10);assert failure['success'] is False;raise RuntimeError('model stopped: '+proposal['rationale'])
        ...
        if proposal['kind']=='view':raise ValueError('view method unavailable after tracked pre-action transition')
        executable_steps=proposal['steps']
        dump(root/f'proposal-{index}.json',{'steps':executable_steps,'rationale':proposal['rationale'],'typed_kind':proposal['kind']});moves=[[step['x'],step['y']] for step in executable_steps if step.get('op')=='pointer_move']
        ...
        observation=applied['result']['state']['continuation']['observation'];current_image=root/'runtime'/Path(observation['image']).name;planner_image=build_effect_sheet(current_image,applied,proposal,root/'runtime',root/f'planner-{index+1}.png');previous={'proposal':proposal,'resolution':applied['result']['state']['last_resolution']};new_required=next_checkpoint_turn(index,proposal)
        if new_required is not None:checkpoint_required_turn=new_required
        elif proposal['kind']=='act' and checkpoint_required_turn is not None and proposal['checkpoint']['status']=='observed':checkpoint_required_turn=None
        print(json.dumps({'turn':index,'kind':proposal['kind'],'status':previous['resolution']['terminal']['status'],'sequence':observation['sequence'],'checkpoint_required_turn':checkpoint_required_turn}),flush=True)
    else:
