"""Explicit model/effort screenshot probe; requested route is recorded per call."""
import hashlib
import json
...
def main():
    node, cli, image, prompt_file, working, output, model, effort = sys.argv[1:]
    allowed = {
        'gpt-5.6-luna': {'low', 'medium', 'high'},
        'gpt-6-astra': {'low', 'medium', 'high'},
    }
    if model not in allowed or effort not in allowed[model]:
        raise ValueError('unsupported explicit model route')
    ...
    plan = dict(args=args, requested_model=model, requested_effort=effort,
                ...
                comparison='adaptive candidate; actual served identity and full context unverified')
