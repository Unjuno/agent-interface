...
H=Path(__file__).resolve().parent;R=H/'results/sampled-decision-calc-01';T=R/'runtime'
...
report={'decision_sampling':[{'turn':r['turn'],'elapsed_s':r['decision_sampling']['elapsed_s'],'captures':len(r['decision_sampling']['result']['checks']),'status':r['decision_sampling']['result']['status']} for r in turns if 'decision_sampling' in r], 'scope':'one actual Calc episode; shared caller; no cross-domain speed claim','events':len(events),'frames':len(obs),'exchanges':len(calls),'models':models,'saved_cells':values,'terminal_statuses':[t['status'] for t in terms],'capture_to_evaluation_s':(evaluation['known_ns']-obs[0]['capture_ns'])/1e9,'handoff_s':[r['phases']['handoff']['elapsed_s'] for r in read(R/'turns.json') if 'handoff' in r.get('phases',{})]}
