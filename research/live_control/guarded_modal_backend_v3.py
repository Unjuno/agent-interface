"""Research-only one-shot guard bound to an ordinary shared first step."""
import time
...
class Backend(Previous):
    def arm_modal(self,identifier,proposal,step):
        if hasattr(self,'modal_guard'):raise ValueError('one pending guard only')
        self.validate([step])
        self.modal_guard=(identifier,proposal,dict(step))
    ...
    def execute(self,step,cancel,identifier,index):
        ...
        proposal=guard[1]
        if index!=0 or step!=guard[2]:raise ValueError('guard requires its bound first step')
