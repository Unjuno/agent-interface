...
class Backend:
    ...
    def raw(self,key,down):
        ...
        self.session.d.sync()
        if down:
            self.emit(dict(event='input_admission',key=key,admitted_ns=admitted,
                           input_ack_ns=time.perf_counter_ns(),valid_until_ns=self.lease.deadline))
...
def main():
    ...
    try:
        hashes={}
        for path in [HERE/'executor_v3.py',HERE/'session_v4.py',HERE/'lease.py',HERE.parent/'observation_tiles/tile_transport.py',
