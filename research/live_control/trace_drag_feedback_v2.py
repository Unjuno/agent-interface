...
def main():
    root = HERE / 'results/cause-drag-feedback-01'
    root.mkdir(exist_ok=False)
    paths = {'visual_correction': [618, 628, 638]}
    names = ['trace_drag_feedback_v2.py', 'cause_session_v1.py', 'input_owner_v10.py',
    ...
    plan = {'paths': paths, 'seed': 214, 'hold_between_samples_ms': 150,
            ...
            'hypothesis': 'one image-based residual correction while held reaches intended object displacement',
            'controller': 'canvas red right edge target +20 pixels from pressed phase; at most one horizontal correction of absolute size <=12; require matching size and y extents',
            'scope': 'one task-specific visual-controller diagnostic; not live model control, no performance comparison'}
                ...
                def execute(self, step, lease, identifier, index):
                    ...
                    try:
                        ...
                        for i, point in enumerate(points[1:], 1):
                            call('move', point)
                            sample('motion-' + str(i))
                        first_box, last_box = phases[0]['red_bbox'], phases[-1]['red_bbox']
                        assert first_box is not None and last_box is not None
                        assert first_box[1] == last_box[1] and first_box[3] == last_box[3]
                        assert abs((first_box[2] - first_box[0]) - (last_box[2] - last_box[0])) <= 1
                        residual = first_box[2] + 20 - last_box[2]
                        assert abs(residual) <= 12, 'residual exceeds declared correction bound'
                        result['visual_correction'] = {'residual_pixels': residual, 'from_bbox': last_box,
                                                      'target_right_edge': first_box[2] + 20}
                        if residual:
                            state = self.owner.call('input_state')
                            assert state['owned_buttons'] == [1] and state['active_lease_time_valid']
                            point = {'x': state['pointer'][0] + residual, 'y': state['pointer'][1]}
                            call('move', point)
                            sample('corrected')
                        assert abs(phases[-1]['red_bbox'][2] - (first_box[2] + 20)) <= 1
                    finally:
                        call('button_up', 1)
                    sample('released')
