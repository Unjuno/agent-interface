...
H=Path(__file__).resolve().parent;R=H/'results/recovery-replan-ink-01';T=R/'runtime'
...
assert len([e for e in events if e['event']=='accepted'])==4+sum(1+int(x['submitted']) for x in read(R/'replans.json'))
...
assert all(t['status'] in ('completed','needs_decision') and t['release']['verified'] and t['release']['keys_down']==[] and t['release']['buttons_down']==[] for t in terms)
