...
H=Path(__file__).resolve().parent;R=H/'results/phased-replan-ink-01';R.mkdir(exist_ok=False)
def dump(n,x):(R/n).write_text(json.dumps(x,indent=2)+'\n',encoding='utf-8')
def win(p):return 'C:'+str(p.resolve())[6:]
names=['phased_replan_ink_v1.py','durable_submit_v4.py','append_checkpoint_v1.py','received_continuation_v1.py','received_exchange_v2.py','cause_servo_socket_v8.py','model_context_runner_v1.py','screenshot_responder_v1.txt','recover_once_v1.py','recover_once_v2.py','recovery_view_v1.py','calc_proposal_schema_v1.py','sampled_target_contract_v1.py','passive_pair_v1.py','activation_handoff_v1.py']
...
p=subprocess.Popen([sys.executable,str(H/'cause_servo_socket_v8.py'),'inkscape','serve','--','--app','inkscape','--seed','239','--out',str(R/'runtime')],stdout=subprocess.PIPE,stderr=(R/'stderr.txt').open('w'),text=True)
...
try:
 ...
 state=load(journal);replans=[]
 feedback={'refusal':verdict,'rejected_proposal':q,'proposal_submitted':False,'observation':observation,'task_completion':'not_achieved'}
 for index in range(2,7):
  source=state['continuation']['observation'];dump(f'evidence-{index}.json',feedback)
  q=model(index,source,feedback)
  if q.get('kind')=='verify':
   assert set(q)=={'kind','visible_x','visible_y','visible_width','visible_height','rationale'}
   assert all(type(q[k]) in (int,float) for k in ['visible_x','visible_y','visible_width','visible_height'])
   assert [q[k] for k in ['visible_x','visible_y','visible_width','visible_height']]==[104,50,40,30]
   dump('visual-verdict.json',q);break
  q=parse_action(json.dumps(q));assert q['kind']=='act'
  first=q['steps'][0];assert first['op']=='pointer_click'
  x,y=first['x'],first['y'];contract={'name':'replan-first-click','box':[max(0,x-12),max(0,y-12),min(1280,x+13),min(800,y+13)],'point':[x,y],'max_age_ms':1000}
  begin_call=len(calls);c=clock()
  ...
  observation=fresh['state']['continuation']['observation'];c=clock()
  with Image.open(R/'runtime'/Path(source['image']).name) as old,Image.open(R/'runtime'/Path(observation['image']).name) as new:
   checked=evaluate(contract,{'intent':contract['name'],'execute_once':True},source,observation,old,new,c['runtime_ns'])
  row={'turn':index,'proposal':q,'source':source,'fresh':observation,'contract':contract,'clock':c,'verdict':checked,'calls_begin':begin_call,'submitted':False}
  if checked['eligible']:
   activation=[q['steps'][0],{'op':'observe'}]
   tail=q['steps'][1:]
   assert all(step['op'] in ('key','chord','text') for step in tail),'tail must be keyboard-only'
   applied=call({'command':{'op':'submit','expected_sequence':checked['expected_sequence'],'valid_until_ns':checked['valid_until_ns'],'steps':activation},'timeout':3})
   assert applied['state']['pending'] is None
   terminal=applied['state']['last_resolution']['terminal']
   row['activation']=applied
   if tail:
    assert terminal['status']=='completed','activation interrupted; no automatic tail'
    handoff_begin=time.perf_counter_ns();c=clock()
    sample=call({'command':{'op':'submit','expected_sequence':c['sequence'],'valid_until_ns':c['runtime_ns']+3_000_000_000,'steps':[{'op':'observe'}]},'timeout':3})
    assert sample['state']['pending'] is None
    c=clock();o=sample['state']['continuation']['observation']
    gate=handoff(applied['request']['command']['id'],activation,terminal,observation,o,c)
    row['handoff']={'source':observation,'fresh':o,'clock':c,'verdict':gate,'elapsed_s':(time.perf_counter_ns()-handoff_begin)/1e9}
    dump('handoff.json',row['handoff'])
    assert gate['eligible'],'handoff refused; no automatic tail'
    applied=call({'command':{'op':'submit','expected_sequence':gate['expected_sequence'],'valid_until_ns':c['runtime_ns']+2_000_000_000,'steps':tail+[{'op':'observe'}]},'timeout':3})
    assert applied['state']['pending'] is None
    assert applied['request']['command']['id']!=row['activation']['request']['command']['id']
   assert applied['state']['last_resolution']['terminal']['status']=='completed','phase interrupted; no automatic replay'
   assert applied['state']['pending'] is None
   row.update(submitted=True,result=applied);feedback={'previous_proposal':q,'outcome':present(applied)}
  else:feedback={'refusal':checked,'rejected_proposal':q,'proposal_submitted':False,'observation':observation,'task_completion':'not_achieved'}
  row['calls_end']=len(calls)
  if row['submitted'] and len(q['steps'])==1 and q['steps'][0]['op']=='pointer_click':
   last=[applied];started=time.perf_counter_ns();readiness_begin=len(calls)
   def sample():
    c=clock();r=call({'command':{'op':'submit','expected_sequence':c['sequence'],'valid_until_ns':c['runtime_ns']+30_000_000_000,'steps':[{'op':'observe'}]},'timeout':3});last[0]=r;o=r['state']['continuation']['observation'];c=clock()
    return {'observation':o,'image':str(R/'runtime'/Path(o['image']).name),'clock':c}
   o=applied['state']['continuation']['observation'];region={'name':'post-selection-x-field','box':[538,94,563,119],'point':[550,106],'max_age_ms':1000}
   ready=collect({'observation':o,'image':str(R/'runtime'/Path(o['image']).name)},sample,region,3)
   row['readiness']={'result':ready,'contract':region,'calls_begin':readiness_begin,'calls_end':len(calls),'elapsed_s':(time.perf_counter_ns()-started)/1e9}
   dump('readiness.json',row['readiness']);assert ready['stable'],'bounded readiness not reached'
   feedback={'previous_proposal':q,'outcome':present(last[0]),'sampled_readiness':ready}
  replans.append(row);dump('replans.json',replans);state=load(journal)
 else:raise RuntimeError('bounded replan limit; no automatic success')
 ...
 code=p.wait(timeout=10);assert code==0;dump('result.json',{'exit_code':code,'success':True,'final_goal_achieved':True,'expected_refusal':True,'recovery_reads':1,'model_calls':index});print(json.dumps({'success':True,'model_calls':index}),flush=True)
