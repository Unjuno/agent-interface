...
HERE=Path(__file__).resolve().parent;out=HERE/'results/live-finalization-faults-02';out.mkdir(exist_ok=False)
results=[]
for fault in ('write','flush','scorer'):
    root=out/fault
    messages=queue.Queue();seen=[]
    process=subprocess.Popen([sys.executable,'-u',str(HERE/'finalization_fault_entry_v2.py'),fault,'--app','xterm','--seed','991024','--out',str(root),'--presentation','compact'],stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
    def read():
    ...
    try:
        ...
        for _ in range(2):
            send(dict(op='finalization_status'));reply=until('finalization_status')
            assert reply['state']=='available' and reply['outcome']==status
        ...
        assert len([r for r in seen if r['event']=='finalization_status'])==2
...
(out/'probe-sources.json').write_text(json.dumps({p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (Path(__file__),HERE/'finalization_fault_entry_v2.py')},indent=2)+'\n')
