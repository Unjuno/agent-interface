...
if stage=='initial':
    save('plan',dict(seed=225,app='calc',scope='One live self-use with executor v8 early response plus at most one bounded read in the same outer operation, full receipt and state table; no fault injection or speed comparison',
        model_tokens=None,verified_model_id=None,sources={n:digest((HERE/n).read_bytes()) for n in
        ['calc_combined_live_v1.py','cause_servo_socket_v4.py','cause_servo_interactive_v4.py','executor_v8.py','post_release_observation_v2.py',
         'input_state_table_v1.py','decision_receipt_v4.py','early_exchange_v1.py','early_resume_v1.py','bounded_followup_v2.py','stopped_client_v1.py','stopped_socket_v1.py','stopped_cursor_v1.py','stopped_scope_v1.py']}))
