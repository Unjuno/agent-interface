...
H=Path(__file__).resolve().parent;R=H/'results/recovery-focus-ink-01';T=R/'runtime'
...
replay(read(R/'initial.json'));calls=read(R/'calls.json');assert len(calls)==6
...
assert load(R/'journal.jsonl')==calls[-1]['result']['state'];count=inspect(R/'journal.jsonl')[1];assert count==13
...
assert len([e for e in events if e['event']=='accepted'])==3
assert sum(s=={'op':'chord','modifier':'Control_L','key':'s'} for c in commands for s in c.get('steps',[]))==1
terms=[e for e in events if e['event']=='terminal'];assert len(terms)==3
...
models=[]
for n,o in [(1,recovered['state']['continuation']['observation'])]:
 ...
 assert json.loads((d/'prompt.txt').read_text(encoding='utf-8').split('Evidence: ')[1])==recovered['recovery']
...
v=read(R/'proposal-1.json');refused=read(R/'refused.json')
...
assert refused['fresh']==calls[-1]['result']['state']['continuation']['observation']
...
assert not any(s.get('op')=='text' and s.get('text')=='104' for c in commands for s in c.get('steps',[]))
assert read(R/'result.json')['final_goal_achieved'] is False
...
report={'events':len(events),'frames':len(obs),'journal_records':count,'model_calls':models,'svg':values,'svg_sha256':sha(T/'shape.svg'),'scope':'one live intentional response abandonment; scripted edit, caller recovery, model new edit refused before input by changed focus; not full autonomous editing or general recovery', 'loss_to_read_resolution_s':(calls[3]['end_ns']-read(R/'loss.json')['send_completed_ns'])/1e9}
