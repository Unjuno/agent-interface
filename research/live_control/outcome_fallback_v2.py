...
def wait_with_status(exchange,request_id,program_id,after,*,timeout=1,status_timeout=1,final_only=False):
    ...
    try:
        ...
        if result['state'] in ('evaluated','effect_observed') and result['evidence'].get('final_program')!=program_id:
            return dict(result=dict(state='needs_reconciliation',reason='outcome_program_mismatch',task_success=None,authority='none',continuation=None),transcript=transcript)
