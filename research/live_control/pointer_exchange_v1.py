...
def run(query,batch,root,program_id,steps,lease_ms,persist):
    ...
    try:
        ...
        reply=call({'after':clock['cursor'],'events':['terminal'],'timeout':15,'action_id':program_id,
        ...
        terminal=records[-1]
        if terminal.get('event')!='terminal' or terminal.get('id')!=program_id or index>=len(records)-1:
        ...
        result.update(state='terminal',terminal=terminal,image=select_image(reply,root))
