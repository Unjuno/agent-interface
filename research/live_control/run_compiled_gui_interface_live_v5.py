"""V5 Chromium pair: propagate typed local yields through caller v2."""
import copy
import hashlib
import json
import os
...
def compiled_interface():
    return {
        ...
        "session_scope": "live-chromium-seed991024",
...
def run_case(index, name, changed_after_first):
    ...
    try:
        ...
        def local_observe(payload):
            ...
            predicates = {
                ...
                "field_target_present": bool(target_name == "form_field" and check
                    and check.get("eligible") is True and check.get("status") == "VALID"),
        ...
        def execute_target(payload):
            ...
            if runtime_receipt["outcome"] == "TASK_SUCCEEDED":
                return {"status": "completed"}
            if runtime_receipt["outcome"] == "SAFE_YIELD":
                return {"status": "safe_yield",
                        "reason": runtime_receipt["reason"],
                        "completed_actions": runtime_receipt["completed_transitions"]}
...
def main():
    ...
    gates = {
        ...
        "changed_stops_second_target_input": (
            ...
            and changed["actual"] == {}),
        "typed_yield_crosses_outer_boundary": (
            changed["adaptive"]["outcome"] == "EXECUTION_INCOMPLETE"
            and changed["adaptive"]["reason"] == changed_runtime.get("reason")
            and changed["adaptive"]["execution_progress"] == {
                "status": "safe_yield", "reason": changed_runtime.get("reason"),
                "completed_actions": 1}
            and changed["adaptive"]["delivery"] == "confirmed_partial"
            and changed["adaptive"]["input_authority"] ==
