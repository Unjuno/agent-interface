"""Explicit per-model-call stages for registered pair 2; no automatic task loop."""
import argparse
import hashlib
import json
import time
...
def main():
    ...
    ap.add_argument('stage', choices=['initial', 'prepare', 'recover1', 'recover2', 'recover3', 'recover4', 'move', 'finish'])
    ...
    def query(q):
        ...
        r = exchange(args.socket, q, timeout=16)
    ...
    try:
        if args.stage == 'initial':
            ...
            save('execution', {'pair': 2, 'domain': 'inkscape', 'seed': 202, 'depth': 3, 'additional_historical_clocks': 3, 'expected_recovery_reads_including_own': 4, 'arm': args.arm,
            ...
            r = act('move-save', read('recover4' if args.arm == 'A' else 'recover1', 'batch'), [
                {'op': 'pointer_click', 'x': 618, 'y': 391},
            ...
            b = read('move', 'move-save-report')['last_reply']
            result = query({'after': b['cursor'], 'events': ['independent_evaluation'], 'timeout': 5,
