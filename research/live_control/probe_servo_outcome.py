...
import numpy as np
from session_v16 import Backend,suite
from executor_v3 import Executor
from patch_servo import PatchServo
HERE=Path(__file__).resolve().parent
root=HERE/'results/servo-outcome-01';root.mkdir(exist_ok=False)
allrows=[]
for offset in (0,40):
    ...
    try:
        ...
        if offset:
            ...
            assert run('setup_offset',[{'op':'pointer_drag','points':[{'x':x,'y':y},{'x':x+offset,'y':y}],'duration_ms':200}]+save)['status']=='completed'
        ...
        terminal=run('servo',[{'op':'pointer_servo','source_sequence':backend.sequence,'box':box,'target_delta':[24,0],'max_corrections':3,'points':[{'x':x,'y':y},{'x':x+12,'y':y}],'duration_ms':100}])
        ...
        row={'setup_offset_requested':offset,'source_box':box,'before':before,'after':after,'saved_dx_screen':dx,'preserved':preserved,'precision_pass':abs(dx-24)<=1,'terminal':terminal,'controller':[r for r in events if r['event']=='servo_feedback'],'program_ms':duration}
        allrows.append(row)
...
sources=[Path(__file__),HERE/'patch_servo.py',HERE/'visual_anchor.py',HERE/'session_v16.py',HERE/'session_v15.py',HERE/'input_owner_v9.py',HERE/'pointer_reply_v2.py',HERE/'executor_v3.py']
