...
def main():
    root = HERE / 'results/append-cost-01'; plan = read(root / 'plan.json')
    ...
    for index, mode in enumerate(plan['order']):
        ...
        assert (load(out / 'journal.json') if mode == 'append' else read(out / 'journal.json')) == calls[-1]['result']['state']
        if mode == 'append': assert inspect(out / 'journal.json')[1] == 17
        ...
        assert location['path'].startswith('/tmp/') and not Path(location['path']).exists()
    ...
    report = {'groups': groups, 'sessions': summaries, 'measured_calls': len(rows),
              'scope': 'ABBA native Linux replacement/append checkpoints; two fresh sessions per mode,17 records for append, no model/power-loss proof',
