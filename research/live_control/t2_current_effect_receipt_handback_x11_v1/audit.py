...
if s['disposition']=='PASS_CURRENT_EFFECT_RECEIPT_HANDBACK_SCOPED':
 if not (b['effect_after_handback']>0 and c['effect_after_handback']==0 and c['physical_possible_after_handback']==0 and c['effect_receipts']==expected_cand and c['handback_completed']==expected_cand and c['wait_p95_ms']<6 and c['wait_max_ms']<8 and ct['effect_receipts']==0 and ct['completed_handbacks']==0 and ct['unresolved_timeouts']==expected_ctrl): errs.append('bad_pass')
