...
def wait_effect(q,deadline_ns,session_id,request_id):
 while True:
  rem=(deadline_ns-time.perf_counter_ns())/1e9
  if rem<=0: return None
  try: r=q.get(timeout=rem)
  except queue.Empty: return None
  if r.get('session_id')==session_id and r.get('request_id')==request_id and r.get('role')=='CURRENT_EFFECT': return r
def run_case(root,idx,arm,offset_ms,app_delay_ms,effect_enabled=True,display_name=':98'):
 ...
 handback=None; status='COMPLETED'; accepted=None
 if arm==BASE: handback=act_recv
 elif arm==CAND:
  accepted=wait_effect(q,act_recv+int(EFFECT_TIMEOUT_MS*1e6),session_id,request_id)
  if accepted is None: status='UNRESOLVED_EFFECT_TIMEOUT'
  else: handback=time.perf_counter_ns()
...
def summarize(pos,ctrl):
 ...
 summary={'positive':{BASE:{'n':len(b),'effect_after_handback':sum(r['effect_after_handback'] for r in b),'correct':sum(r['effect_correct'] for r in b),'terminal_released':sum(not r['terminal_key_down'] for r in b)},CAND:{'n':len(c),'effect_after_handback':sum(r['effect_after_handback'] for r in c),'correct':sum(r['effect_correct'] for r in c),'terminal_released':sum(not r['terminal_key_down'] for r in c),'local_completion_after_handback':sum(r['local_completion_after_handback'] for r in c),'physical_possible_after_handback':sum(r['physical_possible_after_handback'] for r in c),'effect_receipts':sum(r['effect_receipt'] is not None for r in c),'handback_completed':sum(r['handback_status']=='COMPLETED' for r in c),'wait_p95_ms':p95(waits),'wait_max_ms':max(waits,default=None)},},'controls':{'n':len(ctrl),'effect_receipts':sum(r['effect_receipt'] is not None for r in ctrl),'completed_handbacks':sum(r['handback_status']=='COMPLETED' for r in ctrl),'unresolved_timeouts':sum(r['handback_status']=='UNRESOLVED_EFFECT_TIMEOUT' for r in ctrl),'terminal_released':sum(not r['terminal_key_down'] for r in ctrl),'correct':sum(r['effect_correct'] for r in ctrl)}}
 ...
 if all(r['effect_correct'] and not r['terminal_key_down'] and not r.get('observer_error') for r in pos+ctrl):
  lineage=all(r['effect_receipt'] and r['effect_receipt']['session_id']==r['session_id'] and r['effect_receipt']['request_id']==r['request_id'] and r['effect_receipt']['role']=='CURRENT_EFFECT' and not r['effect_receipt']['input_authority'] and not r['effect_receipt']['semantic_authority'] for r in c)
  if cc['effect_after_handback']>0: disp='FAIL_EFFECT_RECEIPT_COVERAGE'
  elif ct['effect_receipts']>0 or ct['completed_handbacks']>0: disp='FAIL_EFFECT_RECEIPT_FALSE_POSITIVE'
  elif not lineage or cc['effect_receipts']!=len(c) or cc['handback_completed']!=len(c): disp='FAIL_LIVE_TRANSFER'
  ...
  else: disp='PASS_CURRENT_EFFECT_RECEIPT_HANDBACK_SCOPED'
