"""Verify archived OpenTTD pair 4 B, retaining presentation failure."""
import hashlib
import json
import sys
...
def main():
    root = HERE / 'results/recovery-pair4-01/B'
    runtime = root / 'runtime'
    metadata = read(root / 'initial/execution.json')
    assert (metadata['pair'], metadata['arm'], metadata['seed']) == (4, 'B', 204)
    ...
    segments = []
    for stage in ('initial', 'prepare', 'recover1', 'open', 'build', 'finish'):
    ...
    assert len(queued) == 3
    for item, old in zip(drain['reads'][:3], queued):
        assert item['reply']['records'][-2:] == old['records']
        assert old['records'][0]['command']['transport_request_id'] != own
    ...
    result = {
        'audit_sha256': sha(Path(__file__)), 'pair': 4, 'arm': 'B',
        'socket_exchanges': len(segments), 'unique_records': len(raw), 'exact_frames': len(observations),
        'recovery_reads': 4, 'recovery_outer_calls_reported': 1,
        ...
        'next': 'Aggregate all four pairs, retaining presentation and focus failures.'}
