...
def main():
    ...
    for row in report["results"]:
        ...
        pre_action = row["adaptive"]["selected_target"]["pre_action_score"]
        assert pre_action["success"] is False
        assert pre_action["reason"] == "expected_crop_missing"
        assert pre_action["binding_status"] in {"CURRENT_EXACT", "CURRENT_TRANSLATED"}
        assert pre_action["observed_crop_sha256"] is not None
