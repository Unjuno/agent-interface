"""Matched live local-handle repair versus Luna-low visual reacquisition."""
...
PREREG=HERE/"matched_semantic_repair_live_v1_prereg.json"
...
def verify(plan):
    ...
    checks.update({"output_absent":not(REPO/plan["output"]).exists(),
        ...
        "same_seed":plan["seed"]==213,"model":plan["model"]=={"name":"gpt-5.6-luna","effort":"low"},
...
def run_arm(plan,root,ordinal,mode):
    ...
    from semantic_repair_model_v1 import call as model_call
    ...
    def wait_binding(predicate,message):
        ...
        return value
    def prompt(goal):
        return ("The attached current 1280x800 screenshot is a private Chromium form for the "
            f"authorized task of submitting the exact token {goal['token']}. The exact token "
            "has already been entered. Identify one point inside the visible text Value field "
    ...
    def ground(label,image,goal):
        return model_call(arm/label,prompt(goal),image,arm/"empty-workspace")
    ...
    def mint_from_grounding(store,grounding,observation,image):
        ...
        return mint,resolution,relation
    ...
    try:
        ...
        backend.snapshot("matched-repair-source",0);source=events[-1];original_geometry=list(source["pointer_binding"]["geometry"])
        source_path=arm/Path(source["image"]).name
        initial_model=ground("initial-model",source_path,goal);initial_grounding=checked_grounding(initial_model)
        ...
        if mode=="local":
            ...
            reacquisition_model=ground("reacquisition-model",resized_path,goal)
            ...
            new_mint,current_resolution,new_relation=mint_from_grounding(
                new_store,reacquired_grounding,resized,resized_image)
            recovered=derive_contract(f"matched_completion_{ordinal+1}","submission_heading",new_mint,
                current_resolution,resized,new_relation,plan["expected_crop_sha256"],"submission_title_exactly_visible")
        ...
        metrics={"recovery_from_resized_capture_ms":(recovery_ready_ns-resized["capture_ns"])/1e6,
            ...
            "model_wait_ms":sum(row["caller_elapsed_ms"] for row in model_records),
            ...
            "model_calls":len(model_records),"model_visible_images":len(model_records),
        ...
        report.update({"passed":all(checks.values()),"checks":checks,"goal":goal,
            ...
            "model_records":model_records,"usage":usage,"source_observation":source,
...
def main():
    ...
    arms=[run_arm(plan,root,index,mode) for index,mode in enumerate(plan["order"])]
    ...
    report={"schema":"matched-semantic-repair-live-v1","allocation_id":plan["allocation_id"],
        "verification":verification,"passed":all(checks.values()),"checks":checks,"preflight":preflight,
