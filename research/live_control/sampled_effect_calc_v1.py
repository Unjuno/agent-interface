...
import hashlib,json,subprocess,sys,tempfile,shutil,time,io
...
H=Path(__file__).resolve().parent;R=H/'results/sampled-effect-calc-01';R.mkdir(exist_ok=False)
def dump(n,x):(R/n).write_text(json.dumps(x,indent=2)+'\n',encoding='utf-8')
def win(p):return 'C:'+str(p.resolve())[6:]
names=['sampled_effect_calc_v1.py','phased_submit_v1.py','activation_handoff_v1.py','sampled_target_contract_v1.py','durable_submit_v4.py','calc_proposal_schema_v1.py','model_context_runner_v1.py','screenshot_responder_v1.txt','cause_servo_socket_v8.py','decision_pair_v1.py']
...
def saved_snapshot(label):
 ...
 try:
  blob=(R/'runtime/sheet.xlsx').read_bytes()
  ...
  entry['cells']=[wb.active['A1'].value,wb.active['A2'].value];wb.close()
  entry['read_status']='parsed'
