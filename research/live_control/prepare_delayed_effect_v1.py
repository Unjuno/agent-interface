...
names = ['prepare_delayed_effect_v1.py', 'delayed_effect_socket_v1.py',
         'delayed_effect_browser_entry_v1.py', 'checkpoint_cause_interactive_v1.py',
...
p = subprocess.Popen([sys.executable, str(H / 'delayed_effect_socket_v1.py'), 'chromium', 'serve',
...
try:
    ...
    clock = call({'command': {'op': 'clock'}, 'timeout': 3})['state']['last_resolution']['clock']
    action_id = 'delayed-submit-242'
    steps = [{'op': 'pointer_click', 'x': 150, 'y': 265, 'duration_ms': 80},
             {'op': 'chord', 'modifier': 'Control_L', 'key': 'a'},
             {'op': 'text', 'text': goal['token']}, {'op': 'key', 'key': 'Return'},
             {'op': 'observe'}]
    submitted = call({'command': {'op': 'submit', 'id': action_id,
                      'expected_sequence': clock['sequence'],
                      'valid_until_ns': clock['runtime_ns'] + 3_000_000_000,
                      'steps': steps}, 'action_id': action_id, 'events': ['terminal'],
                      'timeout': 4})
