"""Independent audit of the caller-v2 typed-yield live pair."""
import hashlib
import json
...
ROOT = HERE / "results/compiled-gui-interface-live-05"
...
def main():
    ...
    assert report["actual_performed_model_usage_totals"] == {
        "input_tokens": 18792, "cached_input_tokens": 6912,
    ...
    # Frozen source retained the v4 seed text in this opaque label.  Each case
    # has a separate process/registry and no reuse path, so record rather than
    # silently correcting the post-preregistration source.
    assert all(row["compiled_runtime"]["session_scope"] ==
               "live-chromium-seed991024" for row in cases.values())
    ...
    assert positive["raw_evidence"][1]["field_changed_pixels"] >= 40
    ...
    assert changed["adaptive"]["reason"] == "unknown_state"
    assert changed["adaptive"]["execution_progress"] == {
        "status": "safe_yield", "reason": "unknown_state",
        "completed_actions": 1}
    assert changed["adaptive"]["delivery"] == "confirmed_partial"
    assert changed["adaptive"]["input_authority"] == (
    ...
    audit = {"passed": True, "formal_mechanics_passed": True,
        ...
        "changed": {"independent_success": False, "local_transitions": 1,
            ...
            "outer_adaptive_reason": "unknown_state",
            "outer_completed_actions": 1,
            "outer_delivery": "confirmed_partial"},
        "fresh_model_calls": 2,
        "fresh_model_usage_totals": report["actual_performed_model_usage_totals"],
        "preflight_model_calls_this_run": 0,
        "known_limit": ("single fresh pair gives no rate; frozen v5 runner "
            "retains stale opaque session_scope label seed991024 while study "
            "seed is 991025; processes and registries are separate and no reuse occurs"),
        "decision": "ADVANCE_TO_MATCHED_EFFICIENCY_COMPARISON",
