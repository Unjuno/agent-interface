...
HERE=Path(__file__).resolve().parent;out=HERE/'results/socket-bounded-write-01';out.mkdir(exist_ok=False)
root=out/'runtime'
p=subprocess.Popen([sys.executable,'-u',str(HERE/'event_socket_v5.py'),'serve','--','--app','xterm','--seed','991024','--out',str(root),'--presentation','compact'],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
...
try:
    ...
    (out/'sources.json').write_text(json.dumps({f.name:hashlib.sha256(f.read_bytes()).hexdigest() for f in (Path(__file__),HERE/'event_socket_v5.py',HERE/'event_cursor.py',HERE/'command_once.py')},indent=2)+'\n')
