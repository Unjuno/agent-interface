...
if stage=='initial':
    save('plan',dict(seed=227,app='chromium',scope='One live self-use with executor v8 early response plus at most one bounded read in the same outer operation, full receipt and state table; no fault injection or speed comparison',
        model_tokens=None,verified_model_id=None,sources={n:digest((HERE/n).read_bytes()) for n in
        ['presentation_emission_v1.py','emission_confirmation_entry_v1.py','browser_emission_live_v1.py','emission_confirmation_socket_v1.py','cause_servo_interactive_v4.py','executor_v8.py','post_release_observation_v2.py',
...
save('result',result);emit(result,sys.stdout.buffer,out/'emission')
