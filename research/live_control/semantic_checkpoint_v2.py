"""Semantic-effect checkpoint admission with safe uncertain/contradicted stop."""
import json
...
def parse(text,required_turn=None):
    ...
    if value['kind']=='act':
        ...
        if value['kind']=='verify' and required_turn is not None and value['checkpoint']['status']!='observed':raise ValueError('verification requires observed pending effect')
    return value
