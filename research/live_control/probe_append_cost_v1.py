...
from durable_submit_v3 import initialize as replace_init, run as replace_run
from durable_submit_v4 import initialize as append_init, run as append_run
...
def main():
    root = HERE / 'results/append-cost-01'; root.mkdir(exist_ok=False)
    modes = ['replace', 'append', 'append', 'replace']
    names = ['probe_append_cost_v1.py', 'received_exchange_v2.py', 'received_continuation_v1.py', 'durable_submit_v3.py', 'durable_submit_v4.py', 'append_checkpoint_v1.py',
    ...
    dump(root / 'plan.json', {'order': modes, 'seed': 236, 'cycles_per_session': 4, 'warmup_cycles': 0,
         'scope': 'replacement versus append checkpoints, same native Linux filesystem, same actual GUI task, no model',
         ...
         'environment': {'python': sys.version, 'platform': platform.platform(), 'journal_filesystem': 'native Linux /tmp on ext4, temporary benchmark only'}})
    for index, mode in enumerate(modes):
        out = root / f'{index+1:02d}-{mode}'; out.mkdir()
        initialize, run = (replace_init, replace_run) if mode == 'replace' else (append_init, append_run)
        ...
        try:
            ...
            journal_home = tempfile.TemporaryDirectory(prefix='agent-interface-cost-')
