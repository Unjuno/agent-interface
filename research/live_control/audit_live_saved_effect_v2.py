...
HERE=Path(__file__).resolve().parent;root=HERE/'results/live-saved-effect-03';report=[]
for case in ('save','unsaved'):
    ...
    early,=[e for e in delivered if e['event']=='effect_evidence']
    assert early['delivery_id'] in receipts and early['effect']==effect
    assert early['emit_started_ns']<evaluation['emit_started_ns']
    assert state['effect_output_flushed']
...
for fault in ('none','evaluate','publish','early_only'):
    trace=[]
    ...
    def publish(record):
        ...
        return fault!='publish' and not (fault=='early_only' and record['event']=='effect_evidence')
    ...
    assert trace[:4]==['closed','effect','publish','evaluate']
    if fault=='early_only':
        assert result['output_flushed'] and not result['effect_output_flushed']
        assert result['evaluation']=={'success':False} and result['failure_stage']=='publish_effect'
    report.append(dict(callback_fault=fault,result=result))
(HERE/'results/live-saved-effect-early-audit.json').write_text(json.dumps(report,indent=2)+'\n')
