...
def _validate_schema(value, schema, path="answer"):
    """Validate the strict JSON Schema subset used by planner action contracts."""
    if "const" in schema and value != schema["const"]:
        raise PlannerProtocolError(f"{path} must equal const")
    ...
    if expected == "number" and not math.isfinite(value):
        raise PlannerProtocolError(f"{path} must be finite")
    if expected == "string":
        if len(value) < schema.get("minLength", 0):
            raise PlannerProtocolError(f"{path} is shorter than minLength")
        if "maxLength" in schema and len(value) > schema["maxLength"]:
            raise PlannerProtocolError(f"{path} is longer than maxLength")
    if expected == "object":
        ...
        if len(value) < schema.get("minItems", 0):
            raise PlannerProtocolError(f"{path} has fewer than minItems")
        if "maxItems" in schema and len(value) > schema["maxItems"]:
            raise PlannerProtocolError(f"{path} has more than maxItems")
        if schema.get("uniqueItems") and len({json.dumps(item, sort_keys=True)
                                              for item in value}) != len(value):
            raise PlannerProtocolError(f"{path} items must be unique")
        if "items" in schema:
            for index, item in enumerate(value):
                _validate_schema(item, schema["items"], f"{path}[{index}]")
    if expected in ("integer", "number"):
        if "minimum" in schema and value < schema["minimum"]:
            raise PlannerProtocolError(f"{path} is below minimum")
        if "maximum" in schema and value > schema["maximum"]:
            raise PlannerProtocolError(f"{path} is above maximum")
