...
def main():
    ...
    if args.stage == 'initial':
        names = ['cause_live_v2.py', 'cause_interactive_v2.py', 'cause_session_v1.py',
                 'cause_socket_entry_v2.py', 'input_owner_v10.py', 'executor_v5.py',
                 'lease_cause_v1.py', 'lease_cause_v2.py', 'pointer_exchange_v1.py', 'decision_receipt_v1.py',
                 'decision_receipt_v2.py', 'event_socket_v11.py', 'receipt_image.py']
        save('plan', {'sources': {n: digest((HERE / n).read_bytes()) for n in names},
                     'allocation': 'one fresh Inkscape seed 208; retain failed stages; explicit recovery only',
