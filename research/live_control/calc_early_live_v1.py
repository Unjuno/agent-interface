...
if stage=='initial':
    save('plan',dict(seed=225,app='calc',scope='One live self-use with executor v8 early stopped response, full receipt and state table; no fault injection or speed comparison',
        model_tokens=None,verified_model_id=None,sources={n:digest((HERE/n).read_bytes()) for n in
        ['calc_early_live_v1.py','cause_servo_socket_v4.py','cause_servo_interactive_v4.py','executor_v8.py','post_release_observation_v2.py',
         'input_state_table_v1.py','decision_receipt_v4.py','early_exchange_v1.py','early_resume_v1.py','stopped_client_v1.py','stopped_socket_v1.py','stopped_cursor_v1.py','stopped_scope_v1.py']}))
    ...
    if stage=='finish':
        ...
        else:report=run(query,batch,runtime,stage,spec['steps'],30000,save)
        save('report',report)
        data=(out/'report.json').read_bytes();review=receipt(data);states=table(data)
        save('receipt',review);save('state-table',states)
        result=dict(lifecycle=report.get('lifecycle'),receipt=review,state_table=states,image=report.get('image'))
