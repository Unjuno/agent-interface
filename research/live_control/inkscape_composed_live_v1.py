...
from composed_result_v1 import build as compose, decode
...
if stage=='initial':
    save('plan',dict(seed=228,app='inkscape',scope='One live self-use with executor v8 early response plus at most one bounded read in the same outer operation, full receipt and state table; invalid numeric entry then screenshot-guided correction; no speed comparison',
        model_tokens=None,verified_model_id=None,sources={n:digest((HERE/n).read_bytes()) for n in
        ['presentation_emission_v1.py','composed_result_v1.py','shared_result_v1.py','inkscape_composed_live_v1.py','cause_servo_socket_v5.py','cause_servo_interactive_v4.py','executor_v8.py','post_release_observation_v2.py',
...
save('result',result)
presented=compose((out/'result.json').read_bytes())
assert canonical(decode(presented))==canonical(result)
save('presented',presented)
emit(presented,sys.stdout.buffer,out/'emission')
