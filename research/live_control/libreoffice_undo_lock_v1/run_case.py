...
def main():
    ...
    ap.add_argument('--scenario', choices=['undo_locked_stable','undo_locked_intervening_write'], required=True)
    ...
    try:
        ...
        um=doc.getUndoManager(); um.lock()
        t_lock=time.monotonic_ns(); lock_state=bool(um.isLocked())
        events.append({'event':'undo_lock_verified','t_ns':t_lock,'locked':lock_state})
        result['undo_lock_verified']=lock_state
        external=None
        if args.scenario=='undo_locked_intervening_write':
        ...
        result['undo_locked_before_unlock']=bool(um.isLocked())
        um.unlock(); t_unlock=time.monotonic_ns()
        events.append({'event':'undo_unlock','t_ns':t_unlock,'locked_after':bool(um.isLocked())})
        result['final_a_x']=x(A); result['final_b_x']=x(B); result['undo_locked_after_unlock']=bool(um.isLocked())
        ok = lock_state and result['undo_locked_before_unlock'] and not result['undo_locked_after_unlock'] and result['final_b_x']==5000
        if args.scenario=='undo_locked_stable': ok = ok and result['final_a_x']==1200
        else:
            ej=(external or {}).get('json') or {}
            ok = ok and external['returncode']==0 and ej.get('ok') is True and ej.get('undo_lock_seen_before') is True and ej.get('undo_lock_seen_after') is True and ej.get('before_x')==1000 and ej.get('after_x')==1700 and result['final_a_x']==1200
        ...
        try:
            if doc is not None:
                um2=doc.getUndoManager()
                if bool(um2.isLocked()): um2.unlock()
        except Exception: pass
        try:
