...
import time
import urllib.parse
import interactive_v28 as runtime


def fixture_server(path, attempt_log):
    lock = threading.Lock()
    class Handler(http.server.BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            with lock:
                with attempt_log.open('a') as log:
...
if __name__ == '__main__':
    ...
    archive = args.out.resolve() / 'submission-attempts.jsonl'
    runtime.suite.fixture_server = lambda path: fixture_server(path, archive)
    runtime.main()
