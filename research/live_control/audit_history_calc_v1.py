...
def main():
 ...
 for n in range(1,count+1):
  ...
  history=read(r/'history.json');expected_history=history[:n-1] if mode=='all' else history[max(0,n-2):n-1]
 ...
 report={'events':len(events),'exact_frames':len(obs),'append_records':1+2*len(calls),'socket_calls':len(calls)+2,'history_mode':mode,'model_turns':models,
         ...
         'scope':'one predeclared matched history-comparison episode; current outcome only, no population/human parity claim',
