...
control=HERE/'results/timing-envelope-openttd-control-08';control.mkdir(exist_ok=False)
def dump(path,value):
...
plan={'scope':'fresh bounded OpenTTD loop with buffered process-scoped end-to-end timing envelope; no matched comparison yet',
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['timing_envelope_openttd_supervisor_v8.py','timing_envelope_openttd_driver_v8.py','timing_envelope_v1.py','timing_envelope_v2.py','openttd_proposal_schema_v5.py','model_pair_runner_v2.py','session_v10.py','openttd_contact_sheet_v1.py']},
...
turns=[]
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_driver_v8.py')],
