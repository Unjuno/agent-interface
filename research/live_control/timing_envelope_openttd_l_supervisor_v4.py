"""Semantic-checkpoint supervisor with typed view recovery for OpenTTD L."""
...
base=HERE/'results/timing-envelope-openttd-l-04';root=base/arm;control=base/(arm+'-control');control.mkdir(parents=True,exist_ok=False)
def dump(path,value):
...
plan={'scope':'fresh seed-991003 OpenTTD five-tile L objective with typed view recovery','arm':arm,
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['timing_envelope_openttd_l_supervisor_v4.py','timing_envelope_openttd_l_driver_v3.py','timing_envelope_v1.py','timing_envelope_v2.py','semantic_checkpoint_v3.py','openttd_finish_outcome_v1.py','model_pair_runner_v2.py','session_v22.py','openttd_contact_sheet_v1.py']},
      'max_turns':12,'seed_semantics':'byte-pinned seed-991003 save; target tiles 977,978,979,1043,1107; closed toolbar',
      'model_route':'all gpt-6-astra medium','prompt_policy':'stateful observed/contradicted/uncertain checkpoint plus one typed tree-transparency view method','initial_ui_state':'closed toolbar; opaque trees; signs A, B corner, C and X keep clear'}
...
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_l_driver_v3.py'),arm,'timing-envelope-openttd-l-04'],stdout=(control/'driver-stdout.txt').open('wb'),stderr=(control/'driver-stderr.txt').open('wb'))
...
Every act must declare intent as progress, inspect or repair, and a concrete expected_effect. Checkpoint is null when no prior drag is pending. After a drag, the next proposal must bind prior_turn to that turn and classify its expected effect as observed, contradicted or uncertain with visual evidence. If uncertain, use intent inspect and only pointer_move/dwell_observe/observe, or request the typed view method below when opaque trees obstruct the road alignment. If contradicted, use intent repair or stop. A new progress mutation is allowed only after observed. Do not call an effect observed from a cost popup alone. The typed view method changes presentation only and keeps the pending checkpoint; inspect its returned screenshot before another task mutation.
...
{{"kind":"view","method":"openttd.transparent_trees","rationale":"why trees obstruct the pending effect","checkpoint":{{"prior_turn":INTEGER,"status":"uncertain","evidence":"visual evidence"}}}}
...
'''
        ...
        proposal=parse(items[0]['text'],checkpoint_required_turn);dump(root/f'typed-{index}.json',proposal);row={'turn':index,'model_runner_started_ns':begin,'model_runner_returned_ns':ended,'kind':proposal['kind'],'requested_model':'gpt-6-astra','requested_effort':'medium','checkpoint_required_turn':checkpoint_required_turn};turns.append(row);dump(control/'turns.json',turns)
        ...
        executable_steps=expand_view(proposal) if proposal['kind']=='view' else proposal['steps']
        ...
        observation=applied['result']['state']['continuation']['observation'];current_image=root/'runtime'/Path(observation['image']).name;planner_image=current_image if proposal['kind']=='view' else build_contact_sheet(current_image,applied,proposal,root/'runtime',root/f'planner-{index+1}.png');previous={'proposal':proposal,'resolution':applied['result']['state']['last_resolution']};new_required=next_checkpoint_turn(index,proposal)
