...
root=HERE/'results/timing-envelope-openttd-matched-05'/arm;root.mkdir(parents=True,exist_ok=False)
def dump(name,value):
...
names=['timing_envelope_openttd_matched_driver_v5.py','durable_submit_v4.py','append_checkpoint_v1.py','received_continuation_v1.py','received_exchange_v2.py','pointer_socket_entry_v7.py','event_socket_v11.py','model_pair_runner_v2.py','openttd_proposal_schema_v5.py','openttd_finish_outcome_v1.py','timing_envelope_openttd_matched_supervisor_v5.py','timing_envelope_v1.py','timing_envelope_v2.py','session_v22.py','openttd_contact_sheet_v1.py']
dump('plan.json',{'seed_semantics':'byte-pinned seed-991002 save; shifted viewport; closed toolbar','scope':'fresh seed-991002 shifted-geometry OpenTTD task with buffered supervisor TimingEnvelope v2',
     'arm':arm,'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in names}})
p=subprocess.Popen([sys.executable,'-u',str(HERE/'pointer_socket_entry_v7.py'),'openttd','serve','--',
...
try:
 ...
 for index in range(1,11):
  ...
  if proposal.get('finish') is True:
   ...
   outcome_name,outcome=classify_finish(path.name,evaluation,exit_code=code,
        proposals_executed=index-1,journal_calls=len(calls),
        reason=proposal.get('reason'),arm=arm)
   dump(outcome_name,outcome)
   break
  ...
  c=clock()
  program=steps+[{'op':'observe'}]
  dwell_seconds=sum(s.get('delay_ms',0) for s in program)/1000
  action_timeout=min(25,max(5,3+dwell_seconds+0.75*len(program)))
  applied=call({'command':{'op':'submit','expected_sequence':c['sequence'],'valid_until_ns':c['runtime_ns']+30_000_000_000,'steps':program},'timeout':action_timeout})
