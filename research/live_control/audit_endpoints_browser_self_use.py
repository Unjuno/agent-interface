...
HERE=Path(__file__).resolve().parent;root=HERE/'results/endpoints-browser-self-use-01'
...
assert all(r['clock_domain_id']==runtime_clock['domain_id'] for r in events)
phases={}
for name,client in (('navigate',nav),('submit',submit)):
    sidecar=json.loads((root/name/'client-endpoints.json').read_text())
    assert sidecar['clock']==client['clock']==runtime_clock
    marks=sidecar['endpoints']
    assert len(marks)==15 and list(marks.values())==sorted(marks.values())
    assert all(v is None for v in sidecar['unobserved_endpoints'].values())
    phases[name]={b.removesuffix('_ns'):(marks[b]-marks[a])/1e6 for a,b in zip(marks,list(marks)[1:])}
    phases[name]['main_to_flush_ms']=(marks['stdout_flush_returned_ns']-marks['client_main_entered_ns'])/1e6
navmarks=json.loads((root/'navigate/client-endpoints.json').read_text())['endpoints']
submarks=json.loads((root/'submit/client-endpoints.json').read_text())['endpoints']
report=dict(client_phase_ms=phases,
    form_client_flush_to_next_main_ms=(submarks['client_main_entered_ns']-navmarks['stdout_flush_returned_ns'])/1e6,
    first_capture_to_final_client_flush_ms=(submarks['stdout_flush_returned_ns']-first['capture_ns'])/1e6,
    actual={'value':['t991029']},exact_frames=count,programs=2,drain_attempted=False,socket_calls_before_cleanup=3,
...
(HERE/'results/endpoints-browser-self-use-audit.json').write_text(json.dumps(report,indent=2)+'\n')
(root/'client-fixture-sources.json').write_text(json.dumps({p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (Path(__file__),HERE/'prefilled_socket_entry_v2.py',HERE/'prefilled_browser_entry_v2.py',HERE/'prepared_exchange_v6.py',HERE/'prepare_program.py',HERE/'receipt_image.py',HERE/'outcome_wait.py',HERE/'drain_final.py',HERE/'unix_json_deadline.py',HERE/'event_socket_v12.py',HERE/'event_cursor_v5.py',HERE/'request_boundary_v2.py',HERE/'timing_clock.py')},indent=2)+'\n')
