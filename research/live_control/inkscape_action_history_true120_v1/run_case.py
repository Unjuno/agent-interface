...
def guard_selection_A(a0,out_path):
 ...
 return {'available':True,'selection_A':handles_selected(counts),'dark_pixels':counts,'box':box,'capture_ns':dt,'pixels':w*h,'rgb_sha256':hashlib.sha256(roi.tobytes()).hexdigest()}
def parse_svg(p):
...
def run(out,context,policy,display_num):
 ...
 try:
  ...
  for attempt in range(10):
   time.sleep(.05); imx=capture(out/f'setup_select_{attempt:02d}.png'); sc=select_score(imx,A,B); rec['setup_selection_samples'].append({'attempt':attempt,'score':sc})
   if sc['success']:im1=imx;break
  if im1 is None:raise RuntimeError('A selection not visibly established')
  rec['timestamps']['revalidate_start_ns']=time.monotonic_ns();im1=capture(out/'revalidated.png');rec['timestamps']['revalidate_capture_end_ns']=time.monotonic_ns();rec['revalidation']=select_score(im1,A,B);rec['timestamps']['revalidation_complete_ns']=time.monotonic_ns()
  if not rec['revalidation']['success']:raise RuntimeError('revalidation failed')
  anchor=rec['timestamps']['revalidation_complete_ns']
  if context=='self_switch':
   t0=time.monotonic_ns();key(s.d,'Tab');t1=time.monotonic_ns();rec['action_receipts'].append({'kind':'selection_navigation','key':'Tab','start_ns':t0,'end_ns':t1,'source':'controller'});anchor=t1
  rec['timestamps']['guard_anchor_ns']=anchor
  deadline=anchor+WAIT_MS*1_000_000
  while time.monotonic_ns()<deadline:time.sleep(.001)
  rec['timestamps']['guard_start_ns']=time.monotonic_ns();rec['guard']=guard_selection_A(A,out/'guard_roi.png');rec['timestamps']['guard_end_ns']=time.monotonic_ns();rec['post_anchor_guard_ns']=rec['timestamps']['guard_start_ns']-rec['timestamps']['guard_anchor_ns']
  relevant=[r for r in rec['action_receipts'] if r['kind']=='selection_navigation' and r['end_ns']>rec['timestamps']['revalidation_complete_ns']]
  ...
  rec['wrong_target_effect']=(context=='self_switch' and rec['effect_input_sent'] and abs(rec['delta']['A'])<.5 and rec['delta']['B']>.5)
  if context=='stable':rec['semantic_correct']=(rec['effect_input_sent'] and rec['delta']['A']>.5 and abs(rec['delta']['B'])<.5)
  elif context=='self_switch' and policy=='history_invalidate':rec['semantic_correct']=(not rec['effect_input_sent'] and rec['decision']=='STOP_HISTORY_INVALIDATED' and abs(rec['delta']['A'])<.5 and abs(rec['delta']['B'])<.5)
