...
def main():
    ...
    read.add_argument('--action-id');read.add_argument('--command-file',type=Path);read.add_argument('--request-id')
    args=ap.parse_args()
    if args.mode=='read':
    ...
    class Handler(socketserver.StreamRequestHandler):
        def handle(self):
            ...
            try:
                ...
                if not isinstance(command,dict) or (set(command)-{'action_id'}) not in ({'after','events','timeout'},{'after','events','timeout','command','request_id'}):raise ValueError('invalid request fields')
