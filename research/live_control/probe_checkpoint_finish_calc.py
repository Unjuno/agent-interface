...
from checkpoint_finish import run as checkpoint_finish
HERE=Path(__file__).resolve().parent;out=HERE/'results/checkpoint-finish-calc-01';out.mkdir(exist_ok=False)
root=out/'runtime';responses=[];clock=describe()
proc=subprocess.Popen([sys.executable,'-u',str(HERE/'event_socket_v16.py'),'serve','--','--app','calc',
...
def request(spec):
    ...
    (out/f'query-{len(responses)}-request.json').write_text(json.dumps(spec,indent=2)+'\n')
...
try:
    ...
    def query(after,identifier):
        ...
        return reply
    before_policy=checkpoint_finish(request,after=entry['cursor'],contract=contract,checkpoint_id='before',finish_id='finish-before',finish_on_match=True)
    (out/'before-policy.json').write_text(json.dumps(before_policy,indent=2)+'\n')
    assert not before_policy['finish_attempted'] and before_policy['task_success'] is None
    before=before_policy['exchanges'][0]['reply'];assert before['records'][-1]['evidence']['status']=='UNKNOWN'
    ...
    after_policy=checkpoint_finish(request,after=confirm['cursor'],contract=contract,checkpoint_id='after',finish_id='finish-after',finish_on_match=True)
    (out/'after-policy.json').write_text(json.dumps(after_policy,indent=2)+'\n')
    assert after_policy['finish_attempted'] and after_policy['task_success'] is True
    after=after_policy['exchanges'][0]['reply'];finish=after_policy['exchanges'][1]['reply']
    assert finish['records'][-1]['evaluation_request']['transport_request_id']=='finish-after'
    ...
    result=dict(verified_reply_to_finish_start_ms=(responses[-1]['started_ns']-responses[-2]['returned_ns'])/1e6,before_unknown_left_open=True,final_evaluation_scoped=True,samples=samples,replayed_after_source_deleted=True,exact_frames=frames,final_success=True,
    ...
    files=[Path(__file__),HERE/'checkpoint_finish.py',HERE/'request_boundary_v4.py',HERE/'event_cursor_v7.py',HERE/'effect_checkpoint_v2.py',HERE/'effect_checkpoint.py',HERE/'saved_effect.py',HERE/'event_socket_v16.py',HERE/'prepared_exchange_v6.py',HERE/'timing_clock.py']
