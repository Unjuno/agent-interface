"""Use one model decision after expiry before or after an irreversible boundary."""
import hashlib
import json
import subprocess
import sys
...
R = H / 'results/partial-terminal-live-01'
...
names = [
    'partial_terminal_live_v1.py', 'delayed_effect_socket_v2.py',
...
dump(R / 'plan.json', {
    'seed': 245, 'deadline_ms': 1100, 'delay_s': 5.0,
    'model': 'gpt-5.6-luna', 'effort': 'low',
    'scope': ('two fresh private Chromium sessions; model must distinguish expiry '
              'before Return from expiry after Return, then reach independent success'),
...
def episode(label, return_before_expiry):
    ...
    process = subprocess.Popen([
        ...
        '--', '--app', 'chromium', '--seed', '245', '--out', str(runtime_out),
...
Evidence: {json.dumps(strict)}'''
        ...
        statuses = []
        verify_deadline = time.monotonic() + 8
        while True:
            observed = checkpoint(contract)
            status = observed['checkpoint']['evidence']['status']
            statuses.append(status)
            if status == 'VERIFIED':
                break
            if time.monotonic() >= verify_deadline:
                raise TimeoutError('effect did not verify after safe decision')
            time.sleep(0.5)
