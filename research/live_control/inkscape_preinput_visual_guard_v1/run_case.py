...
def marker_box(a0):
 l,t,r,bt=a0[:4];return [l-18,t-10,l-4,bt+10]
...
def handles_selected(counts):
 return counts['top']>=30 and counts['right']>=80 and counts['bottom']>=30
...
def guard_selection_A(a0):
 l,t,r,b=map(int,a0[:4]);box=[l,t-20,r+20,b+20]
 ts=time.perf_counter_ns();roi=ImageGrab.grab(bbox=tuple(box)).convert('RGB');dt=time.perf_counter_ns()-ts
 # A-selection handles in current Inkscape theme are black arrows on top/right/bottom.
 # Avoid the left strip because ruler/window chrome contaminates it.
 ...
 selected=counts['top']>=30 and counts['right']>=80 and counts['bottom']>=30
 return {'available':True,'selection_A':selected,'dark_pixels':counts,'box':box,'capture_ns':dt,'pixels':w*h}
def parse_svg(p):
...
def run(out,context,policy,display_num):
 ...
 s=XSession(out,display_num);rec={'context':context,'policy':policy,'display':s.name,'wait_ms':WAIT_MS,'right_presses':RIGHT_PRESSES,'timestamps':{}}
 try:
  ...
  for attempt in range(10):
   time.sleep(.05)
   pth=out/f'setup_select_{attempt:02d}.png'; imx=capture(pth); sc=select_score(imx,A,B); rec['setup_selection_samples'].append({'attempt':attempt,'score':sc})
   if sc['success']:
    im1=imx; break
  if im1 is None: raise RuntimeError(f'A selection not visibly established: {rec["setup_selection_samples"][-1]}')
  ...
  if context=='switch':key(s.d,'Tab')
  deadline=phase+WAIT_MS*1_000_000
  while time.monotonic_ns()<deadline:time.sleep(.001)
  if policy=='guard':
   rec['timestamps']['guard_start_ns']=time.monotonic_ns();rec['guard']=guard_selection_A(A);rec['timestamps']['guard_end_ns']=time.monotonic_ns()
   if not rec['guard']['available'] or not rec['guard']['selection_A']:
    rec['decision']='DEPENDENCY_UNAVAILABLE';rec['effect_input_sent']=False
   else:rec['decision']='ADMIT';rec['effect_input_sent']=True
  else:
   rec['guard']=None;rec['decision']='ADMIT';rec['effect_input_sent']=True
  ...
  if context=='stable':rec['semantic_correct']=(rec['effect_input_sent'] and rec['delta']['A']>.5 and abs(rec['delta']['B'])<.5)
   ...
   rec['semantic_correct']=(policy=='guard' and not rec['effect_input_sent'] and rec['decision']=='DEPENDENCY_UNAVAILABLE' and abs(rec['delta']['A'])<.5 and abs(rec['delta']['B'])<.5)
   rec['wrong_target_effect']=(policy=='baseline' and rec['effect_input_sent'] and abs(rec['delta']['A'])<.5 and rec['delta']['B']>.5)
  (out/'result.json').write_text(json.dumps(rec,indent=2,sort_keys=True));return rec
 finally:s.close()
if __name__=='__main__':
 ap=argparse.ArgumentParser();ap.add_argument('--out',required=True);ap.add_argument('--context',choices=['stable','switch'],required=True);ap.add_argument('--policy',choices=['baseline','guard'],required=True);ap.add_argument('--display',type=int,required=True);a=ap.parse_args();print(json.dumps(run(a.out,a.context,a.policy,a.display),sort_keys=True))
