"""Single-owner executor with cancellation and accepted-program attestation."""
import threading
import time
import copy
import hashlib
...
class Executor:
    ...
    def submit(self,identifier,steps,expected_sequence,valid_until_ns):
        with self.lock:
            ...
            lease.check()
            attestation=program_sha256(steps)
            cancel=lease
            ...
            self.emit(dict(event='accepted',id=identifier,steps=len(steps),
                           program_sha256=attestation,
    ...
    def _run(self,identifier,steps,cancel):
        status='completed'; error=None; completed=0
        try:
