...
from durable_submit_v1 import initialize, run
...
def main():
    root = HERE / 'results/durable-inkscape-01'; root.mkdir(exist_ok=False)
    names = ['probe_durable_inkscape_v1.py', 'durable_submit_v1.py', 'received_continuation_v1.py', 'received_exchange_v2.py',
             'cause_servo_socket_v5.py', 'cause_servo_interactive_v4.py', 'stopped_socket_v1.py', 'unix_json_deadline.py']
    dump(root / 'plan.json', {'scope': 'scripted actual GUI, deliberate caller process exit after socket write, new process read-only recovery',
         'seed': 233, 'expected_svg': {'x': 96, 'y': 50, 'width': 40, 'height': 30},
         'sources': {n: hashlib.sha256((HERE / n).read_bytes()).hexdigest() for n in names}})
    p = subprocess.Popen([sys.executable, str(HERE / 'cause_servo_socket_v5.py'), 'inkscape', 'serve', '--',
                          '--app', 'inkscape', '--seed', '233', '--out', str(root / 'runtime')],
    ...
    try:
        ...
        dump(root / 'submit-spec.json', {'events': ['terminal'], 'timeout': 3,
             'command': {'op': 'submit', 'expected_sequence': c['sequence'], 'valid_until_ns': c['runtime_ns'] + 30_000_000_000,
        ...
        assert recovered['last_resolution']['terminal']['status'] == 'completed'
