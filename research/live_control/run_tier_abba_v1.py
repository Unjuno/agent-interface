...
root=HERE/'results/tier-abba-01';root.mkdir(exist_ok=False)
order=[('tier-abba-1-default','default'),('tier-abba-2-fast','fast'),('tier-abba-3-fast','fast'),('tier-abba-4-default','default')]
plan={'order':order,'seed':238,'model':'gpt-5.6-luna','effort':'low','max_turns':5,
      'scope':'same supervisor/runtime/prompt/full last outcome; requested default versus fast; effective tier unavailable; bounded to five turns',
      ...
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['run_tier_abba_v1.py','tier_calc_supervisor_v1.py','tier_calc_driver_v1.py','calc_proposal_schema_v1.py','model_tier_runner_v1.py']}}
(root/'plan.json').write_text(json.dumps(plan,indent=2)+'\n',encoding='utf-8');runs=[]
for label,mode in order:
    begin=time.perf_counter_ns()
    p=subprocess.Popen([sys.executable,str(HERE/'tier_calc_supervisor_v1.py'),label,mode],
