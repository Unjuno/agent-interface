...
H=Path(__file__).resolve().parent;R=H/'results/recovery-focus-ink-01';R.mkdir(exist_ok=False)
def dump(n,x):(R/n).write_text(json.dumps(x,indent=2)+'\n',encoding='utf-8')
def win(p):return 'C:'+str(p.resolve())[6:]
names=['recovery_focus_ink_v1.py','durable_submit_v4.py','append_checkpoint_v1.py','received_continuation_v1.py','received_exchange_v2.py','cause_servo_socket_v7.py','model_context_runner_v1.py','screenshot_responder_v1.txt','recover_once_v1.py','recover_once_v2.py','recovery_view_v1.py','calc_proposal_schema_v1.py']
...
try:
 ...
 # Identify only this bridge's descendant Xvfb, never the user's display.
 def descendants(pid):
  out=[]
  for child in Path(f'/proc/{pid}/task/{pid}/children').read_text().split():
   out.append(int(child));out.extend(descendants(int(child)))
  return out
 displays=[]
 ...
 dump('focus-fault.json',{'display':displays[0],'original_focus':original.id,'sink':sink.id,'injected_ns':time.perf_counter_ns(),'scope':'private descendant Xvfb after model return, before fresh admission'})
 try:
  c=clock()
  ...
  observation=fresh['state']['continuation']['observation']
  allowed=observation['pointer_binding']==source['pointer_binding'] and observation['input_focus_before']==observation['input_focus_after']
  assert not allowed,'fault did not invalidate existing pre-input gate'
  dump('refused.json',{'reason':'binding_changed_before_input','proposal':q,'source':source,'fresh':observation,'proposal_submitted':False,'refused_ns':time.perf_counter_ns()})
  state=fresh['state']
 finally:
  original.set_input_focus(X.RevertToParent,X.CurrentTime);sink.destroy();controller.sync();controller.close()
