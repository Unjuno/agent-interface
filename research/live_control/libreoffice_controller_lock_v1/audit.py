...
def main():
    ...
    errors=[]; counts={'locked_stable':0,'locked_intervening_write':0}; passes={'locked_stable':0,'locked_intervening_write':0}
    ...
    for i,scenario in enumerate(sched):
        ...
        expected=['fixture_ready','lock_verified'] + (['external_write_complete'] if scenario=='locked_intervening_write' else []) + ['stale_write','unlock']
        ...
        if not r.get('lock_verified') or not r.get('locked_before_unlock') or r.get('locked_after_unlock'): errors.append(f'{i}:lock_state')
        if r.get('final_b_x')!=5000: errors.append(f'{i}:B={r.get("final_b_x")}')
        if r.get('final_a_x')!=1200: errors.append(f'{i}:A={r.get("final_a_x")}')
        if scenario=='locked_intervening_write':
            ej=(r.get('external') or {}).get('json') or {}
            checks=[ej.get('ok') is True,ej.get('lock_seen_before') is True,ej.get('lock_seen_after') is True,ej.get('before_x')==1000,ej.get('requested_x')==1700,ej.get('after_x')==1700]
    ...
    if counts!={'locked_stable':5,'locked_intervening_write':5}: errors.append(f'counts={counts}')
    if passes!={'locked_stable':5,'locked_intervening_write':5}: errors.append(f'passes={passes}')
    outcome='CONTROLLER_LOCK_NOT_SEMANTIC_EXCLUSION_SCOPED' if not errors else 'AUDIT_FAIL'
