...
from phased_submit_v1 import execute as phased_submit
...
H=Path(__file__).resolve().parent;R=H/'results/shared-phased-ink-01';R.mkdir(exist_ok=False)
def dump(n,x):(R/n).write_text(json.dumps(x,indent=2)+'\n',encoding='utf-8')
def win(p):return 'C:'+str(p.resolve())[6:]
names=['shared_phased_ink_v1.py','durable_submit_v4.py','append_checkpoint_v1.py','received_continuation_v1.py','received_exchange_v2.py','cause_servo_socket_v8.py','model_context_runner_v1.py','screenshot_responder_v1.txt','recover_once_v1.py','recover_once_v2.py','recovery_view_v1.py','calc_proposal_schema_v1.py','sampled_target_contract_v1.py','passive_pair_v1.py','activation_handoff_v1.py','phased_submit_v1.py']
...
try:
 ...
 for index in range(2,7):
  ...
  if checked['eligible']:
   phases=phased_submit(call,q,checked,observation)
   dump(f'phases-{index}.json',phases)
   assert phases['status']=='completed',phases.get('reason')
   applied=phases['result'];row['activation']=phases['activation']
   if 'handoff' in phases:row['handoff']=phases['handoff']
