"""Matched visual-planner preparation after release versus after terminal."""
import contextlib
import hashlib
import json
import os
...
def verify(plan):
    ...
    checks.update({"output_absent": not (REPO / plan["output"]).exists(),
        ...
        "thresholds": plan["thresholds_ms"] == {
            "focus_to_early_useful_ready_lte": 200,
            "useful_ready_advantage_gte": 5,
            "focus_to_terminal_useful_ready_lte": 300}})
    return checks
...
def main():
    ...
    try:
        ...
        identifier = "release-planner-pointer-click-01"
        ...
        checks = {
            ...
            "no_candidate_input_admitted": len([row for row in events
                if row.get("event") == "accepted"]) == 1 and
                early["reconciled_state"]["may_submit_to_executor"] is False,
            "round_trip_accounting": early["terminal_exchange"]["request"]["request_id"] ==
                "early-resume",
            "early_useful_ready_threshold": metrics["focus_to_early_useful_ready_ms"] <= 200,
            "useful_ready_advantage_threshold": metrics["useful_ready_advantage_ms"] >= 5,
            "terminal_useful_ready_threshold":
                metrics["focus_to_terminal_useful_ready_ms"] <= 300,
