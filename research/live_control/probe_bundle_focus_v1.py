"""Three real X11 focus faults in the registered nine-step bundle; no reruns."""
import contextlib
import json
import shutil
import threading
...
root=HERE/'results/bundle-focus-01';root.mkdir(exist_ok=False)
source=HERE/'results/bundle-pair-01/B/bundle/steps.json'
steps=json.loads(source.read_text())
assert len(steps)==9
names=['probe_bundle_focus_v1.py','cause_servo_session_v1.py','cause_session_v1.py','executor_v5.py',
       'lease_cause_v1.py','lease_cause_v2.py','input_owner_v10.py','session_v21.py','session_v15.py','session_v13.py','session_v9.py']
plan=dict(seed=224,cases=[['select_button',0,'button'],['field_button',2,'button'],['select_all_modifier',3,'key']],
          bundle_source_sha256=digest(source.read_bytes()),sources={n:digest((HERE/n).read_bytes()) for n in names},
          protocol='Block emitter after chosen physical admission; transfer focus using separate X connection, verify release before unblocking, restore focus, verify no tail and fresh-observation cause isolation.',
...
for name,target,kind in plan['cases']:
    ...
    events=[];result=dict(case=name,target_step=target,kind=kind,success=False)
    ...
    try:
        def emit(event):
            ...
            chosen=(event.get('step')==target and event.get('id')=='bundle' and
        ...
        original.set_input_focus(X.RevertToParent,X.CurrentTime);controller.sync();resume.set()
        ...
        assert all(e['step']<=target and e['admitted_ns']<=cause['verified_ns'] for e in admitted)
        ...
        result['fresh_observation_same_deadline_no_cause']=True;result['success']=True
