...
def main():
    ...
    report=dict(request_id=identifier,program_id=args.program_id,started_ns=time.perf_counter_ns(),authority='none')
    try:
        ...
        if args.drain_final and args.boundary=='outcome':
            def read_tail(spec):
                ...
                with socket.socket(socket.AF_UNIX,socket.SOCK_STREAM) as connection:
                    ...
                    with connection.makefile('rb') as response:tail=json.loads(response.readline())
