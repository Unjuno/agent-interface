"""Audit retained calibration failures and the selected live partial-terminal run."""
import hashlib
import json
import sys
import urllib.parse
...
LIVE = H / 'results/partial-terminal-live-01'
...
assert live_plan['seed'] == 245 and live_plan['deadline_ms'] == 1100
...
for label, want in expected.items():
    ...
    assert result['usage']['input_tokens'] == 9803
...
report = {
    'scope': ('two fresh private Chromium sessions; same page and goal; real expiry '
              'before/after Return drives different model decisions and safe completion'),
