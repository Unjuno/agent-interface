...
SUPPORTED=ADMITTED|{'command','rejected','finalization_status','effect_checkpoint'}


def request_boundary(record,events,request_id):
    name=record['event']
    if name not in events and name!='rejected':return None
    if name=='independent_evaluation' and 'evaluation_request' in record:
        metadata=record['evaluation_request']
        if not isinstance(metadata,dict):return 'identity_unknown'
        if 'admitted_request' in record or metadata.get('command_op')!='finish':return 'identity_conflict'
