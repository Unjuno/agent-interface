...
H=Path(__file__).resolve().parent;R=H/'results/expiry-observation-controls-02';R.mkdir(exist_ok=False);rows=[]
for case in ['expiry','release_failed','explicit_stop']:
 ...
 with executor.lock:job=executor.active
 if job is not None:
  job[2].join(timeout=2);assert not job[2].is_alive()
 executor.close()
