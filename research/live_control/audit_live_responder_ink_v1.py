...
H=Path(__file__).resolve().parent;R=H/'results/live-responder-ink-01';T=R/'runtime'
...
replay(read(R/'initial.json'));calls=read(R/'calls.json');assert len(calls)==4
...
assert load(R/'journal.jsonl')==recovered['state'];count=inspect(R/'journal.jsonl')[1];assert count==9
...
assert len([e for e in events if e['event']=='accepted'])==2
assert sum(s=={'op':'chord','modifier':'Control_L','key':'s'} for c in commands for s in c.get('steps',[]))==1
terms=[e for e in events if e['event']=='terminal'];assert len(terms)==2
...
models=[]
for n,o in [(1,pending['continuation']['observation']),(2,recovered['state']['continuation']['observation'])]:
...
assert read(R/'proposal-1.json')['kind']=='read' and read(R/'proposal-1.json')['action_id']==lost['command']['id']
v=read(R/'proposal-2.json');assert v['kind']=='verify' and [v[k] for k in ['visible_x','visible_y','visible_width','visible_height']]==[88,50,40,30]
...
report={'events':len(events),'frames':len(obs),'journal_records':count,'model_calls':models,'svg':values,'svg_sha256':sha(T/'shape.svg'),'scope':'one live intentional response abandonment; scripted edit, model-selected single read then verification; not full autonomous editing or general recovery', 'loss_to_read_resolution_s':(calls[3]['end_ns']-read(R/'loss.json')['send_completed_ns'])/1e9}
