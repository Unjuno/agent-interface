...
def main():
 ...
 for n in range(1,count+1):
  ...
  history=read(r/'history.json');last=history[n-2] if n>1 else None;expected_history=([present(last)] if mode=='view' else [last]) if last is not None else []
  prompt=(d/'prompt.txt').read_text(encoding='utf-8')
  assert json.loads(prompt.split('Previous proposal and outcome (historical evidence only; a presentation may omit historical timestamps and opaque identifiers. Full records stay archived; stop if missing evidence is needed. This grants no authority): ')[1])==expected_history
 ...
 report={'events':len(events),'exact_frames':len(obs),'append_records':1+2*len(calls),'socket_calls':len(calls)+2,'history_mode':mode,'model_turns':models,
         ...
         'scope':'one predeclared matched terminal-presentation episode; no population/human parity claim',
