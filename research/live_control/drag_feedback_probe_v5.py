...
from session_v11 import Backend,suite
from input_owner_diagnostic import InputOwner
...
def main():
    ...
    names=['session_v11.py','session_v9.py','session_v8.py','session_v7.py','session_v6.py','session_v5.py','session_v4.py','input_owner_v5.py','executor_v3.py','lease.py','quiet_window.py']
    sources=[HERE/n for n in names]+[HERE/'input_owner_diagnostic.py',Path(__file__).resolve(),HERE.parent/'observation_gating/gui_suite.py',HERE.parent/'observation_tiles/tile_transport.py',HERE.parent/'observation_tiles/image_artifact.py',HERE.parent/'real_apps_v1/real_app_suite_v1.py']
    ...
    try:
        ...
        backend.owner.close();backend.owner=InputOwner(s.name)
        ...
        time.sleep(.6);backend.snapshot('complete-fresh',0)
        ...
        for mode in ['cancel','expiry']:
            # Separate gestures in test setup and establish fresh post-action focus.
            # Without this, rapid repeated clicks can change the real application's tool/focus.
            time.sleep(.6);backend.snapshot(mode+'-fresh',0)
