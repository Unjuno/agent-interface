...
assert row['feedback'] == present(losses[0]['recovered']['state']['last_resolution'], recovered=True)
model_dir = R / 'model-1'
...
assert 'strict compact planner view' in prompt and 'one command-free read' in prompt
...
report = {'scope': 'one fresh private Chromium form episode; strict compact v2 evidence drives actual model form input; deliberate local connection abandonment',
          ...
          'input_after_verified': 0, 'model_evidence_bytes': len(json.dumps(row['feedback'], separators=(',', ':')).encode()),
          'model_evidence_profile': 'strict planner_evidence_v2; raw durable records retained outside model view',
