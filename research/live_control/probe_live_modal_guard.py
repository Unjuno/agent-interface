...
from guarded_modal_backend import Backend,suite
...
HERE=Path(__file__).resolve().parent;out=HERE/'results/live-modal-guard-01';out.mkdir(exist_ok=False)
...
for case in ('normal','focus_change','move','age'):
    ...
    try:
        ...
        else:assert terminal['status']=='needs_decision'
        engine.close()
...
paths=[Path(__file__),HERE/'guarded_modal_backend.py',HERE/'bound_modal_proposal.py',HERE/'modal_visual_predicate_v2.py',HERE/'modal_visual_predicate.py',HERE/'executor_v3.py',HERE/'session_v16.py',HERE/'input_owner_v9.py',template]
