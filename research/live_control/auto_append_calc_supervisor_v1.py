...
root=HERE/'results/auto-append-calc-01'
control=HERE/'results/auto-append-calc-control-01';control.mkdir(exist_ok=False)
def dump(path,value):
...
plan={'scope':'bounded autonomous screenshot proposal/action handoff on isolated Calc; no general supervisor claim',
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['auto_append_calc_supervisor_v1.py','auto_append_calc_driver_v1.py','calc_proposal_schema_v1.py','model_pair_runner_v1.py']},
...
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'auto_append_calc_driver_v1.py')],
...
def wait_file(path,seconds=120):
    ...
    return read(path)
try:
    ready=wait_file(root/'ready.json');goal=ready['goal'];observation=ready['observation'];previous=None
