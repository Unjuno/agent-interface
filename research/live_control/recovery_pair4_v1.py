"""Explicit per-model-call stages for registered OpenTTD pair 4; no automatic task loop."""
import argparse
import hashlib
import json
import time
...
def main():
    ...
    ap.add_argument('stage', choices=['initial', 'prepare', 'recover1', 'recover2', 'recover3', 'recover4', 'open', 'build', 'finish'])
    ...
    try:
        if args.stage == 'initial':
            ...
            save('execution', {'pair': 4, 'domain': 'openttd', 'seed': 204, 'seed_semantics': 'pair label; canonical save fixes world, no RNG seed override', 'depth': 3, 'additional_historical_clocks': 3, 'expected_recovery_reads_including_own': 4, 'arm': args.arm,
                 ...
                 'presentation': 'same assembled history at recovery; reversible caller views for open and build',
            ...
            cursor = advance['last_reply']['cursor']
            queued = []
            for index in range(3):
                own = uuid.uuid4().hex
                r = query({'after': cursor, 'events': ['clock'], 'timeout': .25, 'request_id': own, 'command': {'op': 'clock'}})
                assert r['records'][-2]['command']['transport_request_id'] == own
                assert r['records'][-1]['event'] == 'clock'
                queued.append(r)
                cursor = r['cursor']
            save('queued-clocks', queued)
            ...
            result = {'advance': pack(advance), 'stale': pack(stale), 'queued_clocks': queued}
        elif args.stage.startswith('recover'):
            stale = read('prepare', 'stale-right-report')
            if args.arm == 'A':
                number = int(args.stage[-1])
                segments = list(stale['exchanges'])
                for index in range(1, number):
                    segments += read('recover' + str(index), 'new-slices')
                previous = assemble(segments)
                q = {'after': previous['review_batch']['cursor'], 'events': ['clock'], 'timeout': .25}
                reply = query(q)
                new = [{'request': q, 'reply': reply}]
                save('new-slices', new)
                history = assemble(segments + new)
                records = history['review_batch']['records']
                own = stale['exchanges'][0]['request']['request_id']
                found = records[-2].get('command', {}).get('transport_request_id') == own
                assert records[-1]['event'] == 'clock'
                assert found == (number == 4)
            else:
                assert args.stage == 'recover1'
            ...
            r = act('open-toolbar', read('recover4' if args.arm == 'A' else 'recover1', 'batch'), [
