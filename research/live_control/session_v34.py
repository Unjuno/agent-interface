"""Add bounded delayed-hover observations to the point-target runtime."""
from executor_v3 import Cancelled
from session_v33 import Backend as Previous
    ...
    def validate(self, steps):
        ...
        for step in steps:
            if isinstance(step, dict) and step.get("op") == "dwell_observe":
                if (set(step) != {"op", "delay_ms"}
                        or type(step["delay_ms"]) is not int
                        or not 100 <= step["delay_ms"] <= 2000):
