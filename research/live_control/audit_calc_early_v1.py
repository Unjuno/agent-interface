...
HERE=Path(__file__).resolve().parent;root=HERE/'results/calc-early-live-01';runtime=root/'runtime'
...
for stage in ('enter','save','save-followup','confirm','confirm-followup'):
    ...
    if stage in ('save','confirm'):
        assert tracker.view()['state']=='input_stopped_capture_pending' and 'terminal' not in r
    else:
    ...
    if stage.endswith('-followup'):
        initial=read(root/(stage.removesuffix('-followup'))/'report.json')
        ...
        metrics.append(dict(action=c['id'],submit_to_early_reply_ms=(exchanges[1]['returned_ns']-exchanges[1]['started_ns'])/1e6,
