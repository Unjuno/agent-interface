...
def main():
    ...
    boundary=HERE/'results/pointer-focus-01'
    for name,h in json.loads((boundary/'sources.json').read_text()).items():assert hashlib.sha256((HERE.parent/name).read_bytes()).hexdigest()==h
    checks=json.loads((boundary/'results.json').read_text());assert len(checks)==8 and all('error' not in r for r in checks)
    diagnostics=json.loads((HERE/'results/drag-feedback-08/owner-events.json').read_text())
    assert any(r.get('event')=='focus_mismatch_diagnostic' and r['actual_focus']==6291463 and r['expected_focus']==6291464 for r in diagnostics)
    (HERE/'results/drag-focus-audit.json').write_text(json.dumps({'boundary_checks':checks,'feedback_cohorts':report},indent=2)+'\n')
