...
def _fake_input(display_obj, *args, **kwargs):
    # Additive allocation instrumentation: record each XTEST request before
    # delegating to the promoted Xlib call. It is disabled unless requested.
    import json, os
    path = os.environ.get('AGENT_INTERFACE_XTEST_LOG')
    if path:
        with open(path, 'a', encoding='utf-8') as stream:
            stream.write(json.dumps({'args': [str(x) for x in args], 'kwargs': kwargs}) + '\n')
    return xtest.fake_input(display_obj, *args, **kwargs)
...
class X11Backend:
    ...
    def pointer_move(self, target: str, frame: str, x: int, y: int) -> None:
        ...
        _fake_input(self.d, X.MotionNotify, x=rx, y=ry)
        self.emissions += 1
        self.d.sync()

    def pointer_button(self, button: str, down: bool) -> None:
        number = BUTTON_MAP[button]
        _fake_input(self.d, X.ButtonPress if down else X.ButtonRelease, number)
        self.emissions += 1
        self.d.sync()
        if down:
    ...
    def key_state(self, key: str, down: bool) -> None:
        ...
        _fake_input(self.d, X.KeyPress if down else X.KeyRelease, code)
        self.emissions += 1
        self.d.sync()
        if down:
    ...
    def scroll(self, dx: int, dy: int) -> None:
        ...
        for number in buttons:
            _fake_input(self.d, X.ButtonPress, number)
            _fake_input(self.d, X.ButtonRelease, number)
            self.emissions += 2
        self.d.sync()
    ...
    def release_all(self) -> dict[str, Any]:
        ...
        for code in list(self.held_keycodes.values()):
            _fake_input(self.d, X.KeyRelease, code)
            self.emissions += 1
        for button in list(self.held_buttons):
            _fake_input(self.d, X.ButtonRelease, BUTTON_MAP[button])
            self.emissions += 1
        self.d.sync()
