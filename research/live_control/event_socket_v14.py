...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='agent-interface-events-') as private:
        ...
        with BoundedServer(str(socket_path),Handler) as server:
            ...
            process=subprocess.Popen([sys.executable,'-u',str(Path(__file__).with_name('interactive_v29.py')),*runtime_args],stdin=subprocess.PIPE,stdout=subprocess.PIPE,bufsize=0)
