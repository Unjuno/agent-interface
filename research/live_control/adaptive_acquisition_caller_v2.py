"""Shared acquisition and execution boundary with typed local yields.

Version 2 preserves the v1 accounting contract and additionally carries a
bounded local executor's safe-yield reason and completed-action count across
...
def _execution_decision(value):
    """Validate the outer contract for one completed execute adapter call."""
    if type(value) is not dict or type(value.get("status")) is not str:
    ...
    if status in {"completed", "delivery_uncertain", "failed"}:
        ...
        return copy.deepcopy(value)
    if status != "safe_yield":
        raise ValueError("unsupported execution decision status " + status)
    if set(value) != {"status", "reason", "completed_actions"}:
        raise ValueError("safe_yield requires status, reason and completed_actions")
    if value["reason"] not in LOCAL_EXECUTION_YIELD_REASONS:
        raise ValueError("unsupported local execution yield reason " +
                         str(value["reason"]))
...
def run(spec, adapters, *, clock=time.perf_counter_ns, id_factory=None):
    ...
    def finish(outcome, reason, *, task_effect=None, delivery=None,
        ...
        result = {"outcome": outcome, "reason": reason,
                  ...
                  "execution_progress": copy.deepcopy(execution_progress),
                  ...
                  "input_authority": (
                      "none" if (execution_progress is not None and
                          execution_progress.get("status") == "safe_yield" and
                          execution_progress.get("completed_actions") == 0)
                      else "consumed_by_recorded_execute_stage"
    ...
    try:
        ...
        execution = _execution_decision(local("execute", {"target": selected,
                                                            "check": revalidation}))
        if execution["status"] == "safe_yield":
            delivery = ("confirmed_partial" if execution["completed_actions"] > 0
                        else "not_attempted")
            return finish("EXECUTION_INCOMPLETE", execution["reason"],
                          delivery=delivery, execution_progress=execution)
