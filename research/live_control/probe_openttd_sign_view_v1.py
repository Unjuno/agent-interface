...
HERE=Path(__file__).resolve().parent;ROOT=HERE/'results/openttd-sign-view-01';RUN=ROOT/'run';RUN.mkdir(parents=True,exist_ok=False)
def dump(name,value):
...
try:
    ...
    trees=call({'command':{'op':'submit','expected_sequence':current['sequence'],'valid_until_ns':current['runtime_ns']+10_000_000_000,'steps':[{'op':'chord','modifier':'Control_L','key':'2'},{'op':'observe'}]},'timeout':5});before_sign=trees['state']['continuation']['observation'];dump('trees-transparent.json',trees);current=clock()
    ...
    result={'executed':True,'model_calls':0,'precondition_step':{'op':'chord','modifier':'Control_L','key':'2'},'step':{'op':'chord','modifier':'Control_L','key':'1'},'before_image':before_image.name,'after_image':after_image.name,'before_sha256':sha(before_image),'after_sha256':sha(after_image),'image_changed':sha(before_image)!=sha(after_image),'evaluation':evaluation,'durable_calls':len(calls),'save_unchanged':json.loads((RUN/'runtime/cleanup.json').read_text())['save_unchanged']}
