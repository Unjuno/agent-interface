...
for n in (4,5,6):
    ...
    for color in ('blue','red'):
        ...
        if color=='blue' and n in (5,6):assert result['precision_pass'] and result['terminal']['status']=='completed'
...
(HERE/'results/servo-trim-audit.json').write_text(json.dumps(rows,indent=2)+'\n');print(json.dumps(rows,indent=2))
