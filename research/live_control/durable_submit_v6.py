"""Linux single-writer submit journal with bounded effect waits.

Adds explicitly scoped artifact checkpoints; existing submit/clock semantics retained. No retry/reset escape hatch.
...
from checkpoint_contract_v1 import validate as validate_contract, valid_reply
...
def initialize(path, continuation):
    with locked(path):
        ...
        store(path, {'format': 'durable-submit-v6', 'continuation': continuation,
...
def reconcile(pending, records):
    ...
    for event in records:
        ...
        if expected['op'] == 'clock':
            ...
            continue
        if expected['op'] == 'effect_checkpoint':
            request_id = p['request']['request_id']
            if (kind == 'rejected' and event.get('op') == 'effect_checkpoint' and
                    event.get('transport_request_id') == request_id):
                return None, {'request_id': request_id, 'rejected': event, 'authority': 'none'}
            if kind == 'effect_checkpoint' and event.get('transport_request_id') == request_id:
                if valid_reply(event, expected['contract'], request_id):
                    return None, {'request_id': request_id, 'checkpoint': event, 'authority': 'none'}
                p['conflict'] = True
            continue
...
def run(path, spec, call=exchange):
    with locked(path):
        state = load(path)
        if state.get('format') != 'durable-submit-v6' or state.get('authority') != 'none':
        ...
        operation = spec.get('command', {}).get('op') or (state['pending']['request']['command']['op'] if state['pending'] else None)
        default_events = ['effect_checkpoint','rejected'] if operation == 'effect_checkpoint' else ['clock'] if operation == 'clock' else ['terminal']
        ...
        if 'command' in spec:
            ...
            if command.get('op') not in ('submit', 'clock', 'effect_checkpoint') or 'id' in command or 'transport_request_id' in command:
                raise ValueError('submit/clock/effect_checkpoint only; journal assigns unique identities')
            if command['op'] == 'clock' and command != {'op': 'clock'}: raise ValueError('clock accepts no extra fields')
            if command['op'] == 'effect_checkpoint':
                if set(command)-{'op','contract','wait_ms','poll_ms'}:raise ValueError('unexpected checkpoint fields')
                if set(command) not in ({'op','contract'}, {'op','contract','wait_ms','poll_ms'}):
                    raise ValueError('checkpoint wait requires both wait_ms and poll_ms')
                command['contract'] = validate_contract(command['contract'])
                if 'wait_ms' in command:
                    if type(command['wait_ms']) is not int or not 0 <= command['wait_ms'] <= 10_000:
                        raise ValueError('wait_ms must be integer 0..10000')
                    if type(command['poll_ms']) is not int or not 10 <= command['poll_ms'] <= 500:
                        raise ValueError('poll_ms must be integer 10..500')
                    if command['wait_ms'] and command['poll_ms'] > command['wait_ms']:
            ...
            if command['op'] == 'effect_checkpoint':q['read_request_id'] = q['request_id']
        ...
        if 'command' not in q and state['pending'] and operation == 'effect_checkpoint':
            q['read_request_id'] = state['pending']['request']['request_id']
