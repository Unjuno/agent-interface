...
def main():
    ...
    ap.add_argument('--domain',choices=list(TASKS),required=True)
    ...
    dump(a.out/'plan.json',{'scope':'scripted adapter readiness, fixed known replay; no performance comparison',
        'domains':[a.domain],'stress':['already expired request: reject before input','cancel active Shift hold: release and cancelled terminal'],
    ...
    for domain,(relative,identifiers) in TASKS.items():
        ...
        try:
            ...
            if domain=='openttd':args += ['--controller','scripted']
