"""Candidate X11 backend for task-relative displacement continuation checks."""
import time

import numpy as np

from executor_v3 import Cancelled, DecisionRequired
from local_displacement_postcondition_v1 import LocalDisplacementPostcondition
from local_visual_barrier_program_v1 import MUTATING
from session_v23 import Backend as Previous, suite


DISPLACEMENT_SCHEMA = {
    "op": "local_displacement_postcondition",
    "placement": "after an admitted mutation, optionally separated by passive settling, and before a later admitted mutation",
    "required": {
        "postcondition_id": "unique nonempty string up to 64 characters",
        "source_sequence": "latest observation sequence at whole-program admission",
        "box": "[x,y,width,height] RGB patch; each dimension 4..96",
        "target_delta": "[dx,dy] integers; each -32..32",
        "tolerance_px": "integer 0..3",
        "required_samples": "integer 2..3",
        "sample_interval_ms": "integer 20..500",
        "timeout_ms": "integer 100..3000; combined at most 3000",
        "on_unmet": "needs_decision",
    },
    "fixed_matcher": {"search_radius_px": 32, "max_error": 0.03,
                      "minimum_margin": 0.005},
    "authority": "condition for already admitted later steps only; no semantic, task-success or new-input authority",
}


class Backend(Previous):
    def __init__(self, session, out, emit):
        super().__init__(session, out, emit)
        self.displacement_postconditions = {}

    def pixels(self):
        return np.asarray(self.image()).copy()

    def validate(self, steps):
        rewritten = []
        conditions = {}
        total_timeout_ms = 0
        for index, step in enumerate(steps):
            if not isinstance(step, dict) or step.get("op") != "local_displacement_postcondition":
                rewritten.append(step)
                continue
            if index == 0 or index + 1 >= len(steps):
                raise ValueError("postcondition must be between admitted steps")
            if not any(isinstance(earlier, dict) and earlier.get("op") in MUTATING
                       for earlier in steps[:index]):
                raise ValueError("postcondition must follow a mutating step")
            if not any(isinstance(later, dict) and later.get("op") in MUTATING
                       for later in steps[index + 1:]):
                raise ValueError("postcondition must guard a later mutating step")
            identifier = step.get("postcondition_id")
            if identifier in conditions:
                raise ValueError("postcondition_id must be unique within a program")
            conditions[identifier] = LocalDisplacementPostcondition(
                step, self.pixels(), self.sequence, self.frame_identity())
            total_timeout_ms += step["timeout_ms"]
            rewritten.append({"op": "observe"})
        if total_timeout_ms > 3000:
            raise ValueError("combined displacement postcondition timeout exceeds 3000ms")
        super().validate(rewritten)
        self.displacement_postconditions = conditions

    def execute(self, step, cancel, identifier, index):
        if step["op"] != "local_displacement_postcondition":
            return super().execute(step, cancel, identifier, index)
        condition = self.displacement_postconditions.pop(step["postcondition_id"], None)
        if condition is None:
            raise DecisionRequired("local displacement postcondition unavailable")
        samples = []
        sequences = []
        bindings = []
        sampled_ns = []
        for sample_index in range(condition.spec["required_samples"]):
            if sample_index:
                if cancel.wait(condition.spec["sample_interval_ms"] / 1000):
                    raise Cancelled()
                self.snapshot(identifier, index)
            if cancel.is_set():
                raise Cancelled()
            samples.append(self.pixels())
            sequences.append(self.sequence)
            bindings.append(self.frame_identity())
            sampled_ns.append(time.perf_counter_ns())
        outcome = condition.evaluate(samples, sequences, bindings, sampled_ns)
        self.emit({"event": "local_displacement_postcondition", "id": identifier,
                   "step": index, "sequence": self.sequence, **outcome})
        if not outcome["continue_program"]:
            raise DecisionRequired("local displacement postcondition " + outcome["reason"])
        return outcome
