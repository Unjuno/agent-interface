...
def main():
    ...
    sources=[Path(__file__).resolve(),HERE/'input_owner_v5.py',HERE/'lease.py',HERE/'executor_v3.py']
    ...
    def lease(ms=1000):
        l=Lease(time.perf_counter_ns()+ms*1_000_000);l.expected_focus=first.id;l.expected_surface=first.id;l.expected_geometry=[50,80,200,180];return l
    ...
    def released(label,l,reason):
        ...
        assert any(r.get('reason') in (reason if isinstance(reason,tuple) else (reason,)) and r['verified'] and r.get('valid_until_ns')==l.deadline for r in owner.records),label
    ...
    try:
        ...
        reject('wrong_focus_move',lambda:owner.call('move',lease(),{'x':100,'y':120}),DecisionRequired)
        first.set_input_focus(X.RevertToParent,X.CurrentTime);d.sync()
        ...
        assert owner.call('surface_context',key=first.id)==[50,80,200,180]
        l=lease();first.configure(x=60);d.sync()
        reject('moved_surface_rejects_old_geometry',lambda:owner.call('move',l,{'x':100,'y':120}),DecisionRequired)
        first.configure(x=50);d.sync()
        l=lease();owner.call('button_down',l,1);first.configure(width=210);d.sync()
        released('resized_surface_releases_hold',l,'surface_changed')
        first.configure(width=200);d.sync()
        l=lease();owner.call('button_down',l,1);first.destroy();d.sync()
        released('destroyed_surface_releases_hold',l,('focus_changed','surface_changed'))
        reject('destroyed_surface_query',lambda:owner.call('surface_context',key=first.id),DecisionRequired)
        first=window(50,80);first.set_input_focus(X.RevertToParent,X.CurrentTime);d.sync()
        # Existing keyboard path remains usable in this same owner.
        l=lease();owner.call('down',l,'a')
        code=d.keysym_to_keycode(XK.string_to_keysym('a'));bitmap=d.query_keymap()
        assert bitmap[code//8] & (1 << (code%8))
        owner.call('up',l,'a');owner.call('release',l)
