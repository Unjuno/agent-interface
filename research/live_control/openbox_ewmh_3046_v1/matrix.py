...
def one(display, configured):
    ...
    try:
        ...
        wm=subprocess.Popen(['openbox','--sm-disable'],env=env,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL); procs.append(wm); time.sleep(.8)
        ...
        return {'configured':configured,'windows':rows,'rootprops':rootprops,'active_before':active,'target':target,'activate':act,'active_after':after}
    finally:
        for p in reversed(procs):
            if p.poll() is None: p.terminate()
results=[one(':143',False),one(':144',True)]
print(json.dumps({'decision':'PASS_EWMH_MATRIX' if any(r['rootprops']['_NET_ACTIVE_WINDOW']['out'] and 'not found' not in r['rootprops']['_NET_ACTIVE_WINDOW']['out'] for r in results) else 'HOLD_NO_EWMH_ACTIVE_WINDOW','results':results,'model_calls':0,'network_calls':0},sort_keys=True))
