"""Typed semantic registry with optional current-surface binding."""
import copy
import time
...
try:
    ...
    from .target_relative_crop_semantic_probe_v1 import (
        CONTRACT_SCHEMA as RELATIVE_CONTRACT, SCORE_SCHEMA as RELATIVE_SCORE,
        reconcile_artifact as reconcile_relative, score_frame as score_relative,
        validate as validate_relative)
except ImportError:
    from exact_crop_semantic_probe_v1 import (CONTRACT_SCHEMA as CROP_CONTRACT,
        SCORE_SCHEMA as CROP_SCORE, reconcile_artifact as reconcile_crop,
        score_frame as score_crop, validate as validate_crop)
...
def _validate(contract):
    ...
    if schema == CROP_CONTRACT: return validate_crop(contract)
    if schema == RELATIVE_CONTRACT: return validate_relative(contract)
    raise ValueError("unsupported typed semantic probe")


def _score(contract, frame, binding):
    schema = contract["schema"]
    if schema == "inkscape-red-target-plan-v1": return score_selection(contract, frame)
    if schema == CROP_CONTRACT: return score_crop(contract, frame)
    return score_relative(contract, frame, binding)


def reconcile_probe_artifact(result, image_path):
    schema = result.get("schema") if type(result) is dict else None
    if schema == SELECTION_SCORE: return reconcile_selection(result, image_path)
    if schema == CROP_SCORE: return reconcile_crop(result, image_path)
    if schema == RELATIVE_SCORE: return reconcile_relative(result, image_path)
...
class SemanticProbeRegistry:
    ...
    def register(self, identifier, contract):
        ...
        return {"schema": "typed-semantic-probe-registration-v3",
                "id": identifier, "contract_schema": contract["schema"],
                "status": "REGISTERED_NO_AUTHORITY", "grants_input_authority": False}

    def probe(self, identifier, index, sequence, capture_ns, frame, binding=None):
        contract = self._contracts.get(identifier)
        if contract is None: return None
        started_ns = time.perf_counter_ns()
        result = _score(contract, frame, binding)
