...
class Executor:
    ...
    def submit(self,identifier,steps):
        with self.lock:
            ...
            self.backend.validate(steps)  # entire program before any input
            ...
            self.emit(dict(event='accepted',id=identifier,steps=len(steps),accepted_ns=time.perf_counter_ns()))
            thread.start()
