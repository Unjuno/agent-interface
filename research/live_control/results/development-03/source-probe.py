...
def probe(app,seed,out):
        ...
        try:
            for line in process.stdout:
                try:lines.put(json.loads(line))
                    ...
                    lines.put(dict(event='transport_error',raw_line=line));return
    ...
    def until(predicate):
        ...
        while True:
            ...
            events.append(row)
            if row['event']=='transport_error':raise RuntimeError('non-JSON control output: '+row['raw_line'])
...
def main():
    ...
    for name in ('executor.py','session.py','probe.py','test_executor.py'):
        (args.out/('source-'+name)).write_bytes((HERE/name).read_bytes())
    reports=[]
    try:
