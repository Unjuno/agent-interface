...
def main():
    ...
    (a.out/'plan.json').write_text(json.dumps({'scope':'scripted drag-resolution comparison, not assistant timing','order':['coarse','dense'],'requested_dx_pixels':24,'duration_ms':200,'seed':991003,'source_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()},indent=2)+'\n')
    for mode in ['coarse','dense']:
