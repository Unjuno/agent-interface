...
HERE=Path(__file__).resolve().parent;out=HERE/'results/timing-clock-live-01';out.mkdir(exist_ok=False)
...
def cli(source,name,boundary,steps):
    ...
    args=[sys.executable,str(HERE/'prepared_exchange_v5.py'),address,str(source),str(root),name,str(stepfile),
...
try:
    ...
    summary=dict(clock_domain_id=runtime_clock['domain_id'],dispatch_interval=dispatch_interval,missing_model_interval=missing_model_interval,
