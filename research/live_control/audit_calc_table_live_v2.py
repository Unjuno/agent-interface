...
HERE=Path(__file__).resolve().parent;root=HERE/'results/calc-table-live-02';runtime=root/'runtime'
...
for stage in ('enter','save','confirm'):
    ...
    if stage in ('save','confirm'):
        ...
        post=terminal['post_release_observation'];assert post['captures']==2 and post['error'] is None
        stop=[e for e in raw if e['event']=='input_stopped' and e['id']==stage]
        assert len(stop)==1 and stop[0]['interruption']==cause and stop[0]['release']==release
        after=[e for e in raw if stop[0]['emit_started_ns']<e['emit_started_ns']<terminal['emit_started_ns']]
        assert len(after)==2 and all(e['event']=='observation' for e in after)
        assert [e['sequence'] for e in after]==post['sequences']
        for e in after:
            for side in ('input_state_before','input_state_after'):
                state=e[side]
                assert state['active_lease_deadline_ns'] is None and not state['active_lease_time_valid']
                assert not state['owned_buttons'] and not state['owned_keycodes'] and state['physical_pointer_mask']==0
        assert release['verified_ns'] < stop[0]['stopped_ns'] < after[0]['capture_ns']
...
assert decision['image_sha256']==digest((runtime/'013.png').read_bytes())
assert raw[index+1]['event']=='clock' and raw[index+1]['sequence']==13
...
result=dict(audit_passed=True,audit_sha256=digest(Path(__file__).read_bytes()),events=len(raw),socket_exchanges=exchanges,
    ...
    post_release=[dict(id=e['id'],details=e['post_release_observation']) for e in raw if e['event']=='terminal' and e.get('post_release_observation')],
