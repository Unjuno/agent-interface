...
HERE=Path(__file__).resolve().parent;root=HERE/'results/send-wait-self-use-01'
...
batches=[json.loads((root/('read-'+name+'.json')).read_text()) for name in ('initial','modal','clock2','effect','final')]
...
(HERE/'results/send-wait-self-use-audit.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))

(root/'transport-sources.json').write_text(json.dumps({p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (HERE/'event_socket_v2.py',HERE/'event_cursor.py',HERE/'command_once.py')},indent=2)+'\n')
