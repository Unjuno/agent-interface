...
OUT=HERE/'results/confirmation-boundary-01';OUT.mkdir(exist_ok=False)
summaries=[]
for mode in ('premature_final','review_then_confirm'):
    out=OUT/mode;out.mkdir();root=out/'runtime';responses=[]
    proc=subprocess.Popen([sys.executable,'-u',str(HERE/'confirmation_socket_entry_v2.py'),
    ...
    try:
        ...
        confirm=cli(out/'submit/reply.json','confirm',[dict(op='key',key='Tab'),dict(op='key',key='space'),
            dict(op='key',key='Tab'),dict(op='key',key='Return')],'outcome')
        if mode=='premature_final':
            ...
            assert confirm['outcome']['task_success'] is True
        request(dict(after=confirm['cursor'],events=['command'],timeout=5,command=dict(op='finish'),request_id='finish'))
        ...
        assert scores[0]['admitted_request']==terminals[-1]['admitted_request']
        delivered=[json.loads(s) for s in (root/'delivered.jsonl').read_text().splitlines()]
        assert initial['records']+nav['records']+attempt['records']+confirm['records']==delivered[:confirm['cursor']]
        ...
        summaries.append(dict(mode=mode,task_success=scores[0]['success'],exact_frames=frames,
...
(OUT/'results.json').write_text(json.dumps(dict(cases=summaries,
    scope='Two scripted runs, fixed order and known fixture; not model recovery, live-tempo or full Issue 34 contract'),indent=2)+'\n')
files=[Path(__file__),HERE/'confirmation_socket_entry_v2.py',HERE/'confirmation_browser_entry_v2.py',
    HERE/'prepared_exchange_v6.py',HERE/'event_socket_v12.py']
