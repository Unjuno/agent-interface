...
def initialize(path, continuation):
    with locked(path):
        ...
        store(path, {'format': 'durable-submit-v5', 'continuation': continuation,
...
def run(path, spec, call=exchange):
    with locked(path):
        state = load(path)
        if state.get('format') != 'durable-submit-v5' or state.get('authority') != 'none':
        ...
        if 'command' in spec:
            ...
            if command['op'] == 'effect_checkpoint':
                if set(command) != {'op','contract'}:raise ValueError('checkpoint requires only an explicit contract')
