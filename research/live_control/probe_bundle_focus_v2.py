...
root=HERE/'results/bundle-focus-02';root.mkdir(exist_ok=False)
source=HERE/'results/bundle-pair-01/B/bundle/steps.json'
steps=json.loads(source.read_text())
assert len(steps)==9
names=['probe_bundle_focus_v2.py','cause_servo_session_v1.py','cause_session_v1.py','executor_v5.py',
...
plan=dict(seed=224,cases=[['select_all_modifier',3,'key']],
          ...
          protocol='Keyboard events lack step/id; use enclosing single-worker step_started context. Block emitter after chosen physical admission; transfer focus using separate X connection, verify release before unblocking, restore focus, verify no tail and fresh-observation cause isolation.',
