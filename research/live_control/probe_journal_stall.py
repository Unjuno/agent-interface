...
for name,phase,lease_ms,explicit_cancel in [('write_expiry','write',700,False),('flush_expiry','flush',700,False),('write_cancel','write',4000,True),('flush_cancel','flush',4000,True)]:
    ...
    try:
        ...
        row={'case':name,'terminal':terminal,'released_while_output_blocked':True,'no_late_motion':True,'pointer':position,'stall_phase':phase,'held_before_stall':held_at_stall,'released_state':state,'journal_confirmed':journal.confirmed}
        rows.append(row)
    finally:
        resume.set()
