...
from session_v21 import Backend,suite
from executor_v3 import Executor
HERE=Path(__file__).resolve().parent
root=HERE/'results/servo-occlusion-02';root.mkdir(exist_ok=False)
rows=[]
for case in ('normal','partial','hidden','replacement'):
    ...
    try:
        ...
        def snapshot(identifier,index):
            if case=='normal':return original_snapshot(identifier,index)
...
sources=[Path(__file__),HERE/'session_v21.py',HERE/'session_v15.py',HERE/'patch_servo_v5.py',HERE/'visual_anchor_v4.py',HERE/'visual_anchor_v2.py',HERE/'visual_anchor_v3.py',HERE/'input_owner_v9.py',HERE/'pointer_reply_v2.py',HERE/'executor_v3.py']
