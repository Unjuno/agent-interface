...
def main():
    root = HERE / 'results/cause-servo-live-01'
    ...
    for stage in ('servo', 'save'):
        ...
        terminal = receipt['program_binding']['terminal']
        assert terminal['status'] == 'completed' and terminal['interruption'] is None
        attention[stage] = len(receipt['attention'])
    ...
    result = {'audit_passed': True, 'audit_sha256': digest(Path(__file__).read_bytes()),
              ...
              'limits': 'One familiar-layout self-use plus one scripted fault. Runtime timing excludes model and save. Receipt falsely labels numeric match errors as exceptions and flags new schemas. No population speed/token or broad generalization claim.'}
