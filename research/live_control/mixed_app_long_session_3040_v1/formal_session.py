...
def window_info(env):
    ...
    for wid in windows(env):
        name = cmd(["xdotool", "getwindowname", wid], env, timeout=2)
        cls = cmd(["xprop", "-id", wid, "WM_CLASS"], env, timeout=2)
        rows.append({"window": wid, "name": name.stdout.strip(),
                     "wm_class": cls.stdout.strip(),
                     "name_rc": name.returncode, "class_rc": cls.returncode})
    return rows

def reacquire_calc(env, ledger):
    rows = window_info(env)
    event(ledger, "calc_reacquire_scan", windows=rows)
    candidates = [r["window"] for r in rows
                  if "libreoffice" in (r["wm_class"] + " " + r["name"]).lower()]
    return candidates[-1] if candidates else None

def wait_usable_surface(env, initial, ledger, label, timeout=20):
    end = time.time() + timeout
    seen = set()
    while time.time() < end:
        for wid in windows(env):
            g = geom(env, wid)
            if wid in seen and wid != initial:
                continue
            seen.add(wid)
            event(ledger, "surface_candidate", label=label, window=wid, geometry=g)
            if wid == initial or not g:
                continue
            if "Geometry:" in g:
                try:
                    dims = g.split("Geometry:", 1)[1].strip().split("x", 1)
                    if int(dims[0]) >= 100 and int(dims[1]) >= 100:
                        return wid
...
def activate(env, wid, ledger, label):
    """Bounded WM-aware activation; never let xdotool hang the session."""
    q = cmd(["xdotool", "windowactivate", "--sync", wid], env, timeout=3)
    event(ledger, "activation_attempt", label=label, window=wid,
          returncode=q.returncode, stdout=q.stdout.strip(), stderr=q.stderr.strip())
    if q.returncode == 0:
        return True
    fallback = cmd(["xdotool", "windowfocus", "--sync", wid], env, timeout=3)
    event(ledger, "activation_fallback", label=label, window=wid,
          returncode=fallback.returncode, stdout=fallback.stdout.strip(),
...
def main():
    ...
    try:
        time.sleep(.7)
        wm = subprocess.Popen(["openbox", "--sm-disable"], env=env,
                              stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        procs.append(wm); time.sleep(.7)
        event(ledger, "session_setup", display=DISPLAY,
        ...
        cw = wait_usable_surface(env, cw, ledger, "calc_launch")
        ...
        old_calc = apps["calc"].copy(); activate(env, apps["inkscape"]["window"], ledger, "focus_drift")
        ...
        activate(env, apps["calc"]["window"], ledger, "modal_open"); cmd(["xdotool","key","ctrl+o"],env); input_ops += 1; time.sleep(1)
        ...
        reacquired_calc = reacquire_calc(env, ledger)
        if reacquired_calc is not None:
            apps["calc"]["window"] = reacquired_calc
            event(ledger, "calc_reacquired", old_window=old_calc["window"],
                  new_window=reacquired_calc, surface_generation=apps["calc"]["surface_generation"])
        ...
        activate(env, apps["calc"]["window"], ledger, "return_to_calc"); time.sleep(.3)
