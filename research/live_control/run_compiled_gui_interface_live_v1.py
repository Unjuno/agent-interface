"""Preregistered Chromium local-continuation positive/changed pair."""
import copy
import hashlib
import json
import os
...
OUT = HERE / "results/compiled-gui-interface-live-01"
SEED = 991021
...
def compiled_interface():
    return {
        ...
        "session_scope": "live-chromium-seed991021",
        ...
        def local_observe(payload):
            ...
            if state == "empty":
                query = submit([{"op": "observe_target_handle",
                                 "target_handle": "form_field", "offset": [12, 7]}])
...
def main():
    ...
    gates = {
        ...
        "positive_two_local_transitions": (
            positive["compiled_runtime"]["outcome"] == "TASK_SUCCEEDED"
            and positive["compiled_runtime"]["completed_transitions"] == 2
            and positive["compiled_runtime"]["frontier_model_resumptions"] == 0
        ...
        "changed_stops_second_target_input": (
            changed["compiled_runtime"]["outcome"] == "SAFE_YIELD"
            and changed["compiled_runtime"]["completed_transitions"] == 1
