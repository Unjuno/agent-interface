...
label=sys.argv[1]
assert label in ('responder-abba-1-builtin','responder-abba-2-responder','responder-abba-3-responder','responder-abba-4-builtin')
root=HERE/'results'/label;root.mkdir(exist_ok=False)
def dump(name,value):
...
names=['responder_calc_driver_v1.py','durable_submit_v4.py','append_checkpoint_v1.py','received_continuation_v1.py','received_exchange_v2.py','cause_servo_socket_v7.py','model_context_runner_v1.py','calc_proposal_schema_v1.py','responder_calc_supervisor_v1.py']
dump('plan.json',{'seed':238,'scope':'actual typed screenshot model proposals through bounded automatic supervisor, isolated Calc',
...
try:
 ...
 for index in range(1,7):
  ...
  if proposal.get('finish') is True:
   ...
   dump('result.json',{'exit_code':code,'proposals_executed':index-1,'journal_calls':len(calls),'scope':'typed model proposals automatically passed through isolated Calc supervisor'})
   break
