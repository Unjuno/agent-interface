...
root=HERE/'results/patch-servo-02';root.mkdir(exist_ok=False)
allrows=[]
for offset in (0,40):
    ...
    try:
        ...
        if offset:
            ...
            assert run('setup_offset',[{'op':'pointer_drag','points':[{'x':x+i,'y':y} for i in range(offset+1)],'duration_ms':200}]+save)['status']=='completed'
