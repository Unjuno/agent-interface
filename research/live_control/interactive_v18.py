...
def main():
    ...
    with (args.out/'delivery-flush.jsonl').open('xb') as receipt_stream:
        ...
        try:
            ...
            emit(dict(event='ready',app=args.app,task=task,goal=goal,presentation=args.presentation,basic_step_examples=[{'op':'text','text':'123'},{'op':'key','key':'Return'},{'op':'chord','modifier':'Control_L','key':'s'}],decision_evidence_schema={'required_for':'submit','fields':{'delivery_id':'successfully flushed observation or review delivery ID','observation_sequence':'must equal expected_sequence','producer':['assistant','scripted','human']},'authority':'none; ordinary admission required'},servo_schema=SERVO_SCHEMA,operations=['submit','pointer_reply','cancel','clock','finish'],guided_fields=['points','duration_ms','reply_timeout_ms','max_updates','feedback_delay_ms'],reply_fields=['ticket','sequence','command']))
