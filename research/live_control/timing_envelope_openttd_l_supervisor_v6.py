"""Pre-action transparent OpenTTD supervisor with sign-to-tile semantics."""
...
base=HERE/'results/timing-envelope-openttd-l-06';root=base/arm;control=base/(arm+'-control');control.mkdir(parents=True,exist_ok=False)
def dump(path,value):
...
plan={'scope':'fresh seed-991003 OpenTTD five-tile L objective after pre-action transparency, with sign-to-tile semantics','arm':arm,
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['timing_envelope_openttd_l_supervisor_v6.py','timing_envelope_openttd_l_driver_v4.py','timing_envelope_v1.py','timing_envelope_v2.py','semantic_checkpoint_v4.py','openttd_finish_outcome_v1.py','model_pair_runner_v2.py','session_v22.py','openttd_contact_sheet_v1.py']},
      'max_turns':12,'seed_semantics':'byte-pinned seed-991003 save; target tiles 977,978,979,1043,1107; closed toolbar',
      'model_route':'all gpt-6-astra medium','prompt_policy':'v5 plus app-semantic distinction between floating sign label boxes and their underlying map-square surfaces','initial_ui_state':'closed toolbar; interface-tracked transparent trees; signs A, B corner, C and X keep clear'}
...
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_l_driver_v4.py'),arm,'timing-envelope-openttd-l-06'],stdout=(control/'driver-stdout.txt').open('wb'),stderr=(control/'driver-stderr.txt').open('wb'))
...
'''
        ...
        observation=applied['result']['state']['continuation']['observation'];current_image=root/'runtime'/Path(observation['image']).name;planner_image=build_contact_sheet(current_image,applied,proposal,root/'runtime',root/f'planner-{index+1}.png');previous={'proposal':proposal,'resolution':applied['result']['state']['last_resolution']};new_required=next_checkpoint_turn(index,proposal)
