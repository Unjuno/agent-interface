...
def main():
    root = HERE / 'results/append-replan-01'; root.mkdir(exist_ok=False)
    names = ['probe_append_replan_v1.py', 'durable_submit_v4.py', 'append_checkpoint_v1.py', 'received_continuation_v1.py', 'received_exchange_v2.py',
    ...
    dump(root / 'plan.json', {'scope': 'scripted actual GUI, deliberate caller process exit after socket write, rejection recovery then journal-owned clock, fresh observation and new distinct edit',
         'seed': 237, 'expected_svg': {'x': 104, 'y': 50, 'width': 40, 'height': 30},
    ...
    p = subprocess.Popen([sys.executable, str(HERE / 'cause_servo_socket_v7.py'), 'inkscape', 'serve', '--',
                          '--app', 'inkscape', '--seed', '237', '--out', str(root / 'runtime')],
    ...
    try:
        ...
        journal_calls = []
        def journal_call(spec):
            result = run(root / 'journal.json', spec)
            journal_calls.append(result); dump(root / 'journal-calls.json', journal_calls)
            assert result['state']['pending'] is None, 'poll same pending command before continuing'
            return result
        def journal_clock():
            result = journal_call({'command': {'op': 'clock'}, 'timeout': 3})
            clock_record = result['state']['last_resolution']['clock']
            assert result['state']['last_resolution']['request_id'] == result['request']['request_id']
            return clock_record
        c = journal_clock()
        fresh = journal_call({'command': {'op': 'submit', 'expected_sequence': c['sequence'],
                    'valid_until_ns': c['runtime_ns'] + 30_000_000_000, 'steps': [{'op': 'observe'}]}, 'timeout': 3})
        assert fresh['state']['continuation']['observation']['sequence'] > state['observation']['sequence']
        c = journal_clock()
        proposal = json.loads((root / 'submit-spec.json').read_text())
        proposal['command']['expected_sequence'] = c['sequence']
        proposal['command']['valid_until_ns'] = c['runtime_ns'] + 30_000_000_000
        proposal['command']['steps'][2]['text'] = '104'
        edited = journal_call(proposal)
        assert edited['state']['last_resolution']['terminal']['status'] == 'completed'
        state = edited['state']['continuation']
        live('after-fresh-edit')
