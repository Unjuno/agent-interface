...
H=Path(__file__).resolve().parent;R=H/'results/recovery-continue-ink-01';R.mkdir(exist_ok=False)
def dump(n,x):(R/n).write_text(json.dumps(x,indent=2)+'\n',encoding='utf-8')
def win(p):return 'C:'+str(p.resolve())[6:]
names=['recovery_continue_ink_v1.py','durable_submit_v4.py','append_checkpoint_v1.py','received_continuation_v1.py','received_exchange_v2.py','cause_servo_socket_v7.py','model_context_runner_v1.py','screenshot_responder_v1.txt','recover_once_v1.py','recover_once_v2.py','recovery_view_v1.py','calc_proposal_schema_v1.py']
...
try:
 ...
 source=state['continuation']['observation'];c=clock()
 ...
 observation=fresh['state']['continuation']['observation'];assert observation['pointer_binding']==source['pointer_binding'] and observation['input_focus_before']==observation['input_focus_after']
 c=clock();applied=call({'command':{'op':'submit','expected_sequence':c['sequence'],'valid_until_ns':c['runtime_ns']+30_000_000_000,'steps':q['steps']+[{'op':'observe'}]},'timeout':3})
 assert applied['state']['pending'] is None
 dump('continued.json',{'source':source,'fresh':observation,'result':applied,'proposal':q})
 state=applied['state'];view=present(applied);dump('continued-view.json',view)
 q=model(2,state['continuation']['observation'],view)
