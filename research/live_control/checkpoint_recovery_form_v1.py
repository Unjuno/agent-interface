...
names = ['checkpoint_recovery_form_v1.py', 'recover_query_once_v1.py', 'phased_submit_v2.py',
...
dump('plan.json', {'seed': 240, 'max_model_decisions': 4,
                   'scope': 'private X11 Chromium; scripted navigation to local fixture; actual model form input; every artifact query deliberately loses its response, then one command-free recovery read',
...
p = subprocess.Popen([sys.executable, str(H / 'checkpoint_cause_socket_v1.py'), 'chromium', 'serve',
                      '--', '--app', 'chromium', '--seed', '240', '--out', str(R / 'runtime')],
...
Act has1..10 steps. First: {{"op":"pointer_click","x":INTEGER,"y":INTEGER,"duration_ms":80}}, bounds0..1279,0..799. Tail permits {{"op":"text","text":"{goal['token']}"}}, {{"op":"key","key":"Return|Tab|Escape"}}, {{"op":"chord","modifier":"Control_L","key":"a"}}. A click-only act is allowed. Verify requests saved evidence again; visual success alone cannot end the task. No extra fields or tools; rationale1..600 characters. Evidence: {json.dumps(feedback)}'''
        ...
        if proposal['kind'] == 'act':
            ...
            state = load(journal)
            assert state['pending'] is None
            feedback = {'proposal': proposal, 'phase_status': phases['status'], 'reason': phases.get('reason'),
                        'resolution': state['last_resolution'], 'target_check': checked}
        else:
            assert proposal['submission_received_visible'] is True
            feedback = {'proposal': proposal, 'verification_not_complete': True}
