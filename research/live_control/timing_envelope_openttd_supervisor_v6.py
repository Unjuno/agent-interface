...
root=HERE/'results/timing-envelope-openttd-06'
control=HERE/'results/timing-envelope-openttd-control-06';control.mkdir(exist_ok=False)
def dump(path,value):
...
plan={'scope':'fresh bounded OpenTTD loop with buffered process-scoped end-to-end timing envelope; no matched comparison yet',
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['timing_envelope_openttd_supervisor_v6.py','timing_envelope_openttd_driver_v6.py','timing_envelope_v1.py','timing_envelope_v2.py','openttd_proposal_schema_v5.py','model_pair_runner_v1.py','session_v10.py','openttd_contact_sheet_v1.py']},
      'max_turns':9,'seed_semantics':'canonical save; no RNG override','model':'gpt-5.6-luna','effort':'low','prompt_policy':'batched hover contact sheet plus tooltip-purpose semantic guard'}
...
turns=[]
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_driver_v6.py')],
...
'''
        ...
        args=[sys.executable,str(HERE/'model_pair_runner_v1.py'),r'C:\Program Files\nodejs\node.exe',
              str(Path.home()/'AppData/Roaming/npm/node_modules/@openai/codex/bin/codex.js'),str(presented_image),str(prompt_path),str(REPO),str(model_dir)]
