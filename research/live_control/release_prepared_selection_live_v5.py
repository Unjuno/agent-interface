...
def verify(plan):
    ...
    checks.update({"output_absent": not (REPO / plan["output"]).exists(),
        ...
        "thresholds": plan["thresholds_ms"] == {
            ...
            "admission_to_semantic_client_lte": 250,
            "probe_compute_lte": 5}})
    return checks
...
def main():
    ...
    from semantic_probe_backend_v1 import Backend, suite
    ...
    try:
        ...
        identifier = "release-prepared-pointer-click-05"
        ...
        selection_id = "prepared-red-selection-05"
        selection_program = [early["candidate"]["action"], {"op": "observe"}]
        probe_registration = backend.register_semantic_probe(selection_id, early["candidate"])
        semantic_client = {"exchanges": []}
        def receive_semantic_feedback():
            after = 0
            for ordinal in range(2):
                request = {"after": after, "events": ["semantic_probe", "terminal"],
                    "timeout": 3, "action_id": selection_id,
                    "request_id": f"selection-semantic-{ordinal}"}
                record = exchange_record(request)
                semantic_client["exchanges"].append(record)
                boundary = record["reply"]["records"][-1]
                if boundary["event"] == "terminal" or boundary["score"]["success"]:
                    break
                after = record["reply"]["cursor"]
            semantic_client["completed_ns"] = time.perf_counter_ns()
        request_count = len(delivery.request_receipts())
        semantic_thread = threading.Thread(target=receive_semantic_feedback)
        threads.append(semantic_thread); semantic_thread.start()
        if not delivery.wait_requests(request_count + 1):
            raise RuntimeError("semantic client must wait before selection submit")
        semantic_request_registered_ns = delivery.request_receipts()[-1]["received_ns"]
        ...
        semantic_thread.join(3)
        if semantic_thread.is_alive(): raise RuntimeError("semantic client timed out")
        probe_events = [record["reply"]["records"][-1]
                        for record in semantic_client["exchanges"]]
        if not probe_events or probe_events[-1]["event"] != "semantic_probe":
            raise RuntimeError("terminal before semantic probe success")
        if not probe_events[-1]["score"]["success"]:
            raise RuntimeError("semantic client ended without success")
        first_probe, useful_probe = probe_events[0], probe_events[-1]
        first_feedback_received_ns = semantic_client["exchanges"][0]["client_returned_ns"]
        semantic_feedback_received_ns = semantic_client["exchanges"][-1]["client_returned_ns"]
        semantic_score = useful_probe["score"]
        semantic_score_completed_ns = useful_probe["probe_completed_ns"]
        ...
        selection_observations = [row for row in events if row.get("event") == "observation"
                                  and row.get("id") == selection_id]
        reconciliations = [row for row in events
                           if row.get("event") == "semantic_probe_reconciled" and
                           row.get("id") == selection_id]
        selection_observation = next(row for row in selection_observations
                                     if row["sequence"] == first_probe["sequence"])
        semantic_observation = next(row for row in selection_observations
                                    if row["sequence"] == useful_probe["sequence"])
        selection_boundary = first_boundary(events, selection_id)
        semantic_scores = probe_events
        metrics = {
            ...
            "selection_admission_to_semantic_feedback_received_ms":
                (semantic_feedback_received_ns - selection_accepted["accepted_ns"]) / 1e6,
            ...
            "first_probe_compute_ms":
                (first_probe["probe_completed_ns"] - first_probe["probe_started_ns"]) / 1e6,
            "useful_probe_compute_ms":
                (useful_probe["probe_completed_ns"] - useful_probe["probe_started_ns"]) / 1e6,
            "useful_probe_to_image_ready_ms":
                (next(row for row in reconciliations if row["sequence"] ==
                      useful_probe["sequence"])["image_ready_ns"] -
                 useful_probe["probe_completed_ns"]) / 1e6,
            ...
            "semantic_client_to_terminal_ms":
                (selection_terminal["terminal_ns"] - semantic_feedback_received_ns) / 1e6,
        }
        checks = {
            ...
            "probe_registered_before_submit": probe_registration["status"] ==
                "REGISTERED_NO_AUTHORITY" and
                probe_registration["grants_input_authority"] is False and
                semantic_request_registered_ns <= selection_submit_ns,
            ...
            "probe_precedes_artifact_and_reconciles": len(reconciliations) ==
                len(semantic_scores) == 2 and all(
                    probe["probe_completed_ns"] < next(row for row in reconciliations
                        if row["sequence"] == probe["sequence"])["image_ready_ns"] and
                    next(row for row in reconciliations
                        if row["sequence"] == probe["sequence"])
                        ["reconciliation"]["matches"] is True and
                    probe["grants_input_authority"] is False
                    for probe in semantic_scores),
            "semantic_client_consumed_ordered_probes":
                [row["request"]["request_id"] for row in semantic_client["exchanges"]] ==
                ["selection-semantic-0", "selection-semantic-1"] and
                all(row["reply"]["records"][-1]["event"] == "semantic_probe"
                    for row in semantic_client["exchanges"]),
            ...
            "admission_to_semantic_client_threshold":
                metrics["selection_admission_to_semantic_feedback_received_ms"] <=
                plan["thresholds_ms"]["admission_to_semantic_client_lte"],
            "probe_compute_threshold": metrics["first_probe_compute_ms"] <=
                plan["thresholds_ms"]["probe_compute_lte"] and
        ...
        report.update({"passed": all(checks.values()), "checks": checks, "goal": goal,
            ...
            "semantic_request_registered_ns": semantic_request_registered_ns,
            ...
            "semantic_feedback_received_ns": semantic_feedback_received_ns,
            "semantic_probe_reconciliations": reconciliations,
