"""Retest the OpenTTD target/guard pair with the omitted toolbar opener restored."""
import hashlib
import json
import os
import shutil
...
def run_case(name, first_y, expected_reason, expected_success):
    ...
    temporary = tempfile.TemporaryDirectory(prefix="agent-interface-target-guard-v2-")
    ...
    try:
        ...
        current = clock()
        preaction = call({"command": {"op": "submit",
        ...
        finish = request_once(endpoint["socket"], state, {"events": ["independent_evaluation"],
            "timeout": 25, "command": {"op": "finish"}, "request_id": f"finish-v2-{name}"})
