...
assert stage in ('initial','select','edit','save','decide','finish')
...
if stage=='initial':
    names=['field_table_live_v1.py','input_state_table_v1.py','decision_receipt_v4.py','pointer_exchange_v1.py',
           'cause_servo_socket_v1.py','cause_servo_interactive_v1.py','cause_servo_session_v1.py']
    save('plan',dict(seed=222,sources={n:digest((HERE/n).read_bytes()) for n in names},
                    target=dict(x=73.25,y=50,width=40,height=30),absolute_tolerance=.01,
                    allocation='one fresh familiar-layout Inkscape; changed X target; no fault injection or automatic retry',
...
elif stage in ('select','edit','save'):
    previous={'select':'initial','edit':'select','save':'edit'}[stage]
    batch=read('initial/batch.json') if stage=='select' else read(previous+'/report.json')['last_reply']
    if stage=='select':steps=[dict(op='pointer_click',x=618,y=391,button=1,duration_ms=80),dict(op='settle',quiet_ms=80,timeout_ms=500)]
    elif stage=='edit':steps=[dict(op='pointer_click',x=560,y=106,button=1,duration_ms=80),
                            dict(op='chord',modifier='Control_L',key='a'),dict(op='text',text='73.25'),
    ...
    report=read('save/report.json');image=select_image(report['last_reply'],runtime)
    ...
    save('commit',dict(sha256=sha,image=image,report_sha256=digest((root/'save/report.json').read_bytes())))
else:
