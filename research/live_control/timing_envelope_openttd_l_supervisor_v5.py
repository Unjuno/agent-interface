"""Semantic-checkpoint supervisor over a pre-action transparent OpenTTD view."""
...
base=HERE/'results/timing-envelope-openttd-l-05';root=base/arm;control=base/(arm+'-control');control.mkdir(parents=True,exist_ok=False)
def dump(path,value):
...
plan={'scope':'fresh seed-991003 OpenTTD five-tile L objective after one pre-planner transparency transition','arm':arm,
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['timing_envelope_openttd_l_supervisor_v5.py','timing_envelope_openttd_l_driver_v4.py','timing_envelope_v1.py','timing_envelope_v2.py','semantic_checkpoint_v4.py','openttd_finish_outcome_v1.py','model_pair_runner_v2.py','session_v22.py','openttd_contact_sheet_v1.py']},
      'max_turns':12,'seed_semantics':'byte-pinned seed-991003 save; target tiles 977,978,979,1043,1107; closed toolbar',
      'model_route':'all gpt-6-astra medium','prompt_policy':'stateful checkpoint; tree view already transparent before turn 1; no view method offered','initial_ui_state':'closed toolbar; interface-tracked transparent trees; signs A, B corner, C and X keep clear'}
...
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_l_driver_v4.py'),arm,'timing-envelope-openttd-l-05'],stdout=(control/'driver-stdout.txt').open('wb'),stderr=(control/'driver-stderr.txt').open('wb'))
