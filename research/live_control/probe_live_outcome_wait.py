...
out=HERE/'results/live-outcome-wait-01';out.mkdir(exist_ok=False);summaries=[]
for case,token in (('success','t991026'),('failure','wrong-token')):
    ...
    process=subprocess.Popen([sys.executable,'-u',str(HERE/'gated_socket_entry.py'),str(gate),'serve','--','--app','xterm','--seed','991026','--out',str(root),'--presentation','compact'],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
    ...
    try:
        ...
        pending_batch=request(wait_request('submit',terminal['cursor'],timeout=.1))
        pending=interpret(pending_batch,'submit')
        assert pending['state']=='pending' and pending['task_success'] is None
        assert not any(r['event']=='independent_evaluation' for r in pending_batch['records'])
        gate.write_text('release evaluation')
        final_batch=request(pending['continuation']);final=interpret(final_batch,'submit')
        assert final['state']=='evaluated' and final['task_success'] is (case=='success')
        assert 'command' not in pending['continuation']
        request(dict(after=final_batch['cursor'],events=['command'],timeout=5,command=dict(op='finish'),request_id='finish'))
        ...
        assert collected==delivered[:responses[-1]['reply']['cursor']]
        ...
        summaries.append(dict(case=case,first_state=pending['state'],final_success=final['task_success'],
            exact_frames=count,submit_commands=1,continuation_read_only=True,
            ...
            pending_call_ms=(responses[-3]['returned_ns']-responses[-3]['started_ns'])/1e6))
    finally:
...
sources=[Path(__file__),HERE/'gated_evaluation_entry.py',HERE/'gated_socket_entry.py',HERE/'outcome_wait.py',HERE/'event_socket_v10.py',HERE/'event_cursor_v4.py',HERE/'request_boundary.py']
