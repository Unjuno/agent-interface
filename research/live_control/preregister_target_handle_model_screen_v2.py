"""Freeze controlled-context ABBA comparison for coordinate and handle actions."""
import hashlib
import json
...
OUT = HERE / "results/target-handle-model-screen-02"
...
def main():
    ...
    for mode, prompt in (("coordinate", coordinate), ("handle", handle)):
        (OUT / f"{mode}-prompt.txt").write_text(prompt, encoding="utf-8", newline="\n")
    sources = ["preregister_target_handle_model_screen_v2.py",
               "target_handle_model_runner_v2.py", "run_target_handle_model_screen_v2.py",
               "gui_action_responder_v1.txt",
               "target_action_union_schema_v1.json"]
    ...
    plan = {
        ...
        "fast_mode": "disabled",
        "codex_cli": version,
        "context_controls": ["model_instructions_file", "common output schema",
                             "project_doc_max_bytes=0", "empty workspace",
                             "ignore user config and rules", "tools disabled"],
        "correctness": {"coordinate": "point inside [270,242,312,260]",
                        "handle": "exact h_save_form action and [20,9] relation"},
        "primary_metrics": ["strict correctness", "reported total/cached/output tokens",
                            "within-arm reported input range", "runner duration"],
        "promotion": {
            "correct": "4/4",
        ...
        "prompt_sha256": {mode: sha(OUT / f"{mode}-prompt.txt")
                          for mode in ("coordinate", "handle")},
        "sources": {name: sha(HERE / name) for name in sources},
        "docs": ["https://developers.openai.com/api/reference/cli/resources/responses/methods/retrieve",
                 "https://developers.openai.com/api/reference/cli/resources/responses/methods/create"],
        "scope": "fixed archived action; no GUI input, causal latency, cost, served identity or general token claim",
    }
