...
def main():
    root = HERE / 'results/cause-live-02'
    ...
    for stage in ('observe', 'recover', 'finish'):
        assert digest((HERE / 'cause_recovery_v2.py').read_bytes()) == read(root / stage / 'source.json')['sha256']
    ...
    for path in sorted((root / 'observe').glob('query-*-request.json')):
        segments.append({'request': read(path), 'reply': read(path.with_name(path.name.replace('-request', '-reply')))})
    segments.append({'request': read(root / 'finish/query-0-request.json'), 'reply': read(root / 'finish/result.json')})
    covered, count = set(), 0
    ...
    for name, report in [('interrupt', first), ('recover', recovery)]:
        ...
        if image['status'] == 'image':
            ...
            assert name == 'interrupt' and image['status'] == 'no_observation'
