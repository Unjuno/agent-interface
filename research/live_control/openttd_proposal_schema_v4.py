"""Strict OpenTTD v4 schema with bounded batched hover observation."""
import json
    ...
    if value['kind'] == 'stop':
        ...
        for step in value['steps']:
            if type(step) is not dict or step.get('op') not in (
                    ...
                    'dwell_observe'):
                raise ValueError('pointer, observe or dwell_observe required')
            if step['op'] == 'dwell_observe':
                if set(step) != {'op', 'delay_ms'} or type(step['delay_ms']) is not int or not 100 <= step['delay_ms'] <= 2000:
                    raise ValueError('bounded dwell_observe required')
