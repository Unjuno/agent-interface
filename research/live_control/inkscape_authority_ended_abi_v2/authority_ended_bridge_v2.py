...
def _capture_contract(post):
    captures=post.get('captures')
    if type(captures) is not int or captures < 1 or captures > 2:
        raise AuthorityEndedNotReady('one or two bounded post-authority captures required')
    seq=post.get('sequence')
    if type(seq) is not int or seq < 1:
        raise AuthorityEndedNotReady('fresh post-authority sequence required')
    if captures == 1:
        seqs=post.get('sequences')
        if seqs is not None and seqs != [seq]:
            raise AuthorityEndedNotReady('single-capture sequence evidence mismatch')
        return
    seqs=post.get('sequences')
    if type(seqs) is not list or len(seqs) != captures or any(type(x) is not int or x < 1 for x in seqs):
        raise AuthorityEndedNotReady('multi-capture sequence evidence required')
    if any(b <= a for a,b in zip(seqs,seqs[1:])):
        raise AuthorityEndedNotReady('post-authority sequences must increase')
    if post.get('selection_rule') != 'latest' or seq != seqs[-1]:
        raise AuthorityEndedNotReady('multi-capture evidence must select latest sequence')
...
def to_caller_execution_decision(receipt):
    ...
    finished=post.get('snapshot_finished_ns'); deadline=post.get('lifecycle_deadline_ns')
