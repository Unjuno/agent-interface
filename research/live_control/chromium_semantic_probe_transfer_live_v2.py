"""Repaired frozen Chromium transfer of pre-artifact semantic feedback."""
import contextlib
import hashlib
import json
import os
...
def main():
    ...
    report = {"schema": "chromium-semantic-probe-transfer-live-v2",
    ...
    try:
        ...
        checks = {
            ...
            "all_exact_reconciliations": (
                all(row["reconciliation"]["matches"] is True for row in reconciliations) and
                all(any(item["sequence"] == probe["sequence"] for item in reconciliations)
                    for probe in negative["feedback"]+positive["feedback"])),
