...
root=HERE/'results/stopped-transport-01';root.mkdir(exist_ok=False)
names=['probe_stopped_transport_v1.py','stopped_transport_entry_v1.py','stopped_transport_fixture_v1.py',
       'stopped_socket_v1.py','stopped_cursor_v1.py','stopped_scope_v1.py','executor_v8.py',
...
try:
    ...
    # New input cannot reuse the still-running backend even after input_stopped.
    r=query(socket,['terminal'],dict(op='submit',id='premature',steps=[dict(op='fault'),dict(op='tail')],expected_sequence=1,valid_until_ns=now+5_000_000_000),action='premature')
    assert r['status']=='unattributed_rejection' and r['records'][-1]['reason']=='closed or busy; no implicit queue'
    ...
    result=dict(success=True,early_notification_before_gate_release=early_returned_ns<gate_release_ns,
        no_terminal_while_capture_blocked=True,busy_rejection=True,cancel_reply_while_capture_blocked=True,
