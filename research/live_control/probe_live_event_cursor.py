...
HERE=Path(__file__).resolve().parent;out=HERE/'results/live-event-cursor-01';out.mkdir(exist_ok=False);results=[]
for case in ('save','unsaved'):
    root=out/case;messages=EventCursor();seen=[];cursor=0;reads=[]
    process=subprocess.Popen([sys.executable,'-u',str(HERE/'interactive_v23.py'),'--app','calc','--seed','991022','--out',str(root),'--presentation','compact'],stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
    def read():
        for line in process.stdout:
            try:r=json.loads(line)
            except ValueError:continue
            seen.append(r);messages.append(r)
        messages.close()
    ...
    def until(kind):
        ...
        while True:
            batch=messages.read_until(cursor,[kind,'rejected'],timeout=min(5,max(0,deadline-time.monotonic())))
            reads.append(dict(wanted=kind,**batch));cursor=batch['cursor']
            for r in batch['records']:
                if r['event']=='rejected':raise AssertionError(r)
                if r['event']==kind:return r
            if batch['status'] in ('closed','gap') or time.monotonic()>deadline:raise TimeoutError(batch)
    ...
    try:
        until('observation')
        if case=='save':
            submit('enter_save',[dict(op='text',text='532'),dict(op='key',key='Return'),dict(op='text',text='590'),dict(op='key',key='Return'),dict(op='chord',modifier='Control_L',key='s'),dict(op='settle',quiet_ms=150,timeout_ms=1500)],False)
        terminal=submit('final',[dict(op='key',key='Return')],True)
        early=until('effect_evidence');evaluation=until('independent_evaluation');effect=evaluation['effect']
        assert early['effect']==effect
        ...
        assert retained['effect']==effect and retained['admission_closed'] and retained['output_flushed'] and retained['effect_output_flushed']
        ...
        results.append(dict(case=case,terminal=terminal['status'],effect=effect['status'],saved_values=values,
                            terminal_to_early_emit_ms=(early['emit_started_ns']-terminal['terminal_ns'])/1e6,
        ...
        (root/'cursor-reads.json').write_text(json.dumps(reads,indent=2)+'\n')
