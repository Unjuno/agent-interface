...
def main():
    ...
    horizon_ms=100
    M_ms=None if a.scenario=='UNKNOWN_BOUND' else 220
    effect_ms={'NORMAL':50,'HORIZON_LATE':170,'FOCUS_LOSS':170,'OWNER_DEATH':170,'BOUND_VIOLATION':300,'UNKNOWN_BOUND':170}[a.scenario]
    censor_ms=70 if a.scenario in ('FOCUS_LOSS','OWNER_DEATH') else None
    start=ns(); events=[]; worker=None; effect_file=out/'worker_effect.txt'
    ...
    events.append({'type':'ready','t_ns':ns(),'focus':str(root.focus_get()),'owner_alive':bool(target.winfo_exists())})
