"""Run the preregistered opposite-order authored-condition replication."""
import hashlib
import json
import shutil
import time
...
OUT = HERE / "results/local-displacement-transfer-04"
...
def run_case(name, allocation, spec, out):
    ...
    try:
        ...
        points = [{"x": x + offset, "y": y}
                  for offset in range(0, allocation["pointer_delta"] + 1, 2)]
        steps = [
        ...
        conditions = [row for row in events if row.get("event") == "local_displacement_postcondition"]
        ...
        result = {
            ...
            "postcondition": conditions[0] if len(conditions) == 1 else None,
            "postcondition_event_count": len(conditions),
            "terminal": terminal, "steps_started": started, "save_started": 4 in started,
...
def main():
    ...
    assert plan["authored_output_sha256"] == plan["prior_authored_output_sha256"]
