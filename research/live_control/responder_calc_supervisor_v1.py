...
assert (label,mode) in [('responder-abba-1-builtin','builtin'),('responder-abba-2-responder','responder'),('responder-abba-3-responder','responder'),('responder-abba-4-builtin','builtin')]
...
plan={'scope':'bounded autonomous screenshot proposal/action handoff on isolated Calc; no general supervisor claim',
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['responder_calc_supervisor_v1.py','responder_calc_driver_v1.py','calc_proposal_schema_v1.py','model_context_runner_v1.py','screenshot_responder_v1.txt']},
      'history_mode':'last','instruction_mode':mode,'max_turns':5,'seed':238,'model':'gpt-5.6-luna','effort':'low','prompt_policy':'one exact typed schema; malformed response stops without model retry'}
dump(control/'plan.json',plan)
started=time.perf_counter_ns();turns=[]
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'responder_calc_driver_v1.py'),label],
...
'''
        ...
        args=[sys.executable,str(HERE/'model_context_runner_v1.py'),r'C:\Program Files\nodejs\node.exe',
