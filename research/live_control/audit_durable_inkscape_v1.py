...
from durable_submit_v1 import reconcile
...
def main():
    r = HERE / 'results/durable-inkscape-01'; runtime = r / 'runtime'
    ...
    assert [e['id'] for e in events if e['event'] == 'accepted'] == ['select', aid]
    ...
    assert len(terms) == 2 and all(e['status'] == 'completed' and e['release']['verified'] is True
    ...
    report = {'events': len(events), 'frames_exact': len(observations), 'distinct_workers': 3,
              ...
              'scope': 'scripted process crash after actual send; one case, includes Python startup, not model or human speed',
