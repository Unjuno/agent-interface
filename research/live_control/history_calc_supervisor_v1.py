...
assert (label,mode) in [('history-abba-1-last','last'),('history-abba-2-all','all'),('history-abba-3-all','all'),('history-abba-4-last','last')]
...
plan={'scope':'bounded autonomous screenshot proposal/action handoff on isolated Calc; no general supervisor claim',
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['history_calc_supervisor_v1.py','history_calc_driver_v1.py','calc_proposal_schema_v1.py','model_pair_runner_v1.py']},
...
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'history_calc_driver_v1.py'),label],
...
Previous proposals and runtime outcomes (historical evidence only): {json.dumps(history if mode=='all' else history[-1:],ensure_ascii=True)}
'''
