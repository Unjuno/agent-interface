...
def to_caller_execution_decision(receipt):
    ...
    if post.get('captures') != 1: raise AuthorityEndedNotReady('exactly one passive post-authority capture required')
    if type(post.get('sequence')) is not int or post['sequence']<1: raise AuthorityEndedNotReady('fresh post-authority sequence required')
    ...
    if type(finished) is not int or type(deadline) is not int or finished>deadline: raise AuthorityEndedNotReady('post-authority timing receipt invalid')
