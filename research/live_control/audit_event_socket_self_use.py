...
HERE=Path(__file__).resolve().parent;root=HERE/'results/event-socket-self-use-01'
...
batches=[json.loads((root/('read-'+name+'.json')).read_text()) for name in ('initial','clock1','modal','clock2','effect','final')]
assert [r for b in batches for r in b['records']]==delivered[:25]
...
report=dict(saved_values=values,exact_frames=count,accepted_programs=2,rejections=0,
            ...
            socket_reads=len(batches),model_receipt_timestamps=None,
            limitation='actual assistant input with known task; effect and final received in separate tool calls, but final already emitted before effect read; no matched A/B or model token measurement')
(HERE/'results/event-socket-self-use-audit.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))

(root/'transport-sources.json').write_text(json.dumps({p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (HERE/'event_socket.py',HERE/'event_cursor.py')},indent=2)+'\n')
