...
HERE=Path(__file__).resolve().parent;out=HERE/'results/rejection-correlation-01';out.mkdir(exist_ok=False);root=out/'runtime'
...
try:
    ...
    cases=[('stale',dict(base,id='stale',expected_sequence=0)),('expired',dict(base,id='expired',valid_until_ns=1)),
           ('malformed_steps',dict(base,id='malformed_steps',steps=[dict(op='unsupported')])),('bad_json','{'),('non_object','[]')]
    for index,(name,command) in enumerate(cases,2):
    ...
    assert [r['id'] for r in events if r['event']=='accepted']==['valid']
