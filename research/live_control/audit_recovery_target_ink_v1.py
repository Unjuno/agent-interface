...
H=Path(__file__).resolve().parent;R=H/'results/recovery-target-ink-01';T=R/'runtime'
...
assert load(R/'journal.jsonl')==calls[-1]['result']['state'];count=inspect(R/'journal.jsonl')[1];assert count==19
...
report={'events':len(events),'frames':len(obs),'journal_records':count,'model_calls':models,'svg':values,'svg_sha256':sha(T/'shape.svg'),'scope':'one live intentional response abandonment; scripted edit, caller recovery, model new edit refused before input by same-focus target-patch change; not full autonomous editing or general recovery', 'loss_to_read_resolution_s':(calls[3]['end_ns']-read(R/'loss.json')['send_completed_ns'])/1e9}
