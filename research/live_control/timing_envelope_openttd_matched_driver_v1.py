...
root=HERE/'results/timing-envelope-openttd-matched-01'/arm;root.mkdir(parents=True,exist_ok=False)
def dump(name,value):
...
names=['timing_envelope_openttd_matched_driver_v1.py','durable_submit_v4.py','append_checkpoint_v1.py','received_continuation_v1.py','received_exchange_v2.py','pointer_socket_entry_v5.py','event_socket_v11.py','model_pair_runner_v2.py','openttd_proposal_schema_v5.py','timing_envelope_openttd_matched_supervisor_v1.py','timing_envelope_v1.py','timing_envelope_v2.py','session_v22.py','openttd_contact_sheet_v1.py']
...
p=subprocess.Popen([sys.executable,'-u',str(HERE/'pointer_socket_entry_v5.py'),'openttd','serve','--',
...
try:
 ...
 for index in range(1,11):
  ...
  if proposal.get('finish') is True:
   ...
   if path.name == 'abort.json':
    dump('failure-evaluation.json',{'exit_code':code,'proposals_executed':index-1,
         'journal_calls':len(calls),'reason':proposal.get('reason'),
         'evaluation':evaluation,'success':False})
   else:
    assert evaluation['success'] is True
    dump('result.json',{'exit_code':code,'arm':arm,'proposals_executed':index-1,'journal_calls':len(calls),'scope':'matched model-route proposal passed through isolated OpenTTD supervisor'})
   break
