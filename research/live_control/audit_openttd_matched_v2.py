...
def audit_arm(arm: str):
    ...
    if success:
        ...
        if (root / "failure-evaluation.json").exists():
            failure = read(root / "failure-evaluation.json")
            assert failure["success"] is False and failure["evaluation"] == evaluation
            failure_mode = "bounded_turn_limit"
        else:
            assert read(root / "visual-verdict.json")["road_visible"] is True
            assert "AssertionError" in (control / "driver-stderr.txt").read_text()
            failure_mode = "visual_verify_false_positive_with_driver_packaging_failure"
...
def main():
    ...
    assert prereg["execution_order"] == ["adaptive", "fixed-astra", "fixed-luna"]
