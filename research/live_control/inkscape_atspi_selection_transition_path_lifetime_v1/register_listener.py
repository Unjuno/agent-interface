...
def errstr(glib,e):
    ...
    s=f'{e.contents.domain}:{e.contents.code}: '+e.contents.message.decode(errors='replace');glib.g_error_free(e);return s
addr=sys.argv[1]; seconds=float(sys.argv[2]); event=sys.argv[3]
gio=C.CDLL(ctypes.util.find_library('gio-2.0')); glib=C.CDLL(ctypes.util.find_library('glib-2.0')); gobj=C.CDLL(ctypes.util.find_library('gobject-2.0')); ptr=C.c_void_p
...
glib.g_variant_new_string.restype=ptr;glib.g_variant_new_string.argtypes=[C.c_char_p];glib.g_variant_new_strv.restype=ptr;glib.g_variant_new_strv.argtypes=[C.POINTER(C.c_char_p),C.c_ssize_t];glib.g_variant_new_tuple.restype=ptr;glib.g_variant_new_tuple.argtypes=[C.POINTER(ptr),C.c_size_t];glib.g_variant_ref_sink.restype=ptr;glib.g_variant_ref_sink.argtypes=[ptr];glib.g_variant_unref.argtypes=[ptr];glib.g_error_free.argtypes=[C.POINTER(GError)];gobj.g_object_unref.argtypes=[ptr]
...
empty=(C.c_char_p*1)();values=[glib.g_variant_new_string(event.encode()),glib.g_variant_new_strv(empty,0),glib.g_variant_new_string(b'')];params=glib.g_variant_ref_sink(glib.g_variant_new_tuple((ptr*3)(*values),3))
...
if not ret: raise SystemExit('REGISTER '+errstr(glib,err))
print(json.dumps({'registered':True,'event':event}),flush=True);time.sleep(seconds)
