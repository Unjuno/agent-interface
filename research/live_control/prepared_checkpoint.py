    ...
    try:
        ...
        if program.get('status')!='boundary' or not records or records[-1].get('event')!='terminal' or records[-1].get('status')!='completed':
            ...
            for item in result['policy']['exchanges']:
                ...
                combined+=reply.get('records',[]);cursor=reply.get('cursor',cursor)
            if result['state']=='needs_decision' and all(item.get('reply',{}).get('status')=='boundary' for item in result['policy']['exchanges']):
