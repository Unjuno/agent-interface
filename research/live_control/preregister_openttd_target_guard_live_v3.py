"""Freeze the reverse-order unchanged OpenTTD target/guard replication."""
import hashlib
import json
...
OUT = HERE / "results/openttd-target-guard-live-03"
...
def main():
    ...
    live = ["preregister_openttd_target_guard_live_v3.py", "run_openttd_target_guard_live_v3.py",
        "run_openttd_target_guard_live_v2.py", "pointer_socket_entry_v10.py",
    ...
    plan = {"status": "preregistered_before_fresh_openttd_execution",
        "study": "openttd-target-guard-live-03", "order": ["target", "wrong-row"],
        "allocations": [
            {"name": "target", "first_y": 240, "expected_reason": "met",
             "expected_success": True},
            {"name": "wrong-row", "first_y": 224, "expected_reason": "target_not_reached",
             "expected_success": False}],
        "unchanged_from_v2": "same runner, seed, preaction, toolbar opener, direction selection, drags, settle, target/guard boxes and thresholds; allocation order reversed",
        ...
        "primary_endpoint": "target admits B-to-C and independently completes the guarded L; wrong-row stops before B-to-C and independently fails",
        ...
        "scope": "reverse-order unchanged scripted fresh seed991003 OpenTTD replication with independent engine score; no model call, model authorship, human comparison, token or generalization claim"}
