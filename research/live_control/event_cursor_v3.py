...
from event_scope import validate_scope,boundary
...
class EventCursor:
    ...
    def read_until(self,after,events,timeout=5,max_records=64,action_id=None,request_id=None):
        ...
        if request_id is not None:
            ...
            if action_id is not None or any(e not in ('command','rejected') for e in events):raise ValueError('request scope supports command/rejected only')
                ...
                for seq,encoded in self.records:
                    ...
                    if request_id is not None:
                        ...
                        if record['event'] in events:
                            identity=record.get('transport_request_id')
                            status='identity_unknown' if not isinstance(identity,str) or not identity else ('boundary' if identity==request_id else None)
