...
H=Path(__file__).resolve().parent;R=H/'results/ready-replan-ink-01';T=R/'runtime'
...
assert len([e for e in events if e['event']=='accepted'])==4+sum(1+int(x['submitted'])+len(x.get('readiness',{}).get('result',{}).get('checks',[])) for x in read(R/'replans.json'))
...
for row in read(R/'replans.json'):
 ...
 if row['submitted']:
  assert checked['eligible'] and subset[-1]['result']==row['result'] and len(subset)==4
  command=row['result']['request']['command']
  assert command['steps']==row['proposal']['steps']+[{'op':'observe'}]
...
expired=[t for t in terms if t['status']=='expired'];assert len(expired)==1
last=read(R/'replans.json')[-1]['result']['request']['command'];t=expired[0]
assert t['id']==last['id'] and t['steps_completed']==4
assert t['interruption']['record']['reason']=='expired'
assert not any(e['event']=='step_started' and e.get('id')==t['id'] and e['step']==5 for e in events)
assert all(e['admitted_ns']<e['valid_until_ns'] for e in events if e['event'] in ('input_admission','pointer_admission'))
report={'expired_terminals':expired,'episode_classification':'saved task succeeded, final program expired; not clean completion','events':len(events),'frames':len(obs),'journal_records':count,'model_calls':models,'svg':values,'svg_sha256':sha(T/'shape.svg'),'scope':'one live intentional response abandonment; scripted edit, caller recovery, initial model edit refused, then fresh model selection/edit/verification after patch checks; not full autonomous editing or general recovery', 'loss_to_read_resolution_s':(calls[3]['end_ns']-read(R/'loss.json')['send_completed_ns'])/1e9}
