...
from openttd_proposal_schema_v2 import parse
from timing_envelope_v1 import interval
from timing_envelope_v2 import Recorder
HERE=Path(__file__).resolve().parent
REPO=HERE.parent.parent
root=HERE/'results/timing-envelope-openttd-02'
control=HERE/'results/timing-envelope-openttd-control-02';control.mkdir(exist_ok=False)
def dump(path,value):
...
plan={'scope':'fresh bounded OpenTTD loop with buffered process-scoped end-to-end timing envelope; no matched comparison yet',
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['timing_envelope_openttd_supervisor_v2.py','timing_envelope_openttd_driver_v2.py','timing_envelope_v1.py','timing_envelope_v2.py','openttd_proposal_schema_v2.py','model_pair_runner_v1.py']},
      'max_turns':6,'seed_semantics':'canonical save; no RNG override','model':'gpt-5.6-luna','effort':'low','prompt_policy':'full-screen absolute coordinate contract plus hover; malformed response stops without retry'}
...
turns=[]
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_driver_v2.py')],
...
try:
    ...
    for index in range(1,7):
        ...
        prompt=f'''You control an isolated OpenTTD road fixture using its current screenshot. Task: {task} Choose the next bounded visual action. Use the road toolbar and straight-road tool when visible. If an icon's meaning is uncertain, move the pointer over it and observe the tooltip before clicking. A prior needs_decision/focus_changed or zero steps_completed does not mean the application effect failed: inspect the screenshot and never blindly replay.
