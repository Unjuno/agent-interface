"""Execute the preregistered OpenTTD tree-transparency transform through the shared runtime."""
...
HERE=Path(__file__).resolve().parent;ROOT=HERE/'results/openttd-view-transform-01';RUN=ROOT/'run';RUN.mkdir(parents=True,exist_ok=False)
def dump(name,value):
...
p=subprocess.Popen([sys.executable,'-u',str(HERE/'pointer_socket_entry_v8.py'),'openttd','serve','--','--root','/home/taka/agent-interface-bench-feasibility','--out',str(RUN/'runtime'),'--controller','assistant'],stdout=subprocess.PIPE,stderr=(RUN/'stderr.txt').open('w'),text=True)
...
try:
    ...
    applied=call({'command':{'op':'submit','expected_sequence':current['sequence'],'valid_until_ns':current['runtime_ns']+10_000_000_000,'steps':[{'op':'chord','modifier':'Control_L','key':'2'},{'op':'observe'}]},'timeout':5});after=applied['state']['continuation']['observation'];dump('applied.json',applied)
    ...
    evaluation=next(e for e in finish['reply']['records'] if e['event']=='independent_evaluation');before_image=RUN/'runtime'/Path(before['image']).name;after_image=RUN/'runtime'/Path(after['image']).name
    result={'executed':True,'model_calls':0,'step':{'op':'chord','modifier':'Control_L','key':'2'},'before_image':before_image.name,'after_image':after_image.name,'before_sha256':sha(before_image),'after_sha256':sha(after_image),'image_changed':sha(before_image)!=sha(after_image),'evaluation':evaluation,'durable_calls':len(calls),'save_unchanged':json.loads((RUN/'runtime/cleanup.json').read_text())['save_unchanged']}
