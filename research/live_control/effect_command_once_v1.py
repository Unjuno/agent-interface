...
class CommandOnce:
    ...
    def send(self,request_id,command):
        ...
        if not isinstance(command,dict) or command.get('op') not in ('submit','clock','cancel','pointer_reply','finalization_status','finish','effect'):raise ValueError('runtime command required')
