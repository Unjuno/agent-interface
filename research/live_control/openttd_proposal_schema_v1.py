"""Strict bounded OpenTTD road-task proposal schema."""
import json
    ...
    if value['kind'] == 'stop':
        ...
        for step in value['steps']:
            if type(step) is not dict or step.get('op') not in (
                    'pointer_click', 'pointer_drag', 'observe'):
                raise ValueError('pointer click, drag or observe required')
            if step['op'] == 'observe':
                if set(step) != {'op'}:
                    raise ValueError('exact observe required')
            elif step['op'] == 'pointer_click':
            ...
            coordinates = ([step] if step['op'] == 'pointer_click' else
                           step.get('points', []))
            if any(not 0 <= point['x'] < 1024 or not 0 <= point['y'] < 720
