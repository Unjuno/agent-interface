...
import hashlib,json,os,types,uuid
from pathlib import Path
import durable_token_state_v2 as base

PIN_SCHEMA='authority-ended-validator-byte-pin-v3'
...
def _load_validator(source,function_name):
    ...
    try:
        ...
        mod=types.ModuleType('_authority_validator_'+sha[:16])
        mod.__file__=str(p)
        exec(code,mod.__dict__)
    except Exception as e: raise ValidatorPinError('validator source execution failed') from e
...
class BytePinnedValidatorLedger:
