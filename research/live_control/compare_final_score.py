...
rows=[];programs=[];goals=[]
for n in (1,2):
    root=HERE/'results'/('journal-calc-02' if n==1 else 'final-score-calc-01')
    ...
    reject=next((r for r in events if r['event']=='rejected' and r['emit_started_ns']<score['known_ns']),None)
    ...
    if n==2:
        ...
        assert score['final_program']=='confirm_excel'
        assert not any(r['event']=='accepted' and r['id']=='post_score_negative' for r in events)
    rows.append(dict(cohort=n,exact_frames=count,submit_commands=sum(r['received_ns']<score['known_ns'] for r in submitted),accepted_programs=len(accepted),
                     rejections=sum(r['event']=='rejected' and r['emit_started_ns']<score['known_ns'] for r in events),clock_commands=sum(r['command']['op']=='clock' for r in commands),
...
(HERE/'results/final-score-comparison.json').write_text(json.dumps(rows,indent=2)+'\n')
