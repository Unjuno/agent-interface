...
out=HERE/'results/final-score-cancel-03'
process=subprocess.Popen([sys.executable,'-u',str(HERE/'interactive_v20.py'),'--app','xterm','--seed','991023','--out',str(out),'--presentation','compact'],stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
...
try:
    ...
    until('accepted');time.sleep(.1);send(dict(op='cancel',id='cancel_final'))
    ...
    finalization=json.loads((out/'finalization-status.json').read_text())
    assert finalization['status']=='finished' and finalization['output_flushed'] and finalization['evaluation']['success'] is False
