...
HERE=Path(__file__).resolve().parent;root=HERE/'results/early-effect-self-use-01'
...
report=dict(saved_values=values,exact_frames=count,accepted_programs=2,rejections=0,
            ...
            model_receipt_timestamps=None,
            limitation='actual assistant input with known task; both result events appeared in same write_stdin tool response; no matched A/B or model token measurement')
(HERE/'results/early-effect-self-use-audit.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))
