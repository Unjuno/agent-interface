...
if len(raw)!=CAP['archive_bytes'] or hashlib.sha256(raw).hexdigest()!=CAP['archive_sha256']:
    raise SystemExit('archive identity')
tar=lzma.decompress(raw)
if len(tar)!=CAP['tar_bytes'] or hashlib.sha256(tar).hexdigest()!=CAP['tar_sha256']:
    raise SystemExit('tar identity')
...
with tarfile.open(fileobj=io.BytesIO(tar),mode='r:') as tf:
    ...
    if len(files)!=CAP['member_files'] or sum(m.size for m in files)!=CAP['member_bytes']:
    ...
    for m in files:
        ...
        with tf.extractfile(m) as src, target.open('wb') as dst:
            dst.write(src.read())
checks={
 'FREEZE.json':'f8a5b6f5b5e09baf5e5cf0d8392f12ac7951957c29bedf5202d1976dda5803c8',
...
for name,want in checks.items():
    ...
    if got!=want: raise SystemExit('retained hash '+name)
...
if audit.get('scientific_decision')!='PASS_X11_TIMESTAMP_ORDER_BOUNDARY_SCOPED' or audit.get('errors')!=[] or not all(audit.get('corruption_controls',{}).values()):
    raise SystemExit('audit decision')
