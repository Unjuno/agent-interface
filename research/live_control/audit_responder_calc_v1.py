...
def main():
 ...
 for n in range(1,count+1):
  ...
  assert plan['instruction_mode']==read(control/'plan.json')['instruction_mode']
  assert plan['requested_model']=='gpt-5.6-luna' and plan['requested_effort']=='low'
  assert plan['runner_sha256']==sha(HERE/'model_context_runner_v1.py') and plan['image_sha256']==sha(runtime/image)
 ...
 report={'events':len(events),'exact_frames':len(obs),'append_records':1+2*len(calls),'socket_calls':len(calls)+2,'history_mode':mode,'instruction_mode':read(control/'plan.json')['instruction_mode'],'model_turns':models,
