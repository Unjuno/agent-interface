"""Versioned backend passing coherent current binding to typed probes."""
import sys
import time
...
from semantic_probe_runtime_v3 import SemanticProbeRegistry, reconcile_probe_artifact


class Backend(Previous):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs); self.semantic_probes = SemanticProbeRegistry()

    def register_semantic_probe(self, identifier, contract):
        return self.semantic_probes.register(identifier, contract)
    ...
    def snapshot(self, identifier, index):
        ...
        coherent_binding = dict(after) if before == after else None
        probe = self.semantic_probes.probe(
            identifier, index, sequence, capture, frame, coherent_binding)
        if probe is not None: self.emit(probe)
        artifact = self.images.publish(frame); reconciliation = None
        if probe is not None:
            reconciliation = reconcile_probe_artifact(probe["score"], artifact["image"])
            self.emit({"event": "semantic_probe_reconciled", "id": identifier,
                       ...
                       "image": artifact["image"], "image_ready_ns": artifact["image_ready_ns"],
                       "reconciliation": reconciliation, "grants_input_authority": False})
        self.last_context = dict(context)
        self.emit({"event": "observation", "id": identifier, "step": index,
                   "sequence": sequence, "capture_ns": capture, "context_ns": context_ns,
                   "context": context, **artifact, "input_focus_before": before["focus"],
