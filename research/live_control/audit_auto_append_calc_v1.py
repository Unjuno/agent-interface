...
def main():
 r=HERE/'results/auto-append-calc-01';control=HERE/'results/auto-append-calc-control-01';runtime=r/'runtime'
 ...
 state=start(ep['socket']);calls=read(r/'calls.json');assert len(calls)==12
 def replay(c):
  nonlocal state
 ...
 assert load(r/'journal.jsonl')==calls[-1]['result']['state'] and inspect(r/'journal.jsonl')[1]==25
 replay(read(r/'finish.json'))
 models=[];handoffs=read(control/'turns.json');assert len(handoffs)==4
 ...
 for n in range(1,5):
  ...
  models.append(row)
 terms=[e for e in events if e['event']=='terminal'];assert len(terms)==6
 assert [t['status'] for t in terms]==['completed','needs_decision','completed','needs_decision','completed','completed']
 assert all(t['release']['verified'] is True and t['release']['buttons_down']==[] and t['release']['keys_down']==[] for t in terms)
 assert len([e for e in events if e['event']=='accepted'])==6 and not any(e['event']=='rejected' for e in events)
 commands=[e['command'] for e in events if e['event']=='command']
 assert len({c['transport_request_id'] for c in commands})==len(commands)
 saves=sum(s=={'op':'chord','modifier':'Control_L','key':'s'} for c in commands for s in c.get('steps',[]));assert saves==2
 obs=[e for e in events if e['event']=='observation'];assert len(obs)==16;decoder=Decoder('live-control')
 ...
 report={'events':len(events),'exact_frames':16,'append_records':25,'socket_calls':14,'model_turns':models,
         ...
         'scope':'one automatic typed handoff episode; redundant model-requested second save; changed prompts/action count versus manual baseline, no causal speed ratio or human parity',
