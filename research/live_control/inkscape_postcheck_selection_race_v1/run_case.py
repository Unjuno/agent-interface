...
def run(args):
    ...
    try:
        ...
        if args.scenario in ('stable_after_check','switch_after_check'): ev('trajectory_precheck_stable')
        ...
        if args.scenario=='switch_after_check':
            key(D,'Tab'); ev('postcheck_switch_b')
        else:
            ev('postcheck_stable')
...
if __name__=='__main__':
    ap=argparse.ArgumentParser(); ap.add_argument('--scenario',required=True,choices=['stable_after_check','switch_after_check']); ap.add_argument('--out',required=True); ap.add_argument('--display',type=int,required=True); ap.add_argument('--construction',action='store_true')
