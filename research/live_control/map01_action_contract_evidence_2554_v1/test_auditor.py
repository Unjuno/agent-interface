...
class AuditTests(unittest.TestCase):
    def rows(self):
        return [{'case_id':f'{a}-{d}','arm':a,'dwell_tics':d,'emitted_vector':[1,0,0,0],'declared_button':a,'ready':True,'pre':{},'post':{},'reward':0,'screen_hash_before':'a','screen_hash_after':'b','process_identity':'p','cleanup_ok':True} for a in ('forward','use','left','right','noop') for d in (4,16)]
    ...
    def test_duplicate_case_identity_fails(self):
        r=self.rows(); r[1]['case_id']=r[0]['case_id']; self.assertIn('CASE_IDENTITY',audit(r)['issues'])
