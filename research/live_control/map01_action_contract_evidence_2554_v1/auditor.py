...
DWELLS=(4,16)
REQUIRED=('case_id','arm','dwell_tics','emitted_vector','declared_button','ready','pre','post','reward','screen_hash_before','screen_hash_after','process_identity','cleanup_ok')
...
def audit(rows):
    ...
    if len(ids)!=len(set(ids)) or any(not x for x in ids): issues.append('CASE_IDENTITY')
    for r in rows:
        if any(k not in r for k in REQUIRED): issues.append('MISSING_FIELDS')
