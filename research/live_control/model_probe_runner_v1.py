...
def main():
    node, cli, image, output = sys.argv[1:]
    root = Path(output).resolve()
    root.mkdir(parents=True, exist_ok=False)
    prompt = ('Inspect the attached screenshot only. Do not use tools, read files, '
              'browse, edit, or run commands. Return one JSON object with keys '
              'saved (boolean), evidence (string), next_action (string). Decide '
              'whether saving has completed from visible evidence.')
    ...
    args = [node, cli, 'exec', '--ignore-user-config', '--ephemeral',
            ...
            '-C', str(root), '-i', str(Path(image).resolve()), '-']
    plan = dict(args=args, requested_model='gpt-5.6-luna', requested_effort='low',
                ...
                comparison='configuration feasibility, not a matched performance arm')
