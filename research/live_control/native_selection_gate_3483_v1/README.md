...
Run the adversarial auditor regressions with `python3 -m unittest -v test_audit_replay.py`. They exercise the untouched bundle and temporary tampered copies (wrong source PNG despite a refreshed manifest, a relabelled duplicate click action, and a manifest path escape); only the untouched bundle may pass.
