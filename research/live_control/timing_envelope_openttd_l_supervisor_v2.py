"""Corrected Windows supervisor for the second OpenTTD L-road allocation."""
...
base=HERE/'results/timing-envelope-openttd-l-02';root=base/arm;control=base/(arm+'-control');control.mkdir(parents=True,exist_ok=False)
def dump(path,value):
...
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_l_driver_v2.py'),arm,'timing-envelope-openttd-l-02'],stdout=(control/'driver-stdout.txt').open('wb'),stderr=(control/'driver-stderr.txt').open('wb'))
