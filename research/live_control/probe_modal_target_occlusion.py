"""Compare guarded keyboard and coordinate activation under post-check faults."""
import hashlib,json,shutil,time,zipfile,subprocess
from pathlib import Path
from PIL import Image,ImageGrab
from Xlib import X,display
import guarded_modal_backend_v3 as guarded
...
out=HERE/'results/modal-target-occlusion-01';out.mkdir(exist_ok=False)
plan=dict(cases=[route+'_'+fault for route in ('key','click') for fault in ('occlusion',)],injection='same-surface child overlay over Excel button after guard passes',
...
for case in plan['cases']:
    root=out/case;root.mkdir();s=guarded.suite.Session();backend=None;engine=None;events=[];injections=[];overlay_display=None
    route,fault=case.split('_')
    try:
        ...
        def interposed(self,step,cancel,identifier,index):
            if self is backend and identifier=='confirm' and index==0:
                entry=dict(entered_ns=time.perf_counter_ns(),binding_before=self.binding(),injected=fault!='normal')
                if fault=='focus':
                    driver.key('Tab');driver.key('Tab');time.sleep(.03)
                if fault=='move':
                    subprocess.run(['wmctrl','-ir',hex(binding['surface']),'-e','0,440,250,-1,-1'],env=s.env,check=True);time.sleep(.03)
                if fault=='occlusion':
                    global overlay_display
                    overlay_display=display.Display(s.name)
                    parent=overlay_display.create_resource_object('window',binding['surface'])
                    overlay=parent.create_window(310,128,178,27,0,X.CopyFromParent,X.InputOutput,X.CopyFromParent,background_pixel=0x808080,event_mask=X.ButtonPressMask|X.ButtonReleaseMask)
                    overlay.map();overlay_display.sync();time.sleep(.03)
                    entry['overlay_window']=overlay.id
                ...
                try:return original(self,step,cancel,identifier,index)
                    ...
                    if overlay_display:
                        overlay_display.sync();entry['overlay_event_types']=[]
                        while overlay_display.pending_events():entry['overlay_event_types'].append(overlay_display.next_event().type)
        ...
        step=dict(op='key',key='Return') if route=='key' else dict(op='pointer_click',x=785,y=463)
        backend.arm_modal('confirm',proposal,step)
        engine.submit('confirm',[step],backend.sequence,time.perf_counter_ns()+2_000_000_000)
        ...
        if overlay_display:overlay_display.close()
        s.close();shutil.rmtree(s.tmp)
paths=[Path(__file__),HERE/'guarded_modal_backend_v3.py',HERE/'bound_modal_proposal.py',HERE/'modal_visual_predicate_v2.py',HERE/'modal_visual_predicate.py',HERE/'session_v16.py',HERE/'executor_v3.py',template]
