...
out=HERE/'results/outcome-fallback-controls-01';out.mkdir(exist_ok=False);rows=[];query_ids=[]
...
(out/'sources.json').write_text(json.dumps({p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (Path(__file__),HERE/'outcome_fallback.py',HERE/'outcome_wait.py',HERE/'request_boundary_v2.py')},indent=2)+'\n')
