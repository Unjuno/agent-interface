...
def main():
    root = HERE / 'results/durable-replan-01'; root.mkdir(exist_ok=False)
    names = ['probe_durable_replan_v1.py', 'durable_submit_v3.py', 'received_continuation_v1.py', 'received_exchange_v2.py',
    ...
    dump(root / 'plan.json', {'scope': 'scripted actual GUI, deliberate caller process exit after socket write, rejection recovery then journal-owned clock, fresh observation and new distinct edit',
         'seed': 235, 'expected_svg': {'x': 104, 'y': 50, 'width': 40, 'height': 30},
    ...
    p = subprocess.Popen([sys.executable, str(HERE / 'cause_servo_socket_v7.py'), 'inkscape', 'serve', '--',
                          '--app', 'inkscape', '--seed', '235', '--out', str(root / 'runtime')],
