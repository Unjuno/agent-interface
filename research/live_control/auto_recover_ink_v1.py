...
H=Path(__file__).resolve().parent;R=H/'results/auto-recover-ink-01';R.mkdir(exist_ok=False)
def dump(n,x):(R/n).write_text(json.dumps(x,indent=2)+'\n',encoding='utf-8')
def win(p):return 'C:'+str(p.resolve())[6:]
names=['auto_recover_ink_v1.py','durable_submit_v4.py','append_checkpoint_v1.py','received_continuation_v1.py','received_exchange_v2.py','cause_servo_socket_v7.py','model_context_runner_v1.py','screenshot_responder_v1.txt','recover_once_v1.py']
...
try:
 ...
 q=model(1,state['continuation']['observation'],{'pending':None,'original_action_id':action,'last_resolution':state['last_resolution'],'image_timing':'received after original execution, reconciled by exact echo/admission/terminal'})
