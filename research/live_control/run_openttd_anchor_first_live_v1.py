"""Use one verified anchor receipt before bounded OpenTTD neighbour expansion."""
import json
import shutil
import subprocess
import sys
...
def main():
    ...
    root = OUT / "live-seed991004"; root.mkdir()
    ...
    try:
        ...
        candidate_model = base.model_call(
            ...
            "or request active hover evidence if the icons are ambiguous.", source_path,
        ...
        if candidate["op"] == "target_reference":
            point = candidate_model["typed"]["point"]; anchor = [point["x"], point["y"]]
        ...
        baseline = json.loads((HERE / plan["baseline_report"]).read_text(encoding="utf-8"))
        old = baseline["result"]
        comparison = {"prior_five_hover_ms": old["timing_ms"]["hover_batches"],
                      "anchor_hover_ms": anchor_submit_ms,
                      "hover_stage_reduction_ms": old["timing_ms"]["hover_batches"]-anchor_submit_ms,
                      "prior_durable_calls": old["durable_calls"],
                      "anchor_first_durable_calls": len(calls),
                      "prior_exact_frames": 50}
        gate = {"anchor_only_branch": result["branch"] == "anchor-accepted",
                "one_initial_hover_receipt": len(anchor_readiness["receipts"]) == 1,
                "strict_anchor_evidence_bound": anchor_decision["status"] == "EVIDENCE_BOUND",
                ...
                "durable_calls_lower": len(calls) < old["durable_calls"]}
        gate["passed"] = all(gate.values())
        report = {"result": result, "comparison": comparison, "promotion_gate": gate,
                  "reported_input_tokens": {"candidate": candidate_model["usage"]["input_tokens"],
                                            "anchor_evidence": anchor_model["usage"]["input_tokens"]},
                  "decision": ("RETAIN_ANCHOR_FIRST_LIVE_CANDIDATE" if gate["passed"]
                               else "HOLD_ANCHOR_FIRST_LIVE_CANDIDATE"),
                  "scope": plan["scope"]}
