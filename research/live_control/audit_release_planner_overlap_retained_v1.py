"""Independent cross-platform audit of retained release-planner overlap v1 evidence."""
import hashlib
import json
from pathlib import Path

HERE = Path(__file__).resolve().parent; REPO = HERE.parents[1]
ROOT = HERE / "results/release-planner-overlap-live-01"
...
def main():
    ...
    checks = {
        ...
        "actual_planner_overlap":
            report["clients"]["early"]["preparation_completed_ns"] <
            report["terminal"]["terminal_ns"] and
            report["clients"]["early"]["candidate"] ==
            report["clients"]["terminal"]["candidate"],
        "fresh_validation_without_input":
            report["clients"]["early"]["validation"] ==
            report["clients"]["terminal"]["validation"] and
            report["clients"]["early"]["validation"]["status"] == "VALID_CURRENT" and
            report["checks"]["no_candidate_input_admitted"] is True,
        "frozen_timing_limits":
            report["metrics_ms"]["focus_to_early_useful_ready_ms"] <=
            plan["thresholds_ms"]["focus_to_early_useful_ready_lte"] and
            report["metrics_ms"]["useful_ready_advantage_ms"] >=
            plan["thresholds_ms"]["useful_ready_advantage_gte"] and
            report["metrics_ms"]["focus_to_terminal_useful_ready_ms"] <=
