...
out=HERE/'results/status-correlation-01';out.mkdir(exist_ok=False)
...
try:
    ...
    status=request(dict(after=pending_batch['cursor'],events=['finalization_status'],timeout=5,command=dict(op='finalization_status'),request_id='status1',read_request_id='status1'))
    ...
    for identifier in ('status1','status2','parallel1','parallel2'):
    ...
    report=dict(wait_state=pending['state'],task_success=None,retained_status=outcome['status'],failure_stage=outcome['failure_stage'],
        exact_frames=count,submit_commands=1,repeated_status_unchanged=True,concurrent_queries_matched=True,replayed_query_not_resent=True,
    ...
    sources=[Path(__file__),HERE/'failed_evaluation_entry_v2.py',HERE/'failed_socket_entry_v2.py',HERE/'outcome_wait.py',HERE/'event_socket_v11.py',HERE/'event_cursor_v5.py',HERE/'request_boundary_v2.py',HERE/'finalization_v3.py']
