...
plan={'scope':'fresh bounded OpenTTD fixed route with pre-opened road toolbar and buffered process-scoped timing envelope',
      'arm':arm,
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['timing_envelope_openttd_matched_supervisor_v4.py','timing_envelope_openttd_matched_driver_v4.py','timing_envelope_v1.py','timing_envelope_v2.py','openttd_proposal_schema_v5.py','openttd_finish_outcome_v1.py','model_pair_runner_v2.py','session_v22.py','openttd_contact_sheet_v1.py']},
      'max_turns':9,'seed_semantics':'canonical save; no RNG override; road toolbar pre-opened before timed initial observation',
      'model_route':routes[arm],
      'prompt_policy':'unchanged batched hover contact sheet, exact target-noun guard and ambiguity escalation',
      'initial_ui_state':'road construction toolbar pre-opened before timed initial observation; one setup click excluded from task actions'}
...
turns=[]
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_matched_driver_v4.py'),arm],
