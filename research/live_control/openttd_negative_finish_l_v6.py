...
HERE=Path(__file__).resolve().parent;BASE=HERE/'results/timing-envelope-openttd-l-06';ROOT=BASE/'negative-control';CONTROL=BASE/'negative-control-supervisor';CONTROL.mkdir(parents=True,exist_ok=False)
def dump(path,value):
...
def wait_file(path,process,seconds=120):
    ...
    while not path.exists():
        ...
        time.sleep(.025)
    return read(path)
source_names=['openttd_negative_finish_l_v6.py','timing_envelope_openttd_l_driver_v4.py','timing_envelope_openttd_l_supervisor_v6.py','semantic_checkpoint_v4.py','openttd_finish_outcome_v1.py']
...
started=time.perf_counter_ns();driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_l_driver_v4.py'),'negative-control','timing-envelope-openttd-l-06'],stdout=(CONTROL/'driver-stdout.txt').open('wb'),stderr=(CONTROL/'driver-stderr.txt').open('wb'))
try:
    ...
    dump(CONTROL/'result.json',{'success':True,'driver_exit_code':code,'task':ready['task'],'failure_outcome':failure,'supervisor_wall_ms':(time.perf_counter_ns()-started)/1e6,'scope':'fresh pre-action view packaging control; no model call or pointer/task mutation'});print('openttd_negative_finish_l_v6: PASS')
finally:
