...
H=Path(__file__).resolve().parent;R=H/'results/recovery-view-ink-01';T=R/'runtime'
...
v=read(R/'proposal-1.json');assert v['kind']=='verify' and [v[k] for k in ['visible_x','visible_y','visible_width','visible_height']]==[88,50,40,30]
...
report={'events':len(events),'frames':len(obs),'journal_records':count,'model_calls':models,'svg':values,'svg_sha256':sha(T/'shape.svg'),'scope':'one live intentional response abandonment; scripted edit, caller-selected single read, model verification; not full autonomous editing or general recovery', 'loss_to_read_resolution_s':(calls[3]['end_ns']-read(R/'loss.json')['send_completed_ns'])/1e9}
