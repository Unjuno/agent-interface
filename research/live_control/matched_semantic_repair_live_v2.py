"""Capacity-aware matched local repair versus Luna-low visual reacquisition."""
...
PREREG=HERE/"matched_semantic_repair_live_v2_prereg.json"
...
def verify(plan):
    ...
    checks.update({"output_absent":not(REPO/plan["output"]).exists(),
        ...
        "same_seed":plan["seed"]==214,"model":plan["model"]=={"name":"gpt-5.6-luna","effort":"low"},
...
def run_arm(plan,root,ordinal,mode):
    ...
    report={"schema":"matched-semantic-repair-arm-v2","ordinal":ordinal,
