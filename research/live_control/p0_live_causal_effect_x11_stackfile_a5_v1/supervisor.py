...
TASK='P0-LIVE-CAUSAL-EFFECT-X11-STACKFILE-A5-20260918-005';ISSUE=1325;BRANCH='research/p0-live-causal-effect-x11-stackfile-a5-1325'
...
def load_science():
    n='science_summary_1325';s=importlib.util.spec_from_file_location(n,SCIENCE);m=importlib.util.module_from_spec(s);sys.modules[n]=m;s.loader.exec_module(m);return m
...
def run_case(root,repo_root,arm,pair,position,display_num):
    ...
    status={'arm':arm,'pair':pair,'position':position,'display_num':display_num,'timeout':False,'returncode':None,'stdout':'','stderr':'','row_exists':False}
    try:
        ...
        stdout_path=sup/'child.stdout';stderr_path=sup/'child.stderr'
        with stdout_path.open('wb',buffering=0) as child_stdout, stderr_path.open('wb',buffering=0) as child_stderr:
            cp=subprocess.Popen(cmd,stdout=child_stdout,stderr=child_stderr,env=env)
            try:
                cp.wait(timeout=8)
            except subprocess.TimeoutExpired:
                status['timeout']=True;cp.kill();cp.wait()
        status.update(returncode=cp.returncode,stdout=stdout_path.read_text(errors='replace')[-8000:],stderr=stderr_path.read_text(errors='replace')[-8000:],row_exists=child_out.exists())
        ...
        supervisor_row.write_text(json.dumps(row,indent=2,sort_keys=True)+'\n')
    return row
...
def main():
    ...
    orders=[['NO_ACTION_CONTROL','V12_EFFECT']] if a.construction else schedule['pair_orders'];n=2300 if a.construction else 2320;rows=[]
