...
from bounded_pipe_writer import BoundedPipeWriter
        ...
        def handle(self):
            ...
            try:
                ...
                if probe['status']=='gap':result=probe
                    ...
                    if receipt is not None and receipt['state']=='write_uncertain':
                        result=dict(status='command_uncertain',records=[],cursor=read_args['after'],recovery='channel unusable; observe existing events only')
