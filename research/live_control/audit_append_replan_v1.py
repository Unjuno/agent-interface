...
def main():
    r = HERE / 'results/append-replan-01'; runtime = r / 'runtime'
    ...
    assert call['state'] == read(r / 'recovered.json')
    aid, rid = sent['command']['id'], sent['request_id']
    journal_calls = read(r / 'journal-calls.json')
    assert [c['request']['command']['op'] for c in journal_calls] == ['clock','submit','clock','submit']
    new_ids = [c['request']['command']['id'] for c in journal_calls if c['request']['command']['op'] == 'submit']
    assert len(set(new_ids + [aid])) == 3
    assert [e['id'] for e in events if e['event'] == 'accepted'] == ['select'] + new_ids
    ...
    assert len(terms) == 3 and all(e['status'] == 'completed' and e['release']['verified'] is True
    ...
    previous_sequence = state['observation']['sequence']
    for index, jc in enumerate(journal_calls):
        state = advance_call(state, jc)
        assert state == jc['state']['continuation'] and jc['state']['pending'] is None
        q = jc['request']; matching = [e for e in events if e['event'] == 'command' and e['command'].get('transport_request_id') == q['request_id']]
        assert len(matching) == 1
        if q['command']['op'] == 'clock':
            resolved_clock = jc['state']['last_resolution']['clock']
            assert state['clocks'][q['request_id']]['record'] == resolved_clock
            assert jc['state']['last_resolution']['request_id'] == q['request_id']
        else:
            clock = journal_calls[index-1]['state']['last_resolution']['clock']
            assert q['command']['expected_sequence'] == clock['sequence']
            assert q['command']['valid_until_ns'] == clock['runtime_ns'] + 30_000_000_000
    assert journal_calls[1]['request']['command']['steps'] == [{'op':'observe'}]
    assert journal_calls[1]['state']['continuation']['observation']['sequence'] > previous_sequence
    assert journal_calls[3]['request']['command']['steps'][2] == {'op':'text','text':'104'}
    assert journal_calls[-1]['state'] == load(r / 'journal.json')
    assert parent[-1]['request']['command']['op'] == 'finish'
    state = advance_call(state, parent[-1]); assert state == parent[-1]['continuation']
    lives = read(r / 'live-checks.json'); assert len(lives) == 5
    ...
    assert inspect(r / 'journal.json')[1] == 11
    report = {'append_records': 11, 'events': len(events), 'frames_exact': len(observations), 'distinct_workers': 3,
              'new_distinct_submissions_after_recovery': 2, 'journal_clock_requests': 2, 'recovery_reads': 1, 'saved_svg': values, 'svg_sha256': sha(runtime / 'shape.svg'),
              ...
              'scope': 'scripted rejected request recovery, fresh observe and distinct changed edit; no autonomous model replan',
