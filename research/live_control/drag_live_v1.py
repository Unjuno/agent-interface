...
def main():
    ...
    def query(q):
        ...
        return reply
    if stage == 'initial':
        names = ['drag_live_v1.py', 'cause_interactive_v2.py', 'cause_session_v1.py',
                 'cause_socket_entry_v2.py', 'input_owner_v10.py', 'executor_v5.py',
                 'lease_cause_v1.py', 'lease_cause_v2.py', 'pointer_exchange_v1.py',
                 'decision_receipt_v1.py', 'decision_receipt_v2.py', 'decision_receipt_v3.py']
        save('plan', {'sources': {n: digest((HERE / n).read_bytes()) for n in names},
                     'allocation': 'one fresh Inkscape seed 214; selected rectangle then 20 screen-pixel horizontal drag',
        ...
        steps = ([{'op': 'pointer_click', 'x': 618, 'y': 391, 'duration_ms': 80},
                 ...
                 [{'op': 'pointer_drag', 'points': [{'x': 618, 'y': 391}, {'x': 628, 'y': 391}, {'x': 638, 'y': 391}], 'duration_ms': 600},
