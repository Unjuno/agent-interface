"""Freeze a two-coordinate-frame transformed pair before fresh input."""
import hashlib
import json
...
OUT = HERE / "results/openttd-target-guard-transform-03"
...
def main():
    ...
    live = ["preregister_openttd_target_guard_transform_v3.py",
        "run_openttd_target_guard_transform_v3.py",
        ...
        "results/openttd-target-guard-transform-02/report.json"]
    ...
    plan = {"status": "preregistered_before_fresh_openttd_execution",
        "study": "openttd-target-guard-transform-03", "seed": 991004,
        "source_resolution": "1024x720", "held_out_resolution": "1280x720",
        "coordinate_frames": {
            "screen_chrome_translation": {"dx": 0, "dy": 0,
                "applies_to": "Road Construction toolbar and road/one-way controls"},
            "viewport_content_translation": {"dx": -128, "dy": 0,
                "applies_to": "map pointer paths and boxes derived from those paths"}},
        "diagnosis": "failed studies 01/02 show centered toolbar chrome and window-relative saved map content require separate transforms",
        "order": ["repeat", "target"],
        "allocations": [
        ...
        "primary_endpoint": "frame-specific transforms move chrome and viewport intents independently; repeat stops before continuation and target independently completes",
        ...
        "scope": "one fresh scripted seed991004 resolution transform repaired from two retained failures, with frame-specific coordinates, path-derived boxes and independent engine score; no model, unseen task, human, token or broad transform claim"}
