"""Repaired frozen live translation and resize test for a surface-relative predicate."""
import contextlib
import hashlib
import json
import os
...
REPO = HERE.parents[1]
PREREG = HERE/"target_relative_semantic_probe_live_v2_prereg.json"
...
def verify(plan):
    ...
    checks.update({"output_absent": not (REPO/plan["output"]).exists(),
        ...
        "zero_model": plan["model_calls"] == 0, "seed": plan["seed"] == 211,
...
def main():
    ...
    report = {"schema": "target-relative-semantic-probe-live-v2",
    ...
    try:
        ...
        backend.snapshot("relative-moved-admission", 0)
        moved_admission = events[-1]
        if moved_admission.get("pointer_binding", {}).get("geometry") != moved_geometry:
            raise RuntimeError("fresh moved binding missing")
        ...
        checks = {"source_geometry_expected": original_geometry == plan["expected_source_geometry"],
            ...
            "fresh_moved_binding": moved_admission["pointer_binding"]["geometry"] == moved_geometry and
                                   moved_admission["sequence"] > source_observation["sequence"],
        ...
        report.update({"passed": all(checks.values()), "checks": checks, "goal": goal,
            ...
            "moved_admission_observation": moved_admission,
