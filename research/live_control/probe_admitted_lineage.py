...
HERE=Path(__file__).resolve().parent;out=HERE/'results/admitted-lineage-01';out.mkdir(exist_ok=False);root=out/'runtime';responses=[]
p=subprocess.Popen([sys.executable,'-u',str(HERE/'event_socket_v9.py'),'serve','--','--app','xterm','--seed','991024','--out',str(root),'--presentation','compact'],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
...
try:
    ...
    good=dict(base,id='same-action',finish_after=True,valid_until_ns=now+10_000_000_000,steps=[dict(op='text',text='t991024'),dict(op='key',key='Return')])
    ...
    score=next(r for r in events if r['event']=='independent_evaluation')
    assert accepted['admitted_request']==terminal['admitted_request']==score['admitted_request']
    assert accepted['admitted_request']['transport_request_id']=='valid' and accepted['id']=='same-action'
    assert all(r.get('admitted_request') is None for r in events if r['event']=='rejected')
    ...
    summary=dict(rejections={name:dict(sequence=r['records'][-1]['runtime_request_sequence'],reason=r['records'][-1]['reason']) for name,r in replies.items()},
                 accepted_lineage=accepted['admitted_request'],same_action_distinct_requests=True,replay_no_resend=True,conflict_and_reserved_rejected=True,subsequent_saved_task=True)
    ...
    files=[Path(__file__),HERE/'event_socket_v9.py',HERE/'event_cursor_v3.py',HERE/'request_correlation_v2.py',HERE/'interactive_v26.py']
