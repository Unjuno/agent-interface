...
REPO = HERE.parents[1]
PREREG = HERE / "release_prepared_selection_live_v3_prereg.json"
...
def main():
    ...
    try:
        ...
        identifier = "release-prepared-pointer-click-03"
        ...
        selection_id = "prepared-red-selection-03"
        ...
        while selection_boundary["status"] == "PENDING" and time.monotonic() < deadline:
            time.sleep(.0005); selection_boundary = first_boundary(events, selection_id)
        if selection_boundary["status"] == "TERMINAL":
            raise RuntimeError("selection terminated before feedback")
        if selection_boundary["status"] != "FEEDBACK":
            raise RuntimeError("selection boundary timeout")
        selection_observation = selection_boundary["record"]
