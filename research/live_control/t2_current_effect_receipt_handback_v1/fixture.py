...
p=argparse.ArgumentParser(); p.add_argument('--ready', required=True); p.add_argument('--events', required=True); p.add_argument('--delay-ms', type=float, required=True); p.add_argument('--stop', required=True); a=p.parse_args()
...
def on_press(e): log('key_press', keysym=e.keysym, keycode=e.keycode)
def on_release(e):
    log('key_release', keysym=e.keysym, keycode=e.keycode); delay=max(0,int(round(a.delay_ms)))
    root.after(delay,effect) if delay else root.after_idle(effect)
root.bind('<KeyPress-F8>',on_press); root.bind('<KeyRelease-F8>',on_release)
root.update_idletasks(); root.update(); root.focus_force(); root.update()
ready.write_text(json.dumps({'window_id':root.winfo_id(),'pid':os.getpid(),'delay_ms':a.delay_ms}),encoding='utf-8'); log('ready',window_id=root.winfo_id())
def poll_stop():
