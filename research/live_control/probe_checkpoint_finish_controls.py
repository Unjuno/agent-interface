...
from checkpoint_finish import run
HERE=Path(__file__).resolve().parent;out=HERE/'results/checkpoint-finish-controls-02';out.mkdir(exist_ok=False)
...
(out/'sources.json').write_text(json.dumps({str(p.relative_to(HERE)):hashlib.sha256(p.read_bytes()).hexdigest() for p in
    (Path(__file__),HERE/'checkpoint_finish.py',HERE/'request_boundary_v4.py',source_path)},indent=2)+'\n')
