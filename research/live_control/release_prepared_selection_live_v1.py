...
def verify(plan):
    ...
    checks.update({"output_absent": not (REPO / plan["output"]).exists(),
        ...
        "thresholds": plan["thresholds_ms"] == {
            "focus_to_semantic_score_lte": 500,
...
def main():
    ...
    try:
        ...
        identifier = "release-prepared-pointer-click-01"
        ...
        backend.snapshot("prepared-pre-admission", 0)
        admission_validation_started_ns = time.perf_counter_ns()
        admission_validation = validate(early["candidate"], root / "003.png")
        admission_validation_completed_ns = time.perf_counter_ns()
        pre_selection_score = score(early["candidate"], root / "003.png")
        selection_id = "prepared-red-selection-01"
        selection_submit_ns = time.perf_counter_ns()
        executor.submit(selection_id, [early["candidate"]["action"], {"op": "observe"}],
        ...
        while selection_observation is None and time.monotonic() < deadline:
            selection_observation = next((row for row in events
                if row.get("event") == "observation" and row.get("id") == selection_id), None)
            if selection_observation is None: time.sleep(.0005)
        if selection_observation is None: raise RuntimeError("selection feedback missing")
        first_feedback_received_ns = time.perf_counter_ns()
        semantic_score = score(early["candidate"], Path(selection_observation["image"]))
        semantic_score_completed_ns = time.perf_counter_ns()
        ...
        checks = {
            ...
            "prepared_candidate_bound_to_acceptance": len([row for row in events
                ...
                selection_accepted["steps"][0] == early["candidate"]["action"] and
            ...
            "independent_visible_selection": pre_selection_score["success"] is False and
                ...
                semantic_score["reason"] == "four_sided_selection_handles_visible",
            ...
            "focus_to_semantic_score_threshold":
                metrics["focus_to_semantic_score_ms"] <= 500,
