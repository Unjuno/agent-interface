...
TASK='P0-LIVE-CAUSAL-EFFECT-X11-STACK-A4-20260918-004';ISSUE=1318;BRANCH='research/p0-live-causal-effect-x11-stack-a4-1318'
...
def load_science():
    n='science_summary_1318';s=importlib.util.spec_from_file_location(n,SCIENCE);m=importlib.util.module_from_spec(s);sys.modules[n]=m;s.loader.exec_module(m);return m
...
def run_case(root,repo_root,arm,pair,position,display_num):
    ...
    try:
        ...
        try:
            cp=subprocess.run(cmd,capture_output=True,text=True,timeout=8,env=env);status.update(returncode=cp.returncode,stdout=cp.stdout[-8000:],stderr=cp.stderr[-8000:],row_exists=child_out.exists())
        except subprocess.TimeoutExpired as e:
            status.update(timeout=True,returncode=None,stdout=(e.stdout or '')[-8000:] if isinstance(e.stdout,str) else '',stderr=(e.stderr or '')[-8000:] if isinstance(e.stderr,str) else '',row_exists=child_out.exists())
