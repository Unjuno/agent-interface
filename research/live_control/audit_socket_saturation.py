...
for cohort in ('socket-saturation-v3','socket-saturation-v4'):
    ...
    blockers=[r for r in responses if r['request'].get('request_id','').startswith('blocker')]
    assert len(blockers)==8
    blocker=min(blockers,key=lambda r:r['reply']['returned_ns'])
    ...
    assert release_before_wait==(cohort.endswith('v4'))
    assert cancel['reply']['status']=='busy' and not cancel['reply']['command_forwarded']
    assert cancel['started_ns']<blocker['reply']['returned_ns']
    assert sum(r['event']=='clock' for r in events)==10
...
(HERE/'results/socket-saturation-audit.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))
