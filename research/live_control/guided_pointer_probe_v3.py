...
def main():
    ...
    names=['session_v15.py','session_v13.py','session_v12.py','session_v11.py','session_v9.py','session_v8.py','session_v7.py','session_v6.py','session_v5.py','session_v4.py','input_owner_v9.py','pointer_reply_v2.py','executor_v3.py','lease.py']
    ...
    def emit(r):
        ...
        if r['event']=='pointer_yield':
            ...
            elif r['id']=='consumed_stall':backend.reply_pointer(r['ticket'],r['sequence'],{'op':'move','x':420,'y':250})
    ...
    try:
        ...
        backend.snapshot('consumed-fresh',0);blocked.clear();resume.clear();original_call=backend.owner.call
        def delayed_call(op,*args,**kwargs):
            if op=='continue_move':blocked.set();assert resume.wait(3)
            return original_call(op,*args,**kwargs)
        backend.owner.call=delayed_call
        submit('consumed_stall',[dict(guide,points=[{'x':400,'y':250},{'x':410,'y':250}],max_updates=1)])
        assert blocked.wait(2);end=time.monotonic()+2
        while time.monotonic()<end:
            state=original_call('input_state')
            if not state['owned_buttons']:break
            time.sleep(.005)
        assert not state['owned_buttons'] and engine.active is not None
        resume.set();terminal=wait();assert terminal['status']=='failed' and terminal['release']['verified']
        assert original_call('input_state')['pointer']==[410,250]
        rows.append({'case':'consumed_reply_stall','released_before_resume':True,'no_late_motion':True,'terminal':terminal['status'],'error':terminal['error']})
        backend.owner.call=original_call
        submit('after_stall_recovery',[{'op':'observe'}]);assert wait()['status']=='completed'
