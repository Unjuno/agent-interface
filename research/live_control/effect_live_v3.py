...
def main():
    ...
    assert stage in ('initial', 'servo', 'save', 'effect', 'finish')
    root = Path(directory)
    ...
    def query(q):
        ...
        return reply
    if stage == 'initial':
        names = ['effect_interactive_v2.py', 'effect_fault_backend_v2.py', 'effect_checkpoint_v1.py', 'effect_socket_v3.py', 'effect_command_once_v1.py', 'effect_live_v3.py', 'cause_servo_session_v1.py', 'cause_servo_interactive_v1.py',
                 'cause_servo_socket_v1.py', 'session_v21.py', 'patch_servo_v5.py',
                 'executor_v5.py', 'input_owner_v10.py', 'lease_cause_v2.py',
                 'pointer_exchange_v1.py', 'decision_receipt_v3.py', 'decision_receipt_v4.py', 'servo_review_v1.py', 'score_drag_v1.py']
        save('plan', {'sources': {n: digest((HERE / n).read_bytes()) for n in names},
                     'allocation': 'one fresh Inkscape seed 220; fixed occlusion before second servo feedback, after first correction; no implicit retries',
        ...
        result = query({'after': batch['cursor'], 'events': ['saved_effect', 'rejected'], 'timeout': 5,
                        'request_id': uuid.uuid4().hex, 'command': {'op': 'effect', 'save_id': 'save'}})
    else:
