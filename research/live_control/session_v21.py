...
from patch_servo_v5 import PatchServo
from executor_v3 import DecisionRequired,Cancelled
...
class Backend(Previous):
    ...
    def execute(self,s,cancel,identifier,index):
        ...
        def check_reply(r):
            cancel.check()
            if time.perf_counter_ns()>=r['reply_until_ns']:
                cancel.set();raise DecisionRequired('local reply deadline elapsed')
            if cancel.cancel.is_set():raise Cancelled()
        def local_emit(r):
            emit(r)
            if r.get('event')=='pointer_yield':
                ...
                check_reply(r)
                try:self.reply_pointer(r['ticket'],r['sequence'],command)
                except ValueError:
                    check_reply(r)
                    raise
