...
R = H / 'results/authorized-redacted-mutation-live-01'
...
names = [
    'authorized_redacted_mutation_live_v1.py',
...
try:
    ...
    initial_prompt = f'''The current Value field is hidden and has no mutation authority. The task would be to replace it with public value {target} and Save, but do not target the hidden region without authorized refinement. Bind any proposal to the supplied observation and policy. Return ONLY one JSON object in the policy-bound replace_and_save or stop schema. Observation: {encoded(first_view)}'''
