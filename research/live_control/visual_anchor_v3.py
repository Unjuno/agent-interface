...
class VisualAnchor:
    ...
    def locate(self,image,observation_id,frame_id,allow_source=False):
        ...
        if observation_id==self.observation_id and not allow_source:return {'status':'same_observation'}
        ...
        for yy in range(max(0,y-r),min(a.shape[0]-h,y+r)+1):
            for xx in range(max(0,x-r),min(a.shape[1]-w,x+r)+1):
                residual=np.abs(a[yy:yy+h,xx:xx+w].astype(np.int16)-self.patch).mean(axis=2).ravel()/255
                residual.sort();error=float(residual[:max(1,int(len(residual)*.9))].mean())
        ...
        result={'status':status,'error':best,'margin':margin,'rival_error':min(rivals) if rivals else 1.0,'source_observation':self.observation_id,
