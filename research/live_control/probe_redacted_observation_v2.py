...
R = H / 'results/redacted-observation-controls-02'
...
names = ['probe_redacted_observation_v2.py', 'redacted_observation_v2.py',
...
with tempfile.TemporaryDirectory(prefix='redaction-controls-') as raw:
    ...
    with Image.open(root / 'out-a.png') as output:
        ...
        with Image.open(source_a).convert('RGB') as original:
            assert ImageChops.difference(original, output.convert('RGB')).getbbox() == (8, 8, 24, 24)
    assert view_a['redacted_regions'][0]['box_semantics'] == \
