"""Semantic-checkpoint supervisor for the third OpenTTD L-road allocation."""
...
base=HERE/'results/timing-envelope-openttd-l-03';root=base/arm;control=base/(arm+'-control');control.mkdir(parents=True,exist_ok=False)
def dump(path,value):
...
plan={'scope':'fresh seed-991003 OpenTTD five-tile L objective with buffered process-scoped timing envelope','arm':arm,
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['timing_envelope_openttd_l_supervisor_v3.py','timing_envelope_openttd_l_driver_v2.py','timing_envelope_v1.py','timing_envelope_v2.py','semantic_checkpoint_v1.py','openttd_finish_outcome_v1.py','model_pair_runner_v2.py','session_v22.py','openttd_contact_sheet_v1.py']},
      'max_turns':12,'seed_semantics':'byte-pinned seed-991003 save; target tiles 977,978,979,1043,1107; closed toolbar',
      'model_route':'all gpt-6-astra medium','prompt_policy':'v2 policy plus stateful observed/contradicted/uncertain checkpoint after pointer_drag','initial_ui_state':'closed toolbar; signs A, B corner, C and X keep clear'}
...
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_l_driver_v2.py'),arm,'timing-envelope-openttd-l-03'],stdout=(control/'driver-stdout.txt').open('wb'),stderr=(control/'driver-stderr.txt').open('wb'))
...
'''
        ...
        elif checkpoint_required_turn is not None and proposal['checkpoint']['status']=='observed':checkpoint_required_turn=None
        print(json.dumps({'turn':index,'kind':'act','status':previous['resolution']['terminal']['status'],'sequence':observation['sequence'],'checkpoint_required_turn':checkpoint_required_turn}),flush=True)
    else:
