...
from phased_outcome_v1 import present,feedback
H=Path(__file__).resolve().parent;R=H/'results/phased-outcome-controls-01';R.mkdir(exist_ok=False)
...
out={'scope':'archived live reports and synthetic evidence controls; no GUI input',
     'sources':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),H/'phased_outcome_v1.py',calc_path,prior_path,phase_path]},
