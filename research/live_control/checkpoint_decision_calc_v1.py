...
H=Path(__file__).resolve().parent;R=H/'results/checkpoint-decision-calc-01';R.mkdir(exist_ok=False)
def dump(n,x):(R/n).write_text(json.dumps(x,indent=2)+'\n',encoding='utf-8')
def win(p):return 'C:'+str(p.resolve())[6:]
names=['checkpoint_decision_calc_v1.py','phased_submit_v1.py','activation_handoff_v1.py','sampled_target_contract_v1.py','durable_submit_v5.py','calc_proposal_schema_v1.py','model_context_runner_v1.py','screenshot_responder_v1.txt','checkpoint_cause_socket_v1.py','decision_pair_v1.py','checkpoint_contract_v1.py','checkpoint_cause_cursor_v1.py','command_once_v3.py','stopped_socket_v2.py','request_boundary_v4.py']
...
p=subprocess.Popen([sys.executable,str(H/'checkpoint_cause_socket_v1.py'),'calc','serve','--','--app','calc','--seed','238','--out',str(R/'runtime')],stdout=subprocess.PIPE,stderr=(R/'stderr.txt').open('w'),text=True)
...
try:
 ...
 saved_contract={'kind':'saved_cells','expected':{'A1':goal['a'],'A2':goal['b']}}
 dump('completion-policy.json',{'contract':saved_contract,'finish_on_verified':True,'profile':'explicit saved-artifact checkpoint; not pixel-only GUI','scope':'declared cells only; final independent task score still required'})
 def call(spec):
...
Act has1..10 exact steps. First: {{"op":"pointer_click","x":INTEGER,"y":INTEGER,"duration_ms":80}}, bounds0..1279,0..799. Tail permits {{"op":"text","text":"1..8 ASCII digits"}}, {{"op":"key","key":"Return|Tab|Escape|Home|Up|Down|Left|Right"}}, {{"op":"chord","modifier":"Control_L","key":"s|a|Home"}}. No extra fields or tools; rationale1..600 characters. Verify only when both visible values match and no confirmation dialog remains; it requests independent saved-file scoring. Evidence: {json.dumps(feedback)}'''
  ...
  if proposal['kind']=='verify':
   ...
   query,verified=checkpoint();row['checkpoint']=query;row['finished_by']='verified_saved_contract' if verified else None
   turns.append(row);dump('turns.json',turns)
   if verified:break
   feedback={'verification_not_complete':True,'checkpoint_resolution':query['state']['last_resolution']}
   continue
  ...
  query,verified=checkpoint();row['checkpoint']=query
  if verified:
   row['finished_by']='verified_saved_contract';dump('turns.json',turns);break
  feedback['checkpoint_resolution']=query['state']['last_resolution']
 ...
 code=p.wait(timeout=10);assert code==0;dump('result.json',{'exit_code':code,'model_calls':index,'saved_success':True,'completion_source':'verified_saved_contract_then_independent_evaluation'});print(json.dumps({'saved_success':True,'model_calls':index}))
