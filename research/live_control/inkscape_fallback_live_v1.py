...
if stage=='initial':
    save('plan',dict(seed=229,app='inkscape',scope='One live self-use with executor v8 early response plus at most one bounded read in the same outer operation, full receipt and state table; injected optional composition failure after move-save; no operation replay or speed comparison',
        model_tokens=None,verified_model_id=None,sources={n:digest((HERE/n).read_bytes()) for n in
        ['presentation_emission_v1.py','presentation_choice_v1.py','composed_result_v1.py','shared_result_v1.py','inkscape_fallback_live_v1.py','cause_servo_socket_v5.py','cause_servo_interactive_v4.py','executor_v8.py','post_release_observation_v2.py',
...
if stage=='move-save':
    def injected_failure(data):
        raise RuntimeError('injected optional composer failure after completed move-save')
    deliver((out/'result.json').read_bytes(),sys.stdout.buffer,out/'presentation',composer=injected_failure)
else:
    deliver((out/'result.json').read_bytes(),sys.stdout.buffer,out/'presentation')
