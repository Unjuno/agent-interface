...
plan=json.loads(pathlib.Path(a.plan).read_text()); out=pathlib.Path(a.out); out.mkdir(parents=True,exist_ok=False); rows=[]; root=pathlib.Path(__file__).parent
for case in plan['cases']:
    cdir=out/case['id']; cp=subprocess.run([sys.executable,str(root/'run_case.py'),'--policy',case['policy'],'--arrival',case['arrival'],'--case-id',case['id'],'--out',str(cdir)],cwd=root,capture_output=True,text=True,check=False)
