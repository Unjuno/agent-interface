...
p=argparse.ArgumentParser(); p.add_argument('--policy',choices=['arrival_pos','sequence_pos'],required=True); p.add_argument('--arrival',choices=['seq_order','reverse_order'],required=True); p.add_argument('--case-id',required=True); p.add_argument('--out',required=True); a=p.parse_args()
out=pathlib.Path(a.out); out.mkdir(parents=True,exist_ok=False); db=out/'state.db'; init_db(db)
root=pathlib.Path(__file__).parent
procs={}
for seq in (4,5):
    procs[seq]=subprocess.Popen([sys.executable,str(root/'worker.py'),str(db),str(seq),a.policy,str(out/f'go{seq}'),str(out/f'worker{seq}.json')],cwd=root,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
order=(4,5) if a.arrival=='seq_order' else (5,4)
for seq in order:
...
result={'case_id':a.case_id,'policy':a.policy,'arrival':a.arrival,'arrival_order':list(order),'workers':workers,'pending_before_ack':pending_before,'ack_first':ack1,'ack_replay':ack2,'drain':drain,'final':snap(db)}
