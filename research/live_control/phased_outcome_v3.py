...
Revision3 retains the v2 output format and covers passive release failures.
...
def present(report):
    ...
    for result in exchanges:
        ...
        # Passive observation programs also release the owner. Failure there
        # must block input even though they are omitted from input-program rows.
        current_terminal = (state.get('last_resolution') or {}).get('terminal')
        if (command.get('op') == 'submit' and current_terminal and
                current_terminal.get('id') == command.get('id')):
            current_release = current_terminal.get('release') or {}
            release_unknown |= not (current_release.get('verified') is True and
                                    current_release.get('keys_down') == [] and
                                    current_release.get('buttons_down') == [])
        ...
        programs.append({'action_id': action_id,
                         ...
                         'effect': 'unknown'})
    unresolved = any(p['admission'] == 'unconfirmed' or
                     (p['admission'] != 'rejected' and p['terminal_status'] not in
                      ('completed', 'needs_decision', 'expired', 'cancelled', 'failed'))
                     for p in programs)
    if pending:
    ...
    return {'format': 'phased-outcome-v2', 'authority': 'none',
