...
def main(raw_path: Path, result_root: Path, out_path: Path) -> None:
    ...
    checks = {
        ...
        "p1_pixel_bytes_captured_while_p1_alive": (
            p1_bytes == base64.b64decode(raw.get("p1_window_pixels_before_exit", {}).get("base64", ""), validate=True)
            and raw.get("p1_window_pixels_before_exit", {}).get("sha256") == sha(p1_bytes)
            and raw.get("p1_pixels_capture_ns", 0) < raw.get("p1_cleanup", {}).get("observed_ns", 0)
            < p2.get("start_observed_ns", 0)
        ),
