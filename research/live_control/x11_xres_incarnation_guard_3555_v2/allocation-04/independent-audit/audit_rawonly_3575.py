...
def classify(raw: dict) -> str:
    ...
    p1_before_exit = raw.get("p1_window_pixels_before_exit", {})
    p1_capture_ns = raw.get("p1_pixels_capture_ns", -1)
    p1_exit_ns = raw.get("p1_cleanup", {}).get("observed_ns", -1)
    p2_start_ns = raw.get("p2", {}).get("start_observed_ns", -1)
    try:
        b1 = base64.b64decode(p1px["base64"], validate=True)
        b2 = base64.b64decode(p2px["base64"], validate=True)
        b1_before_exit = base64.b64decode(p1_before_exit["base64"], validate=True)
        pixel_ok = (p1px["size"] == p2px["size"] == [240, 160]
                    ...
                    and b1 == b2 == b1_before_exit
                    and p1_before_exit["sha256"] == p1px["sha256"]
                    and p1_before_exit["size"] == p1px["size"]
                    and p1_capture_ns < p1_exit_ns < p2_start_ns
...
def corruption_controls(raw: dict) -> list[dict]:
    ...
    mutations = [
        ...
        ("missing-p1-before-exit-pixels", lambda x: x.pop("p1_window_pixels_before_exit")),
        ("p1-pixels-captured-after-exit", lambda x: x.update(p1_pixels_capture_ns=x["p1_cleanup"]["observed_ns"] + 1)),
