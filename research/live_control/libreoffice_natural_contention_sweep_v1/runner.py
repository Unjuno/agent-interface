...
def case(kind,idx,delay_ms=20,controller_delay_ms=5,schedule_ms=None):
    ...
    try:
        ...
        time.sleep(controller_delay_ms/1000)
        current=x(doc,'A')
        if kind=='recovery':
            mutate(doc,'A',1700)
            current=x(doc,'A')
        ...
        result={'case_id':idx,'kind':kind,'observed_a':observed,'checked_a':current,'final_a':x(doc,'A'),'final_b':x(doc,'B'),'outcome':outcome,'writer':wp,'events':events,'cleanup':'pending','writer_delay_ms':delay_ms,'controller_delay_ms':controller_delay_ms,'schedule_ms':schedule_ms}
...
def main():
    ...
    for i in range(5): rows.append(case('stable',f'stable-{i+1}',20+i*7,5))
    schedule=[0,1,5,10,20,50]
    for delay in schedule:
        for i in range(5): rows.append(case('natural',f'natural-{delay}ms-{i+1}',delay,5,delay))
    for i in range(5): rows.append(case('external',f'external-{i+1}',20+i*7,5))
    rows.append(case('recovery','recovery-1',20,5))
    conflicts=any(r['outcome']=='CONFLICT_REJECTED' for r in rows if r['kind']=='natural')
    summary={'schema':'lo2975_predeclared_schedule_result_v1','allocation':'lo-natural-contention-sweep-20260920-a1','image':'agent-interface-lo2966:20260920','network':'none','schedule_ms':schedule,'rows':rows,'decision':'PASS_NATURAL_CONFLICT_BOUNDARY_SCOPED' if conflicts else 'HOLD_NO_NATURAL_CONTENTION_OBSERVED'}
