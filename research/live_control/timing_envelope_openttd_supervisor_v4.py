...
from openttd_proposal_schema_v4 import parse
...
root=HERE/'results/timing-envelope-openttd-04'
control=HERE/'results/timing-envelope-openttd-control-04';control.mkdir(exist_ok=False)
def dump(path,value):
...
plan={'scope':'fresh bounded OpenTTD loop with buffered process-scoped end-to-end timing envelope; no matched comparison yet',
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['timing_envelope_openttd_supervisor_v4.py','timing_envelope_openttd_driver_v4.py','timing_envelope_v1.py','timing_envelope_v2.py','openttd_proposal_schema_v4.py','model_pair_runner_v1.py','session_v10.py','openttd_contact_sheet_v1.py']},
...
turns=[]
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_driver_v4.py')],
...
try:
    ...
    for index in range(1,9):
        ...
        prompt=f'''You control an isolated OpenTTD road fixture using its current screenshot. Task: {task} Choose the next bounded visual action. Use the road toolbar and straight-road tool when visible. If toolbar meanings are uncertain, inspect up to three distinct icons in one act using repeated pointer_move + dwell_observe pairs around 800ms. The next planner image will preserve those intermediate tooltips as labeled strips, reducing model round trips. Do not revisit an already inspected coordinate within about 12 pixels unless the visible state changed. Ordinary observe immediately after a move may be too early. A prior needs_decision/focus_changed or zero steps_completed does not mean the application effect failed: inspect the screenshot and never blindly replay.
