...
def main():
    ...
    useful = report["semantic_observation"]; terminal = report["selection_terminal"]
    metrics = report["metrics_ms"]
    admission_image = root / Path(report["admission_observation"]["image"]).name
    pre_score = score(early["candidate"], admission_image)
    first_score = score(early["candidate"], root / Path(feedback["image"]).name)
    post_score = score(early["candidate"], root / Path(useful["image"]).name)
    checks = {
        ...
        "coherent_binding_before_admission":
            1 <= len(report["binding_attempts"]) <= plan["binding_attempt_limit"] and
            report["binding_attempts"][-1]["status"] == "READY" and
            report["binding_attempts"][-1]["may_submit_pointer_input"] is True and
            report["binding_attempts"][-1]["binding"] ==
            report["admission_observation"]["pointer_binding"],
        "candidate_exactly_admitted": accepted["event"] == "accepted" and
            accepted["steps"] == len(report["selection_program"]) and
            accepted["program_sha256"] == program_sha256(report["selection_program"]) and
            len([row for row in events if row.get("event") == "accepted"]) == 2,
        "first_boundary_is_feedback": report["selection_boundary"]["status"] == "FEEDBACK" and
            report["selection_boundary"]["record"] == feedback,
        "independent_visible_effect": pre_score == report["pre_selection_score"] and
            pre_score["success"] is False and first_score["success"] is False and
            post_score == report["semantic_score"] and post_score["success"] is True,
        "first_useful_is_later_observation": len(report["semantic_scores"]) >= 2 and
            report["semantic_scores"][0]["observation"] == feedback and
            report["semantic_scores"][0]["score"] == first_score and
            report["semantic_scores"][-1]["observation"] == useful and
            report["semantic_scores"][-1]["score"] == post_score and
        ...
        "feedback_precedes_score": feedback["capture_ns"] >= accepted["accepted_ns"] and
            ...
            report["semantic_score_completed_ns"] >= report["first_feedback_received_ns"] and
            useful["capture_ns"] >= feedback["capture_ns"],
