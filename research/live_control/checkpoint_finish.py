...
def run(exchange,*,after,contract,checkpoint_id,finish_id,finish_on_match,timeout=5):
    ...
    result=dict(state='needs_decision',task_success=None,finish_attempted=False,authority='none',exchanges=[])
    def call(spec):
    ...
    try:
        ...
        if query.get('status')!='boundary':return result
        ...
        if not isinstance(evidence,dict) or evidence.get('status')!='VERIFIED':return result
