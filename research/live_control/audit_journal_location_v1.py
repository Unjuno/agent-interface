...
def main():
    root = HERE / 'results/journal-location-01'; plan = read(root / 'plan.json')
    ...
    for index, mode in enumerate(plan['order']):
        ...
        for cycle, measured in enumerate(calls):
            ...
            if mode != 'received': assert c['state']['pending'] is None
        ...
        if mode != 'received': assert read(out / 'journal.json') == calls[-1]['result']['state']
        ...
        location = read(out / 'journal-location.json')
        if mode == 'native': assert location['path'].startswith('/tmp/') and not Path(location['path']).exists()
        else: assert location['path'].startswith('/mnt/c/')
    ...
    report = {'groups': groups, 'sessions': summaries, 'measured_calls': len(rows),
              'scope': 'ABBA same durable API two fresh sessions per location; mounted versus native Linux journal; identical fsync code, no power-loss proof/model',
