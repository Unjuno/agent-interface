...
from request_correlation import correlation
...
def main():
    ...
    lock=threading.Lock();events=[];projection=Presentation();ledger=DeliveryLedger()
    def emit(record):
        output_flushed=True
        try:
            with lock:
    ...
    with (args.out/'delivery-flush.jsonl').open('xb') as receipt_stream:
        ...
        try:
            ...
            emit(dict(event='ready',app=args.app,task=task,goal=goal,presentation=args.presentation,finish_after_contract='submit finish_after=true reserves final program; after terminal close input admission and score; cancel/clock remain available; finish cleans up without rescoring',basic_step_examples=[{'op':'text','text':'123'},{'op':'key','key':'Return'},{'op':'chord','modifier':'Control_L','key':'s'}],decision_evidence_schema={'required_for':'submit','fields':{'delivery_id':'successfully flushed observation or review delivery ID','observation_sequence':'must equal expected_sequence','producer':['assistant','scripted','human']},'authority':'none; ordinary admission required'},servo_schema=SERVO_SCHEMA,operations=['submit','pointer_reply','cancel','clock','finish','finalization_status'],guided_fields=['points','duration_ms','reply_timeout_ms','max_updates','feedback_delay_ms'],reply_fields=['ticket','sequence','command']))
            ...
            for line in sys.stdin:
                ...
                try:
                    ...
                    if command['op']=='submit':
                        ...
                        emit(dict(event='finalization_status',state='available' if status_snapshot is not None else ('pending' if final_id is not None else 'not_requested'),outcome=status_snapshot,scope='read-only retained result; no rescore or new authority'))
