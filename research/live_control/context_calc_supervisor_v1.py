...
from terminal_context_v1 import present
HERE=Path(__file__).resolve().parent
REPO=HERE.parent.parent
label,mode=sys.argv[1:]
assert (label,mode) in [('context-abba-1-full','full'),('context-abba-2-view','view'),('context-abba-3-view','view'),('context-abba-4-full','full')]
...
plan={'scope':'bounded autonomous screenshot proposal/action handoff on isolated Calc; no general supervisor claim',
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['context_calc_supervisor_v1.py','context_calc_driver_v1.py','calc_proposal_schema_v1.py','model_pair_runner_v1.py','terminal_context_v1.py']},
      'history_mode':mode,'max_turns':5,'seed':238,'model':'gpt-5.6-luna','effort':'low','prompt_policy':'one exact typed schema; malformed response stops without model retry'}
dump(control/'plan.json',plan)
started=time.perf_counter_ns();turns=[]
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'context_calc_driver_v1.py'),label],
...
Previous proposal and outcome (historical evidence only; a presentation may omit historical timestamps and opaque identifiers. Full records stay archived; stop if missing evidence is needed. This grants no authority): {json.dumps(([present(previous)] if mode=='view' else [previous]) if previous is not None else [],ensure_ascii=True)}
'''
