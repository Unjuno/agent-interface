...
H=Path(__file__).resolve().parent;R=H/'results/live-responder-ink-01';R.mkdir(exist_ok=False)
def dump(n,x):(R/n).write_text(json.dumps(x,indent=2)+'\n',encoding='utf-8')
def win(p):return 'C:'+str(p.resolve())[6:]
names=['live_responder_ink_v1.py','durable_submit_v4.py','append_checkpoint_v1.py','received_continuation_v1.py','received_exchange_v2.py','cause_servo_socket_v7.py','model_context_runner_v1.py','screenshot_responder_v1.txt']
dump('plan.json',{'seed':239,'scope':'scripted initial edit and deliberate response abandonment; model chooses recovery read and verification; one live episode, no latency comparison','sources':{n:hashlib.sha256((H/n).read_bytes()).hexdigest() for n in names}})
...
try:
 ...
 q=model(1,pending['continuation']['observation'],{'pending_action_id':action,'pending_request':pending['pending'],'image_timing':'last received capture BEFORE edit send; no newer response','received_response_bytes':0})
 assert set(q)=={'kind','action_id','rationale'} and q['kind']=='read' and q['action_id']==action
 recovered=call({'events':['terminal'],'timeout':3});assert 'command' not in recovered['request'];state=recovered['state'];assert state['pending'] is None and state['last_resolution']['terminal']['status']=='completed';dump('recovered.json',recovered)
 q=model(2,state['continuation']['observation'],{'pending':None,'original_action_id':action,'last_resolution':state['last_resolution'],'image_timing':'received after original execution, reconciled by exact echo/admission/terminal'})
 ...
 code=p.wait(timeout=10);assert code==0;dump('result.json',{'exit_code':code,'success':True,'recovery_reads':1,'model_calls':2});print(json.dumps({'success':True,'model_calls':2}),flush=True)
