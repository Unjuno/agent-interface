"""Run fresh target, partial and guard X11 programs without retry."""
import hashlib
import json
import shutil
import time
...
def run_case(name, allocation, spec, out):
    ...
    try:
        ...
        dx, dy = allocation["pointer_delta"]
        length = max(abs(dx), abs(dy))
        points = [{"x": x + (dx * offset // length), "y": y + (dy * offset // length)}
        ...
        outcomes = [row for row in events if row.get("event") == "local_target_guard_postcondition"]
        ...
        result = {
            ...
            "postcondition": outcomes[0] if len(outcomes) == 1 else None,
            "postcondition_event_count": len(outcomes), "terminal": terminal,
            "steps_started": started, "save_started": 4 in started,
            "owner_focus_release": any(row.get("reason") == "focus_changed" for row in backend.owner.records),
        }
...
def main():
    ...
    report = {
        ...
        "decision": ("RETAIN_FRESH_TARGET_GUARD_INTEGRATION;_AUDIT_BEFORE_TRANSFER" if passed else
                     "HOLD_TARGET_GUARD_INTEGRATION;_PRESERVE_FAILURE"),
        "scope": plan["scope"],
    }
