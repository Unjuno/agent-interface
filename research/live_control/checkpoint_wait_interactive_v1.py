"""Cause runtime with an opt-in bounded saved-effect long poll."""
import argparse,contextlib,hashlib,json,shutil,sys,threading,time
from pathlib import Path
from cause_servo_session_v1 import Backend,suite,SERVO_SCHEMA
from executor_v9 import Executor
from effect_checkpoint_v3 import Checkpoints
from checkpoint_contract_v1 import validate as validate_contract
...
def main():
    ...
    checkpoints=Checkpoints(emit,args.out/'checkpoint-artifacts')
    session=None;server=None;engine=None;output=None;backend=None
    try:
        hashes={}
        for path in [HERE/'checkpoint_wait_interactive_v1.py',HERE/'checkpoint_contract_v1.py',HERE/'effect_checkpoint_v3.py',HERE/'effect_checkpoint_v2.py',HERE/'effect_checkpoint.py',HERE/'saved_effect.py',HERE/'request_correlation_v2.py',HERE/'rejection_identity_v1.py',HERE/'cause_servo_session_v1.py',HERE/'cause_session_v1.py',HERE/'session_v21.py',HERE/'patch_servo_v5.py',HERE/'visual_anchor_v2.py',HERE/'visual_anchor_v3.py',HERE/'visual_anchor_v4.py',HERE/'executor_v9.py',HERE/'post_release_observation_v2.py',HERE/'lease_cause_v1.py',HERE/'lease_cause_v2.py',HERE/'input_owner_v10.py',HERE/'session_v16.py',HERE/'patch_servo.py',HERE/'visual_anchor.py',HERE/'session_v15.py',HERE/'session_v13.py',HERE/'session_v12.py',HERE/'session_v11.py',HERE/'input_owner_v9.py',HERE/'pointer_reply_v2.py',HERE/'session_v9.py',HERE/'input_owner_v5.py',HERE/'presentation.py',HERE/'session_v8.py',HERE/'quiet_window.py',HERE/'session_v7.py',HERE/'session_v6.py',HERE/'input_owner_v2.py',HERE/'session_v5.py',HERE/'input_owner.py',HERE/'executor_v3.py',HERE/'session_v4.py',HERE/'lease.py',HERE.parent/'observation_tiles/tile_transport.py',
        ...
        emit(dict(event='ready',app=args.app,task=task,goal=goal,presentation=args.presentation,servo_schema=SERVO_SCHEMA,operations=['submit','pointer_reply','cancel','clock','finish','effect_checkpoint'],guided_fields=['points','duration_ms','reply_timeout_ms','max_updates','feedback_delay_ms'],reply_fields=['ticket','sequence','command']))
        ...
        for line in sys.stdin:
            ...
            try:
                ...
                if command['op']=='submit':engine.submit(command['id'],command['steps'],command['expected_sequence'],command['valid_until_ns'])
                    ...
                    if set(command)-{'op','contract','transport_request_id','wait_ms','poll_ms'}:raise ValueError('unexpected checkpoint fields')
                    contract=validate_contract(command['contract'])
                    allowed={'calc':'saved_cells','chromium':'saved_form_value'}
                    if allowed.get(args.app)!=contract['kind']:raise ValueError('contract unsupported for application')
                    checkpoints.request(output,contract,correlation(request_sequence,command),
                                        wait_ms=command.get('wait_ms',0),poll_ms=command.get('poll_ms',50))
                elif command['op']=='finish':
                    engine.close()
                    emit(dict(event='independent_evaluation',evaluation_request=correlation(request_sequence,command),**suite.evaluate(args.app,output,goal)))
                    break
                else:raise ValueError('unsupported command')
            except (ValueError,KeyError,TypeError,Expired) as exc:
                emit(rejected(command,engine.used_ids,exc))
    finally:
        checkpoints.close()
