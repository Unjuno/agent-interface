...
HERE=Path(__file__).resolve().parent;root=HERE/'results/outcome-client-self-use-01'
...
assert result['evidence']['actual']==(root/'submitted.txt').read_text()=='t991028'
...
report=dict(actual='t991028',exact_frames=count,submit_commands=1,outcome_exchange_calls=1,status_queries=0,
...
(HERE/'results/outcome-client-self-use-audit.json').write_text(json.dumps(report,indent=2)+'\n')
(root/'client-sources.json').write_text(json.dumps({p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (Path(__file__),HERE/'outcome_client.py',HERE/'outcome_fallback_v2.py',HERE/'outcome_wait.py',HERE/'receipt_image.py',HERE/'event_socket_v11.py',HERE/'event_cursor_v5.py',HERE/'request_boundary_v2.py')},indent=2)+'\n')
