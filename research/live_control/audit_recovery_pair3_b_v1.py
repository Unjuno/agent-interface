"""Verify archived OpenTTD pair 3 B, retaining presentation failure."""
import hashlib
import json
import sys
...
def main():
    root = HERE / 'results/recovery-pair3-01/B'
    runtime = root / 'runtime'
    metadata = read(root / 'initial/execution.json')
    assert (metadata['pair'], metadata['arm'], metadata['seed']) == (3, 'B', 203)
    ...
    result = {
        'audit_sha256': sha(Path(__file__)), 'pair': 3, 'arm': 'B',
        ...
        'next': 'Pair 3 A using unchanged runner and canonical save.'}
