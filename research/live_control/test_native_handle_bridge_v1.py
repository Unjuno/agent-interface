...
class NativeHandleBridgeTests(unittest.TestCase):
    ...
    def test_child_focus_ancestry_is_bounded_and_does_not_grant_other_surface(self):
        bridge = self.review_bridge(focus=21, captured_focus=21)
        parent = SimpleNamespace(id=20)
        child = SimpleNamespace(id=21, query_tree=lambda: SimpleNamespace(parent=parent))
        bridge.backend.d.get_input_focus.return_value.focus = child
        self.assertTrue(bridge._focus_within_target(20))
        self.assertFalse(bridge._focus_within_target(99))
        row = bridge.review_window(20)
        self.assertEqual(row['status'], 'reviewed')
        self.assertEqual(row['observation']['pointer_binding']['focus'], 21)
        self.assertFalse(row['authority_granted'])
        child.query_tree = lambda: SimpleNamespace(parent=child)
        self.assertFalse(bridge._focus_within_target(20))

    def test_guarded_focus_preserves_validated_child_and_refuses_outsider(self):
        backend = object.__new__(_GuardedBackend)
        backend.owner = SimpleNamespace(active=('field', [1,1]), check=mock.Mock(),
                                        _focus_within_target=mock.Mock(return_value=True))
        with mock.patch.object(X11Backend, 'focus') as native_focus:
            backend.focus('app')
            native_focus.assert_not_called()
            backend.owner.check.assert_called_once_with('before_focus')
            backend.owner._focus_within_target.return_value = False
            with self.assertRaises(X11BackendError):
                backend.focus('app')
            native_focus.assert_not_called()

    def test_focused_client_uses_managed_ancestor_without_mutation_or_input(self):
        bridge = self.review_bridge()
        top = SimpleNamespace(id=30)
        child = SimpleNamespace(id=31, query_tree=lambda: SimpleNamespace(parent=top))
        bridge.backend.d.get_input_focus.return_value.focus = child
        bridge.backend.root = mock.Mock()
        bridge.backend.root.get_full_property.return_value = SimpleNamespace(format=32,value=[20,30])
        before = (bridge.scope, bridge.backend.targets['app'].id, dict(bridge.history))
        self.assertEqual(bridge.focused_client_window(),30)
        self.assertEqual((bridge.scope,bridge.backend.targets['app'].id,bridge.history),before)
        bridge.session.dispatch.assert_not_called()
        bridge.observe.assert_not_called()
        bridge._save.assert_not_called()

    def test_focused_client_missing_list_cycle_unknown_and_read_failure_stop(self):
        bridge = self.review_bridge()
        bridge.backend.root = mock.Mock()
        for value in (None, SimpleNamespace(format=8,value=[20])):
            bridge.backend.root.get_full_property.return_value = value
            self.assertIsNone(bridge.focused_client_window())
        bridge.backend.root.get_full_property.return_value = SimpleNamespace(format=32,value=[99])
        child = SimpleNamespace(id=21)
        child.query_tree=lambda: SimpleNamespace(parent=child)
        bridge.backend.d.get_input_focus.return_value.focus=child
        self.assertIsNone(bridge.focused_client_window())
        bridge.backend.d.get_input_focus.return_value.focus=0
        self.assertIsNone(bridge.focused_client_window())
        bridge.backend.root.get_full_property.side_effect=RuntimeError('unavailable')
        self.assertIsNone(bridge.focused_client_window())

    def test_window_review_does_not_clear_unverified_input_release(self):
        bridge = self.review_bridge()
        bridge.session.recovery_required = True
        self.assertEqual(bridge.review_window(20)['status'], 'reviewed')
        self.assertTrue(bridge.session.recovery_required)
        with self.assertRaisesRegex(X11BackendError, 'input recovery'):
            bridge.mint('new_target', 8, [1, 1])
        row = bridge.click('old_target', [1, 1])
        self.assertEqual(row['error'], 'INPUT_RECOVERY_REQUIRED')
    ...
    def test_feedback_title_prefers_utf8_net_name_and_falls_back_only_if_absent(self):
        ...
        bridge.target = 'app'
        window = mock.Mock()
        window.get_full_property.return_value = SimpleNamespace(value='菫晏ｭ俶ｸ医∩'.encode('utf-8'))
        window.get_wm_name.return_value = 'legacy'
        bridge.backend = SimpleNamespace(targets={'app': window}, d=mock.Mock())
        self.assertEqual(bridge._window_title(), '菫晏ｭ俶ｸ医∩')
    ...
    def feedback_bridge(self, titles, *, focus=20):
        ...
        bridge._focus_within_target = mock.Mock(return_value=focus == 20)
    ...
    def test_keyboard_continuation_has_no_pointer_ops_and_requires_release(self):
        bridge = object.__new__(NativeHandleBridge)
        bridge.active = None
        bridge.check = mock.Mock(return_value={'point': [10, 20]})
        bridge._save = mock.Mock()
        bridge.sequence, bridge.binding_revision = 7, 2
        bridge.scope, bridge.target = 'native-x11:test', 'app'
        bridge.session = SimpleNamespace(dispatch=mock.Mock(return_value={'status': 'completed'}))
        row = bridge.keyboard('anchor', [1, 1], tail=[{'op': 'key_chord', 'keys': ['Right']}])
        program = bridge.session.dispatch.call_args.args[0]
        self.assertEqual([op['op'] for op in program['ops']], ['focus', 'key_chord', 'release_all'])
        self.assertTrue(program['terminal']['release_all_required'])
        self.assertEqual(program['source'], {'observation_seq': 7, 'binding_revision': 2})
        self.assertEqual(row['status'], 'completed')
        self.assertIsNone(bridge.active)

    def test_keyboard_refusal_does_not_fall_back_to_click(self):
        for recovery in (False, True):
            with self.subTest(recovery=recovery):
                bridge = object.__new__(NativeHandleBridge)
                bridge.active = None
                bridge.check = mock.Mock(side_effect=X11BackendError('stale anchor'))
                bridge._save = mock.Mock()
                bridge.session = SimpleNamespace(recovery_required=recovery, dispatch=mock.Mock())
                row = bridge.keyboard('old', [1, 1], tail=[{'op': 'key_chord', 'keys': ['Right']}])
                self.assertEqual(row['status'], 'refused')
                self.assertFalse(row['input_dispatched'])
                bridge.session.dispatch.assert_not_called()

    def test_keyboard_rejects_empty_or_pointer_tail_before_admission(self):
        bridge = object.__new__(NativeHandleBridge)
        bridge.active = None
        bridge.check = mock.Mock()
        for tail in ([], [{'op': 'wait_update', 'timeout_ms': 10}],
                     [{'op': 'pointer_button', 'button': 'left', 'down': True}]):
            with self.subTest(tail=tail), self.assertRaises(ValueError):
                bridge.keyboard('anchor', [1, 1], tail=tail)
    ...
    def test_stale_drawable_recovery_uses_read_only_discovery_then_explicit_review(self):
        ...
        # The old anchor is genuinely unreadable. Recovery must not keep asking
        # its geometry while discovering the currently focused managed client.
        with self.assertRaisesRegex(X11BackendError, "BadDrawable"):
            bridge._binding()
        parent = SimpleNamespace(id=30)
        child = SimpleNamespace(
            id=31, query_tree=lambda: SimpleNamespace(parent=parent))
        bridge.backend.d.get_input_focus.return_value.focus = child
        self.assertEqual(bridge.focused_client_window(), 30)
        bridge.backend.geometry.assert_called_once()
        self.assertEqual(bridge.backend.targets["app"].id, 10)
        self.assertEqual(bridge.session.dispatch.call_count, 0)
        bridge.observe.assert_not_called()

        # Rebinding is a separate explicit caller choice. It rotates the
        # binding scope and clears the old observation/handle source.
        previous_scope = bridge.scope
        row = bridge.review_window(30)
        self.assertEqual(row["status"], "reviewed")
        self.assertFalse(row["authority_granted"])
        self.assertFalse(row["input_dispatched"])
