...
def main():
    ...
    try:
        def wm_ready():
            if s.openbox.poll() is not None:raise RuntimeError('private window manager exited')
            prop=root.get_full_property(d.intern_atom('_NET_SUPPORTING_WM_CHECK'),X.AnyPropertyType)
            return prop is not None and len(prop.value)>0
        s._wait(wm_ready,5,'private window manager readiness')
        ...
        w.set_wm_name('Pointer backend fixture');w.set_wm_class('pointerbackend','PointerBackend')
        w.change_property(d.intern_atom('_NET_WM_NAME'),d.intern_atom('UTF8_STRING'),8,b'Pointer backend fixture')
        w.map();d.sync();time.sleep(.5)
        (a.out/'setup-windows.txt').write_text(s.windows())
        (a.out/'setup-diagnostics.json').write_text(json.dumps({'suite':suite.__file__,'display':s.name,'env_display':s.env['DISPLAY'],'wm_status':s.openbox.poll(),'window':w.id,'map_state':w.get_attributes().map_state,'parent':w.query_tree().parent.id,'root':root.id,'root_properties':[d.get_atom_name(p) for p in root.list_properties()]},indent=2))
        s.wait_window('Pointer backend fixture');s.focus('Pointer backend fixture')
        ...
        submit('completed_drag',[{'op':'pointer_drag','points':[{'x':x+80,'y':y+80},{'x':x+90,'y':y+90},{'x':x+100,'y':y+100}],'duration_ms':100}])
        pointer=root.query_pointer()
        assert (pointer.root_x,pointer.root_y)==(x+100,y+100) and not pointer.mask & X.Button1Mask
        checks.append({'case':'drag_endpoint_and_release','point':[pointer.root_x,pointer.root_y],'button1_down':False})
