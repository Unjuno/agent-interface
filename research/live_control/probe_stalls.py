...
import session_v4 as current
from executor_v3 import Executor

HERE=Path(__file__).resolve().parent


def run(out,fault,seed):
    out.mkdir(parents=True,exist_ok=False);s=None;events=[];samples=[];stop=threading.Event()
    def emit(r):
        events.append(r)
    ...
    try:
        ...
        report=dict(fault=fault,seed=seed,deadline_ns=deadline,
...
if __name__=='__main__':
    ...
    names=['probe_stalls.py','session_v4.py','executor_v3.py','lease.py']
    ...
    try:
        for seed in (910101,910102):
            for fault in ('log','capture'):
                r=run(a.out/f'{seed}-{fault}',fault,seed);rows.append(r);print(json.dumps(r),flush=True)
