...
root=HERE/'results/timing-envelope-openttd-01'
control=HERE/'results/timing-envelope-openttd-control-01';control.mkdir(exist_ok=False)
def dump(path,value):
...
plan={'scope':'fresh bounded OpenTTD loop with buffered process-scoped end-to-end timing envelope; no matched comparison yet',
      'sources':{n:hashlib.sha256((HERE/n).read_bytes()).hexdigest() for n in ['timing_envelope_openttd_supervisor_v1.py','timing_envelope_openttd_driver_v1.py','timing_envelope_v1.py','timing_envelope_v2.py','openttd_proposal_schema_v1.py','model_pair_runner_v1.py']},
      'max_turns':5,'seed_semantics':'canonical save; no RNG override','model':'gpt-5.6-luna','effort':'low','prompt_policy':'one exact typed schema; malformed response stops without model retry'}
...
turns=[]
driver=subprocess.Popen(['wsl','-d','Ubuntu','--','python3',linux(HERE/'timing_envelope_openttd_driver_v1.py')],
...
try:
    ...
    for index in range(1,6):
        prompt=f'''You control an isolated OpenTTD road fixture using its current 1024x720 screenshot. Task: {task} Choose the next bounded visual action. Use the road toolbar and straight-road tool when visible. A prior needs_decision/focus_changed or zero steps_completed does not mean the application effect failed: inspect the screenshot and never blindly replay.
...
{{"op":"observe"}}
Pointer bounds x0..1023,y0..719. No extra fields, keys, tools or shell. The harness obtains a fresh observation and checks unchanged focus/binding before input. Propose only actions justified by the current image.
