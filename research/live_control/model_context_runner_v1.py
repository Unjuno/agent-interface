...
def main():
    node, cli, image, prompt_file, working, output, mode = sys.argv[1:]
    assert mode in ("builtin", "responder")
    ...
    instructions=Path(__file__).with_name('screenshot_responder_v1.txt')
    args[args.index('-C'):args.index('-C')]=['--disable','fast_mode']
    if mode=='responder':
        args[args.index('-C'):args.index('-C')]=['-c','model_instructions_file='+json.dumps(instructions.resolve().as_posix())]
    plan = dict(instruction_mode=mode, instructions_sha256=hashlib.sha256(instructions.read_bytes()).hexdigest() if mode=="responder" else None, args=args, requested_model='gpt-5.6-luna', requested_effort='low',
