...
def main():
    ...
    sources=[HERE/'run.py',HERE/'score.py',HERE/'plan.json',*sorted((HERE/'script').glob('*')),
    ...
    for seed in manifest['plan']['seeds']:
        ...
        try:
            ...
            shutil.copytree(r/'share/games/openttd/game',data/'game');shutil.copytree(HERE/'script',data/'game/interface_oracle')
            ...
            with (out/'stdout.txt').open('w') as so,(out/'stderr.txt').open('w') as se:
                ...
                while p.poll() is None and time.monotonic()<end:
                    ...
                    if 'AIO_DONE' in log or 'AIO_FIXTURE_ERROR' in log:break
                    time.sleep(.2)
