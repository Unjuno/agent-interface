...
function InterfaceOracle::Emit(stage, x, y) {
    ...
    for(local dy=0;dy<2;dy++) for(local dx=0;dx<3;dx++) {
        ...
        tiles+=format("{\"id\":%d,\"road\":%s,\"owner\":%d}",t,
            GSRoad.IsRoadTile(t)?"true":"false",GSTile.GetOwner(t));
    }
    local edges="";
    for(local dx=0;dx<2;dx++) {
        local a=GSMap.GetTileIndex(x+dx,y), b=GSMap.GetTileIndex(x+dx+1,y);
        if(dx>0) edges+=",";
        edges+=format("[%d,%d,%s,%s]",a,b,
            GSRoad.AreRoadTilesConnected(a,b)?"true":"false",
            GSRoad.AreRoadTilesConnected(b,a)?"true":"false");
    }
    GSLog.Info("AIO "+format("{\"stage\":\"%s\",\"x\":%d,\"y\":%d,\"width\":%d,\"tiles\":[%s],\"edges\":[%s]}",
        stage,x,y,GSMap.GetMapSizeX(),tiles,edges));
}
function InterfaceOracle::Start() {
    ...
    if(!GSRoad.BuildRoad(a,b)) {GSLog.Error("AIO_FIXTURE_ERROR partial");return;}
    this.Emit("partial",px,py);
    if(!GSRoad.BuildRoad(b,c)) {GSLog.Error("AIO_FIXTURE_ERROR complete");return;}
    this.Emit("complete",px,py);
    if(!GSRoad.BuildRoad(GSMap.GetTileIndex(px,py+1),GSMap.GetTileIndex(px+2,py+1))) {GSLog.Error("AIO_FIXTURE_ERROR extra");return;}
