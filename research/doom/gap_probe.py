...
def probe(out,seed,mode,gap_seconds):
    p=subprocess.Popen([sys.executable,str(HERE/'session_v6.py'),'--out',str(out),'--seed',str(seed)],
    ...
    try:
        ...
        steps=[dict(op='hold',keys=['Right'],duration_ms=100 if mode=='ordinary' else 1000)]
...
def main():
    ...
    cases=[dict(mode='ordinary',seed=990202,gap_seconds=gap) for gap in (0,20)]
    manifest=dict(cases=cases,scope='same-seed exploratory planner-gap rendering comparison; not qualification',
        ...
        sources={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (Path(__file__),HERE/'session_v6.py')})
