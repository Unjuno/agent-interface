...
def main():
    ...
    a.out.mkdir(parents=True,exist_ok=False);lock=threading.RLock();latest=None;s=None;g=None;e=None;backend=None
    def emit(row):
        nonlocal latest
        with lock:
            row['emit_ns']=time.perf_counter_ns()
            with (a.out/'events.jsonl').open('a') as f:f.write(json.dumps(row)+'\n')
            if row['event']=='observation':latest=row.copy()
            with (a.out/'delivered.jsonl').open('a') as f:f.write(json.dumps(row)+'\n')
            print(json.dumps(row),flush=True)
    reference=json.loads((HERE.parent/'live_control/results/presentation-assistant-01/sources.json').read_text())
    for name,digest in reference.items():
        if hashlib.sha256((HERE.parent/name).read_bytes()).hexdigest()!=digest:
            raise RuntimeError('Shared runtime source mismatch: '+name)
    sources={**reference,'doom/session_delta_diagnostic.py':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
    ...
    try:
        with (a.out/'setup.txt').open('w') as diagnostics,contextlib.redirect_stdout(diagnostics):
            ...
            g.set_available_buttons([vd.Button.TURN_LEFT,vd.Button.TURN_RIGHT,vd.Button.MOVE_FORWARD,
                vd.Button.MOVE_BACKWARD,vd.Button.MOVE_LEFT,vd.Button.MOVE_RIGHT,vd.Button.ATTACK,vd.Button.TURN_LEFT_RIGHT_DELTA])
            g.init()
        ...
        variables=['ANGLE','POSITION_X','POSITION_Y','AMMO2']
        initial_state={name:g.get_game_variable(getattr(vd.GameVariable,name)) for name in variables}
        ...
        with (a.out/'backend-setup.txt').open('w') as diagnostics,contextlib.redirect_stdout(diagnostics):
            backend=Backend(s,a.out,emit)
        e=Executor(backend,emit)
        emit(dict(event='ready',presentation='full',operations=['submit','cancel','clock','finish'],task='Basic ViZDoom scenario using Freedoom assets',bindings={'Left':'turn left','Right':'turn right','Up':'forward','Down':'back','space':'attack'}))
        backend.snapshot('initial',0)
        for line in sys.stdin:
            try:
                c=json.loads(line);emit(dict(event='command',command=c,received_ns=time.perf_counter_ns()))
                if c['op']=='submit':e.submit(c['id'],c['steps'],c['expected_sequence'],c['valid_until_ns'])
                elif c['op']=='cancel':e.cancel(c['id'])
                elif c['op']=='clock':emit(dict(event='clock',runtime_ns=time.perf_counter_ns(),sequence=backend.sequence))
                elif c['op']=='finish':
                    e.close()
                    if not g.is_episode_finished():g.advance_action(1,True)
                    (a.out/'post-control-angle.json').write_text(json.dumps(dict(initial=initial_state,final={name:g.get_game_variable(getattr(vd.GameVariable,name)) for name in variables})))
                    backend.snapshot('post-control-refresh',0)
            ...
            except (ValueError,TypeError,KeyError,Expired) as exc:emit(dict(event='rejected',reason=str(exc)))
    finally:
