...
def main():
    ...
    sources={**reference,'doom/session_refresh_diagnostic.py':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
    ...
    try:
        ...
        initial_angle=g.get_game_variable(vd.GameVariable.ANGLE)
        ...
        with (a.out/'backend-setup.txt').open('w') as diagnostics,contextlib.redirect_stdout(diagnostics):
            backend=RefreshBackend(s,a.out,emit)
        ...
        for line in sys.stdin:
            try:
                ...
                if c['op']=='submit':e.submit(c['id'],c['steps'],c['expected_sequence'],c['valid_until_ns'])
                    ...
                    e.close()
                    if not g.is_episode_finished():g.advance_action(1,True)
                    (a.out/'post-control-angle.json').write_text(json.dumps(dict(initial_angle=initial_angle,final_angle=g.get_game_variable(vd.GameVariable.ANGLE))))
