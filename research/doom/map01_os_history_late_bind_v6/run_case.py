...
def main():
 ...
 try:
  ...
  roll=Rolling(s,ctx,plan['rolling_period_s']);roll.start();time.sleep(.08);target=7 if case['state']=='opening' else 161;score['current_rel']=wait(g,origin,target);time.sleep(plan['rolling_period_s']*1.15)
  if roll.error:raise RuntimeError('rolling_capture:'+roll.error)
  if case['acquisition']=='snapshot_bound':
   rows=roll.snapshot();prev,cur,pi,ci=freshest_pair(rows,plan['history_min_gap_ns'],plan['history_max_gap_ns'],plan['history_target_gap_ns']);score['selected_indices']=[pi,ci];score['prewait_selected_current_ns']=cur['end_ns'];wait0=time.perf_counter_ns();score['planner_wait_started_ns']=wait0;time.sleep(plan['planner_wait_s']);wait1=time.perf_counter_ns();score['planner_wait_ended_ns']=wait1;score['planner_wait_ns']=wait1-wait0
  elif case['acquisition']=='late_bind':
   wait0=time.perf_counter_ns();score['planner_wait_started_ns']=wait0;time.sleep(plan['planner_wait_s']);wait1=time.perf_counter_ns();score['planner_wait_ended_ns']=wait1;score['planner_wait_ns']=wait1-wait0;rows=roll.snapshot();prev,cur,pi,ci=freshest_pair(rows,plan['history_min_gap_ns'],plan['history_max_gap_ns'],plan['history_target_gap_ns']);score['selected_indices']=[pi,ci]
  else:raise ValueError('unknown acquisition')
  write(out/'rolling-ledger.json',[{'index':i,'start_ns':r['start_ns'],'end_ns':r['end_ns'],'rgb_sha256':r['rgb_sha256']} for i,r in enumerate(rows)]);score['rolling_capture_count']=len(rows)
  acq_end=time.perf_counter_ns();score['acquisition_wall_ns']=acq_end-acq_start;score['selected_gap_ns']=cur['end_ns']-prev['end_ns'];prev['im'].save(out/'previous.png');cur['im'].save(out/'current.png');score['evidence_write_age_ns']=time.perf_counter_ns()-cur['end_ns'];score['setup_owner_records']=inp.owner.records.copy();score['setup_release_ok']=all(r.get('verified') and not r.get('keys_down') and not r.get('buttons_down') for r in inp.owner.records if r['event']=='owner_release');inp.close();inp=None
  evidence={'schema':'map01-os-history-late-bind-v6','previous_path':str((out/'previous.png').resolve()),'current_path':str((out/'current.png').resolve()),'previous_capture_started_ns':prev['start_ns'],'previous_capture_ns':prev['end_ns'],'current_capture_started_ns':cur['start_ns'],'current_capture_ns':cur['end_ns'],'previous_rgb_sha256':prev['rgb_sha256'],'current_rgb_sha256':cur['rgb_sha256'],'selected_gap_ns':cur['end_ns']-prev['end_ns'],'acquisition':case['acquisition']};write(out/'evidence.json',evidence);write(out/'context.json',ctx)
  cmd=[sys.executable,str(Path(__file__).with_name('controller.py')),'--source',str(source),'--context',str(out/'context.json'),'--evidence',str(out/'evidence.json'),'--calibration',a.calibration,'--mode','history','--out',str(out/'controller')];t0=time.perf_counter_ns();r=subprocess.run(cmd,capture_output=True,text=True,timeout=8);score['controller_rc']=r.returncode;score['controller_wall_ns']=time.perf_counter_ns()-t0;(out/'controller.stdout').write_text(r.stdout);(out/'controller.stderr').write_text(r.stderr);sync(g);score['final_x']=float(g.get_game_variable(vd.GameVariable.POSITION_X));tr=json.loads((out/'controller'/'trace.json').read_text());score['predicted_label']=tr['predicted_label'];score['use_count']=tr['use_count'];score['observation_age_ns_at_start']=tr['observation_age_ns_at_start'];score['transit_success']=score['final_x']>705;score['source_phase_match']=(score['predicted_label']==(0 if case['state']=='opening' else 1)) if score['predicted_label'] is not None else None;score['controller_release_ok']=tr['all_release_verified'];score['controller_decision']=tr['decision'];score['task_input_events']=sum(1 for x in tr.get('events',[]) if x.get('kind')=='input');score['stale_at_controller']=score['observation_age_ns_at_start']>plan['observation_max_age_ns'];score['safe_yield']=bool(score['controller_decision']=='YIELD' and score['task_input_events']==0 and not score['transit_success']);score['goal_success']=bool(score['transit_success'] and score['setup_release_ok'] and score['controller_release_ok'] and not score['stale_at_controller']);score['condition_success']=bool(score['safe_yield'] and score['setup_release_ok'] and score['controller_release_ok']) if case['acquisition']=='snapshot_bound' else score['goal_success'];score['error']=None
