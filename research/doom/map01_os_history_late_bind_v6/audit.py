...
def main():
    ...
    for c in plan['cases']:
        ...
        if not isinstance(wait_ns,int) or not isinstance(ws,int) or not isinstance(we,int) or we-ws!=wait_ns or wait_ns<400_000_000:errors.append(f"wait_receipt_{c['case']}")
    ...
    if fail:decision='FAIL_LATE_BIND'
    elif snap.get('safe_yields',0)>=6 and med is not None and med>=250_000_000:decision='PASS_EXECUTION_TIME_LATE_BIND'
    else:decision='HOLD_INSUFFICIENT_LATENCY_EXPOSURE'
    out={'schema':'agent-interface/map01-os-history-late-bind-audit-v6','status':'PASS_AUDIT' if not errors else 'FAIL_AUDIT_INTEGRITY','scientific_decision':decision,'errors':errors,'counts':counts,'paired_age_rows':pair_rows,'paired_median_snapshot_minus_late_ns':med}
