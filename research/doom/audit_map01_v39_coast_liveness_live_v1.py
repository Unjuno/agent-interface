"""Audit the first frozen v39 unauthored-coast MAP01 live allocation."""
import hashlib
import json
...
def result(plan, root):
    ...
    if not report_path.is_file():
        value = {"schema": "map01-v39-coast-liveness-live-audit-v1", "formal_pass": False,
    ...
    statuses = {}
    coast_mode_correct = True
    empty_coast_count = 0
    coast_completed_after_damage = False
    ...
    for decision in report["decisions"]:
        ...
        admission = decision["cover_validity_admission"]
        unauthored_coast = (decision.get("cover_policy_source_iteration") is None and
                            decision.get("cover_policy") == [] and
                            admission["authored"] is None)
        expected_mode = ("unauthored_coast_no_policy" if unauthored_coast
                         else "authored_policy_guard")
        if admission.get("monitor_mode") != expected_mode:
            coast_mode_correct = False
        if unauthored_coast:
            empty_coast_count += 1
            if (decision.get("policy_invalidation") is not None or
                    decision.get("planner_interrupt") is not None):
                coast_mode_correct = False
            source_value = admission["source_signal"]["value"]
            later = [row for row in typed if row["capture_ns"] >
                     decision["controller_model_started_ns"] and row["capture_ns"] <
                     decision["controller_model_ended_ns"] and
                     row.get("signals", {}).get("health", {}).get("value") is not None]
            if (decision["planner_turn_status"] == "completed" and
                    any(row["signals"]["health"]["value"] < source_value
                        for row in later)):
                coast_completed_after_damage = True
    ...
    checks["coast_monitor_mode_correct"] = coast_mode_correct
    early_release = True
    ...
    exposed = any(decision.get("running_action_guard") is not None
                  for decision in report["decisions"])
    return {"schema": "map01-v39-coast-liveness-live-audit-v1", "formal_pass": formal,
            "completed": True, "integrated_running_path_exposed": exposed,
            "dynamic_revocation_exposed": bool(released),
            "unauthored_coast_decisions": empty_coast_count,
            "coast_completed_after_damage": coast_completed_after_damage,
