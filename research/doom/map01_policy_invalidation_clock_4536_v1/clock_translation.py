"""Fail-closed host-to-runtime conversion for policy invalidation receipts."""

from copy import deepcopy


SCHEMA = "policy-invalidation-clock-translation-v1"
MAX_UNCERTAINTY_NS = 1_000_000_000
MAX_SAMPLE_AGE_NS = 5_000_000_000


def translate_invalidation(receipt, calibration):
    """Return a preserved two-domain record and a runtime-domain receipt.

    Calibration is the exact three-probe protocol used by MAP01 decision
    admission. We use the conservative lower offset, and refuse to extrapolate
    farther back than five seconds or across a wider-than-one-second interval.
    """
    if type(receipt) is not dict or set(receipt) != {
            "timestamp_domain", "outcome_evaluated_ns", "outcome"}:
        raise ValueError("exact host-domain invalidation receipt required")
    ...
    translated = deepcopy(receipt)
    translated.pop("timestamp_domain")
    translated["outcome_evaluated_ns"] = host_ns + lower
    record = {
        "schema": SCHEMA,
        "source_receipt": deepcopy(receipt),
        "runtime_receipt": deepcopy(translated),
