...
_RUNTIME_CLOCK_OFFSET_LOWER = None
...
def main():
    ...
    for index in range(args.iterations):
        ...
        _runtime_policy_invalidation=None
        if invalidation is not None:
            from clock_translation import SCHEMA as _policy_clock_schema
            from clock_translation import translate_invalidation as _translate_policy_invalidation
        ...
        def refresh_between_segments(identifier):
            ...
            process.stdin.write(json.dumps({"op":"submit","id":identifier,
              "expected_sequence":latest["sequence"],"valid_until_ns":clock_ns+15_000_000_000,
              "steps":[{"op":"observe"}]})+"\n");process.stdin.flush()
            accepted=wait(lambda r:r["event"] in ("accepted","rejected") and
                          (r.get("id")==identifier or r["event"]=="rejected"))
            if accepted["event"]!="accepted":raise RuntimeError(accepted)
