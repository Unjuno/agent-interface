...
def probe(out,seed,mode,gap_seconds):
    p=subprocess.Popen([sys.executable,str(HERE/'session_response_v2.py'),'--out',str(out),'--seed',str(seed)],
    ...
    try:
        ...
        steps=[dict(op='hold',keys=['space'],duration_ms=1000)]
...
def main():
    ...
    manifest=dict(cases=cases,scope='same-seed exploratory planner-gap rendering comparison; not qualification',
        ...
        sources={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (Path(__file__),HERE/'session_response_v2.py')})
