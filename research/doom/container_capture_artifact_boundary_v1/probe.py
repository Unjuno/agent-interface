...
root=tk.Tk(); root.geometry("900x600+20+20"); root.title("capture-artifact-boundary-probe")
...
def retained_pipeline(display):
    ...
    with tempfile.TemporaryDirectory(prefix='capture-artifact-') as td:
        ...
        with Image.open(path) as reopened:
            reopened.load(); _=reopened.getpixel((10,10))
        t_reopen=time.perf_counter_ns()
    return {
        'pipeline_total_ms':(t_reopen-t0)/1e6,
        'capture_ms':(t_capture-t0)/1e6,
        'png_write_fsync_ms':(t_write-t_capture)/1e6,
        'reopen_load_ms':(t_reopen-t_write)/1e6,
    }
...
def main():
    ...
    try:
        ...
        result={
            'schema':'capture-artifact-boundary-development-v1','pairs':args.pairs,'summary':summary,
