...
def direct_bounds(events, program_id):
    ...
    if len(accepted)!=1 or not accepted[0].get('intent_token'):
        raise AssertionError('exact accepted intent token required')
    token=accepted[0]['intent_token']
    admissions=[r for r in events if r.get('event')=='input_admission' and r.get('intent_token')==token]
    releases=[r for r in events if r.get('event')=='input_release_transition' and r.get('intent_token')==token and r.get('release_batch_identifier')==program_id]
    if not admissions:
        return {'measurement_ready': True, 'intent_token': token, 'hold_count': 0, 'retained_lower_ms': 0.0, 'retained_upper_ms': 0.0}
    verified=[r for r in releases if r.get('owner_transition_verified') is True]
    by={r.get('key'):r for r in verified}
    lower=upper=0
    for a in admissions:
        rel=by.get(a.get('key'))
        if rel is None: raise AssertionError('unmatched admission')
        lower += max(0, rel['release_call_started_ns']-a['input_ack_ns'])
        upper += max(0, rel['release_call_returned_ns']-a['admitted_ns'])
    if len(verified)!=len(admissions): raise AssertionError('release/admission count mismatch')
    return {'measurement_ready': True, 'intent_token': token, 'hold_count':len(admissions),
...
def run_arm(here,out,seed,mode,wait_ms,recovery_ms,health_loss):
    ...
    try:
        ...
        if mode=='coast': pid='wait-coast';steps=[{'op':'coast','duration_ms':wait_ms,'sample_ms':50}]
        else:
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--repo',type=Path,required=True);ap.add_argument('--out',type=Path,required=True);ap.add_argument('--seed',type=int,default=990616);ap.add_argument('--wait-ms',type=int,default=600);ap.add_argument('--recovery-ms',type=int,default=240);ap.add_argument('--health-loss',type=int,default=8);a=ap.parse_args()
    ...
    c,r=arms
    result={'schema':'map01-recovery-mechanics-development-v2','seed':a.seed,'arms':arms,
