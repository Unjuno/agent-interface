"""Test whether available-button inventory changes passive async tic progress."""
...
for index, expose_buttons in enumerate(conditions):
    game = vd.DoomGame()
    observed = phase_runner.ObservedGame(game)
    row = {
        "repetition": index // 2,
        "order_in_pair": index % 2,
        "expose_buttons": expose_buttons,
        "setup": "not_started",
        "samples": [],
    }
    try:
...
print(json.dumps([
    {
        ...
        "expose_buttons": row["expose_buttons"],
        "setup": row["setup"],
