"""Independent audit for no-button vs idle-button passive-clock probe."""
...
def audit(raw_path: Path, source_path: Path):
    ...
    for index, row in enumerate(rows):
        ...
        if row.get("new_episode_completed") is not True:
            errors.append(f"episode_start:{index}")
        if len(row.get("available_buttons_readback", [])) != (9 if condition else 0):
            errors.append(f"button_readback:{index}")
        ...
        summary.append({
            ...
            "expose_buttons": condition,
            "button_count": len(row.get("available_buttons_readback", [])),
    ...
    return {
        "schema": "map01-button-inventory-passive-clock-audit-v1",
        "decision": "HOLD_BUTTON_INVENTORY_DID_NOT_UNLOCK_PASSIVE_TIC" if not errors else "FAIL_AUDIT",
