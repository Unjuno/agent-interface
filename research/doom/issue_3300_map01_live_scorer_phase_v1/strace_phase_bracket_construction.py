"""Clock-mapped VIZ_Tic construction trace with post-scorer bracket events."""
...
def clock_anchor() -> dict[str, int]:
    ...
    return {"wall_before_ns": wall_before, "monotonic_ns": monotonic,
            "wall_after_ns": wall_after}


def marker(kind: str, repetition: int) -> dict:
    payload = {"kind": kind, "repetition": repetition, **clock_anchor()}
    print("PHASE_EVENT " + json.dumps(payload, sort_keys=True), flush=True)
    return payload
...
for repetition in range(3):
    ...
    try:
        ...
        begin = marker("passive_begin", repetition)
        row["anchors"].append(begin)
        ...
        while time.monotonic_ns() < deadline:
            ...
            time.sleep(0.05)
        row["passive_start_monotonic_ns"] = start
        ...
        row["elapsed_ns"] = row["passive_end_monotonic_ns"] - start

        # Observe upcoming engine tic records after the scorer so its getter
        # calls can be bracketed by prior/following logged tic markers.
        row["post_scorer_begin"] = marker("post_scorer_begin", repetition)
        post_start = time.monotonic_ns()
        post_end = post_start + 150_000_000
        while time.monotonic_ns() < post_end:
            row["anchors"].append(clock_anchor())
            time.sleep(0.01)
        row["post_scorer_end_monotonic_ns"] = time.monotonic_ns()
        row["post_scorer_end"] = marker("post_scorer_end", repetition)
        row["anchors"].append(row["post_scorer_end"])
    except BaseException as exc:
...
print("PHASE_SUMMARY " + json.dumps([
    {"repetition": r["repetition"], "setup": r["setup"],
     ...
     "post_window": r.get("post_scorer_end_monotonic_ns", 0) > 0}
    for r in rows
