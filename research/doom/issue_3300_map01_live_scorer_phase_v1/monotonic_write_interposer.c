...
ssize_t writev(int fd, const struct iovec *iov, int iovcnt) {
    if (fd != STDOUT_FILENO || iovcnt < 0) {
        return (ssize_t)syscall(SYS_writev, fd, iov, iovcnt);
    }
    size_t total = 0;
    for (int i = 0; i < iovcnt; ++i) {
        if (iov[i].iov_len > 131000 - total) {
            return (ssize_t)syscall(SYS_writev, fd, iov, iovcnt);
        }
        total += iov[i].iov_len;
    }
    ...
    int prefix_len = snprintf(combined, 64, "CLOCK_MONOTONIC_NS=%llu ",
                              (unsigned long long)mono_ns);
    ...
    size_t at = (size_t)prefix_len;
    for (int i = 0; i < iovcnt; ++i) {
        memcpy(combined + at, iov[i].iov_base, iov[i].iov_len);
        at += iov[i].iov_len;
    }
    ssize_t written = (ssize_t)syscall(SYS_write, fd, combined, at);
    pthread_mutex_unlock(&guard);
    if (written < prefix_len) return written < 0 ? written : 0;
    return written - prefix_len;
}
