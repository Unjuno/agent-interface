...
Run53 compared the passive boundary with the existing one-tic action boundary
at the requested ~35 Hz cadence. Across 20 samples per condition, passive
public tic stayed at 1367 while the action-driven condition advanced by 21
ticks over 600.764 ms. This is construction-only evidence that an explicit
action boundary exposes progress; it is not passive async progress, an
independent clock witness, or formal phase data. Formal remains 0/120 and
`HOLD_LIVE_SPAN_UNIDENTIFIED`. See `results/construction-clock-53/`.

Run54 preflighted Linux `perf_event_open` in the pinned container. Default
access stopped with EPERM; adding only `CAP_PERFMON` opened a self software
clock event, but the exact runtime had no usable `VIZ_Tic` symbol. No engine
breakpoint or scorer row was produced. Disposition remains
`HOLD_TARGET_ADDRESS_UNRESOLVED`; this does not establish hardware-breakpoint
support or impossibility. See `results/construction-clock-54/`.

Run55 built an unmodified ViZDoom source binary with a linker map, but its
`.text` differed from the pinned wheel executable and none of the tested
prefix lengths at the mapped `VIZ_Tic` address matched. The address cannot be
transferred to the runtime; do not attach a breakpoint there. No formal row
was produced. Disposition: `HOLD_BINARY_CODE_IDENTITY_NOT_ESTABLISHED`. See
`results/construction-clock-55/`.

Run56 then used that exact symbol-bearing binary as the runtime in an excluded
construction probe with a kernel hardware execute breakpoint. Independent
audit reports 23 matching IP/PID/TID samples, strictly increasing kernel
timestamps, and a 29.672 ms recorded-event bracket around one scorer call.
This is a `PASS_BREAKPOINT_TIMESTAMP_CONSTRUCTION_ONLY`, not proof that the
event timestamp equals instruction-entry time, not a bound on perturbation,
and not a narrow phase estimate. Formal remains 0/120 and
`HOLD_LIVE_SPAN_UNIDENTIFIED`. See `results/construction-clock-56/`.

## Recovery boundary

This recovered snapshot ends at run56. The later remote source-branch head
`4f77a79b87c49e5f408207f28069fe9beb1c0a88` contains runs57, 59, and 60, but 21
files under those result directories contain literal `@work/...` pointers
instead of the referenced payloads. Those incomplete records were excluded
from this integration; see `RECOVERY_NOTE_2026-09-21.md`. The original PR
branch is retained until those missing payloads are resolved or explicitly
closed as unrecoverable.
