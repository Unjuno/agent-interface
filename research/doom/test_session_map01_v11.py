...
import session_map01_v11 as session
...
class Map01FixtureContractTests(unittest.TestCase):
    ...
    def test_chained_fixture_metadata_is_allowed_but_does_not_replace_hash_checks(self):
        with tempfile.TemporaryDirectory() as directory:
            manifest, save = self.fixture(Path(directory))
            record = json.loads(manifest.read_text())
            record["parent_fixture"] = {
                "manifest_sha256": "parent-manifest",
                "save_sha256": "parent-save",
                "source_episode_tic": 12,
                "loaded_episode_tic": 12,
            }
            manifest.write_text(json.dumps(record))
            loaded, loaded_save = session.validate_fixture_manifest(
                manifest, "1.3.0", "iwad", 1)
            self.assertEqual(loaded_save, save.resolve())
            self.assertEqual(loaded["parent_fixture"]["source_episode_tic"], 12)
