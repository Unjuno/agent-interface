...
from map01_stagnation_v1 import descriptor, find_revisit, recovery_commands
...
def model_call(root, image, session_id):
    ...
    command = [WINDOWS_PYTHON, win(HERE / "map01_persistent_model_runner_v1.py"),
               ...
               win(HERE / "map01_motor_responder_v1.txt"),
...
def main():
    ...
    visual_history=[];visual_cluster_ids=[];recovery_attempts={}
    for index in range(args.iterations):
        ...
        stagnation_revisit=find_revisit(current_descriptor,visual_history)
        if stagnation_revisit:
            cluster_id=visual_cluster_ids[stagnation_revisit["prior_iteration"]]
            recovery_attempt=recovery_attempts.get(cluster_id,0)
            recovery_attempts[cluster_id]=recovery_attempt+1
        else:
            cluster_id=index
            recovery_attempt=None
        visual_history.append(current_descriptor)
        visual_cluster_ids.append(cluster_id)
        ...
        action,usage,model_ns,observed_session_id=model_call(model_root,image,model_session_id)
        ...
        original_commands=action["commands"]
        selected_commands=(recovery_commands(recovery_attempt)
                           if stagnation_revisit else original_commands)
        steps=compile_commands(selected_commands)+[{"op":"observe"}]
        ...
        decisions.append({"iteration":index,"source_image":str(source_image),"model_image":str(image),
          ...
          "stagnation_revisit":stagnation_revisit,
          "compiled_override":bool(stagnation_revisit),
          "recovery_cluster_id":cluster_id if stagnation_revisit else None,
          "recovery_attempt":recovery_attempt,
          "original_commands":original_commands,
          "compiled_commands":selected_commands,
    ...
    report={"claim":"pre-formal overlapping inference feasibility", "model":"gpt-5.6-luna",
      ...
      "stagnation_contract":"visual revisit at non-adjacent lag 2..8 and normalized grayscale MAE <=0.065 triggers bounded escalating recovery",
      "stagnation_overrides":sum(bool(x.get("compiled_override")) for x in decisions),
