...
SCHEMA='agent-interface/map01-door-action-effect-history-v1'
TRAIN_CASES=12
...
SEEDS=list(range(994100,994120))
    ...
    try:
        ...
        for t in range(1,175):
            ...
            if t in OPEN_TICS or t in CLOSE_TICS:
                ...
                name=f'{t:03d}-{"opening" if label==0 else "closing"}.png';Image.fromarray(cur).save(case_dir/name)
                samples.append({'case':case_index,'seed':seed,'yaw':yaw,'wait':wait,'tic':t,'label':label,
                    'current':cd,'delta':delta,'rgb_sha256':sha_bytes(cur.tobytes()),'png':str(pathlib.Path(case_dir.name)/name)})
            prev=cur
    finally:g.close()
    return samples
...
def main():
    ...
    summary={'schema':SCHEMA,'decision':decision,'development_seeds_excluded':[994001,*range(994010,994015)],'formal_seeds':SEEDS,'train_cases':TRAIN_CASES,
