"""One fresh real X11 case. Refuse reused output IDs and mismatched source pins."""
...
def run_case(runtime,out,spec):
    ...
    env=os.environ.copy();env.update(AI_RUNTIME_ROOT=str(runtime),LIFECYCLE_GUARD=str(int(spec['guard'])),XAUTHORITY=str(auth))
    ...
    p=subprocess.Popen(cmd,env=env,cwd=runtime/'research/doom',stdin=subprocess.PIPE,stdout=subprocess.PIPE,
    ...
    def send(row):
        row=dict(row);row['driver_sent_ns']=time.perf_counter_ns();sent.append(row)
        p.stdin.write(json.dumps(row)+'\n');p.stdin.flush()
    try:
        wait(lambda r:r.get('event')=='ready');initial=wait(lambda r:r.get('event')=='observation' and r.get('id')=='initial')
        send({'op':'clock'});clock=wait(lambda r:r.get('event')=='clock')
        deadline=clock['runtime_ns']+spec['cutoff_ms']*1_000_000
        steps=[{'op':'hold','keys':['a','d'],'duration_ms':5000}]
        send({'op':'submit','id':'primary','steps':steps,'expected_sequence':latest['sequence'],'valid_until_ns':deadline})
        terminal=wait(lambda r:r.get('event')=='terminal' and r.get('id')=='primary',12)
        # Give both arms the same opportunity time for a fresh post-primary action.
        ...
        dump(out/'failure.json',dict(error=repr(e),traceback=traceback.format_exc(),sent=sent));raise
    finally:
        if p.poll() is None:
...
if __name__=='__main__':
    ...
    ap.add_argument('--plan',type=Path,required=True);ap.add_argument('--index',type=int,required=True);a=ap.parse_args()
