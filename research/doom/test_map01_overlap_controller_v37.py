...
class Map01OverlapControllerV36Tests(unittest.TestCase):
    ...
    def test_monitor_uses_preartifact_typed_epoch_without_reading_png(self):
        class Reader:
            def read(self, row):
                raise AssertionError("typed runtime path must not reopen PNG")
        candidate = action()
        ready = controller.prepare_action_admission(
            initial_receipt(), candidate, candidate["action_validity"][0],
            signal("health", 85, 1, 100), signal("ammo", 47, 1, 100),
        ...
        guard.admit_program(
            {"role": "primary", "semantic_commands": candidate["commands"],
             "command_indices": [0], "contingency_after": None,
             "compiled_steps": steps},
            {"command": {"op": "submit", "id": "typed", "expected_sequence": 2,
                         "valid_until_ns": 1000, "steps": steps}, "sent_ns": 202},
            {"event": "accepted", "id": "typed", "steps": 1,
             "program_sha256": program_sha256(steps), "accepted_ns": 203})
        monitor = controller.DoomRunningActionMonitor(guard, Reader(), Reader())
        typed = {"event": "typed_observation",
                 "schema": "doom-typed-observation-v1", "id": "typed", "step": 0,
                 "sequence": 3, "capture_ns": 300, "pointer_binding": BINDING,
                 "signals": {"health": signal("health", 84, 3, 300),
                             "ammo": signal("ammo", 46, 3, 300)},
                 "frame_rgb_sha256": "0" * 64, "frame_size": [1280, 800],
                 "typed_extraction_started_ns": 300, "typed_ready_ns": 301,
                 "capture_to_typed_ready_ms": 0.000001,
                 "artifact_published": False, "grants_input_authority": False}
        self.assertIsNone(monitor.observe(typed, 302))
        self.assertTrue(monitor.last_receipt["current_input_authority"])
        self.assertEqual(monitor.event_types, {"typed_observation"})

    def test_v37_uses_typed_session_v11(self):
        args = SimpleNamespace(seed=1,
            load_fixture_manifest=Path("fixture.json"))
        command = controller.session_command(args, Path("runtime"))
        self.assertIn("session_map01_v11.py", command[1])

    def test_invalid_typed_decision_persists_before_terminal_handling(self):
        class Reader:
            def read(self, row):
                raise AssertionError("typed path must not read artifact")
        candidate = action()
        ready = controller.prepare_action_admission(
            initial_receipt(), candidate, candidate["action_validity"][0],
            signal("health", 85, 1, 100), signal("ammo", 47, 1, 100),
        ...
        guard.admit_program(
            {"role": "primary", "semantic_commands": candidate["commands"],
             "command_indices": [0], "contingency_after": None,
             "compiled_steps": steps},
            {"command": {"op": "submit", "id": "invalid", "expected_sequence": 2,
                         "valid_until_ns": 1000, "steps": steps}, "sent_ns": 202},
            {"event": "accepted", "id": "invalid", "steps": 1,
             "program_sha256": program_sha256(steps), "accepted_ns": 203})
        monitor = controller.DoomRunningActionMonitor(guard, Reader(), Reader())
        typed = {"event": "typed_observation",
                 "schema": "doom-typed-observation-v1", "id": "invalid", "step": 0,
                 "sequence": 3, "capture_ns": 300, "pointer_binding": BINDING,
                 "signals": {"health": signal("health", 70, 3, 300),
                             "ammo": signal("ammo", 46, 3, 300)},
                 "frame_rgb_sha256": "0" * 64, "frame_size": [1280, 800],
                 "typed_extraction_started_ns": 300, "typed_ready_ns": 301,
                 "capture_to_typed_ready_ms": 0.000001,
                 "artifact_published": False, "grants_input_authority": False,
                 "emit_ns": 301}
        invalidation = monitor.observe(typed, 302)
        with tempfile.TemporaryDirectory() as directory:
            path = controller.persist_running_invalidation(
                directory, "invalid", invalidation)
            retained = json.loads(path.read_text(encoding="utf-8"))
        self.assertEqual(retained["source_event"]["capture_ns"], 300)
        self.assertEqual(retained["running_action_guard"]["state"],
                         "CANCEL_REQUIRED")
