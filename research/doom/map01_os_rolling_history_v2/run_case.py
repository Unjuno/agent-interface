...
class Rolling:
    def __init__(self,s,ctx,period_s=.035,maxlen=64):
        self.s=s;self.ctx=ctx;self.period_s=period_s;self.rows=deque(maxlen=maxlen);self.stop_evt=threading.Event();self.lock=threading.Lock();self.error=None
...
def choose_pair(rows,desired_gap_ns):
    if len(rows)<2:raise RuntimeError('rolling_history_insufficient')
    cur=rows[-1];prev=min(rows[:-1],key=lambda r:abs((cur['end_ns']-r['end_ns'])-desired_gap_ns));return prev,cur
...
def main():
    ...
    try:
        ...
        sync(g);score['door_x']=float(g.get_game_variable(vd.GameVariable.POSITION_X));inp.press('e',.03,0,'setup_use');origin=sync(g)
        roll=Rolling(s,ctx,period_s=plan['rolling_period_s']);roll.start();time.sleep(.08)
        target=7 if case['state']=='opening' else 161;score['current_rel']=wait(g,origin,target);time.sleep(plan['rolling_period_s']*1.15)
        rows=roll.snapshot();
        if roll.error:raise RuntimeError('rolling_capture:'+roll.error)
        desired=int((2/35 if case['state']=='opening' else 3/35)*1e9);prev,cur=choose_pair(rows,desired)
        prev['im'].save(out/'previous.png');cur['im'].save(out/'current.png')
        score['rolling_capture_count']=len(rows);score['rolling_selected_gap_ns']=cur['end_ns']-prev['end_ns'];score['rolling_latest_age_ns_at_evidence_write']=time.perf_counter_ns()-cur['end_ns'];score['rolling_period_s']=plan['rolling_period_s']
