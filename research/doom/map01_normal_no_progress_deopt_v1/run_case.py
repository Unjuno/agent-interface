...
def run(out,arm,seed,source=SOURCE_DEFAULT):
    ...
    try:
      ...
      score={'schema':'agent-interface/map01-no-progress-deopt-case-v1','arm':arm,'seed':seed,'decisions':len(rows),'deopt_escalations':escalations,'map_exit':bool(g.is_episode_finished() and not g.is_player_dead()),'player_dead':bool(g.is_player_dead()),'wall_seconds':time.perf_counter()-start,'release_ok':release_ok,'nomonsters':False,'kill_count':float(g.get_game_variable(vd.GameVariable.KILLCOUNT)),'final_health':float(g.get_game_variable(vd.GameVariable.HEALTH)),'final_ammo':float(g.get_game_variable(vd.GameVariable.AMMO2))}
