...
Before freeze, run the construction-only classifier controls into a separate fresh `results/construction-01/` mount using the same network/read-only/resource restrictions and the command below. It writes `CONSTRUCTION.json`; this is not the formal allocation. Supply all six `EXPECTED_*` source hashes from the working tree. Preserve any STOP.
