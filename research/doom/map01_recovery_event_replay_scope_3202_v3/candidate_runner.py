...
class JsonSession:
    ...
    def wait(self, predicate, timeout: float = 20.0, *, replay: bool = False) -> dict:
        ...
        if replay:
            matches = [row for row in self.events if predicate(row)]
            if len(matches) > 1:
                raise SessionError("ambiguous retained replay")
            if len(matches) == 1:
                return matches[0]
    ...
    try:
        ...
        if fallback_terminal is None:
            ...
            try:
                ...
                pass
            fallback_terminal = session.wait(lambda r: r.get("event") == "terminal" and r.get("id") == fallback_id, timeout=5, replay=True)
