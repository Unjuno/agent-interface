"""Run one fresh lifecycle-freshness MAP01 case."""
...
def run_case(runtime,out,spec):
    ...
    env=os.environ.copy();env.update(AI_RUNTIME_ROOT=str(runtime),XAUTHORITY=str(auth),
        LIFECYCLE_FRESHNESS_MS=str(spec['freshness_ms']),LIFECYCLE_FAULT=spec['fault'],
        LIFECYCLE_DELAY_MS=str(spec.get('delay_ms',0)))
