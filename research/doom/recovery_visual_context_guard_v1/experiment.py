...
def visual_context_verdict(mae):
    return 'ADMIT' if mae <= VISIBLE_CHANGE_THRESHOLD else 'REJECT_CONTEXT_CHANGED'
...
def run_case(case, out, runtime_root, python):
    ...
    result={**case,'schema':'recovery-visual-context-guard-case-v1','source_sequence':source_t['sequence'],'post_sequence':post_t['sequence'],
      ...
      'source_image':source_o['image'],'post_image':post_o['image'],'viewport_normalized_mae':frame_mae,
      'visual_context_threshold':VISIBLE_CHANGE_THRESHOLD,'visual_context_verdict':guard,
      'baseline_validity_status':validity['status'],'baseline_validity_reason':validity['reason'],'baseline_validity_checks':validity['checks'],'snapshot_age_ms':(decided-post_t['capture_ns'])/1e6,
...
def main():
    ...
    summary={'schema':'recovery-visual-context-guard-summary-v1','cases':results,'pairs':pairs}
