...
def main():
    ...
    for index in range(args.iterations):
        ...
        side="a" if index % 2 == 0 else "d"
        cover_cmd={"op":"submit","id":cover,"expected_sequence":latest["sequence"],
          "valid_until_ns":clock_ns+25_000_000_000,
          "steps":[{"op":"hold","keys":[side,"space"],"duration_ms":4500},
                   {"op":"hold","keys":["Left","space"],"duration_ms":4500}]}
        ...
        model_root.mkdir()
        image=Path(latest["image"])
        action,usage,model_ns=model_call(model_root,image)
        ...
        decisions.append({"iteration":index,"image":str(image),"image_sha256":hashlib.sha256(image.read_bytes()).hexdigest(),
          "action":action,"usage":usage,"model_ns":model_ns,"cover_terminal_before_plan":True,
