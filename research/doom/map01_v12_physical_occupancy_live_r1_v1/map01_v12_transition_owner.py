"""Measurement-only v12 physical-edge owner under the current v3 transition ABI."""
import time
from input_owner_v12 import InputOwner as Previous
...
class InputOwner:
    ...
    def _lease_state_at(lease, now_ns):
        ...
        return {
            ...
            'ordinary_release_candidate': time_valid and cancel_requested is False and not focus_invalid,
        }
    ...
    def call(self, operation, lease=None, key=None):
        ...
        measurement = None
        if operation == 'up':
            if not isinstance(result, dict) or result.get('event') != 'input_release_measurement':
                raise AssertionError('InputOwner v12 key release did not return physical measurement')
            measurement = result.get('physical_key_measurement')
            if not isinstance(measurement, dict):
                raise AssertionError('InputOwner v12 key release missing physical measurement')
        elif result is not None:
            raise AssertionError('InputOwner v12 button release unexpectedly returned payload')
        ...
        row = {
            ...
            **lease_state,
            'measurement_contract': (
                'caller brackets exact retained InputOwner v12 explicit release; nested physical '
                'measurement is copied unchanged and remains separately typed from v3 verification'
            ),
        }
        if measurement is not None:
            row['physical_key_measurement'] = measurement
        return row
