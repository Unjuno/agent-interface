...
ALLOCATION_ID = "map01-terminal-sync-diagnostic-3211-07"
EXPECTED_OWNER_SCHEMA = "formal-allocation-global-owner-v1"
EXPECTED_WORKFLOW_PATH = ".github/workflows/map01-terminal-sync-diagnostic-3211-v7.yml"
...
class JsonSession:
    def __init__(self, command: list[str]):
        ...
        out_index = command.index("--out") if "--out" in command else -1
        self.trace_path = Path(command[out_index + 1]).resolve().parent / "session-events.jsonl" if out_index >= 0 else None
    ...
    def wait_retained(self, predicate, timeout: float = 20.0) -> dict:
...
def _run_arm(root: Path, pair_index: int, arm: str, fixture: Path) -> dict:
    ...
    try:
        ...
        lease_cancel_event = threading.Event()
        lease_timer = None
        ...
        if arm == "COAST_CONTROL":
            ...
            lease_delay = max(0.0, (valid_until - session.runtime_clock()) / 1_000_000_000 - 0.05)
            lease_timer = threading.Timer(lease_delay, lease_cancel_event.set)
            lease_timer.start()
        else:
            raise ValueError(f"unknown arm: {arm}")

        while not timer_done.is_set():
            if arm == "BOUNDED_RECOVERY" and lease_cancel_event.is_set() and guard_cancel_reason is None:
                guard_cancel_reason = "lease_expiry_guard"
        ...
        if fallback_terminal is None:
            ...
            try:
                ...
                pass
            fallback_terminal = session.wait_retained(lambda r: r.get("event") == "terminal" and r.get("id") == fallback_id, timeout=5)
