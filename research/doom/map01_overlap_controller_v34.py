"""MAP01 controller with immediate and running typed action validation."""
import argparse
import atexit
...
class DoomRunningActionMonitor:
    ...
    def __init__(self, guard, health_reader, ammo_reader):
        self.guard = guard
        required = set(guard.contract["source"]["signals"])
    ...
    def observe(self, observation, decided_ns=None):
        snapshot = build_action_snapshot(
            observation, self.guard.contract, self.readers)
...
def cancel_invalidated_action(process, wait, identifier, guard):
    """Request cancellation and require exact matched release evidence."""
...
def main():
    ...
    for index in range(args.iterations):
        ...
        trace=[]
        running_guard=RunningActionGuard(
            action["commands"],final_action_admission["action_validity"]["contract"],
            final_action_admission["action_validity"])
        ...
        def execute_segment(identifier,commands,role,command_indices):
            ...
            running_guard.admit_program(guard_acceptance)
            if first_accepted is None:
                first_accepted=accepted["accepted_ns"]
                final_action_admission=bind_first_plan_acceptance(
                    final_action_admission,guard_acceptance)
        ...
        def refresh_between_segments(identifier):
            ...
            if (terminal.get("status")!="completed" or release.get("verified") is not True or
                    ...
                    running_guard.state!=RUNNING_READY):
        ...
        for segment_end in boundaries:
            ...
            if segment_start<len(action["commands"]) and not refresh_between_segments(
                ...
                running_invalidation=running_guard.receipt()["invalidation"]
                break
        if running_guard.state==RUNNING_BETWEEN:
    ...
    report={"claim":"persistent typed planner plus immediate and running action invalidation from a fixed real-MAP01 threat state", "model":args.model,
      ...
      "final_action_admission_contract":"planner eligibility, controller policy/validation decision and fresh Executor acceptance are distinct typed states; hard evidence observed before admission wins under either terminal ordering",
      "running_action_contract":"reuse exact Executor hold observations to re-evaluate the same action contract during active input; invalidation requires matched cancel and verified empty release; inter-segment input requires a fresh passive observation",
      ...
      "policy_invalidation_contract":"effective health floor is max(critical_health_minimum, fresh source health - schema-bounded maximum_health_loss); soft change may only preserve admitted cover; hard, unknown, expired or binding-mismatched evidence cancels cover and discards the dependent model action; it never grants input authority or proves success",
      ...
      "final_action_admission_statuses":dict(Counter(
