...
def compile_cover(commands):
    ...
    cycle=[]
    for command in commands:
        if command["action"]=="coast":
            cycle.append({"op":"coast","duration_ms":coast_ms[command["extent"]],"sample_ms":250})
        else:
            cycle.extend(compile_commands([command]))
            cycle.append({"op":"coast","duration_ms":1000,"sample_ms":250})
    steps=[];duration=0
    while cycle:
        added=False
        for step in cycle:
            step_ms=step["duration_ms"]
            if len(steps)>=16 or duration+step_ms>10000:
                break
            steps.append(dict(step));duration+=step_ms;added=True
        if not added or len(steps)>=16 or duration>=10000:
            break
    while duration<10000 and len(steps)<16:
        tail=min(5000,10000-duration)
        steps.append({"op":"coast","duration_ms":tail,"sample_ms":250});duration+=tail
    if not steps:
        raise ValueError("cover policy compiled to no steps")
    return steps
