...
def model_call(root, image, session_id, effect_memory):
    ...
    command = [WINDOWS_PYTHON, win(HERE / "map01_persistent_model_runner_v1.py"),
               ...
               win(HERE / "map01_motor_responder_v5.txt"),
               win(HERE / "map01_contingency_schema_v1.json"), session_id or "-"]
    ...
    if action["state"] != "active" and (action["commands"] or action["contingencies"]):
        raise ValueError("terminal state requires empty commands and contingencies")
...
def main():
    ...
    for index in range(args.iterations):
        ...
        action,usage,model_ns,observed_session_id=model_call(
            model_root,image,model_session_id,effect_memory)
        ...
        trace=[]
        def execute_one(identifier,command,role,command_index):
            ...
            process.stdin.write(json.dumps({"op":"submit","id":identifier,
              ...
              "steps":compile_commands([command])})+"\n");process.stdin.flush()
            ...
            receipt=effect_receipts([command],before,observations,accepted["accepted_ns"])[0]
            record={"id":identifier,"role":role,"command_index":command_index,
              "command":command,"accepted_ns":accepted["accepted_ns"],
              "terminal_ns":terminal["terminal_ns"],"receipt":receipt,
              "observation_samples":len(observations),
              "observation_capture_ms":sum(row["capture_ms"] for row in observations)}
            trace.append(record);return record
        contingencies={row["after_command"]:row for row in action["contingencies"]}
        branch=None;first_accepted=None
        for command_index,command in enumerate(action["commands"]):
            record=execute_one(f"plan-{index}-primary-{command_index}",command,
                               "primary",command_index)
            if first_accepted is None:first_accepted=record["accepted_ns"]
            contingency=contingencies.get(command_index)
            if record["receipt"]["result"]=="no_visible_effect" and contingency:
                branch={"after_command":command_index,"condition":"no_visible_effect",
                  "skipped_primary_commands":len(action["commands"])-command_index-1,
                  "fallback_commands":contingency["commands"],"latency_ms":None}
                for fallback_index,fallback in enumerate(contingency["commands"]):
                    fallback_record=execute_one(
                        f"plan-{index}-fallback-{command_index}-{fallback_index}",fallback,
                        "fallback",fallback_index)
                    if fallback_index==0:
                        branch["latency_ms"]=(fallback_record["accepted_ns"]-
                          record["receipt"]["effect_observed_ns"])/1e6
                break
