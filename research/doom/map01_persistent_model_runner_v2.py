"""Audited initial/resume image call with an explicit model and effort."""
import hashlib,json,subprocess,sys,time
from pathlib import Path

def sha(path): return hashlib.sha256(Path(path).read_bytes()).hexdigest()
def main():
    node,cli,prompt_file,working,output,image,instructions,schema,session_id,model,effort=sys.argv[1:]
    root=Path(output).resolve();root.mkdir(parents=True,exist_ok=False)
    prompt=Path(prompt_file).read_text();(root/"prompt.txt").write_text(prompt)
    common=["--ignore-user-config","--ignore-rules","--json","--model",model,
    ...
    plan={"mode":mode,"session_id_requested":None if session_id=="-" else session_id,
      "args":args,"requested_model":model,"requested_effort":effort,
