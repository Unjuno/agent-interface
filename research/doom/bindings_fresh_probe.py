...
def probe(out,seed,mode,gap_seconds,key):
    p=subprocess.Popen([sys.executable,str(HERE/'session_bindings_v7.py'),'--out',str(out),'--seed',str(seed)],
    ...
    try:
        ...
        steps=[dict(op='hold',keys=[key],duration_ms=250)]
        ...
        return dict(mode=mode,seed=seed,gap_seconds=gap_seconds,key=key,readiness_pass=True,observed_tics_per_second=rate,
...
def main():
    ...
    cases=[dict(mode='ordinary',seed=990401+i,gap_seconds=0,key=key) for i,key in enumerate(('Left','Right','Up','Down'))]
    manifest=dict(cases=cases,scope='same-seed exploratory planner-gap rendering comparison; not qualification',
        ...
        sources={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in (Path(__file__),HERE/'session_bindings_v7.py')})
    (a.out/'manifest.json').write_text(json.dumps(manifest,indent=2))
    rows=[]
    for case in cases:
        try:r=probe(a.out/case['key'],**case)
