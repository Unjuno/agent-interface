...
def main():
    ...
    paths=[HERE/'session.py']+[HERE.parent/p for p in ('live_control/session_v2.py','live_control/executor_v2.py',
    ...
    try:
        with (a.out/'setup.txt').open('w') as diagnostics,contextlib.redirect_stdout(diagnostics):
            ...
            g.add_game_args('+bind left +left +bind right +right +bind up +forward +bind down +back +bind space +attack')
            g.init()
        s.wait_window('ViZDoom');s.focus('ViZDoom')
        ...
        emit(dict(event='clock_probe',before_tic=before,after_tic=after,wall_seconds=elapsed,
                  no_advance_calls_during_wait=True))
        ...
        if s is not None:s.close();shutil.rmtree(s.tmp)
