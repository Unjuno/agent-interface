...
import map01_overlap_controller_v32 as controller
...
class Map01TypedCoverValidityV30Tests(unittest.TestCase):
    ...
    def test_v32_maps_planner_and_policy_to_typed_final_admission(self):
        result = SimpleNamespace(
            handle=SimpleNamespace(turn_id="turn-9"),
            status="completed", answer_eligible=True)
        invalidation = {
            "outcome_evaluated_ns": 20,
            "outcome": {"status": "HARD_INVALIDATED",
                        "reason": "below_hard_minimum",
                        "requires_new_decision": True,
                        "grants_input_authority": False}}
        rejected = controller.final_admission_from_planner_result(
            result, 10, invalidation, 21)
        self.assertEqual(rejected["status"], "REJECTED_POLICY_INVALIDATED")
        ready = controller.final_admission_from_planner_result(result, 10, None, 11)
        self.assertEqual(ready["status"], "READY_FOR_FRESH_EXECUTOR_ADMISSION")
        admitted = controller.bind_first_plan_acceptance(
            ready, {"id": "plan-9", "accepted_ns": 12, "other": "retained"})
        self.assertEqual(admitted["status"], "INPUT_ADMITTED")
        self.assertEqual(admitted["executor_admission"]["id"], "plan-9")
