"""MAP01 controller with typed final action-admission receipts."""
import argparse
import atexit
...
def begin_model_turn(planner, root, image, effect_memory, source_health,
...
def main():
    ...
    planner = PersistentPlannerAdapter(
        ...
        base_instructions=(HERE / "map01_motor_responder_v9.txt").read_text())
    planner.start_session()
    output_schema = json.loads((HERE / "map01_cover_policy_schema_v5.json").read_text())
    ...
    for index in range(args.iterations):
        ...
        planner_handle=begin_model_turn(
            ...
            validity_admission["source_signal"]["value"],
            prior_soft_event_summary,output_schema)
        ...
        for segment_end in boundaries:
            ...
            if first_accepted is None:
                first_accepted=records[0]["accepted_ns"]
                final_action_admission=bind_first_plan_acceptance(
                    final_action_admission,records[0])
