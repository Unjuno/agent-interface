"""MAP01 controller with exact programs and pre-artifact typed feedback."""
import argparse
import atexit
...
def session_command(args, runtime):
    return [sys.executable, str(HERE / "session_map01_v10.py"),
...
def main():
    ...
    for index in range(args.iterations):
        ...
        trace=[]
        running_guard=RunningActionGuardV2(
            action,final_action_admission,compile_commands,
            "map01_overlap_controller_v36.compile_commands")
    ...
    report={"claim":"persistent typed planner plus immediate and running action invalidation from a fixed real-MAP01 threat state", "model":args.model,
      ...
      "running_action_contract":"the root running-action-v2 receipt is authoritative after guard creation; every primary/fallback semantic slice is deterministically recompiled and bound to the exact submitted steps and Executor program SHA-256 attestation; fallback additionally binds the latest completed primary terminal and full no-visible-effect receipt; pre-artifact typed observations re-evaluate current validity, invalidation requires matched cancel and verified empty release, and inter-segment input requires a fresh passive observation",
