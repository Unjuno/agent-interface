"""One frozen live probe for two-phase physical release and terminal closure."""
import argparse
import atexit
import hashlib
import json
...
from map01_overlap_controller_v38 import (
...
def verify(plan):
    ...
    checks["latency_thresholds"] = (
        ...
        plan["thresholds_ms"]["capture_to_physical_release_lte"] == 90 and
        plan["thresholds_ms"]["capture_to_terminal_closure_lte"] == 200)
    return checks
...
def main():
    ...
    (cancel_event, physical_release_event, release_pending_guard,
    ...
    metrics = {
        ...
        "capture_to_physical_release_ms":
            (physical_release_event["owner_release"]["verified_ns"] - capture_ns) / 1e6,
        "guard_decision_to_physical_release_ms":
            (physical_release_event["owner_release"]["verified_ns"] - decided_ns) / 1e6,
        "physical_release_to_publication_ms":
            (physical_release_event["published_ns"] -
             physical_release_event["owner_release"]["verified_ns"]) / 1e6,
        "capture_to_terminal_closure_ms":
            (terminal["terminal_ns"] - capture_ns) / 1e6,
        "physical_release_to_terminal_closure_ms":
    ...
    checks = {
        ...
        "lease_token_bound_release":
            physical_release_event["intent_token"] == accepted["intent_token"],
        "physical_release_before_artifact":
            physical_release_event["owner_release"]["verified_ns"] <
            invalidating_full["artifact_ready_ns"],
        "physical_release_published_before_terminal":
            physical_release_event["published_ns"] < terminal["terminal_ns"],
        "release_pending_state_exposed":
            release_pending_guard["state"] ==
            "REVOKED_INPUT_RELEASED_AWAITING_TERMINAL" and
            release_pending_guard["physical_release_verified"] is True and
            release_pending_guard["program_terminal_pending"] is True and
            release_pending_guard["current_input_authority"] is False,
        "cancelled_terminal": terminal["status"] == "cancelled",
        "terminal_closes_pending_state":
            guard_receipt["physical_release_verified"] is True and
            guard_receipt["program_terminal_pending"] is False,
        ...
        "capture_to_physical_release_within_threshold":
            metrics["capture_to_physical_release_ms"] <=
            plan["thresholds_ms"]["capture_to_physical_release_lte"],
        "capture_to_terminal_closure_within_threshold":
            metrics["capture_to_terminal_closure_ms"] <=
    ...
    report = {"allocation_id": plan["allocation_id"], "passed": all(checks.values()),
              ...
              "release_pending_guard": release_pending_guard,
