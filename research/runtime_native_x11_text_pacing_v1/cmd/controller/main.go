...
func main() {
	...
	flag.Parse()
	xid, err := tp.ParseXID(*xidText); if err != nil { panic(err) }
	b, err := tp.Open(*display, time.Duration(*pacingMS*float64(time.Millisecond))); if err != nil { panic(err) }
	defer b.Close(); b.RegisterTarget("calc", xid)
	now := b.NowNS(); expires := now + 60_000_000_000
	...
	for _, s := range corpus { ops = append(ops, core.Op{Op: "text", Text: s}, core.Op{Op: "key_chord", Keys: []string{"ENTER"}}) }
	...
	t0 := time.Now(); tr := b.Execute(task, 5, 1); editNS := time.Since(t0).Nanoseconds()
	time.Sleep(400 * time.Millisecond)
	confirm := program("native-tight-confirm", 5, 1, expires, []core.Op{{Op: "key_chord", Keys: []string{"ENTER"}}, {Op: "release_all"}})
	cr := b.Execute(confirm, 5, 1); time.Sleep(1200 * time.Millisecond)
	...
	data, _ := json.MarshalIndent(result, "", "  "); data = append(data, '\n')
	if *out != "" { if err := os.WriteFile(*out, data, 0644); err != nil { panic(err) } }
	os.Stdout.Write(data); if !passed { os.Exit(1) }
}
