"""Declared common-runtime interruption stress; not freeze qualification or model timing."""
...
def probe(app,seed,stress,out):
    ...
    try:
        ...
        deadline=time.perf_counter_ns()+(2_000_000_000 if stress=='cancel' else 200_000_000)
        send(dict(op='submit',id='interrupt',expected_sequence=sequence,valid_until_ns=deadline,
            steps=[dict(op='hold',keys=['Control_L'],duration_ms=1000),text('bad')]))
        held=event('keys_held')
        if stress=='cancel':
            send(dict(op='cancel',id='interrupt'))
            requested=event('cancel_requested');assert requested['matched']
            anchor=requested['requested_ns']
        else:anchor=deadline
        stopped=event('terminal')
        assert stopped['id']=='interrupt' and stopped['status']==('cancelled' if stress=='cancel' else 'expired'),stopped
        assert stopped['steps_completed']==0 and stopped['release']['verified']
        assert not any(r['event']=='step_started' and r.get('id')=='interrupt' and r['step']>0 for r in events)
        interruption=dict(status=stopped['status'],release_after_anchor_ms=(stopped['release']['verified_ns']-anchor)/1e6,
            anchor_ns=anchor,deadline_ns=deadline,held_observed=True,tail_started=False)
        ...
        return dict(app=app,seed=seed,stress=stress,task_success=score['success'],actual=score['actual'],
            interruption=interruption,accepted_programs=counter+1,
...
def main():
    ...
    cases=[dict(app=app,seed=980101+i,stress=('cancel' if i==0 else 'expiry')) for app in ('xterm','chromium','calc','inkscape') for i in (0,1)]
    manifest=dict(reference_commit='7427bdd',reference_sources=reference,
        ...
        presentation='full',task_intent_validity_seconds=10,quiet_ms=80,timeout_ms=750,
        semantics='same interactive_v10 and transitive runtime in all cases',
        stress='cancel active 1000ms Control hold, or expire it after 200ms; discard text tail; fresh task afterward',
