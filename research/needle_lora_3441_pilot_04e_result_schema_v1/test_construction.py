"""Construction-only and result-schema regression coverage for Issue #4492; no training."""
...
class Construction(unittest.TestCase):
 ...
 def test_fresh_seed_and_fixed_shapes(self):
  self.assertIn("SEED = 5920",self.text);self.assertNotIn("SEED = 3927",self.text)
 ...
 def test_dataset_reproducibility_and_cardinality(self):
  x1,y1,h1=self.mod["dataset"](32,5921)
  x2,y2,h2=self.mod["dataset"](32,5921)
 ...
 def test_full_module_state_roundtrip_and_rollback(self):
  core=self.mod["Core"](); model=self.mod["Adapter"](core,5931)
 ...
 def test_result_parameter_schema_is_pure_and_json_serializable(self):
  ...
  self.assertEqual(params["seed"],5920)
