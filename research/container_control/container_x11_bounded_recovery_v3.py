#!/usr/bin/env python3
"""Container/X11 matched mechanics experiment v3: coast vs bounded recovery.
...
def controller(out: Path, mode: str, decisions: int, planner_wait_s: float, cover_budget_s: float) -> None:
    d = display.Display()
    win = find_window(d)
    ...
    for decision in range(1, decisions + 1):
        ...
        if mode == "recovery" and previous_policy:
            key = previous_policy
            valid = (key == "Left" and source_x > 0.06) or (key == "Right" and source_x < -0.06)
            if valid:
                ...
                while time.monotonic() - cover_start < min(cover_budget_s, planner_wait_s):
                    ...
                    if x is None:
                        continue
                    still_valid = (key == "Left" and x > 0.02) or (key == "Right" and x < -0.02)
...
def run_arm(root: Path, pair: int, mode: str, order_index: int, decisions: int, planner_wait_s: float, cover_budget_s: float, display_num: int) -> dict:
    ...
    try:
        ...
        cp = subprocess.run([
            ...
            "--decisions", str(decisions), "--planner-wait", str(planner_wait_s), "--cover-budget", str(cover_budget_s),
...
def run_experiment(root: Path, pairs: int, decisions: int, planner_wait_s: float, cover_budget_s: float) -> dict:
    ...
    for pair in range(pairs):
        ...
        for idx, mode in enumerate(order):
            result[mode] = run_arm(root, pair, mode, idx, decisions, planner_wait_s, cover_budget_s, 100 + pair * 2 + idx)
    ...
    summary = {
        "schema": "container-x11-bounded-recovery-v3",
...
def main() -> None:
    ...
    if a.app:
        ...
        return
    if a.controller:
        controller(Path(a.controller), a.mode, a.decisions, a.planner_wait, a.cover_budget)
        return
    summary = run_experiment(Path(a.out), a.pairs, a.decisions, a.planner_wait, a.cover_budget)
