...
def run():
 ...
 port=9222 if ROLE=='A' else 9223; profile='/state/profile-'+ROLE
 ...
 rec=json.loads(Path('/state/receipt.json').read_text()); dispatch_count=0
 def admit_old_receipt(receipt,current):
  if receipt.get('generation') != current.get('generation'): return (False,'generation_mismatch')
  if receipt.get('process_start') != current.get('process_start'): return (False,'process_epoch_mismatch')
  return (True,'admitted')
 current={'generation':2,'process_start':start,'display':disp,'owner':'B'}
 old_allowed,old_reason=admit_old_receipt(rec,current); new_allowed=False
 ...
 if new_allowed: dispatch_count += 1; cdp(port,"document.getElementById('value').value='abc';document.getElementById('save').click();'dispatch'")
 ...
 result={'role':'B','ready':ready,'old_receipt_loaded':True,'old_receipt_admitted':old_allowed,'old_receipt_rejection_reason':old_reason,'old_receipt_dispatch_count':0 if not old_allowed else dispatch_count,'old_receipt_effect':old_effect,'new_generation':new['generation'],'pid':p.pid,'process_start':start,'new_receipt_admitted':new_allowed,'new_receipt_dispatch_count':dispatch_count,'dom_effect':effect,'display':disp}; Path('/state/result-B.json').write_text(json.dumps(result)); p.terminate(); p.wait(timeout=5); xv.terminate(); xv.wait(timeout=5); print(json.dumps(result));
run()
