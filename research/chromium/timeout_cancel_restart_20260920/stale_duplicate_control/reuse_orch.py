...
def run():
 ...
 if ROLE=='U':
  old_source='source-v1'; cdp(port,"document.body.dataset.source='source-v2';document.title='ReceiptFixture';document.body.dataset.saved='';document.getElementById('value').value=''")
  unbound_admitted=True; cdp(port,"document.getElementById('value').value='unsafe';document.getElementById('save').click();'dispatch'")
  ...
  result={'role':'U','ready':ready,'receipt_source':old_source,'live_source':'source-v2','unbound_admitted':unbound_admitted,'effect':effect,'expected':'FAIL_REUSABLE_RECEIPT_WEAKENS_GATE'}; Path('/state/result-U.json').write_text(json.dumps(result)); p.terminate(); p.wait(timeout=5); xv.terminate(); xv.wait(timeout=5); print(json.dumps(result)); return result
 if ROLE=='TA':
  journal=[{'receipt_id':'timeout-r1','terminal':'timeout','generation':1},{'receipt_id':'cancel-r1','terminal':'cancel','generation':1},{'receipt_id':'fresh-r1','terminal':'pending','generation':1}]
  Path('/state/journal.jsonl').write_text(''.join(json.dumps(x)+'\n' for x in journal)); result={'role':'TA','ready':ready,'journal_written':len(journal),'exit_code':0,'xlib_loaded':display is not None}; Path('/state/result-TA.json').write_text(json.dumps(result)); p.terminate(); p.wait(timeout=5); xv.terminate(); xv.wait(timeout=5); print(json.dumps(result)); return result
 if ROLE=='TB':
  journal=[json.loads(x) for x in Path('/state/journal.jsonl').read_text().splitlines() if x]; terminal=[x for x in journal if x['terminal'] in ('timeout','cancel')]; replay=[]
  for x in terminal: replay.append({'receipt_id':x['receipt_id'],'admitted':False,'dispatch_count':0,'effect':False,'reason':'terminal_after_restart'})
  fresh=next(x for x in journal if x['terminal']=='pending'); cdp(port,"document.getElementById('value').value='fresh';document.getElementById('save').click();'dispatch'"); effect={'title':cdp(port,'document.title'),'saved':cdp(port,'document.body.dataset.saved||""'),'value':cdp(port,'document.getElementById("value").value')}
  result={'role':'TB','ready':ready,'journal_loaded':len(journal),'terminal_replay':replay,'fresh_receipt_id':fresh['receipt_id'],'fresh_admitted':True,'fresh_dispatch_count':1,'fresh_effect':effect,'xlib_loaded':display is not None}; Path('/state/result-TB.json').write_text(json.dumps(result)); p.terminate(); p.wait(timeout=5); xv.terminate(); xv.wait(timeout=5); print(json.dumps(result)); return result
 if ROLE=='FA':
  journal=[{'receipt_id':'timeout-cleanup-fail','terminal':'timeout','cleanup':'release_failed','release_receipt':None},{'receipt_id':'fresh-cleanup-control','terminal':'pending','cleanup':'not_applicable'}]
  Path('/state/journal.jsonl').write_text(''.join(json.dumps(x)+'\n' for x in journal)); result={'role':'FA','ready':ready,'journal_written':2,'cleanup_release':False,'exit_code':0}; Path('/state/result-FA.json').write_text(json.dumps(result)); p.terminate(); p.wait(timeout=5); xv.terminate(); xv.wait(timeout=5); print(json.dumps(result)); return result
 if ROLE=='SA':
  journal=[{'receipt_id':'duplicate-terminal','terminal':'timeout','source':'source-v1','generation':1,'prior_dispatch':True},{'receipt_id':'stale-source','terminal':'pending','source':'source-v1','generation':1},{'receipt_id':'fresh-after-restart','terminal':'pending','source':'source-v2','generation':2}]
  Path('/state/journal.jsonl').write_text(''.join(json.dumps(x)+'\n' for x in journal)); result={'role':'SA','ready':ready,'journal_written':3,'exit_code':0}; Path('/state/result-SA.json').write_text(json.dumps(result)); p.terminate(); p.wait(timeout=5); xv.terminate(); xv.wait(timeout=5); print(json.dumps(result)); return result
 if ROLE=='SB':
  journal=[json.loads(x) for x in Path('/state/journal.jsonl').read_text().splitlines() if x]; dup=journal[0]; stale=journal[1]; duplicate_replay={'receipt_id':dup['receipt_id'],'admitted':False,'dispatch_count':0,'effect':False,'reason':'duplicate_terminal_after_restart'}; stale_replay={'receipt_id':stale['receipt_id'],'admitted':False,'dispatch_count':0,'effect':False,'reason':'source_generation_mismatch'}; cdp(port,"document.getElementById('value').value='fresh-after-restart';document.getElementById('save').click();'dispatch'"); effect={'title':cdp(port,'document.title'),'saved':cdp(port,'document.body.dataset.saved||""'),'value':cdp(port,'document.getElementById("value").value')}; result={'role':'SB','ready':ready,'journal_loaded':len(journal),'duplicate_replay':duplicate_replay,'stale_replay':stale_replay,'fresh_admitted':True,'fresh_effect':effect}; Path('/state/result-SB.json').write_text(json.dumps(result)); p.terminate(); p.wait(timeout=5); xv.terminate(); xv.wait(timeout=5); print(json.dumps(result)); return result
 if ROLE=='FB':
  journal=[json.loads(x) for x in Path('/state/journal.jsonl').read_text().splitlines() if x]; old=journal[0]; terminal={'receipt_id':old['receipt_id'],'admitted':False,'dispatch_count':0,'effect':False,'reason':'cleanup_release_failed'}; cdp(port,"document.getElementById('value').value='fresh-cleanup';document.getElementById('save').click();'dispatch'"); effect={'title':cdp(port,'document.title'),'saved':cdp(port,'document.body.dataset.saved||""'),'value':cdp(port,'document.getElementById("value").value')}; result={'role':'FB','ready':ready,'journal_loaded':len(journal),'cleanup_release_verified':old.get('release_receipt') is None,'terminal_replay':terminal,'fresh_admitted':True,'fresh_effect':effect}; Path('/state/result-FB.json').write_text(json.dumps(result)); p.terminate(); p.wait(timeout=5); xv.terminate(); xv.wait(timeout=5); print(json.dumps(result)); return result
 if ROLE=='A':
