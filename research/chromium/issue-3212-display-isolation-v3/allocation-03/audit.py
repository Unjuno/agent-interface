...
stale_effect = stale["decoy_title_after"] == "DECOY-3430-03-EFFECT-1" and \
...
gates = {
    ...
    "independent_readiness_before_stale_send": (
        stale["decoy_title_before"] == "READY-DECOY-3430-03" and
        stale["p2_title_before"] == "READY-P2-3430-03"
    ),
    "generation_changed": old["generation"] != target["generation"],
    "old_xid_reused_by_decoy": stale_reused_to_decoy,
    ...
    "decoy_did_not_change_p2": decoy["p2_title_after"] == p2["p2_title_before"],
    "p2_positive_effect": p2_effect_count_after == p2_effect_count_before + 1,
    "cleanup_reaped": cleanup["all_processes_reaped"],
}
if gates["stale_input_retargeted_decoy"] and gates["independent_readiness_before_stale_send"]:
    decision = "FAIL_STALE_XID_RETARGETED_DECOY"
elif gates["stale_input_retargeted_decoy"]:
