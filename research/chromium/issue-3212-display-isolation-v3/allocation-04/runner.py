...
def ready_window(title, timeout=15):
    ...
    while time.monotonic() < deadline:
        marker = "READY-" + title
        r = cmd("xdotool", "search", "--name", "^" + marker + "$")
        if r.returncode == 0 and r.stdout.strip():
            wid = int(r.stdout.splitlines()[0])
            p = cmd("xdotool", "getwindowpid", str(wid))
            if p.returncode == 0 and p.stdout.strip() and title_of(wid) == marker:
...
def main():
    ...
    rows = []
    try:
        p1 = launch("p1", "P1-3430-04", 9801)
        old_w, old_pid = ready_window("P1-3430-04")
        old_ready_title = title(old_w)
        old_start = STARTED["p1"]
        stop("p1", p1)
        time.sleep(0.25)

        p2 = launch("p2", "P2-3430-04", 9802)
        decoy = launch("decoy", "DECOY-3430-04", 9803)
        p2_w, p2_pid = ready_window("P2-3430-04")
        decoy_w, decoy_pid = ready_window("DECOY-3430-04")
        rows.append({
            ...
            "ready_titles": {"old": old_ready_title, "p2": title(p2_w),
                             "decoy": title(decoy_w)},
        })
    ...
    print(json.dumps({"allocation": "OBSTAC_3430_ALLOCATION_04",
