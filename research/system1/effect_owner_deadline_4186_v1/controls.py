...
def main():
 ...
 def ch(name,mut):
  ...
  with tempfile.TemporaryDirectory() as d:
   p=pathlib.Path(d); (p/'ROWS.json').write_text(json.dumps(data)); out.append({'name':name,'rejected':bool(audit(p,3)['errors'])})
 ...
 result={'controls':out,'all_rejected':all(v['rejected'] for v in out)}; pathlib.Path('CONTROLS.json').write_text(json.dumps(result,indent=2,sort_keys=True)+'\n'); print(json.dumps(result,sort_keys=True)); raise SystemExit(0 if result['all_rejected'] else 1)
