...
def run_case(root,policy,schedule,rep,mode):
    ...
    effect=None
    if effect_path.exists(): effect=json.loads(effect_path.read_text())
    row={"case_id":cid,"mode":mode,"rep":rep,"policy":policy,"schedule":schedule,
         ...
         "source_hashes":{x:sha256(x) for x in ("run.py","app_worker.py","sink_worker.py")}}
    (cdir/"row.json").write_text(json.dumps(row,indent=2,sort_keys=True)+"\n")
    return row

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--out",required=True); ap.add_argument("--mode",choices=["construction","formal"],required=True); ap.add_argument("--reps",type=int,default=1); a=ap.parse_args()
    root=pathlib.Path(a.out); root.mkdir(parents=True,exist_ok=False); rows=[]
    for rep in range(a.reps):
      for sched in SCHEDULES:
       for policy in POLICIES: rows.append(run_case(root,policy,sched,rep,a.mode))
