...
**D.** `PASS_AUDIT_INTENT_ALIGNMENT_HOLD_SCOPED`. Independent auditor recomputed 1,024 paired rows, all teacher labels and both model prediction vectors; errors=[]; recomputed predictive gates and reproduced the recorded outcome. It reproduced `HOLD_OR_FAIL_GATE_MISS`, not a model PASS. Accuracy: baseline 0.607421875, conditioned 0.79296875, gain 0.185546875; paired-intent exact: 0.41796875 vs 0.732421875. Conditioned p95: 0.0281 ms, recomputed from retained latency samples and not newly measured. Accuracy, gain and paired exact gates remain false. The stale-intent/evidence/unknown YIELD and matched PROPOSE control gates were checked against stored aggregate `gate_counts`; their individual control decisions were not independently replayed. Exact stdout SHA-256 is recorded in SHA256SUMS.
