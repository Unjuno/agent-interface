...
class Pilot09AuditLogicTests(unittest.TestCase):
    def test_public_issue_contract_digest_is_pinned(self):
        digest=hashlib.sha256(Path("ISSUE_CONTRACT.md").read_bytes()).hexdigest()
        self.assertEqual(digest,audit.ISSUE_CONTRACT_SHA256)
    ...
    def test_dataset_sizes_and_seed_count_fixed(self):
        self.assertEqual(audit.SEEDS,(3490,3491,3492))
    ...
    def test_training_rows_and_batch_digest_reconstruct(self):
        control,_=audit.training_rows_expected(3490,False)
        treatment,_=audit.training_rows_expected(3490,True)
        self.assertTrue(torch.equal(control[:2048],treatment[:2048]))
        self.assertTrue(torch.equal(control[4096:],treatment[4096:]))
        self.assertTrue(audit.stratified_band_membership(treatment))
        treatment[3584,0]=.1105
        self.assertFalse(audit.stratified_band_membership(treatment))
        self.assertNotEqual(audit.expected_batch_hash(3490),audit.expected_batch_hash(3491))
