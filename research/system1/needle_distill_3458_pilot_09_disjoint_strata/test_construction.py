...
class Pilot09ConstructionTests(unittest.TestCase):
    ...
    def test_training_data_size_and_mixture(self):
        control,_=runner.training_rows(3490,False)
        augmented,_=runner.training_rows(3490,True)
        ...
        for dx,dy,vx,vy,conf,visible in augmented[3072:3584].tolist():
            self.assertTrue(.071<=abs(dx)<=.110)
        ...
        for dx,dy,vx,vy,conf,visible in augmented[3584:4096].tolist():
            self.assertTrue(.111<=abs(dx)<=.149)
            self.assertLessEqual(abs(dy),.10)
            self.assertLessEqual(abs(vx),.05); self.assertLessEqual(abs(vy),.05)
            self.assertTrue(.80<=conf<=1.); self.assertEqual(visible,1.)

    def test_eval_seeds_are_disjoint_from_training_seed_map(self):
        train={s+10+c for s in runner.SEEDS for c in range(3)}|{s+40 for s in runner.SEEDS}|{s+41 for s in runner.SEEDS}
    ...
    def test_paired_minibatch_stream_is_fixed(self):
        a=runner.minibatch_stream(3490); b=runner.minibatch_stream(3490)
