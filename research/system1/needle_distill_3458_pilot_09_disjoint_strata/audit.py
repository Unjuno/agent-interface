"""Independent audit for paired pilot-09; does not import runner.py."""
import hashlib
import json
import math
import statistics
...
SEEDS=(3490,3491,3492); LABELS=("CONTINUE","CORRECT","WATCH")
...
def stratified_band_membership(train_x):
    rows=train_x.tolist()
    band_a=rows[3072:3584]
    band_b=rows[3584:4096]
    def valid_band(samples,lo,hi):
        return len(samples)==512 and all(
            lo<=abs(float(row[0]))<=hi
            and abs(float(row[1]))<=.10+TOL
            and abs(float(row[2]))<=.05+TOL
            and abs(float(row[3]))<=.05+TOL
            and .80-TOL<=float(row[4])<=1+TOL
            and float(row[5])==1.0 for row in samples)
    return valid_band(band_a,.071,.110) and valid_band(band_b,.111,.149)


def audit(payload,raw):
    errors=[]; reports=[]; overall=[]
    raw_sha256=hashlib.sha256(raw).hexdigest()
    if payload.get("allocation")!="needle-intent-distill-3458-pilot-09-disjoint-strata": errors.append("allocation")
    contract_sha=hashlib.sha256(Path("ISSUE_CONTRACT.md").read_bytes()).hexdigest()
    if contract_sha!=ISSUE_CONTRACT_SHA256 or payload.get("issue_contract_sha256")!=ISSUE_CONTRACT_SHA256:
        errors.append("public_issue_contract_binding")
    ...
    for rec,seed in zip(records,SEEDS):
        ...
        for arm in ("balanced_control","targeted_augmented"):
            ...
            if arm=="targeted_augmented" and not stratified_band_membership(expected_x): errors.append(pref+":band_membership")
    ...
    scientific="PASS_DISJOINT_STRATA_AUGMENTATION_SCOPED" if len(overall)==3 and all(overall) and recall_std<=.05 else "FAIL_DISJOINT_STRATA_AUGMENTATION"
