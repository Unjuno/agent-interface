"""Frozen #3847 selective Needle covariate-shift experiment."""
import argparse
import hashlib
import json
import math
...
def shifted_class(label, n, seed):
    ...
    if label == 2:
        xy = (torch.rand(n, 2, generator=g) - .5) * .5
        velocity = (torch.rand(n, 2, generator=g) - .5) * .2
        confidence = .60 + .119 * torch.rand(n, 1, generator=g)
        visible = torch.ones(n, 1)
        return torch.cat([xy, velocity, confidence, visible], 1)
...
def run_seed(seed):
    ...
    invalid = []
    invalid_cases = [
        (META | {"epoch": 8}, torch.tensor([.2, .2, 0., 0., .9, 1.])),
        (META | {"scope": "other"}, torch.tensor([.2, .2, 0., 0., .9, 1.])),
        ({"intent": "other", "scope": META["scope"], "epoch": META["epoch"]}, torch.tensor([.2, .2, 0., 0., .9, 1.])),
        (META, torch.tensor([1.3, 0., 0., 0., .9, 1.])),
        (META, torch.tensor([float("nan"), 0., 0., 0., .9, 1.])),
    ]
    for m, x in invalid_cases:
        pred, reason = proposal(m, x, model)
        invalid.append({"proposal": pred, "reason": reason})
...
def main():
    ...
    result = {"allocation": "needle-intent-distill-3458-pilot-05-near-boundary-shift",
              "issue": 3847, "seeds": [run_seed(s) for s in SEEDS],
