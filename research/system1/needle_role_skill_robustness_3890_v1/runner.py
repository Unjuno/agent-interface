...
def main():
    ...
    artifact_path=os.path.join(OUT,"skill.json")
    with open(artifact_path,"rb") as f: artifact_raw=f.read()
    meta={"seed":SEED,"allocation":"needle-role-skill-robustness-3890-v1",
          "optimizer_steps":BASE_STEPS+2*ADAPTER_STEPS,"base_steps":BASE_STEPS,
          "adapter_steps_per_role":ADAPTER_STEPS,"optimizer_seed_offsets":{"base":10,"B":11,"C":12},
          ...
          "artifact_file_sha256":hashlib.sha256(artifact_raw).hexdigest(),"artifact_bytes":len(artifact_raw),
          "environment":{"platform":platform.platform(),"python":platform.python_version(),"torch":torch.__version__,"device":"cpu","threads":torch.get_num_threads()}}
    with open(os.path.join(OUT,"builder_meta.json"),"w",encoding="utf-8") as f: json.dump(meta,f,sort_keys=True,separators=(",",":"),allow_nan=False)
