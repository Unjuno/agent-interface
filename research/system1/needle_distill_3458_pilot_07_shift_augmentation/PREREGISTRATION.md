# Issue #3918 — pilot-07 targeted shift augmentation
...
- Allocation `needle-intent-distill-3458-pilot-07-targeted-shift-augmentation`, seeds 3470–3472, additive path `research/system1/needle_distill_3458_pilot_07_shift_augmentation/`.
- Per seed paired balanced control and targeted-augmentation treatment. Same 6→16→16→3 tanh MLP, identical deterministic initial state, AdamW lr .008, batch size 64, 700 fixed minibatch updates, and identical minibatch-index RNG stream.
...
`PASS_TARGETED_SHIFT_AUGMENTATION_SCOPED` iff all treatment seeds satisfy shifted accepted accuracy≥.95; per-class coverage≥.75 and accepted recall≥.95; false-CORRECT fraction of rows≤.005; boundary 100% YIELD; all invalid controls YIELD; CPU p95<60 ms; IID accepted accuracy≥.95; mean paired shifted CORRECT recall lift≥.10; and each paired IID accuracy loss≤.02. All three control and treatment metrics are reported. IID cannot rescue shifted misses. Scientific gate miss=FAIL; evidence/protocol defect=HOLD; environment/formal failure=STOP. One allocation, no retries/tuning/seed replacement.
