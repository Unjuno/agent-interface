"""Independent audit for paired pilot-07; does not import runner.py."""
import hashlib
import json
import math
...
SEEDS=(3470,3471,3472); LABELS=("CONTINUE","CORRECT","WATCH")
...
def shifted_class(label,n,seed):
    ...
    dx=sign*(.071+.078*torch.rand(n,1,generator=g)); dy=(torch.rand(n,1,generator=g)-.5)*.20
...
def training_rows_expected(seed,augmented):
    ...
    if augmented: classes[1]=torch.cat((classes[1][:1024],shifted_class(1,1024,seed+40)),0)
...
def audit(payload,raw):
    ...
    if payload.get("allocation")!="needle-intent-distill-3458-pilot-07-targeted-shift-augmentation": errors.append("allocation")
    ...
    for rec,seed in zip(records,SEEDS):
        ...
        if len(errors)==seed_errors_before:
            pass
    scientific="PASS_TARGETED_SHIFT_AUGMENTATION_SCOPED" if len(overall)==3 and all(overall) else "FAIL_TARGETED_SHIFT_AUGMENTATION"
    return {"audit":"PASS" if not errors else "FAIL","errors":errors,"decision_recomputed":scientific,
            "treatment_seed_gates":overall,"reports":reports,"feature_tolerance":TOL,
