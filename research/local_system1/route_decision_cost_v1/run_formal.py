...
TASK='LOCAL-SYSTEM1-ROUTE-DECISION-COST-20260917-001'
SEED=89220260917
UNIQUE=8192
MEASURED=32768
WARMUP=512
...
def compile_backends():
    ...
    t=time.perf_counter_ns(); linear=LogisticRegression(C=1e6,max_iter=10000,solver='lbfgs',random_state=SEED).fit(X,y); linear_ms=(time.perf_counter_ns()-t)/1e6
    t=time.perf_counter_ns(); tree=DecisionTreeClassifier(random_state=SEED).fit(X,y); tree_ms=(time.perf_counter_ns()-t)/1e6
    ...
    return linear,tree,{'linear_ms':linear_ms,'tree_ms':tree_ms,'tree_depth':tree.get_depth(),'tree_leaves':tree.get_n_leaves()}
def corpus():
