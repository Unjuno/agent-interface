...
def canonical(out):
    ...
    if isinstance(out,Decision): return [out.kind,out.value]
    if isinstance(out,Request):
        return {'contract_id':out.contract_id,'facts':dict(sorted(out.facts.items())),'allowed':sorted([list(x) for x in out.allowed]),'evidence_roles':sorted(out.evidence_roles),'authority':out.authority}
...
def timed(fn,rows):
    ...
    try:
        for i in range(TIMING_N):
            row=rows[i%len(rows)]; t0=time.perf_counter_ns(); out=fn(row); t1=time.perf_counter_ns(); vals.append(t1-t0); outs.append(canonical(out))
    finally: gc.enable()
