...
def check_row(path, requested_layout):
    ...
    if row.get("allocation") != "issue3668-german-xkb-text-v2":
    ...
    extensions = row.get("server_extensions", [])
    if row.get("server_xkb_extension") is not True or "XKEYBOARD" not in extensions:
        errors.append("server did not advertise XKEYBOARD")
    if row.get("server_xtest_extension") is not True or "XTEST" not in extensions:
        errors.append("server did not advertise XTEST")
    if type(row.get("python_xlib_xkb_binding")) is not bool:
        errors.append("Python-Xlib XKB binding observation missing")
    if row.get("python_xlib_xtest_binding") is not True:
        errors.append("Python-Xlib XTEST binding unavailable")
