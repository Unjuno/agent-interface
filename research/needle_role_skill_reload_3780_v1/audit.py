...
import hashlib,json,math,sys,pathlib
def fail(x): raise SystemExit("STOP:"+x)
def canonical(x): return json.dumps(x,sort_keys=True,separators=(",",":"),allow_nan=False).encode()
def main():
    root=sys.argv[1]; source=pathlib.Path(__file__).parent; freeze=json.load(open(source/"FREEZE.json",encoding="utf-8")); results=[]; errors=[]
    ...
    for seed in freeze["seeds"]:
        ...
        if artifact.get("graph",{}).get("edges")!=[["A","B"],["B","C"]] or artifact.get("provenance",{}).get("predecessor_issue")!=3780: errors.append(f"{seed}:manifest")
        ...
        for run in ("load1","load2"):
            ...
            for g in graphs:
                if g.get("flow")!=["ADVANCE","ADVANCE"] or g.get("cursor")!="C" or g.get("old_receipt")!="YIELD": errors.append(f"{seed}:{run}:graph")
