...
func Open(display string) (*Backend, error) {
	var c *C.char
	if display != "" {
		c = C.CString(display)
	...
	b := &Backend{d: d, started: time.Now(), targets: map[string]C.ulong{}, heldKeys: map[C.uint]bool{}, heldButtons: map[C.uint]bool{}}
...
func (b *Backend) Manifest() core.Manifest {
	...
	if b.xtest {
		for _, c := range []string{"input.keyboard", "input.pointer", "input.release_all"} {
			m.Capabilities[c] = core.Capability{State: "supported"}
		}
	}
