...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--out',type=Path,required=True);ap.add_argument('--plan',type=Path,required=True);ap.add_argument('--index',type=int,required=True);a=ap.parse_args();a.out.mkdir(parents=True,exist_ok=False);p=json.loads(a.plan.read_text());exit_code=0
 ...
 env=os.environ.copy();env['DISPLAY']=disp;env['XAUTHORITY']=str(auth);os.environ['DISPLAY']=disp;os.environ['XAUTHORITY']=str(auth);log=(a.out/'xdummy.log').open('w');proc=subprocess.Popen(['Xdummy',disp,'-auth',str(auth),'-nolisten','tcp','-noreset'],stdout=log,stderr=subprocess.STDOUT,text=True)
 startup=False
 for _ in range(50):
  ...
  time.sleep(.1)
 result={'schema':'agent-interface/xkb-xdummy-arm-v2','display':disp,'startup':startup,'xauth_returncode':xa.returncode,'input_operations':0}
 try:
  if not startup:
   result['decision']='SETUP_BLOCKED_XDUMMY'
  else:
   ...
   exit_code=0 if decision!='FAIL_INTEGRITY' else 2
 except Exception as exc:
  result['decision']='HARNESS_FAIL';result['error']=repr(exc);exit_code=2
 finally:
