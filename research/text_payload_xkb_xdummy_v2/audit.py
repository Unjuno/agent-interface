...
def main():
 ...
 for i in range(3):
  ...
  if r.get('decision')=='HARNESS_FAIL': ds.append('HARNESS_FAIL');continue
  if not r.get('startup'):
   exp='SETUP_BLOCKED_XDUMMY'
   if r.get('decision')!=exp:e.append(f'{i}:decision')
   ds.append(exp);continue
  for f in ('de.resolved.xkb','baseline.server.xkb','after.server.xkb'):
   if not (d/f).exists(): e.append(f'{i}:missing:{f}')
  if any(x.startswith(f'{i}:missing:') for x in e): ds.append('FAIL_INTEGRITY');continue
  ...
  if r.get('decision')!=exp:e.append(f'{i}:decision')
  ds.append(exp)
 if all(x=='PASS_XDUMMY_NATIVE_XKB_MAP_SCOPED' for x in ds): exp='PASS_XDUMMY_NATIVE_XKB_MAP_SCOPED'
 elif all(x=='SETUP_BLOCKED_XDUMMY' for x in ds): exp='SETUP_BLOCKED_XDUMMY'
 elif all(x=='SETUP_BLOCKED_NATIVE_XKB_APPLY' for x in ds): exp='SETUP_BLOCKED_NATIVE_XKB_APPLY'
 elif any(x=='HARNESS_FAIL' for x in ds): exp='HARNESS_FAIL'
 else: exp='FAIL_INTEGRITY'
