...
def amap(s): return {r[0]:(r[1],r[2],r[3]) for r in s['active']}
def run_chunk(chunk):
    if not (0<=chunk<CHUNKS): raise ValueError('chunk')
    start=chunk*PER_CHUNK; end=min(start+PER_CHUNK,LIFETIMES)
    names=['steps','mismatches','false_mint','retired_reuse','cross_lineage_retirement','multi_id_per_hold','repeated_down_instability','unconfirmed_fabrication','active_retired_overlap']
    counts={k:0 for k in names}; h=hashlib.sha256()
    for idx in range(start,end):
        ...
        for pos,e in enumerate(sequence(idx)):
            ...
            retired_after=set(after['retired']); status,aid=co
            if status=='MINTED':
            ...
            if not e.confirmed:
                known_reuse=(e.op=='down' and e.key in before_active and before_active[e.key][:2]==(e.owner,e.intent) and status=='ACTIVE_REUSED' and aid==before_active[e.key][2] and before==after)
                if before!=after or (aid is not None and not known_reuse): counts['unconfirmed_fabrication']+=1
    ...
    return {'task':'PHYSICAL-KEY-HOLD-ACTUATION-IDENTITY-AUDITABLE-20260918-004','seed':SEED,'chunk':chunk,'start':start,'end':end,'lifetimes':end-start,'counts':counts,'digest_sha256':h.hexdigest(),'reruns':0}
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--chunk',type=int,required=True); ap.add_argument('--out',required=True); a=ap.parse_args(); out=Path(a.out)
