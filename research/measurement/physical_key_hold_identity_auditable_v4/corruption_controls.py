...
HERE=Path(__file__).resolve().parent; chunks=json.loads((HERE/'CHUNK_SUMMARIES.json').read_text()); result=json.loads((HERE/'RESULT.json').read_text()); fixed=json.loads((HERE/'FIXED.json').read_text()); controls=[]
...
out={'passed':all(x['rejected'] for x in controls),'controls':controls,'controls_total':len(controls),'controls_rejected':sum(x['rejected'] for x in controls)}; (HERE/'CORRUPTION.json').write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print(json.dumps(out,sort_keys=True))
