...
def classify(events:list[dict[str,Any]])->dict[str,Any]:
    ...
    for e in task:
        ...
        bound.append(e)
    return {"physical_count":len(physical),"task_effect_count":len(bound),"task_effects":bound,"errors":sorted(set(errors)),"task_effect_authority":False,"decision":"TASK_EFFECT_BOUND" if bound and not errors else ("UNRESOLVED_NO_TASK_EFFECT" if not bound and not errors else "REJECT")}
def oracle(events:list[dict[str,Any]])->dict[str,Any]:
    ...
    effects=[e for e in events if e.get("role")=="TASK_EFFECT" and e.get("scored") is True]; errors=[]
    for e in effects:
