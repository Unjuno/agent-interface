...
TASK='CONCURRENT-FAST-DECISION-T1-X11-PROCESS-ISOLATION-20260918-002'
...
SCENARIO={'name':'ACTIVATE_18_CONSTRUCTION','initial':WATCH,'transitions':[[18_000_000,CLEAR],[30_000_000,WATCH]]}
...
def sample(win):
    b=time.perf_counter_ns(); raw=bytes(win.get_image(*CTRL,X.ZPixmap,0xffffffff).data); e=time.perf_counter_ns()
...
def count_color(win,rect,rgb):
    data=bytes(win.get_image(*rect,X.ZPixmap,0xffffffff).data); n=0
...
def child(case_id,arm,root_path,out_path):
    ...
    try:
        ...
        start=time.perf_counter_ns()+20_000_000; result.update({'start_ns':start,'initial_state':SCENARIO['initial'],'frontier_request_offset_ns':0,'frontier_return_offset_ns':FRONTIER,'controller_capture_rect':list(CTRL),'progress_score_rect':list(PROG_RECT),'harm_score_rect':list(HARM_RECT),'initial_rgb':list(rgb)})
        def trans():
            for off,state in SCENARIO['transitions']:
                sleep_until(start+off)
                if stop.is_set():return
                root.after(0,lambda s=state,o=off:fx.set_state(s,o))
        tt=threading.Thread(target=trans,daemon=True); tt.start(); threads.append(tt)
        def ctl():
            sleep_until(start)
            ...
            for off in range(0,FRONTIER,SAMPLE):
                sleep_until(start+off)
                if stop.is_set():return
                sb,se,st,rgb=sample(cw); d0=time.perf_counter_ns(); disp=select_disposition(st); d1=time.perf_counter_ns(); row={'nominal_sample_offset_ns':off,'sample_begin_ns':sb,'sample_end_ns':se,'state':st,'rgb':list(rgb),'disposition':disp,'decision_compute_ns':d1-d0}; samples.append(row)
                if disp not in ALLOWED:raise RuntimeError('out of vocabulary')
                if disp=='ADVANCE':ab,ae=send_f8(cd,k); sends.append({'send_begin_ns':ab,'send_end_ns':ae,'nominal_sample_offset_ns':off}); row['send_index']=len(sends)-1
                elif disp=='YIELD':return
        ct=threading.Thread(target=ctl,daemon=True); ct.start(); threads.append(ct); end=start+FRONTIER
        while time.perf_counter_ns()<end or ct.is_alive():
            root.update(); time.sleep(.0002)
            if time.perf_counter_ns()>end+20_000_000:raise RuntimeError('controller overrun')
        ct.join(.1); stop.set(); tt.join(.1)
...
def supervise(out_path,root_dir):
    ...
    for pos,arm in enumerate(ARMS):
        cid=f'p00-{pos}-{arm}-{SCENARIO["name"]}'; row=root/f'{cid}.json'; cr=root/cid
        cp=subprocess.run([sys.executable,__file__,'child','--case-id',cid,'--arm',arm,'--root',str(cr),'--out',str(row)],text=True,capture_output=True)
    ...
    r={'task':TASK,'phase':'construction','formal_invocations':0,'reruns':0,'replacements':0,'tuning':0,'pairs':1,'process_isolation':True,'cases':cases,'child_runs':runs,'scientific_disposition':'NONE'}; out.write_text(json.dumps(r,indent=2,sort_keys=True)+'\n'); print(json.dumps({'child_exit':[x['returncode'] for x in runs],'rows':len(cases)},sort_keys=True)); return 0 if len(cases)==2 and all(x['returncode']==0 for x in runs) else 3
...
def pure():
    ...
    for off in range(0,FRONTIER,SAMPLE):
        ...
        for t,nxt in SCENARIO['transitions']:
            if off>=t:st=nxt
        nominal.append([off,st,select_disposition(st)])
    print(json.dumps({'task':TASK,'selector':'PASS','construction_nominal':nominal,'process_isolation':'subprocess-per-session'},sort_keys=True));return 0

def main():
    ap=argparse.ArgumentParser(); sub=ap.add_subparsers(dest='mode',required=True)
    p=sub.add_parser('child');p.add_argument('--case-id',required=True);p.add_argument('--arm',required=True);p.add_argument('--root',required=True);p.add_argument('--out',required=True)
    p=sub.add_parser('supervise');p.add_argument('--root',required=True);p.add_argument('--out',required=True)
    p=sub.add_parser('audit');p.add_argument('result');p.add_argument('--out',required=True)
    sub.add_parser('pure');a=ap.parse_args()
    if a.mode=='child':return child(a.case_id,a.arm,a.root,a.out)
    if a.mode=='supervise':return supervise(a.out,a.root)
    if a.mode=='audit':return audit(a.result,a.out)
    return pure()
