        ...
        try:
            ...
            if arm=='capture':
                ...
                while True:
                    ...
                    try:
                        img=rwin.get_image(ROI_X,ROI_Y,ROI_W,ROI_H,X.ZPixmap,0xffffffff); data=bytes(img.data)
            ...
            return {'arm':arm,'display':display_num,'measure_wall_ns':stop-start,'process_cpu_ns':cpu,'cpu_fraction':cpu/max(1,stop-start),
                    ...
                    'frame_bytes':frame_bytes,'max_ring_bytes':max_ring,'capture_region':[ROI_X,ROI_Y,ROI_W,ROI_H],
                    'fixture_count':len(ts),'fixture_gap_ns':{'p50':pct(gaps,.5),'p95':pct(gaps,.95),'max':max(gaps) if gaps else None},
...
def main():
    ...
    base_display=310 if args.construction else 320
    for i in range(pairs):
        ...
        rows.append(pair)
    result={'construction':args.construction,'formal':not args.construction,'formal_invocations':0 if args.construction else 1,'reruns':0,'pairs':rows,
            'platform':platform.platform(),'python':platform.python_version(),'grants_input_authority':False,'task':'TEMPORAL-BUFFER-X11-ROI-CAPTURE-OVERHEAD-20260918-006'}
