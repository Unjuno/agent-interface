...
def main():
    ...
    if d.get('task')!='TEMPORAL-BUFFER-X11-ROI-CAPTURE-OVERHEAD-20260918-006': errs.append('task_identity')
    ...
    for i,pair in enumerate(d.get('pairs',[])):
        ...
        if c.get('capture_region')!=[80,60,160,120]: errs.append(f'pair{i}_region')
    ...
    decision='HOLD_HOST_SCHEDULING_NOISE' if severe>=2 else 'PASS_X11_TEMPORAL_ROI_CAPTURE_OVERHEAD_SCOPED'
    if severe<2:
        ...
        if errs: decision='REJECT_ROI_CAPTURE_COST' if any(('capture_' in x or x.endswith('_cpu') or x.endswith('_ring') or 'paired_' in x or 'max_gap' in x) for x in errs) else 'FAIL_INTEGRITY'
