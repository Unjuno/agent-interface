# Independent history-replay oracle. No candidate imports.

def sid(scope):
    return (scope.session, scope.target)

def replay(history):
    epochs={}; requests={}; decisions={}; seen=set(); last=None
    for op,args in history:
        if op=='begin':
            scope,rid,*_=args; key=sid(scope)
            if rid in requests: raise ValueError('duplicate_request_id')
            ep=epochs.get(key,0); requests[rid]=[key,ep,False,scope]
            ...
            if e.event_id in seen:
                ...
                seen.add(e.event_id); epochs[key]=epochs.get(key,0)+1
                last={'status':'CURRENTNESS_INVALIDATED','event_id':e.event_id,'scope':e.scope,'epoch':epochs[key],'grants_input_authority':False}
        elif op=='install':
            rid,did,pgen,*_=args; req=requests.get(rid)
            if req is None:
                last={'status':'UNKNOWN_REQUEST_REFUSED','request_id':rid,'decision_id':did,'planner_generation':pgen,'grants_input_authority':False}
            else:
                key,origin,cons,scope=req; cur=epochs.get(key,0)
                if cons:
    ...
    snap={
      ...
      'requests':tuple(sorted((rid,key[0],key[1],ep,cons) for rid,(key,ep,cons,_) in requests.items())),
