...
def main():
    ...
    rows=[]
    for i in range(6):
        order=['baseline','capture'] if i%2==0 else ['capture','baseline'];rows.append([one_arm(root,180+i*2+j,arm,.3,1.5) for j,arm in enumerate(order)])
    result={'formal':True,'formal_invocations':1,'reruns':0,'pairs':rows,'platform':platform.platform(),'python':platform.python_version(),'grants_input_authority':False,'task':'TEMPORAL-BUFFER-X11-CAPTURE-CADENCE-20260918-005'};out.write_text(json.dumps(result,indent=2,sort_keys=True)+'\n');print(json.dumps(result))
