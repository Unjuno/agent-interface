...
def main():
    chunks=json.loads((HERE/'CHUNK_SUMMARIES.json').read_text())['chunks']; fixed=json.loads((HERE/'FIXED.json').read_text())
    errors=[]
    ...
    keys=sorted(chunks[0]['counts']) if chunks else []
    totals={k:sum(x['counts'][k] for x in chunks) for k in keys}
    h=hashlib.sha256()
    ...
    if not fixed.get('passed') or fixed.get('controls_passed')!=fixed.get('controls_total') or fixed.get('controls_total')!=11: errors.append('fixed')
    result={'task':'PHYSICAL-KEY-HOLD-ACTUATION-IDENTITY-AUDITABLE-20260918-003','decision':'PASS_PHYSICAL_HOLD_IDENTITY_AUDITABLE_V3_SCOPED' if not errors else 'FAIL_CONSTRUCTION','seed':SEED,'chunks':CHUNKS,'chunks_complete':len(chunks),'lifetimes':sum(x['lifetimes'] for x in chunks),'steps':totals.get('steps',0),'counts':{k:v for k,v in totals.items() if k!='steps'},'fixed_controls_passed':fixed.get('controls_passed'),'fixed_controls_total':fixed.get('controls_total'),'chunk_digest_sha256':h.hexdigest(),'reruns':0,'x11_actions':0,'model_calls':0,'task_input_actions':0,'errors':errors}
