...
def main():
    ...
    base=json.loads(pathlib.Path(a.formal).read_text()); tests=[]; muts=[]
    ...
    for name,obj in muts:
        ...
        except Exception as e: tests.append({'name':name,'rejected':True,'exception':repr(e)})
        finally: os.unlink(p)
    out={'all_rejected':all(t['rejected'] for t in tests),'tests':tests}; pathlib.Path(a.out).write_text(json.dumps(out,separators=(',',':'),sort_keys=True)); print(json.dumps(out,sort_keys=True))
