...
def run_session(session_id:int,direction:int,phases:list[float],display_num:int):
    ...
    with tempfile.TemporaryDirectory(prefix=f'x11centroid_{session_id}_') as td:
        ...
        try:
            ...
            # Sole A2 repair: set the runner process environment itself before Xlib Display().
            ...
            for phase in phases:
                ...
                for label,x in [('x0',x0),('x1',x1)]:
                    ...
                    if centroid is None:
                        ...
                        point_error=centroid-authored_root_x
                    fr={
                        'session_id':session_id,'display':display_name,'direction':direction,'phase':phase,
                        'label':label,'authored_local_x':x,'fixture_rootx':ack['rootx'],'fixture_rooty':ack['rooty'],
                        'authored_root_x':authored_root_x,'scan_y':SCAN_Y,'scanline_b64':base64.b64encode(raw).decode('ascii'),
                        'scanline_bytes':len(raw),'red_count':nred,'centroid_x':centroid,'point_error_px':point_error,
                        'capture_start_ns':t0,'capture_end_ns':t1,'capture_duration_ns':t1-t0,
                    }
                    pair.append(fr)
                r0,r1=pair
                ...
                rows.append({
                    'session_id':session_id,'display':display_name,'direction':direction,'phase':phase,
                    'authored_displacement_px':direction*D,'observed_displacement_px':observed_disp,
                    'displacement_residual_px':residual,
                    'direction_agreement': observed_disp is not None and ((observed_disp>0)==(direction>0)),
                    'frames':pair,
                })
        finally:
            if dpy is not None:
...
def summarize(rows,cleanups,formal):
    ...
    return {
        'task':TASK,'formal_invocation':1 if formal else 0,'reruns':0,
        'pairs':len(rows),'frames':len(frames),'missed_red':missed,
        'direction_agreement_count':sum(bool(p['direction_agreement']) for p in rows),
        'max_abs_point_error_px':max(point) if point else None,
        'p99_abs_point_error_px':percentile_nearest(point,0.99),
        'max_abs_displacement_residual_px':max(resid) if resid else None,
        'capture_duration_ns_median':statistics.median([f['capture_duration_ns'] for f in frames]) if frames else None,
        'cleanup':cleanups,
    }
...
def main():
    ...
    if formal:
        specs=[(0,1,FORMAL_PHASES,221),(1,-1,FORMAL_PHASES,222),(2,1,FORMAL_PHASES,223),(3,-1,FORMAL_PHASES,224)]
    else:
        specs=[(90,1,CONSTRUCTION_PHASES,220)]
