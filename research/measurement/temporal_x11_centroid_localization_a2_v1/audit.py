...
def audit(path,formal=True):
    ...
    errors=[]; science=[]
    rows=p.get('pairs_rows',[])
    exp_pairs=400 if formal else 4; exp_frames=800 if formal else 8
    ...
    for pr in rows:
        ...
        recom=[]
        for f in fs:
            raw=base64.b64decode(f['scanline_b64'])
            c,n=decode(raw)
        ...
        if od is not None:
            ...
            residuals.append(abs(rr))
            da=((od>0)==(pr['direction']>0))
    ...
    elif 'missed_red' in science or 'direction' in science: decision='FAIL_X11_LOCALIZATION_OBSERVATION'
    elif science: decision='HOLD_X11_LOCALIZATION_BOUND_WIDER'
    elif formal: decision='PASS_X11_CENTROID_LOCALIZATION_ENVELOPE_SCOPED'
