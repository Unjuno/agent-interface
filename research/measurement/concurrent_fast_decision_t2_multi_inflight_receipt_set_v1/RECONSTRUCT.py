...
with tempfile.TemporaryDirectory(prefix='ai1469-reconstruct-') as td:
    root=Path(td)
    bundle=root/'source.tar.gz'; bundle.write_bytes(raw)
    ...
    got=json.loads((root/'AUDIT.json').read_text())
    retained=json.loads((HERE/'AUDIT.json').read_text())
    assert got==retained
