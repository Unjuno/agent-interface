...
for name,fn in [
 ...
 ('return_gen',lambda x:(x['rows'][2].__setitem__('return_generation',x['rows'][2]['capture_generation']),x['rows'][2].__setitem__('return_window_id',x['rows'][2]['capture_window_id']))),
 ...
 ('window',lambda x:(x['rows'][5].__setitem__('capture_window_id',x['rows'][5]['request_window_id']),x['rows'][5].__setitem__('capture_generation',x['rows'][5]['request_generation'])))]:
    td=pathlib.Path(tempfile.mkdtemp(prefix='o4mut-')); shutil.copytree(src,td/'f'); x=json.loads(json.dumps(raw)); fn(x); (td/'f'/'RAW.json').write_text(json.dumps(x))
...
print(json.dumps({'controls':mut,'rejected':sum(v for _,v in mut),'total':len(mut)}));
