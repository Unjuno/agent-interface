...
W=320; H=240; PERIOD_NS=100_000_000; RING_AGE_NS=500_000_000
...
def one_arm(root,display_num,arm,warmup_s,measure_s):
    ...
    try:
        ...
        try:
            ...
            return {'arm':arm,'display':display_num,'measure_wall_ns':stop-start,'process_cpu_ns':cpu,'cpu_fraction':cpu/max(1,stop-start),'capture_count':len(caps),'capture_dropped_slots':dropped,'capture_exceptions':exceptions,'capture_latency_ns':{'p50':pct(caps,.5),'p95':pct(caps,.95),'max':max(caps) if caps else None},'frame_bytes':frame_bytes,'max_ring_bytes':max_ring,'fixture_count':len(ts),'fixture_gap_ns':{'p50':pct(gaps,.5),'p95':pct(gaps,.95),'max':max(gaps) if gaps else None},'fixture_geometry':fj['geometry'],'max_rss_kb':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}
        finally:
...
def main():
    ...
    pairs=1 if args.construction else 6; warm=.15 if args.construction else .3; dur=.7 if args.construction else 1.5; rows=[]; base_display=100 if args.construction else 110
    for i in range(pairs):
        order=['baseline','capture'] if i%2==0 else ['capture','baseline']; pair=[]
        for j,arm in enumerate(order): pair.append(one_arm(root,base_display+i*2+j,arm,warm,dur))
        rows.append(pair)
    result={'construction':args.construction,'formal':not args.construction,'formal_invocations':0 if args.construction else 1,'reruns':0,'pairs':rows,'platform':platform.platform(),'python':platform.python_version(),'grants_input_authority':False,'task':'TEMPORAL-BUFFER-X11-CAPTURE-CADENCE10-20260918-005'}
