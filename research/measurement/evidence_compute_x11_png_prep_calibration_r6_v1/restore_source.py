...
m=json.loads(Path("SOURCE_MANIFEST.json").read_text());t="".join(Path(p["name"]).read_text() for p in m["bundle"]["parts"]);assert hashlib.sha256(t.encode()).hexdigest()==m["bundle"]["b64_sha256"];gz=base64.b64decode(t);assert hashlib.sha256(gz).hexdigest()==m["bundle"]["gzip_sha256"];tar=gzip.decompress(gz);assert hashlib.sha256(tar).hexdigest()==m["bundle"]["tar_sha256"]
