...
EXPECTED=['PLAN.md','SCHEDULE.json','audit.py','fixture.py','runner.py']
def main():
    ap=argparse.ArgumentParser();ap.add_argument('bundle');ap.add_argument('out');a=ap.parse_args()
    raw=base64.b64decode(Path(a.bundle).read_text().strip(),validate=True); tar_bytes=gzip.decompress(raw)
