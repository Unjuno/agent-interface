...
def main():
    ...
    decision='HOLD_HOST_SCHEDULING_NOISE' if severe>=2 else 'PASS_X11_TEMPORAL_CAPTURE_OVERHEAD_SCOPED'
    if severe<2:
        ...
        if errs: decision='REJECT_CONTINUOUS_CAPTURE_COST' if any(('capture_' in x or x.endswith('_cpu') or x.endswith('_ring') or 'paired_' in x or 'max_gap' in x) for x in errs) else 'FAIL_INTEGRITY'
