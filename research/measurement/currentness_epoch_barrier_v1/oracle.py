...
def replay(history):
    ...
    scopes={sc for _,sc in seen}
    snap={
        'epochs': tuple(sorted((sc.session,sc.target,sum(1 for _,s in seen if s==sc)) for sc in scopes)),
