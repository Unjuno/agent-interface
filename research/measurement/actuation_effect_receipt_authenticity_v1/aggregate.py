...
def main(paths):
    ...
    sums={k:0 for k in ['count','mismatches','authenticated_false_self_credit','unauthenticated_false_self_credit','replay_escape','authority_promotions']}
    for r in rows:
    ...
    if expected!=330000: errors.append(f'end:{expected}')
    decision='PASS_ACTUATION_EFFECT_RECEIPT_AUTHENTICITY_SCOPED' if (not errors and sums['mismatches']==0 and sums['authenticated_false_self_credit']==0 and sums['replay_escape']==0 and sums['unauthenticated_false_self_credit']>0 and sums['authority_promotions']==0) else 'FAIL_GATE'
