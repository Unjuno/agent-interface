...
def select_image_bytes(batch,run_directory):
    ...
    if not candidates:return dict(status='no_observation',authority='none'), None
    ...
    return (dict(status='image',sequence=newest,capture_ns=captured,path=str(path),relative_path=str(path.relative_to(root)),
                sha256=hashlib.sha256(payload).hexdigest(),authority='none',freshness='historical referenced capture; no new observation'), payload)
