...
def trace(i, klass):
    ...
    c=Candidate(); o=Oracle(); comp=StateOnlyComparator(); mismatch=0
    def both(op,*a,**kw):
      nonlocal mismatch
...
def main():
  ...
  rng.shuffle(classes); t0=time.monotonic_ns(); rows=[]; counts={k:0 for k in CLASSES}
  ...
  raw=json.dumps({"summary":summary,"rows":rows},sort_keys=True,separators=(",",":"))
