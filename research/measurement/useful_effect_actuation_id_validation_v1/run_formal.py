...
defects={}
for bad in (None,'',0): defects[repr(bad)]=analyze(W,[A(bad)],[E(bad)])['effects']['useful_bound']
if defects!={"None":1,"''":1,"0":1}: raise AssertionError(defects)
start=time.perf_counter_ns()
rng=random.Random(VALID_SEED); valid_equal=0
for case in range(N_VALID):
    wait,acts,events=make_valid(case,rng)
    base=analyze(wait,acts,events); cand=analyze_validated(wait,acts,events)
    if base!=cand: raise AssertionError(('valid_regression',case,base,cand))
    valid_equal+=1
rng=random.Random(MALFORMED_SEED); malformed_rejected=0; actuation_bad=0; effect_bad=0
for case in range(N_MALFORMED):
    wait=Interval(0,100)
    if case%2==0:
        bad=rng.choice(BAD_HASHABLE); acts=[A(bad)]; events=[E(bad)]; actuation_bad+=1
    else:
        bad=rng.choice(BAD_EFFECT); acts=[A(f'valid-{case}')]; events=[E(bad)]; effect_bad+=1
    if not expect_reject(wait,acts,events): raise AssertionError(('malformed_escape',case,repr(bad)))
...
result={'task':'USEFUL-EFFECT-ACTUATION-ID-VALIDATION-20260917-001','base':'d994bc01a6a23d21bd565b84d58a43a99a562f89','parent_git_blob':git_blob(ROOT/'interval_contract.py'),'parent_sha256':sha256(ROOT/'interval_contract.py'),'source_sha256':{n:sha256(ROOT/n) for n in ['PLAN.md','id_validation.py','test_static.py','run_formal.py','audit.py']},'formal_invocations':1,'formal_reruns':0,'valid_seed':VALID_SEED,'valid_cases':N_VALID,'valid_exact_equal':valid_equal,'malformed_seed':MALFORMED_SEED,'malformed_cases':N_MALFORMED,'malformed_rejected':malformed_rejected,'malformed_actuation_cases':actuation_bad,'malformed_effect_cases':effect_bad,'predecessor_defect_useful_bound':defects,'none_unbound_effects':none_unbound,'unicode_valid_equal':unicode_equal,'duplicate_valid_id_rejected':duplicate_rejected,'authority_actions':0,'task_input_actions':0,'network_actions':0,'model_calls':0,'elapsed_ns':elapsed,'disposition':'PASS_ACTUATION_ID_VALIDATION_SCOPED' if (valid_equal==N_VALID and malformed_rejected==N_MALFORMED and none_unbound['useful_unbound']==1 and none_unbound['useful_bound']==0 and unicode_equal and duplicate_rejected) else 'FAIL'}
