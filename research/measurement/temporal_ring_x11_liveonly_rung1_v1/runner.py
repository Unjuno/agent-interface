...
def nearest(ring,target_ns):
    return min(ring,key=lambda x:abs(x['capture_ns']-target_ns))
    ...
    try:
        ...
        try:
            ...
            if arm=='ring':
                ...
                while time.perf_counter_ns()<end:
                    ...
                    if reqi<len(request_offsets_ns) and now>=next_req:
                        ...
                        trials.append({'request_index':reqi,'request_ns':request_ns,'query_latency_ns':q1-q0,'target_ns':targets,
                          'selected_capture_ns':[x['capture_ns'] for x in selected],
                          'target_error_ns':[abs(x['capture_ns']-t) for x,t in zip(selected,targets)],
                          'distinct_frames':selected[0]['capture_ns']!=selected[1]['capture_ns'],
                          'both_before_request':all(x['capture_ns']<request_ns for x in selected),
                ...
                for reqi,off in enumerate(request_offsets_ns):
                    ...
                    trials.append({'request_index':reqi,'request_ns':request_ns,'evidence_latency_ns':done-request_ns,
                      'current_capture_ns':first['capture_ns'],'future_capture_ns':second['capture_ns'],
                      'future_delay_ns':second['capture_ns']-request_ns,'exact_requested_past_available':False,
                      'evidence_role':'future_equivalent','extra_acquisition_boundaries':1,'promoted_future_as_history':False})
            ...
            return {'arm':arm,'display':display_num,'trials':trials,'capture_count':len(caps),'capture_exceptions':exceptions,
              'capture_latency_ns':{'p50':pct(caps,.5),'p95':pct(caps,.95),'max':max(caps) if caps else None},
              'capture_payload_types':payload_types,'frame_bytes':EXPECTED_FRAME_BYTES,'max_ring_bytes':max_ring_bytes,
              'capture_region':list(ROI),'fixture_geometry':fj['geometry'],'fixture_count':len(fj['times_ns'])}
        finally:
...
def main():
    ...
    for p in schedule['pairs']:
        pair=[]
        for j,arm in enumerate(p['order']):pair.append(one_arm(root,560+(p['pair']-1)*2+j,arm,[x*1_000_000 for x in p['request_offsets_ms']]))
        pairs.append({'pair':p['pair'],'arms':pair})
    result={'formal':True,'formal_invocations':1,'reruns':0,'pairs':pairs,'platform':platform.platform(),'python':platform.python_version(),
      'grants_input_authority':False,'task':schedule['task']}
