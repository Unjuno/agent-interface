...
W=320; H=240; ROI_X=80; ROI_Y=60; ROI_W=160; ROI_H=120; PERIOD_NS=50_000_000; RING_AGE_NS=500_000_000
EXPECTED_FRAME_BYTES=ROI_W*ROI_H*4
...
def normalize_image_data(raw):
    if isinstance(raw, bytes):
        ...
        raise TypeError('unsupported image payload type: '+type(raw).__name__)
    if len(data)!=EXPECTED_FRAME_BYTES:
        raise ValueError(f'roi payload length {len(data)} != {EXPECTED_FRAME_BYTES}')
    return data
...
def one_arm(root,display_num,arm,warmup_s,measure_s):
    ...
    try:
        ...
        try:
            ...
            d=display.Display(env['DISPLAY']) if arm=='capture' else None; rwin=d.screen().root if d else None
            start=time.perf_counter_ns(); startf.write_text(str(start)); end=start+int(measure_s*1e9)
            cpu0=time.process_time_ns(); caps=[]; dropped=0; ring=collections.deque(); ring_bytes=0; max_ring=0; frame_bytes=None; exceptions=[]; payload_types={}
            if arm=='capture':
                nxt=start
                while True:
                    ...
                    try:
                        img=rwin.get_image(ROI_X,ROI_Y,ROI_W,ROI_H,X.ZPixmap,0xffffffff); raw=img.data
                        payload_types[type(raw).__name__]=payload_types.get(type(raw).__name__,0)+1
                    ...
                    while ring and after-ring[0][0]>RING_AGE_NS: ring_bytes-=len(ring.popleft()[1])
            ...
            return {'arm':arm,'display':display_num,'measure_wall_ns':stop-start,'process_cpu_ns':cpu,'cpu_fraction':cpu/max(1,stop-start),
                    ...
                    'capture_payload_types':payload_types,'frame_bytes':frame_bytes,'max_ring_bytes':max_ring,'capture_region':[ROI_X,ROI_Y,ROI_W,ROI_H],
                    'fixture_count':len(ts),'fixture_gap_ns':{'p50':pct(gaps,.5),'p95':pct(gaps,.95),'max':max(gaps) if gaps else None},'fixture_geometry':fj['geometry'],'max_rss_kb':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}
        finally:
...
def main():
    ...
    pairs=1 if args.construction else 6; warm=.15 if args.construction else .3; dur=1.3 if args.construction else 1.5; rows=[]
    base_display=460 if args.construction else 470
    for i in range(pairs):
        order=['baseline','capture'] if i%2==0 else ['capture','baseline']; pair=[]
        for j,arm0 in enumerate(order): pair.append(one_arm(root,base_display+i*2+j,'capture' if arm0=='capture' else 'baseline',warm,dur))
        rows.append(pair)
    result={'construction':args.construction,'formal':not args.construction,'formal_invocations':0 if args.construction else 1,'reruns':0,'pairs':rows,'platform':platform.platform(),'python':platform.python_version(),'grants_input_authority':False,'task':'TEMPORAL-BUFFER-X11-ROI-PAYLOAD-NORMALIZATION-20260918-007'}
