...
def call_pair(c,o,op,*args,**kwargs):
    if op=="req": cr=c.frontier_request(*args); oo=o.req(*args); ok=cr.get("ok") == oo[0]
        ...
        cr=c.try_admit(*args,**kwargs)
        okw=dict(kwargs)
        if "claimed_generation" in okw:
            okw["claimed"]=okw.pop("claimed_generation")
        oo=o.admit(*args,**okw)
        ok=cr.get("admit") == oo
...
def main():
  ...
  summary={
    "allocation":"issue1460-generation-aba-v3-20260922-01","seed":SEED,"traces":len(rows),"class_counts":counts,
