# Issue #1460 fresh allocation v3 — T2 generation ABA guard

Historical #1460 comment 5728762210 claimed a local construction at formal0 but no remote branch/freeze/result exists. v2 is retained STOP_FORMAL_RUNNER_KEYWORD_MISMATCH at formal1/rows0. This fresh v3 changes only the harness keyword translation and adds a regression check; scientific semantics and gates remain unchanged.
...
Construction is excluded: py_compile and 7 unit methods only; no formal rows. No model/provider/GUI/X11/network/task input/runtime mutation.
