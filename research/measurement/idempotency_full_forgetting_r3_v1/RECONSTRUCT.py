# Audit reconstruction only; not a new formal allocation.
...
with tempfile.TemporaryDirectory(prefix='ai24-forgetting-r3-') as td:
    root=Path(td); p=root/'src.tar.gz'; p.write_bytes(raw)
    with tarfile.open(p,'r:gz') as tf: tf.extractall(root/'src')
    src=root/'src'
