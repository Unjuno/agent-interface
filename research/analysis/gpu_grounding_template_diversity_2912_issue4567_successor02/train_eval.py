...
def load_rows(repo, manifest):
    corpus_root = (repo / "research/analysis/gpu_grounding_template_diversity_2912_v2/corpus").resolve()
    rows = []
    for record in manifest["records"]:
        image_path = resolve_corpus_image(corpus_root, record)
...
def resolve_corpus_image(corpus_root, record):
    corpus_root = Path(corpus_root).resolve()
    image_path = (corpus_root / record["image"]).resolve()
    if not image_path.is_relative_to(corpus_root):
        raise ValueError(f"manifest image path escapes corpus: {record['record_id']}")
    return image_path
...
def main():
    ...
    if not os.environ.get("LOCAL_GPU_FORMAL_ACK") == "4567-one-shot":
    ...
    if os.environ.get("LOCAL_GPU_SERIALIZATION_ACK") != "4567-idle-confirmed":
        raise RuntimeError("fresh local GPU-idle/serialization acknowledgement missing")
    records = load_rows(repo, manifest)
    if len(records) != freeze["corpus"]["records"]:
        raise ValueError("preflight image denominator mismatch")
    by_family = {}
    for row in records:
    ...
    result = {"format": "gpu-grounding-template-diversity-result-v1",
              "issue": 4567, "allocation": "gpu-grounding-template-diversity-2912-successor-02",
