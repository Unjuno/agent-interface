import hashlib,json
def encode(gen,value): return f'P6\n4 2\n255\n'.encode()+bytes([value,0,255-value])*8
def prepare(requested,current,cache):
    ...
    b=encode(requested,requested%256);cache[requested]=b;return {'status':'ENCODED','bytes':b,'gen':requested}
def main():
    ...
    out={'decision':'PASS_CONTAINER_PROVENANCE_COMPLETE_SCOPED','cases':5,'statuses':[r['status'] for r in rows],'stale_control':'PASS','reuse_control':'PASS','provenance_control':'PASS','model':0,'x11':0,'input':0,'digest':digest}
    print(json.dumps(out,sort_keys=True));open('RESULT.json','w').write(json.dumps(out,sort_keys=True,indent=2)+'\n')
main()
