...
def main():
    ...
    v=json.loads((ROOT/"PROVENANCE.json").read_text(encoding="utf-8"))
    ...
    assert subprocess.check_output(["git","hash-object",m["parent_path"]],text=True).strip()==m["parent_blob_sha"]
    ...
    assert v["parent_blob_sha"]==m["parent_blob_sha"]
    assert v["parent_path"]==m["parent_path"]
