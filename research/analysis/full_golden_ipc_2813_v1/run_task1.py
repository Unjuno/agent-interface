"""Run only task-1 through the existing persistent client, once."""
...
def main() -> int:
    ...
    (out / "doctor.json").write_text(json.dumps({"schema":"task1-route-doctor-v1","authority_granted":False,"task_limit":1,"chromium":str(chromium)})+"\n", encoding="utf-8")
    ...
    with client_type(out / "arms" / "persistent", args.seed) as client:
        ...
        rows.append(row); details.append(detail); independent = client.finish("finish-task1-route-2813")
    report={"schema":"task1_route_correction_result_v1","issue":2813,"status":"PASS_TASK1_EFFECT" if row["exact_submission"] and row["releases_verified"] and independent.get("success") is True else "HOLD_TASK1_EFFECT_NOT_REACHED","authority_granted":False,"preflight":preflight_result,"tasks":rows,"independent_evaluation":independent,"task_limit":1}
