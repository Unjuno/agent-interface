...
def mutations(wd,rc,a2):
    ...
    a=rc.splitlines(); i=next(i for i,v in enumerate(a) if "if a.arm=='watchdog_journal'" in v); block=a[i:i+2]; del a[i:i+2]; j=next(i for i,v in enumerate(a) if '# Ordinary data path recovers only now.' in v); a[j:j]=block; x=chr(10).join(a)+chr(10)
    tests['recovered_before_recovery']=bool(structural(wd,x,a2,False)[1])
