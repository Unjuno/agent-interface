...
def audit(root_path,formal_path):
    ...
    if rows:
        ...
        if any(r['policies']['COMPLETENESS_GATED']['events']['READY_TO_SUBMIT']['hit'] for r in rows): errors.append('gated_incomplete_hit')
        if sum(bool(r['policies']['COMPLETENESS_GATED']['events']['TARGET_MATCH']['hit']) for r in rows)<1: errors.append('gated_complete_no_hit')
        for idx in (4,):
    ...
    decision='PASS_DEPENDENCY_COMPLETENESS_GATE_SCOPED' if not errors else ('FAIL_COMPLETENESS_GATE_FALSE_REUSE' if any(x.endswith(':unsafe') for x in errors) else 'FAIL_INTEGRITY_OR_GATE')
