...
def expected_pooling_matrix_sha256():
    def matrix(source, target):
        ...
        return values
    return digest(matrix(40, 8).tobytes() + matrix(25, 10).tobytes())
...
def audit_file(repo, raw_path, audit_path):
    ...
    if raw.get("format") != "gpu-photometric-2912-v3-result-v1":
        raise ValueError("result format mismatch")
    if raw.get("pooling_operator") != "deterministic_adaptive_avg_pool2d_separable_v1":
        raise ValueError("pooling operator mismatch")
    if raw.get("source_commit") != "9e3ee7ea02b27e935a50130a0be45e9938a4dfdb":
