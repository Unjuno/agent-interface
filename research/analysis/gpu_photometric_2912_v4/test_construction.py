...
class ConstructionTests(unittest.TestCase):
    ...
    def test_fixed_matrices_match_adaptive_pool_cpu_oracle(self):
        torch.manual_seed(4482)
        x = torch.randn((2, 16, 25, 40), dtype=torch.float32)
    ...
    def test_complete_cnn_cpu_shape_path_matches_pool_oracle(self):
        torch.manual_seed(4482)
        model = runner.TinyGrounder().eval()
        source = torch.randn((16, 1, 100, 160), dtype=torch.float32)
        with torch.no_grad():
            actual_feature_map = model.features[:6](source)
            self.assertEqual(tuple(actual_feature_map.shape), (16, 16, 25, 40))
            fixed = model.features[6](actual_feature_map)
            oracle = F.adaptive_avg_pool2d(actual_feature_map, (8, 10))
            self.assertEqual(tuple(fixed.shape), (16, 16, 8, 10))
            self.assertLessEqual(float((fixed - oracle).abs().max()), 1e-6)
            logits = model(source)
...
class CudaConstructionTests(unittest.TestCase):
    def test_repeated_cuda_forward_backward_is_deterministic(self):
        ...
        source = torch.randn((16, 1, 100, 160), dtype=torch.float32)
        model = runner.TinyGrounder().cuda().eval()

        def forward_backward():
            x = source.cuda().detach().clone().requires_grad_(True)
            logits = model(x)
            logits.square().sum().backward()
            return logits.detach(), x.grad.detach()

        y1, g1 = forward_backward()
        y2, g2 = forward_backward()
        torch.cuda.synchronize()
        self.assertEqual(tuple(y1.shape), (16, 2))
