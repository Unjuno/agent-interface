...
p=Path(__file__).with_name("RAW_RESULT.json")
d=json.loads(p.read_text())
rows=d["rows"]
def oracle(f,r):
    if f["epoch"]!=r["epoch"]: return False
    if f["identity"]!=r["identity"]: return False
...
assert len(rows)==256 and not mismatches
assert sum(bool(x["candidate"]) for x in rows)==8
