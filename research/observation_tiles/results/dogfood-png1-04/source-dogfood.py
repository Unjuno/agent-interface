...
SUPPORTED_TEXT="abcdefghijklmnopqrstuvwxyz0123456789:/-._ "
...
def main():
    ...
    try:
        ...
        images=ImageArtifactSink(args.out,compress_level=args.png_compress_level,reuse=True)
        ...
        emit(dict(event="ready",app=args.app,goal=goal,operations=["observe","text","key","chord","drag","finish"],
                  ...
                  supported_text_characters=SUPPORTED_TEXT,
        ...
        for line in sys.stdin:
            ...
            emit(dict(event="command",command=command,received_ns=time.perf_counter_ns()))
            ...
            if op=="text" and (not isinstance(command.get("text"),str) or
                               any(ch not in SUPPORTED_TEXT for ch in command["text"])):
                emit(dict(event="rejected",action_id=action_id,input_events_sent=0,
                          reason="Text contains unsupported characters; whole command rejected before input"))
                continue
            wait_ms=command.get("wait_changed_ms",0)
            if not isinstance(wait_ms,(int,float)) or not 0 <= wait_ms <= 5000:
                raise ValueError("wait_changed_ms must be between 0 and 5000")
            prior_frame=dec.frame
            prior_context=dec.metadata["context"]
            ...
            observe(action_id)
            if wait_ms:
                deadline=ack+int(wait_ms*1e6)
                while dec.frame==prior_frame and dec.metadata["context"]==prior_context:
                    if time.perf_counter_ns() >= deadline: break
                    time.sleep(min(.016,max(0,(deadline-time.perf_counter_ns())/1e9)))
                    observe(action_id)
                changed=dec.frame!=prior_frame or dec.metadata["context"]!=prior_context
                emit(dict(event="change_wait_result",action_id=action_id,
                          changed=changed,timed_out=not changed,
                          known_ns=time.perf_counter_ns(),semantic_completion="unknown",
                          note="A visual/context change may be unrelated to the action; inspect it"))
    except Exception:
