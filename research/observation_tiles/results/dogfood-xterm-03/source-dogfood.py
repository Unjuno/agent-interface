...
def main():
    ...
    sources={}
    for name in ("dogfood.py","tile_transport.py"):
    ...
    try:
        ...
        def observe(action_id):
            ...
            reused=enc.last["kind"]=="unchanged" and last_image is not None
            path=last_image if reused else args.out/f"{seq:03d}.png"
            if not reused: suite.image_for(frame).save(path)
            last_image=path
            ...
            record=dict(event="observation",sequence=seq,action_id=action_id,
                        ...
                        reconstruction_verified=True,image_reused=reused,**enc.last,
