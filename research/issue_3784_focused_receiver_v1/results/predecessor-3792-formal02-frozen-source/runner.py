...
def capture_state(env, expected_layout):
    ...
    try:
        ...
        fresh.close()
    layout_ok = any(line.strip() == f"layout: {expected_layout}" for line in query["stdout"].splitlines())
...
def run_row(case_id, layout, source, out):
    ...
    try:
        ...
        row["receiver_control_ok"] = [(e["type"], e["keycode"], e["lookup_text"]) for e in control] == [
            ("KeyPress", keycode, "a"), ("KeyRelease", keycode, "a")]
...
def main():
    ...
    raw = {"schema": "agent-interface/issue3784-raw-v2", "allocation": ALLOCATION,
