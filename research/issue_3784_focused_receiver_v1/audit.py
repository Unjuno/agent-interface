...
RUNNER_SHA = "604ff7238e7ba6053456787520696589bbd7cd9d6ffc877fe771b2ab82d7eb50"
MANIFEST_SHA = "34be24b090cbcaec586613a868ecd7a16008d8977b4f7073c9dee70d593e8a6d"
...
def verify(raw, formal: Path, source: Path, manifest: dict, manifest_bytes: bytes,
    ...
    if raw.get("schema") != "agent-interface/issue3794-raw-v1": errors.append("SCHEMA")
    ...
    for index, row in enumerate(rows):
        ...
        if (proc.get("reaped") is not True or proc.get("start_ticks") is None
                or (proc.get("launched", True) is False and not status.startswith("STOP_"))):
            errors.append("XVFB_CLEANUP:" + cid)
        phase = row.get("phase")
        ...
        if phase_at_least(row, "receiver_control"):
            ...
            control_ok = (len(ctl) == 2 and observed[0] == ("KeyPress", keycode, "a")
                and observed[1][0:2] == ("KeyRelease", keycode)) if len(observed) == 2 else False
            if not keycode or not control_ok or row.get("receiver_control_ok") is not True:
        ...
        for name in required_map_phases(row):
            ...
            if q.get("returncode") != 0 or not state.get("layout_ok") or not any(
                re.fullmatch(r"layout:\s*" + re.escape(want_layout), line.strip())
...
def main():
    ...
    manifest_bytes = (source / "research/issue_3784_focused_receiver_v1/source_manifest.json").read_bytes()
    manifest = json.loads(manifest_bytes)
    runner_bytes = (source / "research/issue_3784_focused_receiver_v1/runner.py").read_bytes()
    source_errors = []
    ...
    result = {"schema": "agent-interface/issue3794-audit-v1", "allocation": ALLOCATION,
