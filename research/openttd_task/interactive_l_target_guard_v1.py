"""Run the seed991003 L fixture through the target/guard X11 backend."""
import argparse
import contextlib
import hashlib
import json
...
def main():
    ...
    (args.out / "manifest.json").write_text(json.dumps({
        "scope": "scripted target/guard controller; seed991003 five-tile L objective; independent dynamic guard score",
    ...
    try:
        with (args.out / "setup.txt").open("w") as setup, contextlib.redirect_stdout(setup):
            ...
            session._wait(lambda: "AIT_READY" in (args.out / "game-stderr.txt").read_text(),
                          40, "saved L observer task ready")
            ...
            try:
                ...
                if command["op"] == "submit":
                    ...
                    session._wait(lambda: len(records()) > count, 5,
                                  "post-control independent L sample")
