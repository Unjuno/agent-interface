"""OpenTTD seed-991002 shifted-geometry self-use through session_v22."""
...
SAVE=HERE/'results/geometry-01/baseline.sav'
SAVE_SHA='2cfdb42ee2f3d1b44e89920387a2299b44072801198cbc76ff88a7609c6c12f7'
...
def main():
    ...
    sources=[Path(__file__).resolve(),HERE/'guarded_score_v2.py',*sorted((HERE/'observer_v2').glob('*.nut')),HERE.parent/'openttd_oracle/score.py']
    ...
    (a.out/'manifest.json').write_text(json.dumps({'scope':a.controller+' controller; seed-991002 shifted geometry; dynamic guard score; no engine actions during control','save_sha256':SAVE_SHA,'sources':{str(p.relative_to(HERE.parent)):sha(p) for p in sources}},indent=2)+'\n')
