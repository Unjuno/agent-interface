"""Generate and restore the seed-991003 five-tile L-road task fixture."""
...
def main():
    ...
    sources=[Path(__file__).resolve(),HERE/'plan_l_geometry_v1.json',HERE/'guarded_l_score_v1.py',
             *sorted((HERE/'setup_l_v1').glob('*')),*sorted((HERE/'observer_l_v1').glob('*')),
             HERE.parent/'observation_gating/gui_suite.py',HERE.parent/'real_apps_v1/real_app_suite_v1.py']
    manifest={'sources':{p.relative_to(HERE.parent).as_posix():sha(p) for p in sources},'plan':json.loads((HERE/'plan_l_geometry_v1.json').read_text())}
    ...
    for phase in ['setup','unsaved','restore-1','restore-2']:
        ...
        try:
            ...
            shutil.copytree(HERE/('setup_l_v1' if phase=='setup' else 'observer_l_v1'),data/'game/interface_task')
            cfg=out/'openttd.cfg';cfg.write_text('[game_creation]\nmap_x = 6\nmap_y = 6\nstarting_year = 1950\n[game_scripts]\nInterfaceTask = \n')
            cmd=[str(r/'games/openttd'),'-c',str(cfg),'-d','script=4','-s','null','-m','null','-r','1024x720','-g']
            cmd+= [str(saved)] if phase.startswith('restore') else ['-G','991003']
            row['command']=cmd
            ...
            with (out/'stdout.txt').open('w') as so,(out/'stderr.txt').open('w') as se:
                ...
                if phase=='setup' and row['ready']:
                    s.focus('OpenTTD');driver=base.Driver(s,settle_ms=20);base.CHAR_GAP_MS=12
                    driver.key('grave');time.sleep(.2)
                    record=next(json.loads(l.split('AIT ',1)[1]) for l in (out/'stderr.txt').read_text().splitlines() if 'AIT {' in l)
                    x,y,w=record['x'],record['y'],record['width']
                    target=[y*w+x,y*w+x+1,y*w+x+2,(y+1)*w+x+2,(y+2)*w+x+2]
                    row['setup_scroll_command']='scrollto '+str((y+4)*w+x+6)
                    row['target_contract']={'x':x,'y':y,'width':w,'target':target}
                    assert len(set(target))==5
                    driver.text(row['setup_scroll_command']);driver.key('Return');time.sleep(.3)
                    temporary=s.tmp/'baseline.sav';row['setup_console_command']='save '+str(temporary.with_suffix(''))
                    driver.text(row['setup_console_command']);driver.key('Return')
                    deadline=time.monotonic()+15;last=None;stable=0
                    while time.monotonic()<deadline:
                        if temporary.exists():
                            size=temporary.stat().st_size;stable=stable+1 if size==last and size>0 else 0;last=size
                            if stable>=3:break
                        time.sleep(.2)
                    if not temporary.exists() or stable<3:raise RuntimeError('save not completed')
                    shutil.copy2(temporary,saved);row['save_created']=sha(saved);driver.key('grave')
                time.sleep(.4);ImageGrab.grab(xdisplay=s.name).save(out/'screen.png');row['screen_sha256']=sha(out/'screen.png')
                if p.poll() is None:
            ...
            row['error']=repr(e);ImageGrab.grab(xdisplay=s.name).save(out/'error-screen.png')
            ...
            shutil.rmtree(s.tmp);(out/'result.json').write_text(json.dumps(row,indent=2)+'\n');print(json.dumps(row),flush=True)
