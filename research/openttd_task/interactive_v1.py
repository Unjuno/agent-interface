...
def main():
    ...
    sources=[Path(__file__).resolve(),*sorted((HERE/'observer').glob('*.nut')),HERE.parent/'openttd_oracle/score.py']
    sources += [HERE.parent/'live_control'/n for n in ['session_v9.py','session_v8.py','session_v7.py','session_v6.py','session_v5.py','session_v4.py','input_owner_v5.py','executor_v3.py','lease.py']]
    (a.out/'manifest.json').write_text(json.dumps({'scope':'assistant visual self-use; no engine actions during control','save_sha256':SAVE_SHA,'sources':{str(p.relative_to(HERE.parent)):sha(p) for p in sources}},indent=2)+'\n')
    ...
    try:
        with (a.out/'setup.txt').open('w') as setup,contextlib.redirect_stdout(setup):
            ...
            shutil.copytree(r/'share/games/openttd/game',data/'game');shutil.copytree(HERE/'observer',data/'game/interface_task')
        ...
        emit({'event':'ready','task':'Build a straight road connecting the ground tiles marked A and C. Keep the adjacent row marked X free of roads. Use visual GUI controls.'})
        ...
        for line in sys.stdin:
            try:
                ...
                if c['op']=='submit':engine.submit(c['id'],c['steps'],c['expected_sequence'],c['valid_until_ns'])
                    ...
                    result=score(observation,{'target':[678,679,680],'forbidden':[742,743,744],'owner':0})
                    (a.out/'evaluation.json').write_text(json.dumps({'observation':observation,**result},indent=2)+'\n')
