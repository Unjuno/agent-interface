// Setup creates only the source and empty floor. Agent must build via GUI.
// After finish closes input, a declared no-input delivery window runs for 600 ticks.
var phase="loading", startTick=0;
...
Events.run(Trigger.update,run(function(){
    if(phase=="control" && Vars.dataDirectory.child("evaluate.txt").exists()){
        ...
        phase="delivery";startTick=Vars.state.tick;
...
Events.run(Trigger.afterGameUpdate,run(function(){
    if(phase=="spawn" && !Vars.player.dead() && Vars.player.unit()!=null){
        ...
        report("before");phase="control";
        Vars.dataDirectory.child("ready.txt").writeString("player ready");
    }else if(phase=="delivery" && Vars.state.tick-startTick>=600){
        Vars.state.set(Packages.mindustry.core.GameState.State.paused);
