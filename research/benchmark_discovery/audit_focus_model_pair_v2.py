...
def main():
 r=HERE/'results/focus-model-pair-02';plan=read(r/'plan.json');rows=[];args=None
 ...
 report={'rows':rows,'all_answers_correct':all(x['correct'] for x in rows),'scope':'archived text-only cases, equal explicit policy with synthetic freshness; full latest records retained; no live recovery, broad reasoning or cost proof','audit_sha256':sha(Path(__file__))}
