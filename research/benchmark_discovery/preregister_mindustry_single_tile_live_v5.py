...
OUT = HERE / "results/mindustry-single-tile-live-05"
...
SOURCES = [
    "preregister_mindustry_single_tile_live_v5.py", "run_mindustry_single_tile_live_v5.py",
    ...
    "mindustry_world_target_responder_v3.txt", "mindustry_world_target_contract_schema_v3.json",
    "mindustry_world_target_contract_v3.py", "compact_world_receipt_v1.py",
...
def main():
    ...
    plan = {
        ...
        "study": "mindustry-single-tile-live-05", "base_main": "cacf6dcd098b5233e839a14813af33654f1430b0",
        "model": "gpt-5.6-luna", "reasoning_effort": "low", "model_calls_max": 4,
        "task": task,
        "policy": ("use structured-output-compatible flat candidate and world-receipt objects with explicit property types and nonredundant points arrays, validate "
        ...
        "sources": {name: sha(HERE.parent / name) if name.startswith("live_control/") else sha(HERE / name)
                    for name in SOURCES},
        "scope": ("one fresh fixed-screen Linux/X11 Mindustry task after retained v1 mixed-field, v2/v3 oneOf and v4 missing-property-type failures; "
