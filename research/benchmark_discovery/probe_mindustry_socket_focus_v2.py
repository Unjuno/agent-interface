...
root=HERE/'results/mindustry-socket-focus-02';root.mkdir()
run=root/'runtime'
names=['mindustry_socket_v2.py','mindustry_focus_fixture_v2.py','mindustry_bend_interactive_v2.py','probe_mindustry_socket_focus_v2.py']
...
p=subprocess.Popen([sys.executable,str(HERE/'mindustry_socket_v2.py'),'--focus-fixture','serve','--','--root','/home/taka/agent-interface-bench-feasibility','--out',str(run)],stdout=subprocess.PIPE,stderr=(root/'stderr.txt').open('w'),text=True)
...
try:
 ...
 for _ in range(10):
  if any(e['event']=='independent_evaluation' for e in r['records']):break
  if r['status']=='closed':raise RuntimeError('runtime closed without evaluation')
  r=query(['independent_evaluation'],timeout=2)
 ...
 (root/'result.json').write_text(json.dumps({'exit_code':code,'tracker':tracker.view(),'calls':len(calls),'scope':'fixture restores game focus after observe-only recovery capture; no agent refocus claim'},indent=2)+'\n')
 print(json.dumps({'exit_code':code,'calls':len(calls),'state':tracker.view()['state']}))
finally:
 if p.poll() is None:
  try:query(['independent_evaluation'],timeout=20);p.wait(timeout=30)
