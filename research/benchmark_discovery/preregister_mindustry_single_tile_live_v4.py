...
OUT = HERE / "results/mindustry-single-tile-live-04"
REFERENCE = HERE / "results/mindustry-bend-v2-self-use-01/004.png"
PRIOR_FAILURE = HERE / "results/mindustry-single-tile-live-03/failure.json"
SOURCES = [
    "preregister_mindustry_single_tile_live_v4.py", "run_mindustry_single_tile_live_v4.py",
    ...
    "mindustry_palette_binding_v2.py", "mindustry_world_candidate_responder_v3.txt", "mindustry_world_hover_receipt_v1.py",
    "mindustry_world_target_responder_v2.txt", "mindustry_world_target_contract_schema_v2.json",
    "mindustry_world_target_contract_v2.py", "compact_world_receipt_v1.py",
...
def main():
    ...
    plan = {
        ...
        "study": "mindustry-single-tile-live-04", "base_main": "cacf6dcd098b5233e839a14813af33654f1430b0",
        "model": "gpt-5.6-luna", "reasoning_effort": "low", "model_calls_max": 4,
        "task": task,
        "policy": ("use structured-output-compatible flat candidate and world-receipt objects with nonredundant points arrays, validate "
                   "direct/probe/coordinate-free cardinality locally, and reject a palette point farther "
                   "than 48 pixels from a fresh detected slot; separately bind Conveyor through a persistent tooltip receipt; select it once; "
        ...
        "failure_policy": "retain this separately versioned first allocation and all available evidence; no model, GUI or task retry",
        ...
        "sources": {name: sha(HERE.parent / name) if name.startswith("live_control/") else sha(HERE / name)
                    for name in SOURCES},
        "scope": ("one fresh fixed-screen Linux/X11 Mindustry task after retained v1 mixed-field, v2 candidate-oneOf and v3 receipt-oneOf failures; "
                  "target remains outside the earlier eight-tile route; integrates "
