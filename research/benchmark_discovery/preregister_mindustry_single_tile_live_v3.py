...
OUT = HERE / "results/mindustry-single-tile-live-03"
REFERENCE = HERE / "results/mindustry-bend-v2-self-use-01/004.png"
PRIOR_FAILURE = HERE / "results/mindustry-single-tile-live-02/failure.json"
SOURCES = [
    "preregister_mindustry_single_tile_live_v3.py", "run_mindustry_single_tile_live_v3.py",
...
def main():
    ...
    plan = {
        ...
        "study": "mindustry-single-tile-live-03", "base_main": "cacf6dcd098b5233e839a14813af33654f1430b0",
        "model": "gpt-5.6-luna", "reasoning_effort": "low", "model_calls_max": 4,
        "task": task,
        "policy": ("use a structured-output-compatible flat candidate object with one nonredundant points array, validate "
        ...
        "sources": {name: sha(HERE.parent / name) if name.startswith("live_control/") else sha(HERE / name)
                    for name in SOURCES},
        "scope": ("one fresh fixed-screen Linux/X11 Mindustry task after retained v1 mixed-field and v2 unsupported-oneOf zero-input failures; "
