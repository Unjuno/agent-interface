...
def main():
    root=HERE/'results/mindustry-bend-v2-self-use-01';manifest=read(root/'manifest.json')
    ...
    assert read(root/'process-result.json')['exit_code']==0
    for e in observations:
        assert 'input_state_before' in e and 'input_state_after' in e
    final=observations[-1]
    for field in ('input_state_before','input_state_after'):
        assert final[field]['owned_buttons']==[] and final[field]['owned_keycodes']==[]
    timings=[]
    for accepted,terminal in zip(admissions,terminals):
        frames=[e for e in observations if e['id']==accepted['id']]
        timings.append({'id':accepted['id'],'accept_to_first_image_ready_ms':(frames[0]['image_ready_ns']-accepted['accepted_ns'])/1e6,'accept_to_terminal_ms':(terminal['terminal_ns']-accepted['accepted_ns'])/1e6})
    controls={}
    ...
    report={'scope':'one known visual construction episode; no held-out performance or global verifier claim',
        'runtime_program_timings':timings,'exact_frames':len(observations),'accepted_programs':len(admissions),'rejected_programs':0,
        'visual_recoveries':[],
        'visual_review_sequences':[1,4,11,14,18],
