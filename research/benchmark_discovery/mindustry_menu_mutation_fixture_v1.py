...
class MutatingBackend(Previous):
 def __init__(self,session,out,emit):
  super().__init__(session,out,emit);self.display_name=session.name;self.worker=None;self.changed=threading.Event();self.restore=threading.Event()
 def mutate(self):
  time.sleep(1)
  d=display.Display(self.display_name);original=d.get_input_focus().focus;kind=os.environ['AI_STUDY_MUTATION']
