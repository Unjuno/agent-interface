"""Visual six-conveyor construction self-use through unchanged shared session_v9/executor_v3."""
import argparse
import contextlib
import hashlib
import json
...
def main():
    ...
    sources = [HERE/'mindustry_build_score_v1.py', HERE/'mindustry_flow_score_v1.py', HERE/'mindustry_flow_plan_v1.json', Path(__file__), *sorted((HERE / 'mindustry_build_mod_v1').rglob('*'))]
    ...
    dump(a.out / 'manifest.json', {'save_sha256':SAVE_SHA,
         ...
         'declared_check':'build six eastward conveyors; no extra placement; finish paused with no pending plans; then no-input delivery measurement',
    ...
    try:
        ...
        emit({'event':'ready','task':'Build six eastward conveyors from the copper source to the core across the empty strip. Leave other tiles unchanged. Resume to finish construction, then pause before finish. Evaluation runs a separate 600-tick no-input delivery window.'})
            ...
            try:
                ...
                if c['op']=='submit':
                    ...
                    if (a.out/'after.json').exists():
                        value=score(*[json.loads((a.out/name).read_text()) for name in ('before.json','delivery-before.json','after.json')],json.loads((HERE/'mindustry_flow_plan_v1.json').read_text()))
