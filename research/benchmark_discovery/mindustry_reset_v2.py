#!/usr/bin/env python3
"""Two fresh paused GUI reloads from an explicit archived save; setup only."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    shutil.copy2(a.save, a.out / 'canonical.msav')
    results = []
    for label in ('reload-1', 'reload-2'):
        out = a.out / label
        out.mkdir()
        s = Session()
        row = {'label': label}
    ...
    if len(results) != 2 or not all(row.get('ready') for row in results):
        raise SystemExit(1)
