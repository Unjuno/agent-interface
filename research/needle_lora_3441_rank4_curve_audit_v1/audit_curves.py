...
def main():
    ...
    if raw_sha!=RAW_SHA256:raise SystemExit("STOP_PREDECESSOR_STDOUT_SHA256")
    old_audit_bytes=old_audit_path.read_bytes()
    old_audit_stdout_sha=hashlib.sha256(old_audit_bytes).hexdigest()
    if old_audit_stdout_sha!=OLD_AUDIT_STDOUT_SHA256:raise SystemExit("STOP_PREDECESSOR_AUDIT_STDOUT_SHA256")
    ...
    if runner_sha!=RUNNER_SHA256 or audit_sha!=OLD_AUDIT_SHA256:raise SystemExit("STOP_PREDECESSOR_SOURCE_SHA256")
    envelope=json.loads(raw)
    payload=gzip.decompress(base64.b64decode(envelope["gzip_b64"],validate=True))
    payload_sha=hashlib.sha256(payload).hexdigest()
    if payload_sha!=PAYLOAD_SHA256 or envelope.get("sha256")!=PAYLOAD_SHA256:raise SystemExit("STOP_CANONICAL_PAYLOAD_SHA256")
