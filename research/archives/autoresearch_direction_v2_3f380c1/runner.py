...
import argparse, hashlib, json, os, select, shutil, statistics, subprocess, sys, time
from pathlib import Path

SCHEMA='map01-direction-source-closed-v2'
...
def run_case(runtime_root:Path, manifest:dict, fixture_manifest:Path, case:dict, out_dir:Path, timeout_seconds=60):
    ...
    p=subprocess.Popen(cmd,cwd=runtime_root/'research/doom',stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,bufsize=1,env=os.environ.copy())
