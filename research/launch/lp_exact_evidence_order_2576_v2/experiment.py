...
def swap_media(html: str) -> str:
    ...
    if not hero or not failed or html.count(hero.group(0)) != 1 or html.count(failed.group(0)) != 1:
        raise RuntimeError("canonical figure boundaries not unique")
    a, b = hero.span(), failed.span()
    h, f = hero.group(0), failed.group(0)
    if a[0] > b[0]: a, b, h, f = b, a, f, h
...
def main():
    ...
    (OUT/"baseline.html").write_text(html,encoding="utf-8")
    (OUT/"candidate.html").write_text(candidate,encoding="utf-8")
