"""Construction-only and result-schema regression coverage for Issue #4505; no training."""
...
class Construction(unittest.TestCase):
 ...
 def test_fresh_seed_and_fixed_shapes(self):
  self.assertIn("SEED = 777731",self.text);self.assertNotIn("SEED = 3927",self.text)
 ...
 def test_dataset_reproducibility_and_cardinality(self):
  x1,y1,h1=self.mod["dataset"](32,777732)
  x2,y2,h2=self.mod["dataset"](32,777732)
 ...
 def test_full_module_state_roundtrip_and_rollback(self):
  core=self.mod["Core"](); model=self.mod["Adapter"](core,777742)
 ...
 def test_result_parameter_schema_is_pure_and_json_serializable(self):
  params=self.mod["parameter_record"]()
  self.assertEqual(params["seed"],777731)
  self.assertEqual(params["base_pretrain_steps"],400)
  self.assertEqual(params["adapter_steps_per_fit"],120)
  self.assertEqual(set(params),{"seed","base_pretrain_steps","adapter_steps_per_fit","features","hidden","classes","rank","batch","lr","support_each","heldout_each"})
  json.dumps(params,sort_keys=True)
 def test_deleted_steps_symbol_cannot_reappear(self):
  names={n.id for n in ast.walk(self.tree) if isinstance(n,ast.Name)}
  self.assertNotIn("STEPS",names)
  self.assertIn('"parameters": parameter_record()',self.text)
