...
def classify(r):
    rows=r.get('rows',[])
    if any(x.get('science_row_present') is not True for x in rows):return 'FAIL_INTEGRITY'
    eff=[x for x in rows if x['arm']=='V12_EFFECT'];ctl=[x for x in rows if x['arm']=='NO_ACTION_CONTROL']
    ...
    for x in rows:
        ...
        if not Path(x.get('child_status_path','')).exists() or not Path(x.get('supervisor_row_path','')).exists():return 'FAIL_INTEGRITY'
    for x in eff:
