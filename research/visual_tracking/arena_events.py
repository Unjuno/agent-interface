...
def main():
    ...
    def paint(t):
        ...
        if not (a.event=='target_loss' and 1<=t<1.5):
            gc.change(foreground=0xff2020);w.fill_rectangle(gc,round(target)-10,135,20,30)
        if a.event=='alert' and 1<=t<1.5:
            gc.change(foreground=0xffff20);w.fill_rectangle(gc,340,70,120,40)
    ...
    while True:
        ...
        target=paint(t)
        if start is not None:rows.append(dict(t=t,player=player,target=target,error=abs(player-target),event_visible=a.event if 1<=t<1.5 else None))
        previous=now
