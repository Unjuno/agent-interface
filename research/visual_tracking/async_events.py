...
from executor_v2 import Executor,Cancelled,DecisionRequired
...
class Backend(BaseBackend):
    def snapshot(self,identifier,index):
        super().snapshot(identifier,index)
        if identifier=='initial':return
        condition=detect(self.decoder.frame)
        if condition:
            self.emit(dict(event='critical_signal',signal_id=f'{identifier}:{self.sequence}',
                           condition=condition,sequence=self.sequence,source_id=identifier,
...
def main():
    ...
    ap.add_argument('--event',choices=['alert','target_loss','none'],default='alert')
    a=ap.parse_args()
    if not number(a.seconds,1,30):raise ValueError('arena duration 1–30 seconds required')
    a.out.mkdir(parents=True,exist_ok=False);lock=threading.RLock();session=None;engine=None;latest=None;seen=set();pending={}
    def emit(row):
        nonlocal latest
        with lock:
            if row['event']=='critical_signal':
                row={**row,'image':latest['image'],'capture_ns':latest['capture_ns']}
                pending[row['signal_id']]=row.copy()
            ...
            if kind=='observation':
                ...
                seen.add(key)
            elif kind in ('motor_feedback','command'):return
            if kind=='terminal' and latest is not None:
                row={**row,'latest_sequence':latest['sequence'],'latest_image':latest['image']}
                line=json.dumps(row)
            print(line,flush=True)
    paths=[HERE/n for n in ('async_events.py','arena_events.py','run.py','signals.py')]
    ...
    try:
        ...
        with (a.out/'app.txt').open('w') as f:
            session.spawn(['python3',str(HERE/'arena_events.py'),'--event',a.event,'--seed',str(a.seed),'--seconds',str(a.seconds),'--out',str(oracle)],stdout=f,stderr=f)
            ...
            for line in sys.stdin:
                try:
                    ...
                    if c['op']=='submit':
                        with lock:
                            if pending and any(s.get('op')!='observe' for s in c['steps']):raise ValueError('acknowledge pending signals before new input')
                        engine.submit(c['id'],c['steps'],c['expected_sequence'])
                    elif c['op']=='ack_signal':
                        with lock:
                            if c['signal_id'] not in pending:raise ValueError('unknown signal id')
                            del pending[c['signal_id']]
                            emit(dict(event='signal_acknowledged',signal_id=c['signal_id']))
                    elif c['op']=='cancel':engine.cancel(c['id'])
                    elif c['op']=='poll':
                        with lock:
                            if latest is None:raise ValueError('no observation yet')
                            emit(dict(event='latest_observation',**{k:latest[k] for k in ('id','step','sequence','image','capture_ns','image_ready_ns')},observation_age_ms=(time.perf_counter_ns()-latest['capture_ns'])/1e6,pending_signals=list(pending.values())))
                    elif c['op']=='finish':
                        engine.close()
