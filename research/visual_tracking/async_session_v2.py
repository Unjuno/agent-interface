...
def main():
    ...
    a.out.mkdir(parents=True,exist_ok=False);lock=threading.RLock();session=None;engine=None;latest=None;seen=set()
    def emit(row):
        nonlocal latest
        with lock:
    ...
    paths=[HERE/n for n in ('async_session_v2.py','arena.py','run.py')]
                ...
                try:
                    ...
                    if c['op']=='submit':engine.submit(c['id'],c['steps'],c['expected_sequence'])
                        ...
                        with lock:
                            ...
                            emit(dict(event='latest_observation',**{k:latest[k] for k in ('id','step','sequence','image','capture_ns','image_ready_ns')},observation_age_ms=(time.perf_counter_ns()-latest['capture_ns'])/1e6))
                    elif c['op']=='finish':
                        engine.close()
