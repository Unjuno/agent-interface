...
root=tk.Tk();root.title('payload-routing-receiver');root.geometry('500x120+80+80')
value=tk.StringVar();entry=tk.Entry(root,textvariable=value,font=('monospace',18));entry.pack(fill='both',expand=True);entry.focus_force();root.update_idletasks();q=queue.Queue()
def server():
...
def poll():
 ...
 root.after(5,poll)
root.after(5,poll);print(json.dumps({'xid':root.winfo_id()}),flush=True);root.mainloop()
