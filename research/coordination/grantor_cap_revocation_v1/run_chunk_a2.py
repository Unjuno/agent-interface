...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--chunk',type=int,required=True);ap.add_argument('--out',required=True);a=ap.parse_args();p=json.loads((ROOT/'plan_a2.json').read_text());rows=p['chunks'][a.chunk];out=pathlib.Path(a.out);out.mkdir(parents=True,exist_ok=False);ledger=[]
