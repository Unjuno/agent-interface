...
| Early-release client wait comparison | First same-stream run descriptively returned early release2.672ms vs terminal113.116ms, but both-read server registration was checked only in memory; its receipts/timestamps were not serialized and frozen audit crashed. | A runner boolean or client start clock cannot independently prove server-side precondition ordering. | Persist exact server request receipts before focus and bind the expected early/terminal request IDs in a new frozen audit. | First allocation retained failed without retry; observed110.444ms difference remains descriptive only. |
