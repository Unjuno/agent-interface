...
def run(out,layout):
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page.wait_for_timeout(6000)  # Exclude transient fullscreen browser chrome before samples.
            ...
            for ci,case in enumerate(CASES):
                ...
                for arm in order:
                    ...
                    append(out/'pre-input.jsonl',{'token':token,'observations':observations})
                    ...
                    if point is not None:
                        ...
                        try:
                            ...
                            press=time.perf_counter_ns();append(out/'input.jsonl',{'token':token,'event':'press_request','time_ns':press,'point':point})
                            xtest.fake_input(d,X.ButtonPress,1);d.sync()
                        finally:
                            xtest.fake_input(d,X.ButtonRelease,1);d.sync();release=time.perf_counter_ns()
                            append(out/'input.jsonl',{'token':token,'event':'release_request_return','time_ns':release})
                        deadline=time.perf_counter_ns()+100_000_000
                        while True:
                            held=bool(d.screen().root.query_pointer().mask & X.Button1Mask);checked=time.perf_counter_ns()
                            append(out/'input.jsonl',{'token':token,'event':'empty_check','held':held,'time_ns':checked})
                            if not held or checked>=deadline:break
                            time.sleep(.002)
